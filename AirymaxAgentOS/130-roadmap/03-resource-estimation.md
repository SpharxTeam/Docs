...
> **文档定位**: AirymaxOS（agentrt-linux，极境智能体操作系统）开发详细方案（路线图）模块第 3 文档
...
本文档估算 AirymaxOS 1.0.1 版本（M0-M8 全部里程碑）所需的人力资源、工时投入与工期分布。0.1.1 版本（占位）已在 `README.md` 单独说明，不计入本文档估算。估算对象包括：
...
3. **IRON-9 同源但独立**——AirymaxOS 工时独立于 agentrt，同源语义对齐成本（约 80h）计入 P0 缓冲。
...
```mermaid
pie showData
    title AirymaxOS 1.0.1 P0+P1 工时分布（按 Part）
...
```mermaid
pie showData
    title AirymaxOS 文档工时分布（按模块前 5 名）
    "50-工程标准(240h)" : 240
    "80-测试(200h)" : 200
    "90-可观测(200h)" : 200
    "110-安全(200h)" : 200
...
```mermaid
pie showData
    title AirymaxOS 1.0.1 工时分布（按工作类别）
    "文档撰写审查(1890h)" : 1890
    "实施编码(580h)" : 580
    "调试集成(180h)" : 180
    "文档同步(100h)" : 100
...
AirymaxOS 1.0.1 的关键路径为 M0 → M1 → M2/M3/M4/M5（并行）→ M6 → M7 → M8，其中 M1 是最长前置任务，决定整体工期。
...
```mermaid
gantt
    title AirymaxOS 1.0.1 关键路径（98 天 = 14 周）
    dateFormat  YYYY-MM-DD
    axisFormat  %m-%d
    section M0 奠基
...
| **IRON-9 同源但独立** | AirymaxOS 工时独立于 agentrt；同源语义对齐成本计入缓冲 | §1.3 + §4 |
...
> **文档结束** | 共 11 节 | Linux 6.6 内核基线 + 五维正交 24 原则 + IRON-9 同源但独立 | 1.0.1 总工时 ~2,750h
