...
> **文档定位**: AirymaxOS（agentrt-linux，极境智能体操作系统）国际化与本地化工程体系主索引
...
AirymaxOS 国际化与本地化体系是面向全球开发者和用户的工程保障。它继承 Linux 发行版 30+ 年沉淀的国际化哲学（locale + gettext + iconv + Unicode），并在其上扩展智能体操作系统专属的多语言 Agent 提示词、多区域记忆卷载、多语言文档同步等。
...
int main() {
    ...
    printf(gettext("AirymaxOS started\n"));
    return 0;
}
```
...
agentrt 的多语言错误码与 AirymaxOS 同源：
- agentrt 用户态：`agentrt_strerror()` 多语言错误码
- AirymaxOS 内核态：`printk` + gettext 多语言内核消息
- 两端通过同源错误码保持一致
