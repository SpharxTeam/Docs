...
# AirymaxOS eBPF 可编程探针

> **文档定位**: AirymaxOS（agentrt-linux）可观测性体系 L2 层——可编程内核探针 eBPF 的工程规范
> **版本**: 0.1.1（占位）/ 1.0.1（开发）
> **最后更新**: 2026-07-06
> **同源映射**: agentrt E-2 可观测性 + Linux 6.6 eBPF/BTF/kfunc
...
- [第 10 章 AirymaxOS Token 能效监控与 Agent 行为追踪](#第-10-章-airymaxos-token-能效监控与-agent-行为追踪)
...
eBPF（extended Berkeley Packet Filter）是 Linux 6.6 内核基线提供的可编程探针框架。它允许在内核态运行经过验证器验证的字节码程序，无需修改内核源码或加载内核模块。AirymaxOS 选择 eBPF 作为可观测性 L2 层，原因有三：
...
**OS-OBS-011: eBPF 是 AirymaxOS 可观测性 L2 层的强制基线，所有动态可编程观测必须经 eBPF 实现，不得自行编写内核模块。**
...
**OS-OBS-013: AirymaxOS 自定义 kfunc 必须用 `__bpf_kfunc` 宏标注，并通过 `BTF_KFUNCS_START`/`BTF_KFUNCS_END` 注册；未注册的内核函数不得被 BPF 程序调用。**
...
BPF helper 是早期内核函数调用接口，签名固定、有限集；kfunc 是新一代机制，签名通过 BTF 描述、可扩展。Linux 6.6 内核基线推荐新代码使用 kfunc。AirymaxOS 遵循此约定：新观测 kfunc 优先，仅当无对应 kfunc 时退回 helper。
...
**OS-OBS-014: AirymaxOS BPF 程序访问变长 Agent payload 必须使用 dynptr（`bpf_dynptr_from_skb`、`bpf_dynptr_slice`），不得直接对裸指针做算术运算。**
...
**OS-OBS-015: AirymaxOS BPF 程序必须通过 `-O2 -g -target bpf` 编译，确保验证器能正确追踪寄存器状态；禁用 `-O0`（验证器会因复杂控制流拒绝）。**

**OS-KER-015: airymaxos-kernel 必须开启 CONFIG_BPFVerifier_STATE_MARK 验证器状态标记能力，确保复杂 BPF 程序（如 Agent 决策追踪）能通过验证。**

### 6.3 验证器日志

加载失败时验证器返回详细日志，可通过 `bpf_attr.log_buf` 获取。验证器日志含程序名、失败指令、错误原因三要素。AirymaxOS 规定所有 BPF 加载失败必须将验证器日志落审计至 `/var/log/airymaxos/bpf-load.log`，便于事后排查。
...
Linux 6.6 内核基线在 `include/uapi/linux/bpf.h` 定义了 30+ 种 BPF 程序类型。AirymaxOS 可观测性主要使用以下四种：
...
kprobe 挂载于函数入口，kretprobe 挂载于函数返回。kretprobe 通过 `bpf_get_func_ip()` 与 `bpf_get_func_arg()` 获取返回值。AirymaxOS Agent 行为追踪同时使用两者：kprobe 记录决策入口，kretprobe 记录决策耗时与结果。
...
tracepoint 程序读取 tracepoint 参数需经 BTF 重定位；raw_tracepoint 性能更高但需手动解析参数。AirymaxOS 在高频路径（Agent 决策）使用 raw_tracepoint，低频路径（配置变更）使用 tracepoint。
...
Linux 6.6 内核基线提供 30+ 种 BPF map 类型。AirymaxOS 可观测性常用以下几种：
...
| `BPF_MAP_TYPE_LRU_HASH` | `hashttb.c` | LRU 淘汰哈希 |
...
eBPF 程序从源码到运行经历五步：编译（clang -target bpf）、加载（bpf(BPF_PROG_LOAD)）、attach（kprobe/tracepoint/perf_event 挂载）、运行、卸载。AirymaxOS 推荐使用 libbpf 框架管理生命周期：
...
**OS-OBS-018: AirymaxOS 必须为每个 BPF 程序实现显式卸载路径（`bpf_link__destroy`），不得依赖进程退出隐式卸载；Agent 重载策略时必须先卸载旧 BPF 程序再加载新版本。**
...
## 第 10 章 AirymaxOS Token 能效监控与 Agent 行为追踪

### 10.1 Token 能效监控

AirymaxOS 通过 eBPF 实现 Token 能效监控：kprobe 挂载 Agent 决策入口，kretprobe 挂载出口，两者时间差为决策耗时；同时通过 `bpf_agent_token_usage_get` kfunc 读取 Token 消耗，计算能效比（token/秒）：
...
### 10.3 与 MicroCoreRT 协同

MicroCoreRT 极简内核契约要求所有 BPF 程序代码体积最小化。AirymaxOS BPF 程序单文件指令数不得超过 1 万，编译后 .o 体积不得超过 64 KB。该约束体现 IRON-9 同源但独立原则——与 agentrt 用户态观测同源（语义共享 trace_event 头），但实现独立（内核态 BPF 字节码 vs 用户态进程）。

**OS-KER-020: AirymaxOS BPF 程序编译后 .o 体积必须 < 64 KB，单程序指令数必须 < 1 万；超限视为违反 MicroCoreRT 极简契约。**
...
| **K-4 可插拔策略** | BPF 程序可动态加载/卸载，策略运行时切换 |
| **A-4 完美主义** | 验证器全路径分析，BTF 类型安全，dynptr 边界检查 |
| **C-3 记忆卷载** | BPF map 追踪 L1→L4 记忆演化命中率与淘汰率 |
| **M-1 极境内核** | eBPF 不增加内核代码体积，扩展逻辑在验证器边界外 |

AirymaxOS 在 Linux 6.6 内核基线上严格遵循五维正交 24 原则——每条 OS-OBS 规则都可追溯至至少一条五维原则。IRON-9 同源但独立原则要求 eBPF 的内核态实现与 agentrt 用户态可观测性保持语义同源、二进制独立。两端通过 MicroCoreRT 极简内核契约与 AgentsIPC 消息协议实现无适配层互操作。
...
|------|----------------|------------------|
| 探针入口 | agentrt hook 函数 | kprobe/kretprobe BPF 程序 |
| 数据存储 | 用户态 ring buffer | BPF_MAP_TYPE_RINGBUF |
| 类型安全 | Rust 类型系统 | BTF + 验证器 |
| 函数调用 | 用户态 RPC | kfunc |
| 安全边界 | 进程隔离 | 验证器静态分析 |
| 上报协议 | AgentsIPC | AgentsIPC（同协议） |

**OS-STD-015: AirymaxOS eBPF 与 agentrt hook 共享 AgentsIPC 128B 消息头协议，两端事件格式必须一致，便于跨态聚合分析。**

agentrt 的 `commons/metrics` 模块定义了 `agentrt_metrics_record(name, value)`，与内核 BPF map 更新同源——两者都遵循"键值累加 + 周期聚合"模式。IRON-9 同源但独立原则在此体现为：语义同源（都是结构化指标采集），实现独立（用户态 metrics 库 vs 内核态 BPF map + verifier）。

```mermaid
graph LR
    A[agentrt 用户态 hook] --> B[AgentsIPC 上报]
    B --> C[AirymaxOS eBPF ringbuf]
    D[内核态 BPF kprobe] --> C
    C --> E[user_events / ringbuf 统一消费]
    style C fill:#fde68a,stroke:#b45309
```

MicroCoreRT 极简内核契约要求：内核态 BPF 程序不解析用户态事件 payload，仅按 trace_event 头透传；用户态守护进程负责跨态聚合与能效计算。
...
**内核源码**：Linux 6.6 `kernel/bpf/{syscall.c,verifier.c,helpers.c,ringbuf.c,btf.c,hashtab.c,arraymap.c}`；`include/uapi/linux/bpf.h`；`Documentation/bpf/{verifier.rst,btf.rst,kfuncs.rst,maps.rst,programs.rst}`。
...
| 1.0.1 | 2026-07-06 | 开发版：补充 Token 能效与 Agent 行为追踪实现 |

**OS-STD-016: 文档中引用的 BPF 程序类型、map 类型、kfunc 名必须与 `/home/spharx/SpharxWorks/01Reference/kernel-OLK-6.6/include/uapi/linux/bpf.h` 保持一致；上游变更时本文档必须同步更新。**

**OS-STD-017: OS-KER / OS-STD / OS-OBS 规则编号一经分配不得复用；废弃规则标记 `DEPRECATED` 但保留编号。**

**维护责任**：文档负责人为 AirymaxOS 可观测性工程组；代码负责人为 airymaxos-kernel BPF 维护者；每个 LTS 小版本发布前重新核对 BPF 接口与规则编号有效性。

---

> **文档结束** | AirymaxOS eBPF 可编程探针 v0.1.1 / 1.0.1
