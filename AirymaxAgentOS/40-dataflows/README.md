...
AirymaxOS 数据流程设计层聚焦于「数据如何流动」，是「接口设计层（30-interfaces）」的动态补充。本文档体系以 Linux 6.6 为内核基线，围绕 8 子仓之间的数据流动路径，刻画 4 大核心数据流：
...
4. **调度数据流（Scheduling Flow）**：任务提交 → SCHED_AGENT → EEVDF → 执行，基于 sched_ext（AirymaxOS 内核增强，主线 6.12+）实现可插拔调度策略，落地于 `airymaxos-kernel`。
...
AirymaxOS 数据流设计与 agentrt 数据流保持「同源但独立」（参考 ADR-010 + IRON-9）：

| AirymaxOS 数据流 | 同源 agentrt 模块 | 同源语义 | AirymaxOS 增强 |
|---|---|---|---|
...
4. **独立演进**：AirymaxOS 在内核态实现（kthread + BPF + io_uring），agentrt 在用户态实现，二者独立演进但保持语义对齐。
...
数据流设计随 AirymaxOS 版本迭代演进，遵循「契约稳定 + 实现可变」原则：

| 版本 | 数据流特征 | 关键能力 |
|---|---|---|
| 0.1.1（占位） | 4 大数据流框架定义 | 文档体系建立 |
...
- [AirymaxOS 总览](../README.md)：5 层文档体系与 8 子仓清单
