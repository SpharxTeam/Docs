...
> **文档定位**: AirymaxOS（agentrt-linux，极境智能体操作系统）工程标准规范第 2 卷——代码格式。本卷规定缩进、行宽、大括号、空格、switch/case、空行与多语句等机械格式规则，并以"配置即代码"形式由 `.clang-format` / `.rustfmt.toml` / `.pyproject.toml` / `.prettierrc` 强制执行。
> **版本**: 1.0.1（开发）
> **最后更新**: 2026-07-06
...
本卷回答"代码长什么样"——纯机械的格式规则。它不讨论"该怎么写代码"（01 代码规范）、"该怎么思考"（03 代码风格）或"工程体系怎么建立"（04 工程思想），只规定视觉层面的可由工具 100% 自动化的规则。AirymaxOS 的格式哲学是：
...
| 行尾空白 | 行末 Tab 或空格序列；AirymaxOS 全部禁用 |
...
缩进是控制流块边界的视觉表达。AirymaxOS 内核 C 代码严格沿用 Linux 6.6 内核基线 8 字符 Tab——这是 30+ 年沉淀的可读性经验，任何"4 空格更美观"的论调在内核上下文中都是噪声。
...
config AIRYMAXOS_IPC
	bool "AirymaxOS AgentsIPC subsystem"
	depends on NET
	help
	  Enable the AirymaxOS AgentsIPC subsystem (128B fixed header).
...
K&R 大括号放置是 Linux 6.6 内核基线的工程惯例，AirymaxOS 完全继承。其核心价值是**最小化空行占用**——在 25 行终端屏幕时代确立，至今仍是代码密度与可读性的最佳平衡。
...
**OS-KER-021**：禁止一行内多个赋值；每个赋值独占一行。AirymaxOS 内核代码风格简单——避免技巧性表达。
...
**OS-STD-207**：airymaxos-kernel / airymaxos-security / airymaxos-services 仓库根目录必须维护 `.clang-format`（基于 Linux 6.6 内核基线 689 行版本 + ForEachMacros 列表 + AirymaxOS 专属宏扩展）。
...
AirymaxOS `.clang-format` 在 Linux 6.6 内核基线 689 行配置基础上扩展。下表列出关键项及其与第 1-7 章规则的对应关系：
...
| `ForEachMacros` | 560 项 | — | 内核 for_each 宏列表，AirymaxOS 扩展 `agentrt_for_each_*` |
...
| 1.0.1 | TBD | 与代码实现同步验证；ForEachMacros 扩展至包含 AirymaxOS 专属宏 |
