...
# agentrt-liunx（AirymaxOS）工程标准规范手册

> **文档定位**: 交付成果 #3——工程标准规范手册。本文档是 agentrt-liunx（AirymaxOS，极境智能体操作系统）工程标准的执行依据，汇总 7 主题文档的核心规范条目、SSoT 规则编号注册表、IRON-9 v2 三层共享模型、合规性禁词清单与乔布斯美学审查标准，作为后续 M1 开发阶段的强制执行依据。
> **版本**: 0.1.1（文档体系完成）/ 1.0.1（开发）
> **最后更新**: 2026-07-08
> **理论根基**: Linux 6.6 内核基线工程思想 + Airymax 体系并行论（Multibody Cybernetic Intelligent System）+ seL4 微内核设计思想 + openEuler 工程标准规范参考
> **配套文档**: [comprehensive-review-report.md](../_review_0.1.1/comprehensive-review-report.md)（交付成果 #1）、[revision-changelog.md](../_review_0.1.1/revision-changelog.md)（交付成果 #2）
...
agentrt-liunx 工程标准由 7 主题文档构成，本手册为其总索引与执行依据：
...
- **适用对象**：agentrt-liunx 全部 8 子仓（airymaxos-kernel/services/security/memory/cognition/clouds/system/tests）
...
agentrt-liunx 与 agentrt 之间的代码共享与语义同源遵循 IRON-9 v2 三层模型：

| 层次 | 标注 | 共享程度 | 共享内容 | 共享方式 |
|------|------|---------|---------|---------|
| **共享契约层** | [SC] | 完全共享头文件 | 4 个头文件（见 §4.2） | `include/airymax/` 目录，两端 `#include` 同一头文件 |
| **语义同源层** | [SS] | 语义一致，实现独立 | MicroCoreRT 调度语义 / AgentsIPC 128B 消息头 / Cupolas 安全模型 / CoreLoopThree 三层循环 | 两端独立实现，通过行为契约测试验证语义一致 |
| **完全独立层** | [IND] | 完全独立 | agentrt-liunx 专属：内核驱动框架、Kbuild、内核内部 API；agentrt 专属：跨平台用户态运行时、SDK 四语言 | 各自独立仓库，无依赖关系 |
...
| 边界 | 共享策略 | 说明 |
|------|---------|------|
| agentrt ↔ agentrt-liunx | [SC] 共享契约层 4 个头文件 | 唯一的代码共享通道 |
| agentrt-liunx ↔ Euler/OLK-6.6 | 仅技术参考，不共享代码 | 参考 Euler 工程思想与实现方式，不共享代码 |
...
以下特性属于 Linux 7.0 专属，禁止在 agentrt-liunx 文档、代码、注释中引用为 Linux 6.6 原生能力：
...
| `sched_ext（Linux 6.15+）` | `sched_ext（OLK-6.6 内置）` | OLK-6.6 已内置 SCHED_EXT=7 |
...
| agentrt-liunx | AirymaxAgentOS 极境智能体操作系统 | agentrt-liunx / AirymaxOS | 操作系统发行版、智能体操作系统 |

#### 6.2.2 命名规则

- **正式英文名**：全称使用 "AirymaxAgentOS"（无空格），简称使用 "AirymaxOS"
- **正式中文名**：全称使用 "极境智能体操作系统"
- **代码仓库**：仓库名为 "agentrt-linux"（注意：目录名与文档中使用 "agentrt-liunx" 作为产品代号）
- **文档表述**：首次出现使用全称 "agentrt-liunx（AirymaxOS，极境智能体操作系统）"，后续使用简称 "agentrt-liunx" 或 "AirymaxOS"

### 6.3 openEuler 表述清零规范

| 范围 | 规范 | 说明 |
|------|------|------|
| 开源文档（docs/AirymaxAgentOS/） | ✅ 禁止出现 openEuler 表述 | 100% 清零，防止抄袭争议 |
| 闭源文档（docs-closed/） | 允许作为技术参考引用 | 内部参考，不公开 |
| 代码注释 | ✅ 禁止出现 openEuler 表述 | 防止开源后引发争议 |
| 与 Euler 项目的关系 | 仅技术参考，不共享代码 | 符合 2026-07-07 用户决策 |
...
| **微内核**（micro-kernel） | agentrt-liunx（OS 发行版） | Microkernel | agentrt-liunx 基于 Linux 6.6 进行微内核化改造，将 VFS/网络栈/部分驱动移到用户态 |
...
| 描述 agentrt-liunx 中的 MicroCoreRT | 微核心运行时 | ~~微内核运行时~~ | agentrt-liunx 虽是 OS，但 MicroCoreRT 源自 agentrt，跨两端统一称"微核心运行时" |
...
| 正确表述 | 错误表述 | 说明 |
|---------|---------|------|
| agentrt-liunx 基于 Linux 6.6 进行微内核化改造 | ~~agentrt-liunx 是从零开发的微内核~~ | 不是从零开发，是基于 Linux 改造 |
...
agentrt-liunx 工程标准贯彻乔布斯美学——不仅用户可见的表面要完美，用户不可见的内部构造（代码结构、编码风格、实现模式、设计思想）也必须达到最高质量标准。**没有"用户看不到所以可以妥协"的隐藏瑕疵。**
...
| openEuler 清零 | `grep -riE "openEuler" docs/AirymaxAgentOS/` | 0 结果（排除审查报告） |
| 微核心概念一致 | `grep -r "微内核" docs/AirymaxAgentOS/ \| grep -i agentrt` | 0 结果（agentrt 不应使用"微内核"） |
| SSoT 编号一致 | 对照 07-maintainers-and-governance.md 第 10 章 | 所有编号引用与注册表一致 |
| Copyright 完整 | `grep -rL "Copyright (c) 2025-2026 SPHARX" --include="*.md" docs/AirymaxAgentOS/` | 0 结果 |
| 基线声明完整 | `grep -rL "Linux 6.6" --include="*.md" docs/AirymaxAgentOS/` | 仅顶层 README 可缺失 |
...
### 9.2 不移植的 openEuler 特有特性

以下 openEuler 特有特性不移植到 agentrt-liunx（2026-07-07 用户决策）：
...
| 第 2 周 | agentrt 微内核原语→微核心原语 | 中 | P1-12 |
