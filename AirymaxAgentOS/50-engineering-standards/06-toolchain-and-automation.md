...
> **文档定位**: AirymaxOS（agentrt-linux，极境智能体操作系统）工程标准规范第 6 卷——工具链与自动化。本卷规定 7 层自动化验证体系、编译期/静态/动态分析工具链、格式化与风格检查、测试框架、覆盖率门槛、CI/CD 流水线与 24 项提交检查清单。
> **版本**: 1.0.1（开发）
> **最后更新**: 2026-07-06
...
本卷是 AirymaxOS 工程标准 7 主题文档中的第 6 卷，回答"规范怎么被执行"这一问题。它在 05 开发流程（补丁生命周期、审查响应）与 07 维护者制度（治理、DCO、成熟度模型）之间形成自动化执行层：
...
| 术语 | 定义 |
|------|------|
| 7 层验证 | AirymaxOS 的核心自动化验证体系，从编译期到发布期共 7 层 |
| checkpatch | Linux 内核风格检查脚本（`scripts/checkpatch.pl`），AirymaxOS 沿用并扩展 |
...
AirymaxOS 在 Linux 6.6 内核基线工程工具链之上，构建了 7 层自动化验证体系。这是 AirymaxOS 区别于普通 Linux 发行版的核心机制——每一层都是一个独立的反馈闭环，任何一层失败都阻断后续流程。
...
- **OS-STD-061**：正式发布版本必须由 AirymaxOS 发布密钥 GPG 签名。
...
- AirymaxOS 在安全敏感且非热路径的子系统（如安全策略、文件系统解析）使用 Rust 内核模块。
...
### 3.6 AirymaxOS 专属：AgentsIPC 协议静态校验器
...
- `scripts/checkpatch.pl` 是 Linux 内核风格检查脚本，AirymaxOS 完全沿用并扩展。
- 三级报告：
  - **ERROR**：极可能错误的项
  - **WARNING**：需仔细审查的项
  - **CHECK**：需思考的项
...
### 6.5 AirymaxOS 专属：Agent 行为契约测试 + 模糊测试
...
- AirymaxOS 的 CI/CD 流水线基于 GitHub Actions。
...
本节是 Linux `Documentation/process/submit-checklist.rst` 的 AirymaxOS 等价物，扩展为 24 项强制检查。

### 9.1 Linux submit-checklist.rst 等价物

AirymaxOS 继承 Linux 内核的 24 项提交检查清单，并新增 AirymaxOS 专属检查项。
...
### 9.3 AirymaxOS 专属新增检查项

在 24 项基础检查之上，AirymaxOS 新增以下专属检查：
...
AirymaxOS 的工具链配置以代码形式存放在仓库中，确保所有开发者与 CI 使用一致配置。
...
- `/home/spharx/SpharxWorks/01Reference/kernel-OLK-6.6/Documentation/dev-tools/testing-overview.rst`
- `/home/spharx/SpharxWorks/01Reference/kernel-OLK-6.6/Documentation/dev-tools/kunit/index.rst`
- `/home/spharx/SpharxWorks/01Reference/kernel-OLK-6.6/Documentation/dev-tools/kselftest.rst`
...
> **文档结束** | AirymaxOS 工程标准第 6 卷 | 共 13 章节 + 规则编号汇总
