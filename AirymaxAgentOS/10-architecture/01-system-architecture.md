...
> **最后更新**: 2026-07-06
> **设计原则**: 微内核设计思想 + AirymaxOS 工程基线 + Airymax 同源性
...
### 1.2 AirymaxOS 工程基线（Linux 6.6 内核基线 / 下一代内核基线）

**核心原则**: 采用 AirymaxOS 自身的模块设计、技术规格、标准和规范

| 维度 | AirymaxOS 工程基线 | AirymaxOS 落地 |
|---|---|---|
...
| 测试 | AirymaxOS 系统级测试套件 | airymaxos-tests 实现集成测试框架 |
| AI 原生 | 认知中枢系统、超节点 OS | airymaxos-cognition + airymaxos-cloudnative |
| 架构支持 | x86, ARM, RISC-V, 鲲鹏, 飞腾 | airymaxos-kernel 多架构支持 |
| 社区治理 | SIG（Special Interest Group）| AirymaxOS 按子仓划分 SIG |

### 1.3 Airymax 同源性（与 agentrt 同源）

**核心原则**: agentrt 和 AirymaxOS 共享 Airymax 设计理念

| agentrt 模块 | AirymaxOS 同源模块 | 同源语义 |
|---|---|---|
...
└─────────────────────────┼───────────────────────────────────────┘
                          │ 同源天然适配
┌─────────────────────────▼───────────────────────────────────────┐
│                 AirymaxOS 用户态服务层 (airymaxos-services)     │
│  ┌─────────┬──────────┬──────────┬──────────┬─────────────┐   │
...
└─────────────────────────┬───────────────────────────────────────┘
                          │ 系统调用 (syscall)
┌─────────────────────────▼───────────────────────────────────────┐
│              AirymaxOS 微内核核心 (airymaxos-kernel)             │
│  ┌──────────┬──────────┬──────────┬──────────┬────────────┐   │
...
**关键改造**: 利用 AirymaxOS 内核增强的 sched_ext + eBPF + io_uring 实现微内核化，而不抛弃 Linux 内核的稳定性和硬件支持。

## 3. 与 agentrt 的同源关系

### 3.1 同源性定义

**同源** = agentrt 和 AirymaxOS 共享 Airymax 设计理念，但**不共享代码**。

| 维度 | 同源体现 |
|---|---|
| 设计理念 | MicroCoreRT/AgentsIPC/Cupolas/MemoryRovol/CoreLoopThree 的语义一致 |
| 接口语义 | 128B 消息头、capability 模型、记忆卷载语义一致 |
| 能力体系 | agentrt 用户态提供的能力，AirymaxOS 在 OS 层提供相同语义的能力 |
| 架构契合 | agentrt 的设计假设和 AirymaxOS 的实现假设一致 |

### 3.2 agentrt 在 AirymaxOS 上的运行模式
...
AirymaxOS 的 IPC 子系统 (airymaxos-kernel + airymaxos-services):
  - 内核原生支持 128 字节消息头（同源）
  - magic 0x41524531 作为内核识别码（同源）
  - 5 种 payload 协议作为内核原生协议（同源）

→ agentrt 在 AirymaxOS 上运行时，IPC 无需任何转换层，天然契合
```
...
| sched_ext | AirymaxOS 内核增强（主线 6.12+） | airymaxos-kernel（SCHED_AGENT）|
...
| Token 能效优化 | AirymaxOS Token 能效框架 2026 | airymaxos-cognition |
| 具身智能 Claw | Linux 6.15+ 内核 | airymaxos-cognition |
| DevStation | AirymaxOS 工程基线 | airymaxos-system |

## 5. 架构决策记录

| 决策 | 内容 | 日期 |
|---|---|---|
| ADR-001 | AirymaxOS 基于 Linux 内核，不从零开发微内核 | 2026-07-06 |
| ADR-002 | 采用微内核化改造策略，而非真正的微内核 | 2026-07-06 |
| ADR-003 | 全面采用 AirymaxOS 工程基线 | 2026-07-06 |
| ADR-004 | agentrt 和 AirymaxOS 同源但独立 | 2026-07-06 |
