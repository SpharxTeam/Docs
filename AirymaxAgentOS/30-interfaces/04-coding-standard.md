...
| 语言 | 命名空间前缀 | 示例 |
|------|-------------|------|
| C | `agentrt_` | `agentrt_sys_task_submit` / `agentrt_ipc_msg_hdr_t` |
...
| 示例 | 模块 | 动作 | 对象 |
|------|------|------|------|
| `agentrt_task_submit` | agentrt | submit | task |
| `agentrt_ipc_send` | agentrt | send | ipc |
| `agentrt_ipc_recv` | agentrt | recv | ipc |
| `agentrt_rovol_snapshot` | agentrt | snapshot | rovol（记忆卷载） |
| `agentrt_capability_request` | agentrt | request | capability |
| `agentrt_clt_phase_notify` | agentrt | notify | clt（CoreLoopThree）阶段 |
...
| 示例 | 用途 |
|------|------|
| `agentrt_ipc_msg.h` | IPC 消息头定义 |
| `agentrt_syscalls.h` | 系统调用接口 |
| `agentrt_errno.h` | 错误码定义 |
...
| 语言 | 类型命名风格 | 示例 |
|------|-------------|------|
| C | snake_case_t（`_t` 后缀） | `agentrt_ipc_msg_hdr_t` / `agentrt_task_desc_t` |
...
- 函数与变量: snake_case（`agentrt_ipc_send` / `payload_len`）。
- 类型: snake_case_t（`agentrt_ipc_msg_hdr_t`）。
- 宏与常量: UPPER_SNAKE_CASE（`AGENTRT_IPC_MSG_HDR_SIZE`）。
...
- 常量: UPPER_SNAKE_CASE（`AGENTRT_IPC_MSG_HDR_SIZE`）。
...
AGENTRT_API void log_write(int level, const char *fmt, ...);
AGENTRT_API void log_write_va(int level, const char *fmt, va_list ap);
```
...
- 所有系统调用与库函数必须检查返回值。
- 错误必须通过 `AGENTRT_E*` 错误码返回，禁止吞掉错误。
- 资源释放使用 `goto cleanup` 模式（C）或 `Drop`（Rust）。

```c
/* 正确：goto cleanup 模式 */
AGENTRT_API int do_work(void)
{
    int ret;
...
- 沙箱内 capability 受限，通过 `agentrt_sys_capability_request` 显式申请。
