...
> **文档定位**: AirymaxOS（agentrt-linux，极境智能体操作系统）性能工程体系主索引
...
AirymaxOS 性能工程体系是系统在高负载场景下稳定运行的工程保障。它继承 Linux 内核 30+ 年沉淀的性能工程哲学（perf 剖析 + sched_ext 可插拔调度 + MGLRU 内存回收 + io_uring 零拷贝 I/O），并在其上扩展智能体操作系统专属的 Token 能效工程、Agent 延迟 SLO、记忆卷载性能等。
...
170-performance/
├── README.md                       # 本文件
├── 01-cpu-performance.md           # CPU 性能（perf + sched_ext）
├── 02-memory-performance.md        # 内存性能（MGLRU + CXL）
├── 03-io-performance.md            # I/O 性能（io_uring）
...
agentrt 的性能基线与 AirymaxOS 同源：
- agentrt 用户态：`agentrt_metrics_record()` 性能指标采集
- AirymaxOS 内核态：`perf_event` + ftrace 性能剖析
...
- AirymaxOS 性能工程（OS 级 + Agent 专属性能）
- 两端独立演进，但通过同源指标保持互操作
