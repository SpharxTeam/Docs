...
> **文档定位**: AirymaxOS（agentrt-linux，极境智能体操作系统）发行版管理工程体系主索引
...
AirymaxOS 发行版管理体系是操作系统作为产品发布的工程保障。它继承 Linux 发行版 30+ 年沉淀的发行版管理哲学（版本号 + RPM 包 + ISO 镜像 + 仓库管理 + 签名验证），并在其上扩展智能体操作系统专属的 Agent 应用商店、记忆卷载分发、多架构镜像等。
...
agentrt 的版本管理与 AirymaxOS 同源：
- agentrt 用户态：语义化版本 + 0.1.1 唯一奠基版本
- AirymaxOS 内核态：0.1.1 占位 + 1.0.1 正式开发
- 两端版本号协同但独立
