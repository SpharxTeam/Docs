...
> **文档定位**: AirymaxOS（agentrt-linux，极境智能体操作系统）运维体系第 1 卷——部署工程。本文档规定从裸机到可用 Agent 工作负载的完整交付链路：RPM 包格式、dnf 包管理器、ISO 镜像制作、Kickstart 自动化安装、PXE 网络安装、系统初始化、12 daemons 部署、DevStation 部署、版本升级路径与回滚机制。
...
> **核心约束**: IRON-9 同源但独立——与 agentrt 同源语义，AirymaxOS 独立承担发行版部署责任
...
AirymaxOS 部署体系继承 Linux 6.6 内核基线沉淀的发行版交付哲学，并在其上扩展智能体操作系统的专属部署需求。完整链路覆盖五个阶段：**构建阶段**（源码 → RPM 包，rpmbuild + mock 隔离构建）→ **组装阶段**（RPM 仓库 → ISO 镜像，lorax + livemedia-creator）→ **交付阶段**（ISO / PXE / Kickstart → 裸机或虚拟机安装）→ **初始化阶段**（firstboot → sysctl 应用 → 12 daemons 启动）→ **运维阶段**（dnf 升级 + 回滚 + DevStation 接管）。
...
MicroCoreRT 是 Airymax 微核心运行时基座，在 AirymaxOS 内核态对其保持同源语义。部署体系必须保证 MicroCoreRT 契约在内核包（`airymaxos-kernel`）与服务包（`airymaxos-services-*`）两端同时落地：内核包提供 MicroCoreRT 内核态实现，服务包提供 MicroCoreRT 用户态适配层。任何一端缺失即视为部署不完整。

**OS-OPS-003**：`airymaxos-kernel` 与 `airymaxos-services-core` 的版本号必须在安装时强一致，dnf 版本锁禁止解耦两者。这是 IRON-9 同源但独立原则在部署层的硬约束——同源语义必须由同源版本承载。
...
AirymaxOS 采用 RPM 包格式作为唯一二进制交付单元。包命名遵循 `name-version-release.arch.rpm` 规范，并在 `name` 上分层：
...
Version:        0.1.1
Release:        1%{?dist}
Summary:        AirymaxOS 12 daemons userspace core
...
%description
AirymaxOS 12 daemons userspace core, IRON-9 同源但独立于 agentrt daemons.
...
**OS-OPS-004**：所有 RPM 包必须经 AirymaxOS 构建密钥 GPG 签名，`rpm --checksig` 必须在安装前由 dnf 自动执行；未签名或签名不匹配的包禁止安装（E-1 安全内生）。
...
AirymaxOS 使用 dnf 作为唯一包管理器。仓库分为四类：
...
[airymaxos-base]
name=AirymaxOS $releasever - Base
...
Kickstart 是 AirymaxOS 自动化安装的核心机制，文件由命令段、包段、脚本段构成：

```kickstart
# AirymaxOS-0.1.1-aarch64.ks
text
lang en_US.UTF-8
...
AirymaxOS 推荐分区方案将 Agent 记忆卷（L1 原始卷）独立挂载到 `/var/lib/agentrt`，与系统根分区隔离。这与 C-3 记忆卷载原则对齐——记忆数据独立于系统升级路径，便于回滚时不丢记忆。
...
[Unit]
Description=AirymaxOS 12 daemons aggregated target
...
agentrt 的 12 个 daemons 在 AirymaxOS 中以 systemd 服务运行，遵循 IRON-9 同源但独立原则——二进制名保持 `*_d` 后缀（同源），systemd 服务名采用 `agentrt-*.service` 格式（独立）。
...
[Unit]
Description=AirymaxOS Gateway Daemon (agentrt gateway_d)
...
DevStation 是 AirymaxOS 提供的 AI 辅助开发运维环境，提升开发者体验（A-3 人文关怀）。它由 `devstation-core`（自然语言交互入口）、`devstation-tools`（开发工具集成）、`devstation-ops`（运维工具集成）三个组件构成。

### 9.2 DevStation 部署模式

DevStation 支持两种部署模式：**侧车模式**（与生产 AirymaxOS 实例同机部署，仅监听 localhost）与**独立模式**（独立 AirymaxOS 实例，作为开发集群的统一入口）。
...
[Unit]
Description=AirymaxOS DevStation (AI dev/ops assistant)
...
AirymaxOS 推荐使用 btrfs 子卷或 LVM thin snapshot 实现根分区快照。升级前由 `airymaxos-upgrade-pre` 服务自动创建快照（`btrfs subvolume snapshot / /snapshots/pre-upgrade-<ts>`）；回滚时执行 `btrfs subvolume set-default` 指向旧快照并重启。
...
AirymaxOS 部署体系是 Airymax 五维正交 24 原则在交付链路的具体落地：
...
| **IRON-9 同源但独立** | 12 daemons 同源二进制 + 独立 systemd unit | §8.1 |
...
AirymaxOS 部署体系与 agentrt 遵循 IRON-9 同源但独立原则：**同源**——12 daemons 二进制名（`*_d`）与 agentrt 完全一致，AgentsIPC 128B 消息头协议两端共享，MicroCoreRT 极简内核契约两端共享，配置目录 `/etc/agentrt/` 两端共享语义；**独立**——AirymaxOS 独立承担发行版部署责任（RPM 打包、dnf 仓库、ISO 制作、Kickstart、PXE、systemd 集成、根分区快照回滚），这些都是 agentrt 用户态运行时不涉及的领域；**互操作**——agentrt 遵循其用户态部署（pip/npm 安装），AirymaxOS 遵循其发行版部署（RPM/dnf），两端通过同源二进制名与 AgentsIPC 协议实现无适配层互操作。

### 13.2 同源映射表

| 维度 | agentrt（用户态运行时） | AirymaxOS（发行版部署） |
|------|------------------------|------------------------|
...
**OS-OPS-024**：AirymaxOS 部署的 12 daemons 必须与同版本 agentrt 用户态 SDK 的 AgentsIPC 协议版本一致；版本不一致时 daemon 拒绝接受 SDK 连接并输出诊断码（IRON-9 同源但独立在协议层的强制）。
...
**参考材料**：`/home/spharx/SpharxWorks/01Reference/kernel-OLK-6.6/Documentation/admin-guide/initrd.rst`（initrd 机制）、`.../kernel-parameters.rst`（内核启动参数）、Linux 6.6 内核基线 systemd 集成实践。
