...
> **核心约束**: IRON-9 同源但独立——继承 Linux 设备模型语义，不耦合上游发行版实现
...
本文档覆盖 device/driver/bus/class 四元关系、probe/remove 生命周期、name/OF/ACPI 三源匹配、sysfs 导出、kref/kobject 引用计数五大主题。IRON-9 同源但独立体现为：内核态结构体与上游保持语义等价（确保可直接复用上游驱动代码）；用户态扩展（`agent_bus_type`、`devm_agentrt_*` 资源族）不进入上游内核，而是作为 AirymaxOS 专属扩展通过 `agentos/daemon/` 适配器接入。
...
同源映射追求"语义等价"——内核态的 `probe` 契约（前置条件、后置条件、所有权）在用户态 `agentrt_agent_module.probe` 中保持一致，但实现语言、内存模型（slab vs glibc/Rust `Box`）、并发模型（tasklet/workqueue vs pthread/async）、错误返回（`-Exxx` vs `agentos_error_t`）、注册接口（`driver_register()` vs `agentrt_agent_module_register()`）、资源托管（`devm_kzalloc` vs `devm_agentrt_*`）可以不同。
...
> **文档结束** | AirymaxOS 驱动模型 60 模块首篇 | IRON-9 同源但独立 | Linux 6.6 内核基线
