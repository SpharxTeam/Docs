...
agentrt-liunx（AirymaxOS）的 OS 层契约规范定义各组件间的交互协议，是 agentrt-liunx 内核态与用户态、进程与进程之间通信的"法律文书"。契约规范位于 agentrt-liunx 文档体系的 `Capital_Specifications/agentrt_contract/` 目录下，与 agentrt 用户态契约规范形成互补——前者定义 OS 层（内核级）契约，后者定义应用层（用户态运行时）契约。
...
### 2.1 [SC] 共享契约层：4 个共享头文件

[SC] 层是 agentrt 与 agentrt-liunx 代码字面一致的头文件库，位于 `include/airymax/` 目录。这 4 个头文件是 agentrt-liunx OS 层契约的"共享基础"：

| 头文件 | 全路径 | 定义内容 | 对应契约 |
|--------|--------|---------|---------|
| `bpf_struct_ops.h` | `include/airymax/bpf_struct_ops.h` | sched_ext BPF 调度器结构体操作（SCHED_AGENT 调度类共享） | syscall API 契约 |
| `memory_types.h` | `include/airymax/memory_types.h` | MemoryRovol 记忆卷载数据结构（L1-L4 层级定义） | syscall API 契约 |
| `security_types.h` | `include/airymax/security_types.h` | Cupolas capability 令牌结构与权限位图 | IPC 协议契约 |
| `cognition_types.h` | `include/airymax/cognition_types.h` | CoreLoopThree 认知循环状态机与阶段枚举 | syscall API 契约 |
...
本契约规范体系是 IRON-9 v2"同源且部分代码共享"原则在 OS 层契约的落地：[SC] 层 4 个头文件完全共享，[SS] 层语义一致但实现独立，[IND] 层完全独立。详见第 2 章 IRON-9 v2 契约分层。
