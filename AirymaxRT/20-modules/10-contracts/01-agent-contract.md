...
**最新**: 2026-06-09
**状态**: 维护中
**路径**: docs/AirymaxRT/20-modules/10-contracts/01-agent-contract.md
---
...
| 术语 | 简要定义 | 来源 |
|------|---------|------|
| 智能体 (Agent) | 具有认知能力的智能体 | [认知层设计](../../00-requirements/02-cognition-design-cn.md) |
| 契约 (Contract) | 机器可读的能力描述文件 | 本规范 |
| 调度官 (Dispatcher) | 负责选择智能体 (Agent) 执行任务的组件 | [认知层设计](../../00-requirements/02-cognition-design-cn.md) |
| 信任边界 (Trust Boundary) | 区分可信与不可信数据的界限 | [架构设计原则](../../10-architecture/00-architectural-principles.md) |
...
{
  "input_schema": {
    "$ref": "https://agentrt.org/schemas/user_intent.json"
  },
  "output_schema": {
    "$ref": "https://agentrt.org/schemas/analysis_result.json"
  }
}
```
...
> **注意**: 模型名称应与 `agentrt/manager/models.yaml` 中定义的名称一致。`system1` 对应 t1-f（快思考），`system2` 对应 t2（主思考）。实际运行时由 triple_coordinator 协调 t2/t1-f/t1-p 三组件，t1-p（专业思考/仲裁）由系统自动配置。
...
```json
{
  "extensions": {
    ...
    "custom_metadata": {
      "author": "agentrt-community",
      "license": "MIT",
      "homepage": "https://github.com/agentrt/product-manager"
    }
  }
}
```
...
```json
{
  ...
  "agent_id": "com.agentrt.product_manager.v1",
...
契约必须符合 JSON Schema 定义。Airymax 提供官方 Schema 文件 [`01-agent-contract-schema.json`](./01-agent-contract-schema.json),可通过任何 JSON Schema 验证器检查。
...
```bash
# 使用官方 CLI 工具验证
agentrt validate-contract ./my_agent/contract.json

# 使用在线验证器
curl -X POST https://agentrt.org/api/validate \
...
3. **模型存在性检查**: `models` 中的模型名称必须在 `agentrt/manager/models.yaml` 中存在
4. **权限合法性检查**: `required_permissions` 中的权限必须在安全策略中定义
5. **能力合理性检查**: 能力描述与实际功能是否一致，避免夸大或误导

### 4.3 验证工具

Airymax 提供命令行工具 `agentrt validate-contract`,用于快速验证契约文件：
...
# 详细输出模式
agentrt validate-contract ./my_agent/contract.json --verbose

# 仅检查语法，不检查语义
agentrt validate-contract ./my_agent/contract.json --syntax-only
```
...
- GitHub: https://github.com/agentrt/schemas
- Gitee: https://gitee.com/agentrt/schemas
...
- **阶段 1**: 制定 Agent Contract 规范，开发验证工具
- **阶段 2**: 开发参考实现 Agent，建立认证机制
- **阶段 3**: 支持动态团队组建，实现跨平台技能市场
...
| 引用规范 | 关系说明 |
|---------|---------||
| [架构设计原则](../../10-architecture/00-architectural-principles.md) | 本规范是架构原则在 Airymax 生态的具体实现，特别是 MicroCoreRT 微核心思想和安全内生原则 |
| [统一术语表](../../TERMINOLOGY.md) | 本规范使用的术语定义和解释，如 Agent、契约、调度官等 |
| [C&C++ 安全编程规范](../../50-engineering-standards/10-coding-style/C_Cpp_coding_style.md) | Agent 的实现应遵循安全编程规范，特别是在权限检查和输入验证方面 |
| [日志打印规范](../../50-engineering-standards/10-coding-style/coding_conventions.md) | Agent 运行时应遵循日志规范，记录关键操作和异常情况 |
...
[2] Airymax 认知层设计。../../00-requirements/02-cognition-design.md  
