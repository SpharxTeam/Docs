...
Airymax 的行动层通过执行单元 (Execution Unit) 来执行具体操作，例如调用 API、运行代码、操作文件等。**技能 (Skill)** 是可复用的执行单元，可以被安装、卸载和动态加载，为 Agent 提供具体能力。**技能市场 (Skill Market)** 是社区贡献技能的中心，用户可以通过 `agentrt skill install` 命令安装技能，并通过 `agentrt skill execute` 调用。
...
```json
{
  "extensions": {
    ...
    "license": "MIT",
    "homepage": "https://github.com/agentrt/github-skill",
    "repository": {
      "type": "git",
      "url": "https://github.com/agentrt/github-skill.git"
    },
...
契约必须符合官方 JSON Schema。Airymax 提供 [`02-skill-contract-schema.json`](./02-skill-contract-schema.json),可用任何 JSON Schema 验证器检查。
...
```bash
# 使用官方 CLI 工具验证
agentrt validate-skill ./my_skill/skill.json

# 详细输出模式
agentrt validate-skill ./my_skill/skill.json --verbose

# 仅检查语法，不检查语义
agentrt validate-skill ./my_skill/skill.json --syntax-only
```
