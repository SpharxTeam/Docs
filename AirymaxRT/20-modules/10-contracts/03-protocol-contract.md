...
| -32000..-32099 | Server error | 服务器自定义错误 (对应 `airy_err_t`) |
...
airy_err_t err = airy_sys_task_submit(input, strlen(input), 30000, &result);
...
```prometheus
# HELP airy_request_total Total number of requests
# TYPE airy_request_total counter
airy_request_total{method="llm.complete",status="success"} 1234
```
...
```python
# 伪代码示例
def map_error(airy_error):
    if airy_error == AIRY_EINVAL:
        return -32602  # Invalid params
    elif airy_error == AIRY_ENOENT:
        return -32601  # Method not found
    elif airy_error == AIRY_EPERM:
...
int main() {
    ...
    // 3. 关闭会话
    airy_sys_session_close(session_id);
