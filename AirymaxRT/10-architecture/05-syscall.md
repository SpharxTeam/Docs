...
**最新**: 2026-06-09
**状态**: 维护中
**路径**: docs/AirymaxRT/10-architecture/05-syscall.md
---
...
| 特性 | 说明 |
| :--- | :--- |
| **统一入口** | 单一系统调用接口 `airy_syscall_invoke()` |
...
1. **POSIX ABI 兼容性** [IEEE Std 1003.1]:
   ...
   Airymax 实现:
   - `airy_syscall_invoke()` → 类似 POSIX `syscall()`
   - `airy_err_t` → 类似 `errno`
   - JSON 参数 → 扩展的灵活接口
...
**乔布斯美学体现**:
- **极简主义**: 单一函数 `airy_syscall_invoke()` 封装所有功能
...
Airymax 系统调用接口的设计深刻体现了 **体系并行 (MCIS)** 与 **五维正交体系** 的设计思想，将运行时接口理论（借鉴操作系统接口理论）、安全模型、性能优化与用户体验系统整合：
...
- **极简主义** → 单一函数 `airy_syscall_invoke()` 封装所有功能，体现 **简约至上 (Simplicity First)** 原则
...
|--------------|----------|----------|----------|
| 统一入口 | `airy_syscall_invoke()` 单一函数 | 设计美学 | 简约至上 |
 ...
 */
airy_err_t airy_syscall_invoke(
...
> **[SC] 共享契约层约束**：任务提交涉及的**任务描述符 magic** 定义于 `include/airymax/sched.h`（[SC] 共享头文件），值为 `AIRY_TASK_MAGIC`（`0x41475453`，ASCII 'AGTS'）。该 magic 独立于 IPC 消息头 magic（`0x41524531` 'ARE1'），用于校验任务描述符完整性。详见 [IRON-9 v2 跨端共享模型](./06-iron9-shared-model.md) §3.3。
...
char* task_id;
airy_err_t err = sys_task_submit(task_json, &task_id);
if (err == AIRY_EOK) {
...
char* record_id;
airy_err_t err = sys_memory_write(memory_json, &record_id);
```
 ...
 * @return airy_err_t
 */
airy_err_t sys_telemetry_metrics(
 ...
 * @return airy_err_t
 */
airy_err_t sys_telemetry_traces(
    const char* trace_id,
    char** traces_json);
```
...
> **SSoT 声明**：错误码权威定义位于 `agentrt/commons/include/airy_types.h`（`airy_err_t` 类型 + `AIRY_E*` POSIX 码）和 `agentrt/commons/utils/error/include/error.h`（`AIRY_ERR_*` 扩展码）。`include/airymax/error.h` 为规划中的 [SC] 共享路径，当前尚未创建。类型 `airy_err_t = int32_t` 定义于 `airy_types.h:41`。本节示例必须与 `docs/AirymaxRT/50-engineering-standards/120-cross-project-code-sharing.md` §2.1（13 个 `AIRY_E*` 宏，负值，对齐 POSIX errno 负值）逐字节一致。

```c
/* 权威源：agentrt/commons/include/airy_types.h + agentrt/commons/utils/error/include/error.h —— [SC] 实际权威源；include/airymax/error.h 为规划路径，当前尚未创建 */
typedef int32_t airy_err_t;        /* 错误码类型（int32_t） */
...
/* 任务相关错误（Airymax 专属，编号负值） */
#define AIRY_ETASK_NOT_FOUND     (-2001)
#define AIRY_ETASK_ALREADY_EXISTS (-2002)
#define AIRY_ETASK_CANCELLED     (-2003)

/* 记忆相关错误（Airymax 专属，编号负值） */
#define AIRY_EMEMORY_NOT_FOUND   (-3001)
#define AIRY_EMEMORY_WRITE_FAILED (-3002)
#define AIRY_EMEMORY_SEARCH_FAILED (-3003)

/* 会话相关错误（Airymax 专属，编号负值） */
#define AIRY_ESESSION_NOT_FOUND  (-4001)
#define AIRY_ESESSION_EXPIRED    (-4002)

/* 系统调用错误（Airymax 专属，编号负值） */
#define AIRY_ESYSCALL_UNKNOWN    (-5001)
#define AIRY_ESYSCALL_FAILED     (-5002)
```
 ...
 * @brief 获取错误码的人类可读描述
 */
const char* airy_strerror(airy_err_t err);
```

**示例**:
```c
airy_err_t err = sys_task_submit(params, &task_id);
if (err != AIRY_EOK) {
...
airy_cognition_process(input, &plan);
...
```c
// agentrt/atoms/syscall/memory_syscall.c
airy_err_t sys_memory_write(
    const char* memory_json,
    char** record_id) {
    ...
    airy_layer1_raw_write(g_layer1, 
...
- 启用 Debug 日志：`export AIRY_LOG_LEVEL=DEBUG`
- 使用 `strace` 跟踪 syscall 调用
- 监控 `/proc/sys/agentrt/stats`
