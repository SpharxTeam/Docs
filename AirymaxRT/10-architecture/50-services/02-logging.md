...
**最新**: 2026-06-09
**状态**: 维护中
**路径**: OpenAirymax/Docs/10-architecture/50-services/02-logging.md
...
Airymax 日志系统是系统可观测性的核心组件，遵循 **体系并行 (MCIS)** 与 **工程两论** 指导原则，通过四级结构化日志（L1→L4）实现从原始事件到高级洞察的全栈日志管理。作为系统诊断、性能分析与智能审计的关键基础设施，日志系统严格遵循 **五维正交体系** 的设计美学，将系统可观测性理论与实时流处理技术工程化为可执行的系统组件。

从 **体系并行** 视角分析，日志系统是 MCIS 中的 **可观测体 (Observability Body)**，负责系统状态的全面感知、记录与分析。其四级日志架构（L1原始日志→L2结构化日志→L3聚合指标→L4洞察报告）体现了 **渐进式抽象 (Progressive Abstraction)** 的思想，模拟了从原始数据到高级洞察的认知过程。

该系统不仅是 Airymax **系统观维度**（全局可观测性）的关键支撑，也是 **工程观维度**（性能与可靠性平衡）的经典实践。通过日志分级压缩与动态采样机制形成控制论负反馈回路，确保日志系统的可持续性与可扩展性，体现了 MCIS 中 **反馈调节 (Feedback Regulation)** 与 **自适应平衡 (Adaptive Balancing)** 的核心原理。同时，日志系统与 MicroCoreRT 微核心、CoreLoopThree、MemoryRovol 等核心组件的紧密集成，实现了对整个智能体系统状态的全面监控与智能分析。

## 1. 日志文件存储位置

### 1.1 集中式日志根目录

**位置**: `AgentRT/agentrt/heapstore/logs/`
...
| 原则 | 说明 |
|------|------|
| **集中存储** | 所有日志统一存放在 `agentrt/heapstore/logs/` 分区 |
...
Airymax 日志系统的设计深刻体现了 **体系并行 (MCIS)** 与 **五维正交体系** 的设计思想，将系统可观测性理论、分布式日志处理、实时分析与性能优化有机结合：

#### 设计依据：体系并行 (MCIS) 的可观测体映射
...
```c
// agentrt/daemon/agentrt/commons/include/svc_common.h (第 18 行)
#include "logger.h"  // 来自 agentrt/commons/utils/observability
```

**关键点**:
- `agentrt/daemon/` 下的所有用户态服务层进程通过 `svc_common.h` 复用 `agentrt/commons/utils/observability` 中的日志接口
...
```log
# agentrt/heapstore/logs/services/llm_d.log
...
# agentrt/heapstore/logs/services/tool_d.log
...
└─────────────────────────────────────────────────┘
                      ↓
┌─────────────────────────────────────────────────┐
│          服务层日志 (C - agentrt/daemon/)                │
│  - llm_d, tool_d, market_d, sched_d, monit_d   │
│  - 通过 svc_common.h 复用统一接口               │
└─────────────────────────────────────────────────┘
                      ↓
┌─────────────────────────────────────────────────┐
│          内核层日志 (C - agentrt/atoms/)                │
│  - CoreLoopThree, MemoryRovol, Syscall         │
│  - agentrt/commons/utils/observability/logger.h          │
└─────────────────────────────────────────────────┘
                      ↓
┌─────────────────────────────────────────────────┐
│          统一日志后端                            │
│  - 文件输出：agentrt/heapstore/logs/*                    │
│  - 格式：结构化 JSON + 人类可读文本             │
...
#### 核心接口：`agentrt/commons/utils/observability/include/logger.h`
...
#### 位置：`agentrt/toolkit/python/agentrt/logger.py`
...
class AgentRTLogger:
    ...
    def __init__(self, service_name: str):
        ...
        file_handler = logging.FileHandler(
            f'agentrt/heapstore/logs/services/{service_name}.log'
        )
...
#### 位置：`agentrt/toolkit/go/agentrt/logger.go`
...
func NewLogger(serviceName string) *Logger {
    ...
    // 文件输出
    file, _ := os.OpenFile(
        fmt.Sprintf("agentrt/heapstore/logs/services/%s.log", serviceName),
...
def execute(skill_name: str, params: dict, trace_id: str):
    logger = AgentRTLogger(skill_name)
...
```yaml
# agentrt/manager/logging/logging.yaml
version: 1
logging:
  level: INFO
  ...
  outputs:
    - type: file
      path: agentrt/heapstore/logs/services/*.log
      rotation: daily
      
    - type: opentelemetry
      endpoint: http://localhost:4317  # OTLP gRPC
      service_name: agentrt-services
...
**全局配置**: `agentrt/manager/logging/logging.yaml`
...
logging:
  ...
  services:
    llm_d:
      level: DEBUG
      file: agentrt/heapstore/logs/services/llm_d.log
      max_size_mb: 100
      retention_days: 7
      
    tool_d:
      level: INFO
      file: agentrt/heapstore/logs/services/tool_d.log
      
  kernel:
    level: WARN
    file: agentrt/heapstore/logs/kernel/agentrt.log
...
```yaml
# agentrt/manager/opentelemetry.yaml
receivers:
  otlp:
    protocols:
      grpc:
...
exporters:
  ...
  file:
    path: agentrt/heapstore/logs/aggregate.jsonl
...
# 统计各服务日志量
sum by (service) (rate({namespace="agentrt"}[1h]))
...
日志系统与 MicroCoreRT 微核心共同实现系统级的可观测性：
- **内核事件日志** → 记录 MicroCoreRT 微核心的关键操作（任务调度、内存分配、IPC通信）
- **性能指标采集** → 通过日志系统收集 MicroCoreRT 微核心的调度延迟、内存使用等关键指标
- **故障诊断** → 结构化日志帮助诊断 MicroCoreRT 微核心运行时的异常和性能瓶颈
...
✅ **集中存储**: `agentrt/heapstore/logs/` 统一分区管理  
✅ **模块独立**: 每个服务拥有独立日志文件  
...
logger = AgentRTLogger("my_app")
logger.info("Hello from Python")

// Go 模块
import "github.com/spharx/agentrt/toolkit/go/agentrt"
logger := agentrt.NewLogger("my_service")
...
**维护者**: Airymax 文档团队  
**相关文档**: 
- [原子日志接口](agentrt/commons/utils/observability/include/logger.h)
- [服务公共接口](agentrt/daemon/agentrt/commons/include/svc_common.h)
- [日志配置示例](agentrt/manager/logging/logging.yaml)
