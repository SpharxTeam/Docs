...
```sql
-- 创建只读用户
CREATE ROLE airy_readonly WITH LOGIN PASSWORD 'secure_password';
GRANT CONNECT ON DATABASE agentrt TO airy_readonly;
GRANT USAGE ON SCHEMA public TO airy_readonly;
GRANT SELECT ON ALL TABLES IN SCHEMA public TO airy_readonly;
