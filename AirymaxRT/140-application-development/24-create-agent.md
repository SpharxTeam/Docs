...
**最新**: 2026-06-09
**状态**: 维护中
**路径**: docs/AirymaxRT/140-application-development/24-create-agent.md
---
...
Agent 以用户态服务 (`_d`) 的形式运行在用户态，通过系统调用与内核交互，受到安全穹顶的保护。
...
- **双思考系统 (Thinkdual)**: 简单任务走 t1 快思考路径，复杂任务走 t2 慢思考路径

### 1.3 设计依据：MCIS视角的Agent创建

Agent 是 **体系并行 (MCIS)** 在 Airymax 中的具体实现。从 MCIS 视角理解 Agent 创建过程，有助于深入把握 Agent 的设计哲学与实现原理：
...
2. **内核观维度** → Agent 需要与微核心安全交互，遵循最小特权原则，通过安全穹顶进行资源隔离
3. **认知观维度** → Agent 需要实现双思考系统 (Thinkdual) 机制，支持 t1 快思考（快速直觉）和 t2 慢思考（深度分析）的协同工作
...
通过 MCIS 视角理解 Agent 创建，你将能够更系统、更深入地设计和实现高质量的 Agent。
...
| Airymax 内核 | v1.0.1 | v1.0.1.6 |
| Airymax CLI | v1.0.1 | v1.0.1.6 |

### 2.2 知识储备

- 熟悉 C11 语言标准
- 了解 Airymax 微核心架构（参见 [00-architectural-principles.md](../architecture/00-architectural-principles.md)）
- 理解 Agent Contract 契约（参见 [agent_contract.md](../specifications/10-contracts/01-agent-contract.md)）
 ...
 * @return 0 成功；负值 错误码
 */
int my_agent_init(my_agent_t* agent, const airy_config_t* manager);

/**
 * @brief 处理接收到的任务
 * 
 * 解析任务描述，选择执行路径（t1 快思考路径 / t2 慢思考路径）。
 * 
 * @param agent Agent 实例指针
 * @param task 任务描述
 * @param result 输出执行结果
 * @return 0 成功；负值 错误码
 */
int my_agent_handle_task(my_agent_t* agent, const airy_task_t* task, airy_result_t* result);
...
int my_agent_init(my_agent_t* agent, const airy_config_t* manager)
{
    ...
    // 初始化基类
    ret = airy_agent_init(&agent->base, "my_agent", MY_AGENT_VERSION);
    if (ret != 0) return ret;
    ...
    ret = airy_memory_init(&agent->base.memory, MEMORY_ROVOL, agent->data_dir);
    if (ret != 0) {
        airy_log_error("my_agent", "记忆初始化失败: %s", airy_strerror(ret));
        return ret;
    }

    airy_memory_enable_layer(&agent->base.memory, MEMORY_LAYER_L1);
    airy_memory_enable_layer(&agent->base.memory, MEMORY_LAYER_L2);

    airy_log_info("my_agent", "Agent 初始化完成, version=%s", MY_AGENT_VERSION);
    return 0;
}
 ...
 * @brief 双思考路径选择器
 */
static int classify_task_complexity(const airy_task_t* task)
{
 ...
 * @brief 处理任务 — t1 快思考路径
 */
static int handle_fast_path(my_agent_t* agent, const airy_task_t* task, airy_result_t* result)
{
    airy_trace_push("my_agent", "fast_path_start");
    ...
    airy_memory_result_t memories;
    int ret = airy_memory_search(&agent->base.memory, &query, &memories);
 ...
 * @brief 处理任务 — t2 慢思考路径
 */
static int handle_deep_path(my_agent_t* agent, const airy_task_t* task, airy_result_t* result)
{
    airy_trace_push("my_agent", "deep_path_start");

    // 1. 将任务描述转换为任务图 (DAG)
    airy_task_graph_t graph;
    int ret = airy_task_decompose(task, &graph);
    if (ret != 0) {
        airy_log_error("my_agent", "任务分解失败: %s", airy_strerror(ret));
        airy_trace_pop("my_agent");
        return ret;
    }
    ...
    for (size_t i = 0; i < graph.stage_count; i++) {
        ret = airy_task_execute_stage(&graph.stages[i], result);
        if (ret != 0) {
            airy_log_warn("my_agent", "阶段 %zu 执行失败，触发补偿", i);
            ret = airy_task_compensate(&graph.stages[i]);
            if (ret != 0) break;
        }

        // 轮次内反馈 — 根据执行结果调整后续规划
        if (i + 1 < graph.stage_count) {
            airy_task_replan(&graph, i, result);
        }
    }
    ...
    airy_memory_store(&agent->base.memory, &record);

    airy_task_graph_destroy(&graph);
    airy_trace_pop("my_agent");
    return ret;
}
...
int my_agent_handle_task(my_agent_t* agent, const airy_task_t* task, airy_result_t* result)
{
    ...
    airy_log_info("my_agent", "收到任务[%llu], 路径=%s",
    ...
    // 记录指标
    airy_metrics_counter("my_agent.tasks.total", 1);
    if (ret == 0) {
        airy_metrics_counter("my_agent.tasks.success", 1);
    } else {
        airy_metrics_counter("my_agent.tasks.failure", 1);
    }
    airy_metrics_histogram("my_agent.tasks.duration_ms", result->elapsed_ms);
...
int my_agent_shutdown(my_agent_t* agent)
{
    airy_log_info("my_agent", "Agent 关闭中, 活跃任务: %d", atomic_load(&agent->active_tasks));

    // 等待活跃任务完成（最多 30 秒）
    int wait_count = 0;
    while (atomic_load(&agent->active_tasks) > 0 && wait_count < 300) {
        airy_time_msleep(100);
        wait_count++;
    }

    if (atomic_load(&agent->active_tasks) > 0) {
        airy_log_warn("my_agent", "强制关闭，%d 个任务未完成", atomic_load(&agent->active_tasks));
    }

    // 持久化记忆
    airy_memory_flush(&agent->base.memory);

    // 释放资源
    free(agent->data_dir);
    airy_agent_destroy(&agent->base);

    airy_log_info("my_agent", "Agent 已关闭");
    return 0;
}
```
...
int main(int argc, char* argv[])
{
    ...
    // 加载配置
    airy_config_t manager;
    int ret = airy_config_load("ecosystem/manager/my_agent.yaml", &manager);
    if (ret != 0) {
    ...
    // 通过系统调用注册到内核
    ret = airy_syscall_agent_register(&agent.base);
    if (ret != 0) {
        fprintf(stderr, "Agent 注册失败: %s\n", airy_strerror(ret));
        return 1;
    }

    airy_log_info("my_agent", "Agent 已启动，等待任务...");

    // 主事件循环 — 通过 IPC 接收任务
    while (g_running) {
        airy_task_t task;
        ret = airy_syscall_task_recv(&task, 1000);

        if (ret == AIRY_ETIMEDOUT) continue;
        if (ret != 0) {
            airy_log_error("my_agent", "任务接收失败: %s", airy_strerror(ret));
            continue;
        }

        airy_result_t result = { 0 };
        ret = my_agent_handle_task(&agent, &task, &result);
        airy_syscall_task_complete(task.id, ret, &result);
    }
...
agentrt-cli agent register \
    ...
    --binary /usr/local/lib/agentrt/daemons/my_agent_d \
...
{
    my_agent_t agent = { 0 };
    airy_config_t manager = { .data_dir = "/tmp/test_agent" };
...
| 场景 | 推荐路径 | 原因 |
|------|---------|------|
| 简单查询、模式匹配 | t1 快思考 | 延迟敏感，模式已知 |
| 数据分析、报告生成 | t2 慢思考 | 需要深度推理和规划 |
| 紧急告警响应 | t1 快思考 → t2 慢思考 | 先快速响应，再深度分析 |
| 长周期任务 | t2 慢思考 + 增量规划 | 需要轮次内反馈 |

### 8.2 错误处理

```c
// 所有系统调用必须检查返回值
int ret = airy_syscall_memory_store(&record);
if (ret != 0) {
    airy_log_error("my_agent", "记忆存储失败: %s (errno=%d)", 
                      airy_strerror(ret), ret);
...
- [Agent 契约规范](../specifications/10-contracts/01-agent-contract.md)
- [架构设计原则](../architecture/00-architectural-principles.md)
- [Skill 创建指南](create_skill.md)
- [编码规范](../50-engineering-standards/10-coding-style/C_coding_style_standard.md)
- [系统调用 API](../10-architecture/05-syscall.md)
