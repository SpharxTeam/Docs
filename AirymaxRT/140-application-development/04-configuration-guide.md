...
providers:
  openai:
    ...
    api_key: "${AIRY_LLM_API_KEY}"  # 从环境变量读取
...
base:
  ...
  # 内核连接
  kernel_url: "${AIRY_KERNEL_URL:-http://localhost:8080}"
...
| 变量名 | 说明 | 默认值 | 必填 |
|--------|------|--------|------|
| `AIRY_IPC_BIND_ADDR` | IPC绑定地址 | 0.0.0.0 | ❌ |
| `AIRY_IPC_BIND_PORT` | IPC绑定端口 | 8080 | ❌ |
| `LOG_LEVEL` | 日志级别 | INFO | ❌ |
| `TZ` | 时区 | Asia/Shanghai | ❌ |
| `AIRY_CUPOLAS_ENABLED` | 启用安全穹顶 | true | ❌ |
| `AIRY_PERMISSION_MODE` | 权限检查模式 | strict | ❌ |
| `AIRY_SANITIZER_MODE` | 输入净化模式 | strict | ❌ |
| `AIRY_AUDIT_ENABLED` | 启用审计日志 | true | ❌ |
| `AIRY_LLM_PROVIDER` | LLM提供商 | openai | ❌ |
| `AIRY_LLM_API_KEY` | LLM API密钥 | - | ✅ |
| `AIRY_LLM_BASE_URL` | LLM API地址 | https://api.openai.com/v1 | ❌ |
| `AIRY_LLM_MODEL` | 默认LLM模型 | gpt-4-turbo | ❌ |
...
# 检查必需的环境变量
./scripts/check-env.sh --required AIRY_LLM_API_KEY,DATABASE_URL
```
