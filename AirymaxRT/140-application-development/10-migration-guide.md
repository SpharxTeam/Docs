...
引入了 `AIRY_API` 宏用于精确控制符号可见性：

```c
/* 所有公共函数声明 */
AIRY_API airy_err_t
airy_cognition_create(const airy_config_t* manager,
                         airy_cognition_engine_t** out_engine);

/* CMake 自动处理 AIRY_BUILDING_SHARED 宏 */
```
...
/* v1.0：结构化错误码 */
airy_err_t err = airy_cognition_create(NULL, &engine);
if (err != AIRY_EOK) {
    AIRY_LOG_ERROR("cognition_init",
        "Failed to create engine: %s (code=0x%04X)",
        airy_strerror(err), err);
    return err;
}
```
...
include_directories(
    ...
    "${CMAKE_CURRENT_SOURCE_DIR}/../agentrt/atoms/memoryrovol/include"
...
v1.0 将所有内核交互统一为 `airy_syscall_invoke()` 入口：

```c
/* v0.x：直接调用内核函数 */
airy_task_submit("process data", &task_id);

/* v1.0：通过系统调用接口 */
airy_syscall_req_t req = {
    .category = AIRY_SYS_TASK,
    .action   = AIRY_SYS_TASK_SUBMIT,
    .payload  = &(airy_task_submit_args_t){
        .description = "process data",
    }
};
airy_syscall_rsp_t rsp;
airy_err_t err = airy_syscall_invoke(&req, &rsp);
```
...
airy_memory_result_t* result = NULL;
err = airy_memory_query(memory, &query, &result);
...
if(BUILD_SHARED_LIBS)
    add_definitions(-DAIRY_BUILDING_SHARED)
endif()
...
| 变更类型 | 内容 | 影响 |
|----------|------|------|
| 新增 | `airy_memory_evolve()` 触发记忆进化 | 可选采用 |
| 改进 | HNSW 向量检索性能优化 30% | 自动生效 |
...
| 变更类型 | 内容 | 影响 |
|----------|------|------|
| 新增 | `airy_compensation_get_human_queue()` | 可选采用 |
| 改进 | 补偿事务可靠性增强 | 自动生效 |
...
valgrind --leak-check=full --error-exitcode=1 \
    ./build/bin/airy_test_integration
```
...
```bash
# 检查 CMake 配置输出中的 AIRY_BUILDING_SHARED
cmake --build build/ -- VERBOSE=1 | grep AIRY_BUILDING_SHARED
```

**解决**: 确保 `airy_rt.h` 在所有自定义头文件之前被包含

### 5.2 链接问题

**问题**: 找不到符号 `airy_some_function`

**诊断**: 检查符号是否使用 `AIRY_API` 导出

```bash
# Linux: 检查动态库导出符号
nm -D libairy_corekern.so | grep airy_some_function

# macOS: 检查动态库导出符号
nm -gU libairy_corekern.dylib | grep airy_some_function
```
...
| v1.1.0 | 2026-05-01 | 新增记忆进化功能，HNSW 检索优化 30% |
