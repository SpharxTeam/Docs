...
# 日志级别
AIRY_LOG_LEVEL=INFO
AIRY_LOG_FORMAT=text

# 追踪
AIRY_ENABLE_TRACING=true
AIRY_TELEMETRY_ENDPOINT=http://otel-collector:4317
```
...
services:
  agentrt-atoms:
    ...
    environment:
      - AIRY_ENV=production
      - AIRY_LOG_LEVEL=WARN
      - AIRY_LOG_FORMAT=json
      - AIRY_ENABLE_TRACING=true
      - AIRY_ENABLE_METRICS=true
      - AIRY_SECURITY_LEVEL=high
    healthcheck:
  ...
  postgres-prod:
    ...
    healthcheck:
      test: ["CMD-SHELL", "pg_isready -U agentrt -d airy_prod"]
      interval: 10s
      timeout: 5s
      retries: 5
      start_period: 60s
...
| 变量 | 说明 | 默认值 | 生产建议 |
|------|------|--------|---------|
| `AIRY_ENV` | 运行环境 | `development` | `production` |
| `AIRY_LOG_LEVEL` | 日志级别 | `INFO` | `WARN` |
| `AIRY_LOG_FORMAT` | 日志格式 | `text` | `json` |
| `AIRY_ENABLE_TRACING` | 启用追踪 | `true` | `true` |
| `AIRY_ENABLE_METRICS` | 启用指标 | `true` | `true` |
| `AIRY_TELEMETRY_ENDPOINT` | OTLP 端点 | `http://otel-collector:4317` | 同左 |
| `AIRY_DEPLOYMENT_TYPE` | 部署类型 | - | `production` |
| `AIRY_SECURITY_LEVEL` | 安全级别 | - | `high` |
...
| 变量 | 说明 | 默认值 |
|------|------|--------|
| `POSTGRES_PASSWORD` | PostgreSQL 密码 | `airy_dev`（开发） |
...
| 变量 | 默认值 | 说明 |
|------|--------|------|
| `AIRY_MAX_WORKERS` | 8 | 最大工作进程数 |
| `LLM_REQUEST_TIMEOUT` | 120 | LLM 请求超时（秒） |
| `AIRY_CACHE_SIZE_MB` | 512 | 缓存大小（MB） |
...
sudo sed -i 's|@AIRY_RUNTIME_DIR@|/var/run/agentrt|g' /etc/systemd/system/agentrt-*.service
...
Grafana 配置文件位于 `deploy/docker/monitoring/grafana_airy_dashboard.json`。
...
```bash
# 环境变量
AIRY_LOG_LEVEL=INFO      # DEBUG, INFO, WARNING, ERROR, CRITICAL
AIRY_LOG_FORMAT=json     # json 或 text
AIRY_STRUCTURED_LOGGING=false
```
...
AIRY_ENABLE_TRACING=true
AIRY_TELEMETRY_ENDPOINT=http://otel-collector:4317
AIRY_TRACING_SAMPLE_RATE=1.0    # 生产环境建议 0.1
```
...
```bash
# 备份 PostgreSQL 数据库
docker exec agentrt-postgres-prod pg_dump -U agentrt -d airy_prod \
...
0 2 * * * docker exec agentrt-postgres-prod pg_dump -U agentrt -d airy_prod \
  | gzip > /backup/airy_$(date +\%Y\%m\%d).sql.gz

# 保留最近 30 天的备份
0 3 * * * find /backup -name "airy_*.sql.gz" -mtime +30 -delete
```
...
# 2. 恢复数据库
docker exec -i agentrt-postgres-prod psql -U agentrt -d airy_prod \
...
# 1. 验证备份完整性
echo "[1/4] 验证备份文件..."
ls -lh /backup/airy_$(date +%Y%m%d).sql.gz

# 2. 在隔离环境恢复
echo "[2/4] 恢复数据库到测试实例..."
gunzip -c /backup/airy_$(date +%Y%m%d).sql.gz | \
  docker exec -i test-postgres psql -U agentrt -d airy_recovery_test

# 3. 验证数据完整性
echo "[3/4] 验证数据完整性..."
docker exec test-postgres psql -U agentrt -d airy_recovery_test \
...
# 设置环境变量
AIRY_JWT_SECRET=<generated-secret>
```
...
# 5. 验证数据库连接
docker exec agentrt-postgres-prod pg_isready -U agentrt -d airy_prod
