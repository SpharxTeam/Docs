...
**最新**: 2026-06-09
**状态**: 维护中
**路径**: docs/AirymaxRT/140-application-development/25-create-skill.md
---
...
### 1.3 设计依据：MCIS视角的Skill创建

Skill（技能）是 **体系并行 (MCIS)** 中 **执行体 (Execution Body)** 的具体实现与能力扩展。从 MCIS 视角理解 Skill 的创建与设计，有助于构建更加模块化、可重用、安全的执行能力单元。

#### Skill 作为执行体的能力单元

在 MCIS 框架中，Skill 对应 **执行体** 的具体能力实现：
...
在 MCIS 中，Skill 与 Agent 的关系体现了 **多体协同 (Multi-Body Collaboration)** 原理：
...
通过 MCIS 视角理解 Skill 创建，你将能够设计出更加模块化、可重用、安全的执行能力单元，为构建复杂的智能体系统奠定坚实基础。
...
typedef struct airy_skill airy_skill_t;
typedef struct airy_skill_input airy_skill_input_t;
typedef struct airy_skill_output airy_skill_output_t;

typedef int (*skill_execute_fn)(const airy_skill_input_t* input, airy_skill_output_t* output);
typedef int (*skill_validate_fn)(const airy_skill_input_t* input);
 ...
 */
airy_skill_t* file_reader_skill_create(void);
 ...
 * @param skill 技能实例指针
 */
void file_reader_skill_destroy(airy_skill_t* skill);
 ...
 * @brief 验证输入参数
 */
static int file_reader_validate(const airy_skill_input_t* input)
{
 ...
 * @brief 执行文件读取
 */
static int file_reader_execute(const airy_skill_input_t* input, airy_skill_output_t* output)
{
    uint64_t start = airy_time_now_ns();
    ...
    // 简化 JSON 解析 — 实际使用 airy_json_parse()
    ...
    // 4. 构建输出
    // 简化 JSON 构建 — 实际使用 airy_json_build()
    ...
    output->error_code = 0;
    output->elapsed_ms = (airy_time_now_ns() - start) / 1000000;
...
airy_skill_t* file_reader_skill_create(void)
{
    airy_skill_t* skill = calloc(1, sizeof(airy_skill_t));
...
{
    // 创建技能
    airy_skill_t* reader = file_reader_skill_create();
    if (!reader) return AIRY_ENOMEM;

    // 验证契约
    int ret = airy_skill_validate_contract(reader, "skill_contract.json");
    if (ret != 0) {
    ...
    // 注册到注册表
    ret = airy_skill_registry_register(reader);
    if (ret != 0) {
        file_reader_skill_destroy(reader);
        return ret;
    }
 ...
 * @brief 通过注册表调用技能
 */
int call_file_reader(const char* file_path, airy_skill_output_t* output)
{
    // 查找技能
    airy_skill_t* skill = airy_skill_registry_find("file_reader");
    if (!skill) return AIRY_ENOENT;

    // 准备输入
    airy_skill_input_t input = {
        .json_payload = "{\"path\":\"/tmp/data.csv\"}",
        .payload_size = 23,
        .trace_id = airy_trace_current_id(),
        .task_id = 0
    };
...
{
    airy_pipeline_t* pipeline = airy_pipeline_create("data_analysis");

    airy_pipeline_add_stage(pipeline, "file_reader", NULL);
    airy_pipeline_add_stage(pipeline, "csv_parser", NULL);
    airy_pipeline_add_stage(pipeline, "stat_analyzer", NULL);

    return airy_pipeline_register(pipeline);
}
```
 ...
 */
static int route_by_extension(const airy_skill_input_t* input, char* skill_name, size_t len)
{
    const char* ext = strrchr(input->json_payload, '.');
    if (!ext) return AIRY_EINVAL;
...
int execute_skill_async(const char* skill_name, const char* input_json,
                        airy_async_callback_t callback, void* user_data)
{
    airy_skill_t* skill = airy_skill_registry_find(skill_name);
    if (!skill) return AIRY_ENOENT;
    ...
    return airy_task_spawn((airy_task_fn)skill->execute, &input, callback, user_data);
}
```
...
CTEST(file_reader, validate_success)
{
    ...
    airy_skill_t* skill = file_reader_skill_create();
