...
**最新**: 2026-06-09
**状态**: 维护中
**路径**: docs/AirymaxRT/140-application-development/backup-recovery.md
---
...
pg_dump -h $DB_HOST -p $DB_PORT -U $DB_USER -d $DB_NAME \
    --format=custom \
        --compress=9 \
        --file="$BACKUP_DIR/airy_$DATE.dump"
...
# 验证备份文件
pg_restore --list "$BACKUP_DIR/airy_$DATE.dump" > /dev/null && echo "Backup valid"

# 加密备份（可选）
gpg --symmetric --cipher-algo AES256 --batch --passphrase "$ENCRYPTION_KEY" \
    --output "$BACKUP_DIR/airy_$DATE.dump.gpg" \
    "$BACKUP_DIR/airy_$DATE.dump"

# 清理未加密文件
rm "$BACKUP_DIR/airy_$DATE.dump"

# 保留最近 30 天备份
find "$BACKUP_DIR" -name "*.dump.gpg" -mtime +30 -delete

echo "[$(date)] PostgreSQL backup completed: airy_$DATE.dump.gpg"
```
...
groups:
  - name: backup_alerts
    rules:
      - alert: BackupFailed
        expr: airy_backup_success{job="agentrt-backup"} == 0
        for: 1h
        labels:
          severity: critical
        annotations:
      ...
      - alert: BackupStale
        expr: time() - airy_backup_timestamp_seconds > 86400
        for: 1h
        labels:
          severity: warning
        annotations:
      ...
      - alert: BackupSizeAnomaly
        expr |
          abs(airy_backup_size_bytes -
              avg_over_time(airy_backup_size_bytes[7d])) / avg_over_time(airy_backup_size_bytes[7d]) > 0.5
        labels:
          severity: warning
        annotations:
