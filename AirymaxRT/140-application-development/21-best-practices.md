...
```c
// ✅ 正确：通过统一框架接口管理生命周期
airy_fw_manager_t* mgr = airy_fw_manager_create();
airy_fw_init(mgr, AIRY_FW_AGENT);
airy_fw_start(mgr, AIRY_FW_AGENT);
...
circuit_breaker_config_t config = {
    ...
    .failover_strategy = AIRY_CB_FAILOVER_RETRY  // 失败后重试
};
```
...
```c
// ✅ 正确：使用内存池减少分配开销
airy_mem_pool_t* pool = airy_mem_pool_create(4096, 64);
void* buf = airy_mem_pool_alloc(pool, 256);
airy_mem_pool_free(pool, buf);
...
```c
// ✅ 正确：使用统一指标系统
airy_metrics_counter_inc("gateway_requests_total", 1, 
    "protocol", "jsonrpc", "method", "agent.list");
airy_metrics_histogram_observe("request_duration_seconds", 
    duration_ms / 1000.0,
    "protocol", "jsonrpc");
...
service_discovery_config_t config = {
    .strategy = AIRY_SD_LEAST_CONNECTION_WEIGHTED,
...
if (rc != AIRY_EOK) {
    AIRY_LOG_ERROR("IPC send failed: %s (rc=%d)", 
        airy_err_string(rc), rc);
    return rc;
}
