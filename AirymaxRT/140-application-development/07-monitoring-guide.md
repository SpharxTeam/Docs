...
**最新**: 2026-06-09
**状态**: 维护中
**路径**: docs/AirymaxRT/140-application-development/monitoring.md
---
...
| 指标名 | 类型 | 说明 | 正常范围 |
|--------|------|------|---------|
| `airy_ipc_requests_total` | Counter | IPC请求总数 | 持续增长 |
| `airy_ipc_request_duration_seconds` | Histogram | IPC请求延迟分布 | P99<50ms |
| `airy_ipc_errors_total` | Counter | IPC错误总数 | 错误率<0.1% |
| `airy_ipc_queue_length` | Gauge | 消息队列长度 | <1000 |

**关键查询**:

```promql
# IPC请求QPS（每秒请求数）
sum(rate(airy_ipc_requests_total[1m])) by (method, target)

# IPC P99延迟
histogram_quantile(0.99,
  sum(rate(airy_ipc_request_duration_seconds_bucket[5m])) by (le)
)

# IPC错误率
sum(rate(airy_ipc_errors_total[5m])) /
sum(rate(airy_ipc_requests_total[5m])) * 100

# 消息队列积压情况
airy_ipc_queue_length
```
...
| 指标名 | 类型 | 说明 | 告警阈值 |
|--------|------|------|---------|
| `airy_memory_allocated_bytes` | Gauge | 已分配内存总量 | >80%池大小 |
| `airy_memory_free_bytes` | Gauge | 可用内存量 | <20%池大小 |
| `airy_memory_allocations_total` | Counter | 分配次数 | - |
| `airy_memory_deallocations_total` | Counter | 释放次数 | - |
| `airy_memory_leak_detected` | Gauge | 检测到泄漏 | =1时告警 |

**关键查询**:

```promql
# 内存利用率
airy_memory_allocated_bytes /
(airy_memory_allocated_bytes + airy_memory_free_bytes) * 100

# 分配/释放比率（接近1表示正常）
sum(rate(airy_memory_allocations_total[5m])) /
sum(rate(airy_memory_deallocations_total[5m]))

# 内存碎片率
1 - (airy_memory_largest_free_block / airy_memory_free_bytes)

# 泄漏检测
airy_memory_leak_detected == 1
```
...
| 指标名 | 类型 | 说明 | 告警阈值 |
|--------|------|------|---------|
| `airy_tasks_created_total` | Counter | 创建任务总数 | - |
| `airy_tasks_completed_total` | Counter | 完成任务总数 | - |
| `airy_tasks_failed_total` | Counter | 失败任务总数 | 失败率>5% |
| `airy_tasks_active` | Gauge | 当前活跃任务数 | >最大并发*0.9 |
| `airy_task_queue_length` | Gauge | 任务队列长度 | >5000 |

**关键查询**:

```promql
# 任务吞吐量（完成速率）
sum(rate(airy_tasks_completed_total[5m]))

# 任务失败率
sum(rate(airy_tasks_failed_total[5m])) /
sum(rate(airy_tasks_created_total[5m])) * 100

# 平均任务执行时间
avg(airy_task_duration_seconds) by (priority)

# 队列积压时间估算
airy_task_queue_length / sum(rate(airy_tasks_completed_total[5m]))
```
...
| 指标名 | 类型 | 说明 | 告警阈值 |
|--------|------|------|---------|
| `airy_cupolas_permission_checks_total` | Counter | 权限检查总次数 | - |
| `airy_cupolas_permission_denied_total` | Counter | 权限拒绝次数 | 拒绝率突然升高 |
| `airy_cupolas_sanitization_blocked_total` | Counter | 净化拦截次数 | 异常增高 |
| `airy_cupolas_audit_events_total` | Counter | 审计事件总数 | - |
| `airy_cupolas_sandbox_violations_total` | Counter | 沙箱违规次数 | >0即告警 |

**安全事件查询**:

```promql
# 权限拒绝率
sum(rate(airy_cupolas_permission_denied_total[5m])) /
sum(rate(airy_cupolas_permission_checks_total[5m])) * 100

# 输入净化拦截率（按类型）
sum(rate(airy_cupolas_sanitization_blocked_total[5m])) by (attack_type)

# 最近1小时的沙箱违规事件
increase(airy_cupolas_sandbox_violations_total[1h])

# 审计事件趋势（检测异常活动）
sum(rate(airy_cupolas_audit_events_total[5m])) by (event_type, severity)
```
...
| 指标名 | 类型 | 说明 | 告警阈值 |
|--------|------|------|---------|
| `airy_llm_requests_total` | Counter | LLM API请求总数 | - |
| `airy_llm_request_duration_seconds` | Histogram | LLM推理延迟 | P99>30s |
| `airy_llm_tokens_used_total` | Counter | Token消耗总量 | 超预算 |
| `airy_llm_errors_total` | Counter | LLM调用错误 | 错误率>1% |
| `airy_llm_cost_usd_total` | Counter | 累计成本(USD) | 日预算超限 |

**LLM相关查询**:

```promql
# 平均推理延迟（按模型）
avg(airy_llm_request_duration_seconds) by (model)

# Token使用效率（输出Token/输入Token）
sum(rate(airy_llm_output_tokens_total[1h])) /
sum(rate(airy_llm_input_tokens_total[1h]))

# 成本追踪
sum(increase(airy_llm_cost_usd_total[1d]))

# 缓存命中率
airy_llm_cache_hits_total /
(airy_llm_cache_hits_total + airy_llm_cache_misses_total) * 100
```
  ...
  - name: agentrt-critical
    ...
    rules:
      ...
      - alert: IPCHighLatency
        expr: histogram_quantile(0.99, sum(rate(airy_ipc_request_duration_seconds_bucket[5m])) by (le)) > 1
        for: 5m
        labels:
          severity: warning
        annotations:
      ...
      - alert: IPCQueueBacklog
        expr: airy_ipc_queue_length > 5000
        for: 3m
        labels:
          severity: critical
        annotations:
      ...
      - alert: TaskFailureRateHigh
        expr: |
          sum(rate(airy_tasks_failed_total[5m])) /
          sum(rate(airy_tasks_created_total[5m])) * 100 > 10
        for: 5m
        labels:
          severity: critical
        annotations:
      ...
      - alert: TaskQueueBacklog
        expr: airy_task_queue_length > 3000
        for: 5m
        labels:
          severity: warning
        annotations:
      ...
      - alert: PermissionDeniedSpike
        expr: |
          sum(rate(airy_cupolas_permission_denied_total[5m])) /
          sum(rate(airy_cupolas_permission_checks_total[5m])) * 100 > 5
        for: 2m
        labels:
          severity: warning
        annotations:
      ...
      - alert: SanitizationBlockSpike
        expr: rate(airy_cupolas_sanitization_blocked_total[5m]) > 10
        for: 2m
        labels:
          severity: critical
        annotations:
      ...
      - alert: SandboxViolation
        expr: increase(airy_cupolas_sandbox_violations_total[5m]) > 0
        for: 0m
        labels:
          severity: critical
        annotations:
  ...
  "dashboard": {
    ...
    "panels": [
      ...
      },
      {
        ...
        "targets": [
          {
            "expr": "sum(rate(airy_ipc_requests_total[5m])) by (method)",
            "legendFormat": "{{method}}"
          }
        ],
      ...
      },
      {
        ...
        "targets": [
          {
            "expr": "histogram_quantile(0.99, sum(rate(airy_ipc_request_duration_seconds_bucket[5m])) by (le))"
          }
        ],
        "fieldConfig": {
          "defaults": {
...
# 5. 关键指标快照
echo "=== IPC QPS ==="
curl -s 'http://localhost:9090/api/v1/query?query=sum(rate(airy_ipc_requests_total[1m]))' | jq '.data.result[0].value[1]'

echo "=== Memory Usage % ==="
curl -s 'http://localhost:9090/api/v1/query?query=(airy_memory_allocated_bytes/(airy_memory_allocated_bytes+airy_memory_free_bytes))*100' | jq '.data.result[0].value[1]'

echo "=== Active Tasks ==="
curl -s 'http://localhost:9090/api/v1/query?query=airy_tasks_active' | jq '.data.result[0].value[1]'
