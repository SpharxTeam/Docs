...
│  │  agentrt/atoms/coreloopthree/src/prompt_loader.c              │ │
...
| 组件 | 路径 | 职责 |
|:------|:-----|:-----|
| **Prompt Loader** | `agentrt/atoms/coreloopthree/src/prompt_loader.c` | C 语言核心，从 YAML 模板加载并渲染 Prompt，支持变量替换和 JSON Schema 校验 |
...
YAML 模板文件
    │
    ▼
Prompt Loader 加载 (airy_prompt_template_load)
    │
    ▼
渲染变量替换 (airy_prompt_render)
    │
    ▼
...
// 加载模板
airy_prompt_template_t *tmpl = NULL;
airy_prompt_template_load("ecosystem/prompts/templates/system/coding_agent.yaml", &tmpl);

// 准备变量
airy_prompt_variable_t vars[] = {
...
// 渲染
airy_prompt_rendered_t *rendered = NULL;
airy_prompt_render(tmpl, vars, 6, &rendered);
...
// 清理
airy_prompt_rendered_free(rendered);
airy_prompt_template_free(tmpl);
```
...
```c
// airy_prompt_loader_config_t 默认值
typedef struct {
...
- [Airymax 架构文档](../10-architecture/01-system-architecture.md) — 系统整体架构和 MemoryRovol 四层记忆系统
