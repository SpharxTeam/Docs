...
**最新**: 2026-06-09
**状态**: 维护中
**路径**: docs/AirymaxRT/140-application-development/testing_guidelines.md
---
...
def test_task_submit_with_valid_input_returns_task_id(self):
    ...
    # Act - 执行被测操作
    result = airy_task_submit(agent_id, "process data", 30000)

    # Assert - 验证结果
    assert result.error_code == AIRY_EOK
...
class TestAgentCreation:
    def test_create_with_valid_config(self, sample_agent_config, mock_airy_client):
        result = mock_airy_client.agent_create(sample_agent_config)
...
async def test_concurrent_task_submission():
    ...
    assert all(r.error_code == AIRY_EOK for r in results)
```
...
@pytest.mark.parametrize("input_data,expected_status", [
    ("valid input", AIRY_EOK),
    ("", AIRY_EINVAL),
    (None, AIRY_EINVAL),
    ("a" * 10001, AIRY_EINVAL),
])
def test_task_submit_input_validation(input_data, expected_status):
    result = airy_task_submit(input_data, len(input_data) if input_data else 0, 30000)
...
def test_agent_invoke_with_mock(mocker):
    ...
    mock_execute.return_value = AIRY_EOK

    result = airy_agent_invoke("agent_0", "test input", 10, None)
    assert result == AIRY_EOK
...
def test_session_with_mock_persistence(mocker):
    ...
    mock_persist.return_value = AIRY_EOK

    session_id = airy_session_create(None)
...
static void test_ipc_channel_create_with_valid_params(void **state) {
    // Arrange
    airy_ipc_config_t config = {
        .type = IPC_CHANNEL_BUFFERED,
        .buffer_size = 4096
    };

    // Act
    airy_ipc_channel_t* ch = airy_ipc_channel_create(&config);

    // Assert
    assert_non_null(ch);
    assert_int_equal(airy_ipc_channel_get_type(ch), IPC_CHANNEL_BUFFERED);

    // Cleanup
    airy_ipc_channel_destroy(ch);
}
```
...
target_link_libraries(test_ipc airy_ipc cmocka)
...
```python
@pytest.fixture
def temp_agent(airy_client):
    agent_id = airy_client.create({"name": "temp"})
    yield agent_id
    airy_client.destroy(agent_id)
```
...
| [编码规范](../../50-engineering-standards/10-coding-style/Python_coding_style_standard.md) | Python 编码规范 |
| [C 编码规范](../../50-engineering-standards/10-coding-style/C_coding_style_standard.md) | C 编码规范 |
