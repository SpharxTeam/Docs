...
记忆管理系统调用提供 MemoryRovol 记忆系统的写入、搜索、获取和删除接口。记忆系统采用四层渐进抽象架构（L1→L4），使用 HNSW 索引进行语义检索。所有函数使用 `airy_sys_memory_*` 前缀。
...
| 函数 | 描述 |
|------|------|
| `airy_sys_memory_write()` | 写入记忆数据 |
| `airy_sys_memory_search()` | 语义搜索记忆 |
| `airy_sys_memory_get()` | 获取记忆记录 |
| `airy_sys_memory_delete()` | 删除记忆记录 |
 ...
 *
 * @ownership 调用者拥有 *out_record_id，通过 airy_sys_free() 释放
 * @threadsafe 是
 * @reentrant 否
 */
AIRY_API airy_err_t airy_sys_memory_write(const void *data,
                                                      size_t len,
                                                      const char *metadata,
                                                      char **out_record_id);
```
...
airy_err_t err = airy_sys_memory_write(
 ...
 * @return airy_err_t (AIRY_EOK=0 成功，负值=错误)
 *
 * @ownership 调用者拥有 *out_record_ids、*out_scores，通过 airy_sys_free() 释放
 * @threadsafe 是
 * @reentrant 是
 */
AIRY_API airy_err_t airy_sys_memory_search(const char *query,
                                                       uint32_t limit,
                                                       char ***out_record_ids,
                                                       float **out_scores,
                                                       size_t *out_count);
...
float *scores = NULL;
size_t count = 0;
airy_err_t err = airy_sys_memory_search(
 ...
 * @return airy_err_t (AIRY_EOK=0 成功，负值=错误)
 *
 * @ownership 调用者拥有 *out_data，通过 airy_sys_free() 释放
 * @threadsafe 是
 * @reentrant 是
 */
AIRY_API airy_err_t airy_sys_memory_get(const char *record_id,
                                                    void **out_data,
                                                    size_t *out_len);
```

### `airy_sys_memory_delete()`
 ...
 * @reentrant 否
 */
AIRY_API airy_err_t airy_sys_memory_delete(const char *record_id);
```
