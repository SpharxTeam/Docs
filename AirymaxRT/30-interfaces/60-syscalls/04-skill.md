...
Skill 系统与 Agent 系统协同工作：Agent 通过 `airy_agent_skill_bind()` 绑定 Skill 获得扩展能力，Skill 也可通过 `airy_sys_skill_execute()` 独立执行。这种设计遵循**关注点分离**原则——Agent 负责任务编排，Skill 负责具体能力实现。
...
| 函数 | 描述 | 复杂度 |
|------|------|--------|
| `airy_sys_skill_install()` | 安装 Skill | O(1) |
| `airy_sys_skill_execute()` | 执行 Skill | O(n) |
| `airy_sys_skill_list()` | 列出已安装 Skill | O(n) |
| `airy_sys_skill_uninstall()` | 卸载 Skill | O(n) |
 ...
 *
 * 描述已安装 Skill 的完整信息。
 * @ownership 调用者拥有返回指针，需通过 airy_sys_skill_list_free() 释放
 */
typedef struct airy_skill_entry {
    /** Skill 唯一标识符（系统自动生成） */
    char skill_id[64];
 ...
 * 定义 Skill 安装时的参数和选项。
 */
typedef struct airy_skill_install_config {
    /** Skill 来源 URL（必填） */
    const char* url;
 ...
 *
 * 描述 Skill 执行的完整结果信息。
 * @ownership 调用者拥有返回指针，需通过 airy_sys_skill_result_free() 释放
 */
typedef struct airy_skill_result {
    /** 执行状态码（0=成功，负值=错误） */
    int status;
 ...
 * @param out_skill_id [out] 返回分配的 Skill ID（调用者负责 airy_sys_free 释放）
 *
 * @return AIRY_EOK 成功
 ...
 *
 * @see airy_sys_skill_uninstall()
 * @see airy_sys_skill_list()
 */
airy_err_t airy_sys_skill_install(const char* skill_url, char** out_skill_id);
```
...
```c
char* skill_id = NULL;
airy_err_t err = airy_sys_skill_install(
 ...
 * @param out_output [out] 返回执行输出（调用者负责 airy_sys_free 释放）
 *
 * @return AIRY_EOK 成功
 * @return AIRY_EINVAL  参数无效（任何参数为 NULL）
 * @return AIRY_ENOMEM  内存不足
 * @return AIRY_ENOENT  Skill 未找到
 * @return AIRY_EPERM  Skill 无执行权限
 * @return AIRY_EBUSY   Skill 正在执行中
 *
 * @ownership 调用者拥有 *out_output，需通过 airy_sys_free() 释放
 * @threadsafe 是。内部使用互斥锁保护 Skill 列表
 ...
 *
 * @see airy_sys_skill_install()
 * @see airy_agent_skill_bind()
 */
airy_err_t airy_sys_skill_execute(const char* skill_id,
                                          const char* input,
                                          char** out_output);
```
...
// 1. 安装 Skill
char* skill_id = NULL;
airy_sys_skill_install("https://market.agentrt.dev/skills/code-review/v1.0.0", &skill_id);

// 2. 创建 Agent 并绑定 Skill
airy_agent_config_t config = {0};
strncpy(config.name, "code-reviewer", sizeof(config.name));
config.type = AGENT_TYPE_TASK;

char* agent_id = NULL;
airy_agent_create(&config, &agent_id);
airy_agent_skill_bind(agent_id, skill_id);

// 3. 通过 Agent 调用 Skill
char* output = NULL;
airy_agent_invoke(agent_id, "Review this code: ...", strlen("Review this code: ..."), &output);
airy_sys_free(output);

// 4. 清理
airy_agent_skill_unbind(agent_id, skill_id);
airy_agent_destroy(agent_id);
airy_sys_skill_uninstall(skill_id);
airy_sys_free(skill_id);
 ...
 *
 * @param out_skills [out] 返回 Skill ID 数组（调用者负责逐个 airy_sys_free 释放，然后释放数组本身）
 * @param out_count  [out] 返回 Skill 数量
 *
 * @return AIRY_EOK 成功
 * @return AIRY_EINVAL  参数无效（out_skills 或 out_count 为 NULL）
 * @return AIRY_ENOMEM  内存不足
 *
 * @ownership 调用者拥有 *out_skills 数组及其每个元素，需逐个 airy_sys_free() 释放元素后 airy_sys_free() 释放数组
 * @threadsafe 是。内部使用互斥锁保护 Skill 列表
 * @reentrant  否。使用全局 Skill 列表
 *
...
if (err == AIRY_EOK) {
    ...
    for (size_t i = 0; i < count; i++) {
        printf("  - %s\n", skills[i]);
        airy_sys_free(skills[i]);
    }
    if (skills) airy_sys_free(skills);
}
```

---

### airy_sys_skill_uninstall
 ...
 *
 * @return AIRY_EOK 成功
 * @return AIRY_EINVAL  参数无效（skill_id 为 NULL）
 * @return AIRY_ENOENT  Skill 未找到
 * @return AIRY_EBUSY   Skill 正在被 Agent 使用，无法卸载
 *
...
int main(void) {
    ...
    for (int i = 0; i < 3; i++) {
        err = airy_sys_skill_install(skill_urls[i], &skill_ids[i]);
        if (err != AIRY_EOK) {
    ...
    // 4. 执行 Skill
    char* output = NULL;
    err = airy_sys_skill_execute(skill_ids[0],
...
cleanup:
    for (int i = 0; i < 3; i++) {
        if (skill_ids[i]) {
            airy_sys_skill_uninstall(skill_ids[i]);
            airy_sys_free(skill_ids[i]);
        }
    }
...
char* agent_id = NULL;
airy_agent_create(&config, &agent_id);
airy_agent_skill_bind(agent_id, review_skill_id);
airy_agent_skill_bind(agent_id, lint_skill_id);

// 3. 通过 Agent 执行任务（Agent 自动选择合适的 Skill）
char* result = NULL;
airy_agent_invoke(agent_id,
...
// 4. 清理
airy_agent_skill_unbind(agent_id, review_skill_id);
airy_agent_skill_unbind(agent_id, lint_skill_id);
airy_agent_destroy(agent_id);
airy_sys_skill_uninstall(review_skill_id);
airy_sys_skill_uninstall(lint_skill_id);
airy_sys_free(review_skill_id);
airy_sys_free(lint_skill_id);
airy_sys_free(agent_id);
```
...
2. **内存管理**：`airy_sys_skill_install()` 和 `airy_sys_skill_list()` 返回的字符串需要调用者通过 `airy_sys_free()` 释放。`airy_sys_skill_uninstall()` 会自动释放 Skill 内部数据。
...
> **SSoT 声明**：错误码权威定义位于 `agentrt/commons/include/airy_types.h`（`airy_err_t` 类型 + `AIRY_E*` POSIX 码）和 `agentrt/commons/utils/error/include/error.h`（`AIRY_ERR_*` 扩展码）。`include/airymax/error.h` 为规划中的 [SC] 共享路径，当前尚未创建。本表与 `docs/AirymaxRT/50-engineering-standards/120-cross-project-code-sharing.md` §2.1（13 个 `AIRY_E*` 宏，对齐 POSIX errno 负值）逐字节一致。
...
| 错误码 | 值 | 描述 |
|-------|-----|------|
| `AIRY_ERROR_SKILL_NOT_FOUND` | 0x6001 | Skill 未找到 |
| `AIRY_ERROR_SKILL_ALREADY_INSTALLED` | 0x6002 | Skill 已安装 |
| `AIRY_ERROR_SKILL_INSTALL_FAILED` | 0x6003 | Skill 安装失败 |
| `AIRY_ERROR_SKILL_EXECUTION_FAILED` | 0x6004 | Skill 执行失败 |
| `AIRY_ERROR_SKILL_PERMISSION_DENIED` | 0x6005 | Skill 权限不足 |
| `AIRY_ERROR_SKILL_BUSY` | 0x6006 | Skill 正在执行 |
| `AIRY_ERROR_SKILL_INVALID_URL` | 0x6007 | Skill URL 无效 |
| `AIRY_ERROR_SKILL_VERIFICATION_FAILED` | 0x6008 | Skill 验证失败 |
