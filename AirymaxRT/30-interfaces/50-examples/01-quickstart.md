...
cmake .. \
    ...
    -DAIRY_ENABLE_EXAMPLES=ON \       # 构建示例程序
    -DCMAKE_INSTALL_PREFIX=/usr/local \   # 安装路径
    -DAIRY_SANITIZE=ON                # 启用地址消毒器(调试)
```
...
int main(int argc, char* argv[]) {
    ...
    printf("[4/6] 提交问题给认知层...");
    char* task_id = NULL;
    err = airy_loop_submit(loop, question, strlen(question), &task_id);
    if (err != AIRY_EOK) {
    ...
    // 设置超时为30秒
    err = airy_loop_wait(loop, task_id, 30000, &answer, &answer_len);
    
    if (err == AIRY_EOK && answer) {
        printf("\n🎯 Agent回答:\n");
        printf("━━━━━━━━━━━━━━━━━━━━━━━━━━━━\n");
...
```bash
# 解决：链接必要的库
-lairy_core -lairy_common -lpthread
```

### 运行时错误

**问题**: `libairy_common.so: cannot open shared object file`
```bash
# 解决：设置库路径
export LD_LIBRARY_PATH=/usr/local/lib:$LD_LIBRARY_PATH
...
cmake .. -DCMAKE_BUILD_TYPE=Debug -DAIRY_SANITIZE=ON
make clean && make
# 再次运行，会显示详细的错误位置
```
...
```bash
# 检查日志级别（debug模式会很慢）
export AIRY_LOG_LEVEL=warn

# 增加执行线程数
# 在创建loop时配置：
airy_loop_config_t config = { .loop_config_execution_threads = 16 };
airy_loop_create(&config, &loop);
```
