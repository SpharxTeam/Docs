...
```c
AIRY_API airy_err_t airy_loop_create(
    const airy_loop_config_t* config,   /* [in] 配置参数，NULL使用默认值 */
    airy_core_loop_t** out_loop        /* [out] 输出循环句柄 */
);
```
...
```c
AIRY_API void airy_loop_destroy(
    airy_core_loop_t* loop  /* [in] 循环句柄 */
);
```

**注意**: 如果循环正在运行，会先调用 `airy_loop_stop`。
...
```c
AIRY_API airy_err_t airy_loop_run(
    airy_core_loop_t* loop  /* [in] 循环句柄 */
);
```
...
**线程安全**: 此函数应在独立线程中调用，或使用 `airy_loop_submit` 提交任务后在另一个线程中等待。
...
```c
AIRY_API airy_err_t airy_loop_submit(
    airy_core_loop_t* loop,      /* [in] 循环句柄 */
...
**返回值**:
- `AIRY_EOK`: 任务已提交
- `AIRY_EINVAL`: 参数无效或引擎未初始化
- `AIRY_ENOMEM`: 内存不足

**示例**:
```c
char* task_id = NULL;
err = airy_loop_submit(loop, "你好", 2, &task_id);
if (err == AIRY_EOK) {
    printf("任务ID: %s\n", task_id);
    // 后续使用 task_id 等待结果
}
...
```c
AIRY_API airy_err_t airy_loop_wait(
    airy_core_loop_t* loop,      /* [in] 循环句柄 */
...
#### `airy_loop_get_engines`
获取三个引擎的句柄（用于高级用法）。

```c
AIRY_API void airy_loop_get_engines(
    airy_core_loop_t* loop,           /* [in] 循环句柄 */
    airy_cognition_engine_t** out_cognition,  /* [out] 认知引擎 */
    airy_execution_engine_t** out_execution,  /* [out] 执行引擎 */
    airy_memory_engine_t** out_memory        /* [out] 记忆引擎 */
);
```
...
#### `airy_cognition_engine_t`
认知引擎句柄（不透明指针）。

#### `airy_cognition_config_t`
```c
typedef struct {
    ...
    /* 回调函数 */
    airy_feedback_callback_t feedback_callback;
...
typedef struct {
    ...
    airy_task_node_t** task_plan_nodes; /* 计划节点数组 */
...
#### `airy_cognition_create`
创建认知引擎（基础版本）。

```c
airy_err_t airy_cognition_create(
    airy_plan_strategy_t* plan_strategy,      /* [in] 规划策略 (可选) */
    airy_coordinator_strategy_t* coord_strategy, /* [in] 协调策略 (可选) */
    airy_dispatching_strategy_t* disp_strategy, /* [in] 分发策略 (可选) */
    airy_cognition_engine_t** out_engine       /* [out] 输出引擎 */
);
```

---

#### `airy_cognition_create_ex`
创建认知引擎（扩展版本，支持完整配置）。

```c
airy_err_t airy_cognition_create_ex(
    const airy_cognition_config_t* config,    /* [in] 完整配置 (可选) */
    airy_plan_strategy_t* plan_strategy,
    airy_coordinator_strategy_t* coord_strategy,
    airy_dispatching_strategy_t* disp_strategy,
    airy_cognition_engine_t** out_engine
);
```
...
#### `airy_cognition_destroy`
销毁认知引擎。
```c
void airy_cognition_destroy(airy_cognition_engine_t* engine);
```

---

#### `airy_cognition_process`
核心处理函数：将输入转换为任务计划。

```c
airy_err_t airy_cognition_process(
    airy_cognition_engine_t* engine,   /* [in] 引擎句柄 */
    const char* input,                   /* [in] 输入文本 */
    size_t input_len,                    /* [in] 输入长度 */
    airy_task_plan_t** out_plan       /* [out] 输出计划 (需调用者释放) */
);
```
...
- `AIRY_EOK`: 计划生成成功
- `AIRY_EINVAL`: 参数无效
- `AIRY_ENOTSUP`: 无可用规划策略

**示例**:
```c
airy_task_plan_t* plan = NULL;
err = airy_cognition_process(engine, "帮我写一份报告", strlen("帮我写一份报告"), &plan);
...
```c
void airy_task_plan_free(airy_task_plan_t* plan);
```
...
#### `airy_cognition_stats`
获取认知引擎统计信息。

```c
airy_err_t airy_cognition_stats(
...
#### `airy_cognition_health_check`
健康检查（JSON格式）。

```c
airy_err_t airy_cognition_health_check(
    airy_cognition_engine_t* engine,
...
#### `airy_cognition_set_fallback_plan`
设置备用规划策略。

```c
void airy_cognition_set_fallback_plan(
    airy_cognition_engine_t* engine,
    airy_plan_strategy_t* fallback
);
```
...
#### `airy_cognition_set_context`
设置认知上下文（如对话历史）。

```c
void airy_cognition_set_context(
    airy_cognition_engine_t* engine,
    void* context,
...
#### `airy_cognition_set_memory`
关联记忆引擎（用于上下文预填充）。

```c
void airy_cognition_set_memory(
    airy_cognition_engine_t* engine,
...
typedef struct {
    ...
    airy_task_status_t task_status; /* 当前状态 */
} airy_task_t;
```
...
#### `airy_execution_unit_t`
执行单元（注册到引擎的工作器）。
```c
typedef struct {
    char* unit_name;                          /* 单元名称 */
    airy_err_t (*execution_unit_execute)( /* 执行函数 */
        struct airy_execution_unit_s* self,
        const void* input,
        void** output
    );
    void* context;                             /* 用户上下文 */
    void (*destroy)(struct airy_execution_unit_s*); /* 销毁函数 */
} airy_execution_unit_t;
```

### 函数接口

#### `airy_execution_create`
创建执行引擎。

```c
airy_err_t airy_execution_create(
    uint32_t max_concurrency,          /* [in] 最大并发任务数 */
    airy_execution_engine_t** out_engine /* [out] 输出引擎 */
);
```
...
#### `airy_execution_destroy`
销毁执行引擎。

```c
void airy_execution_destroy(airy_execution_engine_t* engine);
```
...
#### `airy_execution_submit`
提交任务到执行引擎。

```c
airy_err_t airy_execution_submit(
    airy_execution_engine_t* engine,  /* [in] 引擎 */
    const airy_task_t* task,          /* [in] 任务描述 */
...
3. 生成唯一任务ID（使用 `airy_generate_task_id`）
4. 初始化同步原语（mutex, cond）
5. 入队并插入哈希表
6. signal 通知工作线程

**返回的任务ID**: 使用 `AIRY_FREE` 释放。

---

#### `airy_execution_query`
查询任务状态（非阻塞）。

```c
airy_err_t airy_execution_query(
    airy_execution_engine_t* engine,
    const char* task_id,
    airy_task_status_t* out_status  /* [out] 当前状态 */
);
```
...
```c
airy_err_t airy_execution_wait(
    airy_execution_engine_t* engine,
    const char* task_id,
    uint32_t timeout_ms,            /* [in] 超时(ms), 0=无限 */
    airy_task_t** out_result    /* [out] 结果副本 (需释放) */
);
```
...
**结果释放**: 使用 `airy_task_free`。

---

#### `airy_execution_cancel`
取消等待中的任务。

```c
airy_err_t airy_execution_cancel(
    airy_execution_engine_t* engine,
    const char* task_id
);
```
...
#### `airy_execution_get_result`
获取已完成任务的结果（非阻塞）。

```c
airy_err_t airy_execution_get_result(
    airy_execution_engine_t* engine,
    const char* task_id,
    airy_task_t** out_result  /* [out] 结果 (需释放) */
);
```
...
```c
void airy_task_free(airy_task_t* task);
```

---

#### `airy_execution_health_check`
健康检查（JSON格式）。

```c
airy_err_t airy_execution_health_check(
    airy_execution_engine_t* engine,
...
#### `airy_memory_engine_t`
记忆引擎句柄（不透明指针）。

#### `airy_memory_record_t`
记忆记录。
```c
typedef struct {
...
#### `airy_memory_result_ext_t`
查询结果。
```c
typedef struct {
    airy_memory_result_item_t** memory_result_items; /* 结果项数组 */
...
```c
airy_err_t airy_memory_create(
    const airy_memory_config_t* config,  /* [in] 配置 (可选, NULL用默认值) */
    airy_memory_engine_t** out_memory    /* [out] 输出引擎 */
);
```
...
销毁记忆引擎。
```c
void airy_memory_destroy(airy_memory_engine_t* memory);
```
...
```c
airy_err_t airy_memory_write(
    airy_memory_engine_t* memory,
    const airy_memory_record_t* record,  /* [in] 记录数据 */
...
char* record_id = NULL;
err = airy_memory_write(memory, &record, &record_id);
printf("记录ID: %s\n", record_id);
AIRY_FREE(record_id);
```
...
```c
airy_err_t airy_memory_query(
    airy_memory_engine_t* memory,
    const airy_memory_query_t* query,      /* [in] 查询参数 */
    airy_memory_result_ext_t** out_result  /* [out] 结果 (需释放) */
);
```
...
**结果释放**: 使用 `airy_memory_result_free`。
...
```c
airy_err_t airy_memory_read(
    airy_memory_engine_t* memory,
    const char* record_id,
    airy_memory_record_t** out_record  /* [out] 记录 (需释放) */
);
```
...
int main() {
    ...
    airy_loop_submit(loop, question, strlen(question), &task_id);
    
    // 5. 等待回答（30秒超时）
    char* answer = NULL;
    size_t answer_len = 0;
    airy_err_t err = airy_loop_wait(loop, task_id, 30000, &answer, &answer_len);
...
// 自定义执行单元：文本分析器
static airy_err_t text_analyzer_execute(
    airy_execution_unit_t* unit,
    const void* input,
    void** output) {
...
void register_text_analyzer() {
    airy_execution_unit_t* unit = AIRY_CALLOC(1, sizeof(*unit));
    unit->unit_name = "text_analyzer";
    unit->execution_unit_execute = text_analyzer_execute;
    unit->context = NULL;
    unit->destroy = free;
    
    airy_registry_register_unit(unit);
}
```
...
- ❌ 不要混用 `malloc/free` 和 `AIRY_MALLOC/AIRY_FREE`
...
### 错误处理最佳实践
```c
airy_err_t err = airy_loop_submit(loop, input, len, &task_id);
switch (err) {
    case AIRY_EOK:
        // 正常处理
        break;
