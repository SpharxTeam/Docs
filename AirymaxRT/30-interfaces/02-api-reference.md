...
```bash
export AIRY_ENDPOINT="http://localhost:8080"
export AIRY_API_KEY="sk-your-api-key-here"
```
...
```json
{
  "resources": [
    {
      "uri": "file:///docs/10-architecture/01-system-architecture.md",
...
curl -X POST http://localhost:8080/mcp \
  ...
  -d '{
    "jsonrpc": "2.0",
    "method": "resources/read",
    "params": {"uri": "file:///docs/10-architecture/01-system-architecture.md"},
    "id": 5
  }'
```
...
| 环境变量 | 默认值 | 说明 |
|:---------|:-------|:-----|
| `AIRY_ENDPOINT` | `http://127.0.0.1:8080` | Gateway 端点地址 |
| `AIRY_API_KEY` | （空） | API 认证密钥 |
