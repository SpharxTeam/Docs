...
**最新**: 2026-06-09
**状态**: 维护中
**路径**: docs/AirymaxRT/30-interfaces/70-toolkit/30-rust/README.md
---
...
```rust
use agentrt::{Agent, AgentConfig, TaskPriority, Result};
...
```rust
use agentrt::{MemoryClient, MemoryQuery, Result};
...
```rust
use agentrt::{Error, ErrorCode};

match agent.submit_task().send().await {
    Ok(task_id) => println!("任务提交成功: {}", task_id),
    Err(Error::AgentRTError { code, message, trace_id }) => {
...
mod tests {
    ...
    use agentrt::testing::{MockAgent, MockMemoryClient};
