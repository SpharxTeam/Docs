...
const client = new AgentRTClient({
  ...
  timeout: 30000,
  apiKey: process.env.AIRY_API_KEY,
});
...
| 变量名 | 描述 | 默认值 |
|--------|------|--------|
| `AIRY_ENDPOINT` | 服务端地址 | `http://localhost:8080` |
| `AIRY_API_KEY` | API 密钥 | - |
| `AIRY_TIMEOUT` | 请求超时(ms) | `30000` |
| `AIRY_LOG_LEVEL` | 日志级别 | `INFO` |
| `AIRY_TELEMETRY_ENABLED` | 启用遥测 | `true` |
| `AIRY_SAMPLING_RATE` | 追踪采样率 | `0.1` |
