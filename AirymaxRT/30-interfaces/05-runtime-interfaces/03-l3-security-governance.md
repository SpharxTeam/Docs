...
Airymax 原生实现：`agentrt/cupolas/include/cupolas.h`（`cupolas_check_permission`、`cupolas_add_permission_rule`、`cupolas_clear_permission_cache`）。L3 标准 ABI 通过适配层桥接至 `are_permission_*`。
...
Airymax 原生实现：`agentrt/atoms/syscall/include/airy_sandbox.h`（`airy_sandbox_*`）。原生权限枚举 `PERM_ALLOW/PERM_DENY/PERM_ASK` 与本标准 `ARE_PERM_*` 一一对应。
...
> 参考实现：`agentrt/atoms/syscall/include/syscalls.h`。原生 `airy_syscall_num_t` 枚举与本标准一致。
...
实现路径（Airymax 主仓库）：`agentrt/commons/utils/error/include/error.h`。
...
| **内核** | -200 ~ -299 | IPC、任务、调度、内存 | `AIRY_EKERN_IPC` | -201 | 内核 IPC 错误 |
| | | | `AIRY_EKERN_TASK` | -202 | 任务错误 |
| **服务** | -300 ~ -399 | 服务状态、依赖、负载均衡 | `AIRY_ESVC_NOT_READY` | -301 | 服务未就绪 |
| | | | `AIRY_ESVC_BUSY` | -302 | 服务忙（背压） |
| **LLM** | -400 ~ -499 | LLM provider、限流、上下文 | `AIRY_ELLM_NO_PROVIDER` | -401 | 无可用 provider |
| | | | `AIRY_ELLM_RATE_LIMIT` | -403 | 限流 |
| | | | `AIRY_ELLM_CONTEXT_LEN` | -404 | 上下文超长 |
| **执行** | -500 ~ -599 | 工具执行、沙箱、超时 | `AIRY_EEXEC_NOT_FOUND` | -501 | 工具未找到 |
| | | | `AIRY_EEXEC_SANDBOX` | -505 | 沙箱违规 |
| **记忆** | -600 ~ -699 | 记忆读写、查询、演化 | `AIRY_EMEM_WRITE` | -601 | 记忆写入失败 |
| | | | `AIRY_EMEM_FULL` | -605 | 记忆存储满 |
| **安全** | -700 ~ -799 | 权限、净化、审计、沙箱 | `AIRY_ESEC_VIOLATION` | -701 | 安全违规 |
| | | | `AIRY_ESEC_SANITIZE` | -702 | 净化失败 |
| | | | `AIRY_ESEC_QUOTA` | -706 | 配额超限 |
| | | | `AIRY_ESEC_PATH_TRAV` | -709 | 路径遍历攻击 |
| **协议** | -800 ~ -889 | 编排、协调、补偿 | `AIRY_ECOORD_PLAN_FAIL` | -801 | 编排计划失败 |
| | | | `AIRY_ECOORD_RETRY_EXCEED` | -806 | 重试超限 |
...
| 标准接口 | 参考实现位置 |
|----------|-------------|
| 权限引擎 | `agentrt/cupolas/include/cupolas.h`（`cupolas_check_permission` 等） |
| 沙箱 | `agentrt/atoms/syscall/include/airy_sandbox.h`（`airy_sandbox_*`） |
| 系统调用号 | `agentrt/atoms/syscall/include/syscalls.h`（`airy_syscall_num_t`） |
| 错误码权威源 | `agentrt/commons/utils/error/include/error.h` |
| 类型定义权威源 | `agentrt/commons/include/airy_types.h`（`airy_err_t`） |
