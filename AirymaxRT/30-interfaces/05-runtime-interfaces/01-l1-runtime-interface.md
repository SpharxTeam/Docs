...
所有接口以 `are_` 前缀命名，避免与宿主系统符号冲突。参考实现位于 Airymax atoms 层（corekern/syscall/memory/taskflow），其原生符号以 `airy_` 前缀提供，由适配层桥接至 `are_` 标准 ABI。
...
| 标准接口 | 参考实现位置 |
|----------|-------------|
| `are_ipc_*` | `agentrt/atoms/corekern/include/ipc.h`（`airy_ipc_*`） |
| `are_mem_alloc/free/realloc` | `agentrt/atoms/corekern/include/mem.h`（`airy_mem_*`） |
| 五级压力框架 | `agentrt/atoms/corekern/include/oom_handler.h`（`airy_oom_*`） |
| `are_task_create/yield/join` | `agentrt/atoms/corekern/include/task.h`（`airy_task_*`） |
| `are_time_ns/monotonic_ns` | `agentrt/atoms/corekern/include/airy_time.h`（`airy_time_*`） |
...
参考实现提供 `are_l1_adapter.c` 将 `airy_*` 符号桥接至 `are_*` 标准 ABI：
...
are_error_t are_ipc_init(void) {
    return (are_error_t)airy_ipc_init();
}

void *are_mem_alloc(size_t size) {
    return airy_mem_alloc(size);
}

uint64_t are_time_monotonic_ns(void) {
    return airy_time_monotonic_ns();
}
/* ... 其余接口一一映射 ... */
```
