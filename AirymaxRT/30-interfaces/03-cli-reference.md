...
| 环境变量 | 默认值 | 说明 |
|:---------|:-------|:-----|
| `AIRY_ENDPOINT` | `http://localhost:8080` | Airymax Gateway 的 HTTP 地址 |
| `AIRY_API_KEY` | （空） | API 认证密钥，设置后将作为 `Authorization: Bearer` 头传递 |
| `AIRY_HEAPSTORE_ROOT` | `~/.agentrt/heapstore` | Heapstore 数据根目录，用于 `db` 相关命令 |

### 示例：设置环境变量

```bash
# 连接到远程 Gateway
export AIRY_ENDPOINT="http://agentrt.example.com:8080"

# 设置 API 认证
export AIRY_API_KEY="sk-xxxxxxxxxxxx"

# 自定义 Heapstore 数据目录
export AIRY_HEAPSTORE_ROOT="/data/agentrt/heapstore"
```
...
**环境变量：** `AIRY_ENDPOINT` — 决定健康检查的目标地址。
...
**环境变量：** `AIRY_ENDPOINT` — 决定查询的 Gateway 地址。如果 Gateway 不可达，CLI 将显示默认协议列表。
...
**环境变量：** `AIRY_ENDPOINT` — 决定消息发送的目标 Gateway。
...
**环境变量：** `AIRY_ENDPOINT` — 决定查询统计信息的目标 Gateway。如果 Gateway 不可达，将显示零值统计。
...
**环境变量：** `AIRY_ENDPOINT` — 决定查询指标的目标 Gateway。
...
**环境变量：** `AIRY_ENDPOINT` — 决定查询告警的目标 Gateway。
...
**环境变量：** `AIRY_ENDPOINT` — 决定 Gateway 健康检查的目标地址。
...
| 环境变量 | 说明 |
|:---------|:-----|
| `AIRY_HEAPSTORE_ROOT` | 指定 Heapstore 数据目录（默认：`~/.agentrt/heapstore`） |
...
**环境变量：** `AIRY_ENDPOINT` — 影响显示的 Gateway 地址。
...
**环境变量：** `AIRY_ENDPOINT` — 决定状态查询的目标地址。
...
| 环境变量 | 默认值 | 影响的命令 |
|:---------|:-------|:-----------|
| `AIRY_ENDPOINT` | `http://localhost:8080` | 所有需要与 Gateway 通信的命令 |
| `AIRY_API_KEY` | （空） | 所有需要认证的 API 请求 |
| `AIRY_HEAPSTORE_ROOT` | `~/.agentrt/heapstore` | `db migrate`, `db migrate-status`, `db migrate-rollback`, `db backup` |