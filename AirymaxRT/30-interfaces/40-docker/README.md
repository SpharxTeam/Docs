...
│   ├── libairy_common.a
│   ├── libairy_core.a
│   ├── libairy_protocols.so → libairy_protocols.so.2.1.0
│   └── ... (其他库文件)
├── include/
│   └── agentrt.h          # 主头文件
└── share/doc/agentrt/
...
# 导出日志到文件
docker-compose logs > airy_$(date +%Y%m%d).log 2>&1
...
# 输出示例:
# HELP airy_requests_total Total requests processed
TYPE airy_requests_total counter
airy_requests_total{service="gateway",endpoint="/api/v1/chat"} 567.000

# HELP airy_request_duration_seconds Request duration in seconds
TYPE airy_request_duration_seconds histogram
airy_request_duration_seconds_bucket{le="0.1"} 100.000
airy_request_duration_seconds_bucket{le="0.5"} 250.000
airy_request_duration_seconds_bucket{le="1.0"} 300.000
airy_request_duration_seconds_sum 125.500
airy_request_duration_seconds_count 300.000
```
...
services:
  gateway:
    environment:
      ...
      # 调整线程池大小
      - AIRY_EXECUTION_THREADS=8
      
      # 启用缓存
      - AIRY_LLM_CACHE_ENABLED=true
      - AIRY_LLM_CACHE_SIZE=2000
