...
> **术语标准化说明**: 遗忘公式统一采用 $\tau$ 版本表示（与 ARCHITECTURAL_PRINCIPLES.md C-4 一致）。历史版本中使用的 $\lambda$ 表示法（$R = e^{-\lambda t}$）数学等价（$\lambda = 1/\tau$），但本规范统一使用 $\tau$ 以保持术语一致性。详见 TERMINOLOGY.md v3.0 裁决 #6。
