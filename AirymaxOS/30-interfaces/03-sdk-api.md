...
- **统一错误**: 错误码对齐 `airy_errno.h`（详见 [01-syscalls.md](01-syscalls.md) 第 6 章）。
...
所有 SDK 错误对齐 `airy_errno.h`（详见 [01-syscalls.md](01-syscalls.md) 第 6 章），通过语言原生的错误机制暴露。
...
| `AIRY_ETIMEOUT` | 是 | 2 | 指数退避（100ms / 200ms） |
...
client = AirymaxClient(
    ...
    retry=RetryPolicy(
        ...
        max_backoff_ms=1000,
        retry_on=(-6, -11),  # AIRY_EAGAIN, AIRY_ETIMEOUT
    ),
)
```
...
- 超时返回 `AIRY_ETIMEOUT`，已提交的任务不会被自动取消（需显式调用 `task_cancel`）。
