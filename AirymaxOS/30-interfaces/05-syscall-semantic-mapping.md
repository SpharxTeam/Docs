...
| 调用入口 | 统一入口 `airy_syscall_invoke()` | 编号 syscall（548-571） | 设计哲学不同 |
...
|------|----------------------|--------------------------------|
| **API 范式** | JSON-RPC 高层 API | 编号 syscall（548-571） |
...
- agentrt-linux 保持编号 syscall 548-571，遵循 Linux 传统 + seL4 借鉴
...
graph TB
    ...
    subgraph "agentrt-linux 内核态 [IND] 编号 syscall"
        OS[SYSCALL_DEFINE*<br/>编号 548-571<br/>Linux 6.6 内核]
    end
...
|---|-------------------|-------------------------|---------|--------------|---------|
| 1 | `airy_sys_task_submit(input, input_len, timeout_ms, out_output)` | #548 `airy_sys_call(COMPILE_BADGE, task_desc, priority)` | 提交 Agent 任务到调度器 | `sched.h`（`struct airy_task_desc` magic 0x41475453） | JSON 字符串 vs 类型化结构体 |
| 2 | `airy_sys_task_cancel(task_id)` | #550 `airy_sys_sched_ctl(AIRY_SCHED_YIELD, task_id)` | 取消 Agent 任务 | `sched.h`（任务描述符） | 字符串 ID vs 整数 ID |
| 3 | `airy_sys_task_query(task_id, out_status)` | #550 `airy_sys_sched_ctl(AIRY_SCHED_GET_LATENCY, task_id, out_status)` | 查询任务状态 | `sched.h`（任务状态枚举） | 字符串 ID + int* vs 整数 ID + 结构体* |
| 4 | `airy_sys_ipc_send(hdr, payload)` → 通过 `airy_syscall_invoke` 路由 | io_uring `IORING_OP_URING_CMD`（数据面，零 syscall） | 发送 IPC 消息 | `ipc.h`（`struct airy_ipc_msg_hdr` magic 0x41524531） | JSON 包装 vs io_uring |
| 5 | `airy_sys_ipc_recv(hdr, payload_buf)` → 通过 `airy_syscall_invoke` 路由 | io_uring CQE（数据面，零 syscall） | 接收 IPC 消息 | `ipc.h`（128B 消息头布局） | JSON 包装 vs io_uring |
| 6 | `airy_sys_memory_write` / `airy_sys_memory_get` | #549 `airy_sys_rovol_ctl(AIRY_ROVOL_SNAPSHOT, pid, snapshot_id_out)` | 进程记忆写入 / 快照 | `memory_types.h`（MemoryRovol L1-L4 结构） | 应用层写入 vs 内核态快照 |
...
|---|-------------------|----------------------------|---------|------|
| 1 | `airy_sys_agent_spawn(agent_spec, out_agent_id)` | #548 `airy_sys_call(COMPILE_BADGE)` | 1 → 1 | agentrt 高层 spawn 语义在内核态通过 COMPILE_BADGE 一次调用完成（提交任务 + 编译 capability） |
| 2 | `airy_sys_agent_terminate(agent_id)` | #550 `airy_sys_sched_ctl(AIRY_SCHED_YIELD)` + #548 `airy_sys_call(REVOKE_BADGE)` | 1 → 2 | 终止 Agent 在内核态分解为"取消任务 + 撤销 capability" |
| 3 | `airy_sys_memory_delete` | #549 `airy_sys_rovol_ctl(AIRY_ROVOL_RESTORE, snapshot_id, pid)` | 1 → 1 | 应用层删除记忆 ↔ 内核态从快照恢复（语义逆向） |
| 4 | `airy_sys_skill_execute` | #551 `airy_sys_clt_notify(AIRY_CLT_WASM_LOAD_MODULE)` + #551 `airy_sys_clt_notify(AIRY_CLT_PHASE_NOTIFY)` | 1 → 2 | Skill 执行在内核态分解为"Wasm 模块加载 + CLT 阶段通知" |
...
**规则 SM-3**：当 agentrt 运行在 agentrt-linux 上时，`airy_syscall_invoke()` 内部可选地走内核加速路径（直接调用编号 syscall 548-571），而非 Linux 平台则走用户态纯软件实现。这一选择对应用层透明。
...
| agentrt API | 独有原因 |
|-------------|---------|
| `airy_sys_agent_spawn()` | 高层 spawn 语义，内核态分解为 COMPILE_BADGE（提交任务 + 编译 capability） |
| `airy_sys_agent_terminate()` | 高层 terminate 语义，内核态分解为 SCHED_YIELD + REVOKE_BADGE |
...
|-------------|---------|---------|
| — | `io_uring_register`（无独立 syscall） | 注册跨进程 io_uring ring，内核态机制，agentrt 用户态使用 POSIX MQ |
...
| OS syscall # | 内核 API | 独有原因 |
|-------------|---------|---------|
| #549 | `airy_sys_rovol_ctl(AIRY_ROVOL_MIGRATE)` | 记忆跨节点迁移，内核态 PMEM 机制 |
| #549 | `airy_sys_rovol_ctl(AIRY_ROVOL_TIER_SET)` | CXL 内存分层策略，内核态硬件接口 |
| #549 | `airy_sys_rovol_ctl(AIRY_ROVOL_MGLRU_CONFIG)` | MGLRU（多代 LRU）配置，内核态页回收策略 |

### 5.3 分层用户态调度器机制（sched_tac）

| OS syscall # | 内核 API | 独有原因 |
|-------------|---------|---------|
| #550 | `airy_sys_sched_ctl(SET_POLICY)` | 设置用户态调度器策略（SCHED_FIFO/SCHED_DEADLINE + cgroup cpuset） |
| #550 | `airy_sys_sched_ctl(AIRY_SCHED_GET_POLICY)` | 查询当前用户态调度器策略（SCHED_FIFO/SCHED_DEADLINE + cgroup cpuset） |

### 5.4 安全内核机制（capability + LSM）

| OS syscall # | 内核 API | 独有原因 |
|-------------|---------|---------|
| #548 | `airy_sys_call(REVOKE_BADGE)` | 撤销 capability，内核态 CNode 操作（agentrt 用户态无 revoke syscall） |
| #548 | `airy_sys_call(LSM_CTL)` | 加载 airy_lsm 策略，内核态 LSM 钩子 |

### 5.5 认知内核机制（CoreLoopThree kthread + Wasm）

| OS syscall # | 内核 API | 独有原因 |
|-------------|---------|---------|
| #551 | `airy_sys_clt_notify(AIRY_CLT_REGISTER_KTHREAD)` | 注册 CoreLoopThree kthread，内核态线程机制 |
| #551 | `airy_sys_clt_notify(AIRY_CLT_WASM_LOAD_MODULE)` | 加载 Wasm 3.0 模块，内核态运行时 |
| #550 | `airy_sys_sched_ctl(AIRY_SCHED_YIELD)` | 跨超节点迁移任务，内核态 cloudnative 机制 |
...
# 在 agentrt-linux：内部可选走编号 syscall #548-551 内核加速
...
| 平台 | 控制面路径 | 数据面路径 | 说明 |
|------|-----------|-----------|------|
| **agentrt-linux** | `airy_syscall_invoke()` → libc syscall() → #548-571 | `airy_ipc_ring_enter()` → io_uring SQE/CQE | 内核加速路径，最优性能 |
...
当检测到 agentrt-linux 时，`airy_syscall_invoke()` 内部可选地将 JSON-RPC 请求解码后，直接调用编号 syscall 548-571，绕过用户态纯软件实现，获得内核加速。这一选择对应用层完全透明。

---

## 8. IRON-9 v3 四层共享模型

> **OS-IFACE-009**： 系统调用语义映射遵循 IRON-9 v3 四层共享模型——agentrt 高层 API（JSON-RPC 统一入口）与 agentrt-linux 编号 syscall（548-571）在 [SS] 语义同源层保持概念操作一致，签名因抽象层级不同而独立演进；SDK 层（AirymaxClient 4 语言）签名确实同源（同一份源码两端编译）。禁止在系统调用层引入签名转换层或 JSON-to-struct 适配层，签名同源仅限于 SDK 层。
...
|--------|-------------|-------------------|---------|
| 调用入口 | `airy_syscall_invoke()` JSON-RPC 统一入口 | `SYSCALL_DEFINE*` 编号 syscall 548-571 | 抽象层级 + 跨平台约束 |
| 参数序列化 | JSON 字符串（跨语言） | 类型化结构体（Linux ABI） | 设计哲学差异 |
| 返回值 | `airy_err_t` + JSON 输出 | `int`（`AIRY_E*` 负值） | 接口范式差异 |
| syscall 表注册 | 无（用户态直接 libc syscall()） | `syscall_64.tbl` 548-571 段 24 项 | 工具链 + 构建系统差异 |
...
- agentrt-linux syscall 编号 548-571 在 MAJOR 版本内不可变更（见 [01-syscalls.md](01-syscalls.md) §2.3 ABI 稳定性）。
