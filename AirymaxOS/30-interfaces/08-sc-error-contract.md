...
> **设计依据**：[15-comprehensive-correction-plan.md](../../docs-closed/agentrt-linux/00-reviews/_review_v2.2/15-comprehensive-correction-plan.md) §2.3（错误码 SSoT）+ §4.2.1（A-UEF 设计）
...
- **前置知识**：理解 [10-unify-design.md](../10-architecture/10-unify-design.md) A-UEF 模块定位、[06-iron9-shared-model.md](../10-architecture/06-iron9-shared-model.md) [SC] 层概念
- **预计阅读时间**：25 分钟
...
| IPC 码 | `[-41, -70]` | A-IPC 协议层 | 30 | `AIRY_EIPC_*` |
...
A-IPC 协议层专用错误码，覆盖 IPC 协议、Ring Buffer、io_uring_cmd 等场景：
...
| 版本 | 日期 | 变更内容 |
|------|------|---------|
| v1.0 | 2026-07-17 | 初始版本：A-UEF [SC] error.h 二进制契约；Error 码 5 子空间分配（POSIX/IPC/Capability/[SC]/[DSL]）；Fault 码 0x1000+ 分配；CI 逐字节校验；[DSL] 降级块；物理宿主 `kernel/include/airymax/error.h` |
