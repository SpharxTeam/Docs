...
#define AIRY_EDSL_CAP_BADGE_SKIP    (-207)  /* C-S9 跳过 Badge 校验（v1.0.1 新增） */
/* [-208, -300] 预留 */
```
...
v1.0.1 起，Fault 码空间按模块重新组织。`0x1001-0x1003` 为 Capability Folding A-IPC 专用，`0x1004-0x1005` 为 A-ULS 专用，`0x1006` 为 MemoryRovol 专用：

```c
/* kernel/include/uapi/linux/airymax/error.h —— Fault 码（正数 0x1000+，不可恢复） */

/* ===== A-IPC Capability Folding Fault 码（v1.0.1 新增） ===== */
...
#define AIRY_FAULT_VM_FAULT      0x1006   /* 共享页映射损坏（v1.0.1 从 0x1002 迁移至此） */

/* ===== 资源配额与驱动 Fault 码（v1.0.1 新增预留） ===== */
...
3. `airy_err_t` 类型定义
4. POSIX 码 `[-1, -40]`
5. IPC 码 `[-41, -70]`（v1.0.1：C-S 检查链对齐）
6. Capability 码 `[-71, -100]`（v1.0.1：含 Badge 校验码 -78~-82）
7. [SC] 码 `[-101, -200]`
8. [DSL] 码 `[-201, -300]`
9. Fault 码 `[0x1000, 0x1FFF]`（v1.0.1：按模块组织）
