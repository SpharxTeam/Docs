...
| 契约源 | 生成器 | 输入文件 | 主输出 |
|--------|--------|---------|--------|
| 系统调用契约（R-01） | `syscall_gen.py` | `include/uapi/airymax/syscall.xml` | C syscall 头文件 + x86 syscall 表 + Rust/Python 绑定 |
| 位域契约（R-02） | `bitfield_gen.py` | `include/airymax/structures_32.bf` / `structures_64.bf` | C 位域结构头文件 |
...
契约源文件位于 `include/uapi/airymax/syscall.xml`，采用自描述 XML Schema，根元素 `<syscalls>` 携带版本与槽位容量约束：

```xml
<?xml version="1.0" encoding="UTF-8"?>
<!--
  syscall.xml — agentrt-linux 系统调用契约源（R-01 SSoT）
  物理宿主: include/uapi/airymax/syscall.xml
  生成器:   tools/codegen/syscall_gen.py
  禁止手工修改生成产物: include/uapi/airymax/syscall.h 等
-->
...
- **12 核心 syscall + 12 预留 = 24 槽位**，与 [01-syscalls.md](01-syscalls.md) §2.2 编号分配完全一致。
...
| 语言 | 生成目标 | 内容 |
|------|---------|------|
| C | `include/uapi/airymax/syscall.h` | 系统调用号宏（`#define AIRY_SYS_CALL 0`）+ `__NR_*` Linux 注册号宏 + 函数原型声明 |
...
/* Source: include/uapi/airymax/syscall.xml (R-01 SSoT) */
```
...
| 输入 | 路径 | 用途 |
|------|------|------|
| 32-bit 位域定义 | `include/airymax/structures_32.bf` | 32 位架构位域布局（兼容 32 位用户态） |
| 64-bit 位域定义 | `include/airymax/structures_64.bf` | 64 位架构位域布局（x86_64 / aarch64 主目标） |
...
-- 物理宿主: include/airymax/structures_64.bf
-- 生成器:   tools/codegen/bitfield_gen.py
-- 生成产物: include/airymax/structures_64_gen.h（禁止手工修改）
...
block ipc_msg_hdr(128) {
    ...
    field payload_len   351..320    -- offset 40, 4B, payload 字节数
    padding reserved    1023..352   -- offset 44, 84B, 保留字段
}
...
| 语言 | 生成目标 | 内容 |
|------|---------|------|
| C | `include/airymax/structures_32_gen.h` | 32 位位域 struct 定义 + 位域操作宏 + getter/setter 函数 |
| C | `include/airymax/structures_64_gen.h` | 64 位位域 struct 定义 + 位域操作宏 + getter/setter 函数 |

生成内容包含三类产物：

**（1）struct 定义**——紧凑 packed 结构体，字段偏移与 `.bf` 声明逐位对应：
...
struct airy_ipc_msg_hdr {
	...
	uint32_t payload_len;    /* 351..320 offset 40 */
	uint8_t  reserved[84];   /* 1023..352 offset 44 */
...
| 语言 | 生成目标 | 内容 |
|------|---------|------|
| C | `include/airymax/ipc.h` 中的 `struct airy_ipc_msg_hdr` | 128B packed 结构体 + magic 宏 + payload 类型宏 + flags 位定义 |
| Rust | `sdk/rust/airyx/ipc.rs` | `#[repr(C, packed)]` 结构体 + 常量 |
...
struct airy_ipc_msg_hdr {
	...
	__u32	payload_len;	/* offset 40, payload 字节数 */
	__u8	reserved[84];	/* offset 44, 保留字段 */
} __attribute__((packed));
 ...
 * 生成产物: include/airymax/security_types.h（禁止手工修改）
 *
...
| 语言 | 生成目标 | 内容 |
|------|---------|------|
| C | `include/airymax/security_types.h` | capability ID 枚举（`enum airy_cap_id`）+ `AIRY_CAP_AGENT_MAX` 上限宏 |
| C | `security/include/airy_lsm_hook_ids.h` | LSM 钩子 ID 枚举（252 ID） |
| Rust | `sdk/rust/airyx/cap_ids.rs` | capability ID 常量 + 枚举 |
| Python | `sdk/python/airyx/cap_ids.py` | capability ID 常量字典 |

### 5.3 LSM 钩子 ID 生成

LSM 钩子 ID（252 个，对齐 Linux 6.6 `security_hook_heads` 钩子数量，详见 [10-architecture/07-directory-structure.md](../10-architecture/07-directory-structure.md) §include/airymax/security_types.h）由 `cap_gen.py` 的第二阶段生成。输入为 `security/include/airy_lsm_hooks.def`（X-macro 风格，每个条目声明一个 LSM 钩子），生成 `enum airy_lsm_hook_id`（0-251）与钩子分派表索引。
...
on:
  pull_request:
    paths:
      - 'include/uapi/airymax/syscall.xml'
      - 'include/airymax/structures_*.bf'
      ...
      - 'include/airymax/structures_*_gen.h'
...
| 模板文件 | 用途 | 输出目标 |
|---------|------|---------|
| `syscall.h.j2` | C syscall 头文件模板 | `include/uapi/airymax/syscall.h` |
...
| `structures.h.j2` | C 位域结构模板 | `include/airymax/structures_{32,64}_gen.h` |
| `ipc.h.j2` | IPC 消息头模板（C） | `include/airymax/ipc.h` |
| `ipc.rs.j2` | IPC 消息头模板（Rust） | `sdk/rust/airyx/ipc.rs` |
| `ipc.py.j2` | IPC 消息头模板（Python） | `sdk/python/airyx/ipc.py` |
| `cap_ids.h.j2` | Capability ID 枚举模板 | `include/airymax/security_types.h` |
