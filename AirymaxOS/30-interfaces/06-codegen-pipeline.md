...
> **v1.0.1 Capability Folding 变更说明**：下方 `syscall.xml` 示例即 v1.0.1 Capability Folding 后的权威 `syscall.xml` 内容，反映 4 核心 syscall + 20 预留 = 24 槽位的最终布局，编号起点 548（避开 x86_64 x32 历史遗留区域 512-547）。0.1.1 的 12 个 syscall（含 `airy_sys_send`/`recv`/`reply`/`notify` 等 seL4 风格 IPC 原语）已全部移除，IPC 数据面改走 io_uring `IORING_OP_URING_CMD`（零 syscall）；控制面精简为 4 核心 syscall（`airy_sys_call`/`rovol_ctl`/`sched_ctl`/`clt_notify`），`core_count` 为 4、`reserved_count` 为 20。v1.0.1 `syscall.xml` 的权威内容见 [01-syscalls.md](01-syscalls.md) §2.2。
...
<syscalls version="0x0100"
  ...
  <syscall>
    ...
    <number>1</number>
    <signature>uint32_t op, uint32_t pid, uint64_t arg</signature>
    <flags>control</flags>
  ...
  <syscall>
    ...
    <since>1.0.1</since>
    <stability>stable</stability>
    <doc>Scheduler control operations.</doc>
  </syscall>
  ...
  <syscall>
    ...
    <since>1.0.1</since>
    <stability>stable</stability>
    <doc>Cross-layer task notification control.</doc>
  </syscall>
...
| `number` | int | 是 | 内部编号（0-23），Linux 注册号 = `number + linux_base`（548） |
...
- 内部编号 0-3 为核心调用，4-23 为预留段（`<reserved>` 元素），生成器对预留段只生成编号宏占位（`#define AIRY_SYS_RESERVED_4 4`），不生成函数声明与 syscall 表实现。
- Linux 注册号映射：内部编号 N → Linux 注册号 `N + 548`，固定偏移，由 `syscall.h` 通过 `#define __NR_airy_sys_call 548` 锁定（OS-IFACE-002）。
...
```c
/* @generated by bitfield_gen.py v1.0.1 from structures_64.bf — DO NOT EDIT */
...
```c
/* @generated by ipc_gen.py v1.0.1 from contracts.md §2 — DO NOT EDIT */
...
| 变量 | 类型 | 说明 |
|------|------|------|
| `generator_version` | string | 生成器版本（如 `1.0.1`），注入 `@generated` 头部 |
| `linux_base` | int | Linux 注册号基址（548） |
