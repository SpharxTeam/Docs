...
6. **版本协商**: 版本协商在 L2 服务协议层处理（见 `50-engineering-standards/30-runtime-interfaces/L2_service_protocol.md`），L1 [SC] 基础消息头不含 `version` 字段，通过 `opcode` 区分消息类型、`reserved[84]` 预留扩展空间。
...
struct airy_ipc_msg_hdr {
    ...
    __u64   timestamp_ns;   /* offset 16, 纳秒时间戳（CLOCK_MONOTONIC） */
...
| `timestamp_ns` | 16 | 8 | 纳秒时间戳（CLOCK_MONOTONIC） |
