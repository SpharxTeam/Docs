...
消息头定义位于 [SC] 共享契约层头文件 `include/airymax/ipc.h`（SSoT 物理宿主见 `50-engineering-standards/120-cross-project-code-sharing.md` §Layout C），遵循 4 空格缩进 + Doxygen 注释规范（详见 [04-coding-standard.md](04-coding-standard.md)）。
...
/* flags 位定义 */
#define AIRY_IPC_FLAG_ZEROCOPY  0x00000001u
#define AIRY_IPC_FLAG_CAP_CARRY 0x00000002u
#define AIRY_IPC_FLAG_COMPRESS  0x00000004u
#define AIRY_IPC_FLAG_ENCRYPT   0x00000008u
...
> **SSoT 声明**：本结构体定义以 `include/airymax/ipc.h`（物理宿主见 `50-engineering-standards/120-cross-project-code-sharing.md` §Layout C）为单一数据源。结构体名 `struct airy_ipc_msg_hdr`，`__attribute__((packed))` 对齐，UAPI 类型 `__u32`/`__u16`/`__u64`/`__u8`，字段顺序 magic/opcode/flags/trace_id/timestamp_ns/src_task/dst_task/payload_len/reserved[84]。本协议主文档与 SSoT 逐字节一致。
                                     ...
                                     const void *payload, size_t payload_len)
{
    ...
    iov[0].iov_len = AIRY_IPC_MSG_HDR_SIZE;
 ...
 */
AIRY_API int airy_sys_ipc_register_ring(int ring_fd, uint32_t dst_pid);
```
...
当消息头 `flags` 中置位 `AIRY_IPC_FLAG_CAP_CARRY` 时，消息可携带 capability 令牌跨进程传递，遵循 seL4 风格的不可伪造语义：
...
1. 发送方在 `flags` 中置位 `AIRY_IPC_FLAG_CAP_CARRY`。
2. 内核校验 `cap_slot` 是否属于 `src_pid`，若不属于则返回 `AIRY_EPERM`。
...
参考 seL4 Endpoint 状态机（ADR-014），agentrt-linux IPC 端点有 3 个状态，由内核维护，用户态不可直接观测：

| 状态 | 含义 | 触发转换 |
|------|------|----------|
| **Idle** | 无消息等待，端点空闲 | Send→SendWaiting；Recv→RecvWaiting |
| **SendWaiting** | 发送方阻塞，等待接收方 | Recv→匹配发送方，双方就绪，转 Idle |
| **RecvWaiting** | 接收方阻塞，等待发送方 | Send→匹配接收方，双方就绪，转 Idle |

```
状态转换图：
  Idle --Send--> SendWaiting --Recv--> Idle（消息传递完成）
  Idle --Recv--> RecvWaiting --Send--> Idle（消息传递完成）
```
...
### 4.8 Reply 原子性（seL4 风格，Step 2.4 #3 补齐）

参考 seL4 Reply 语义（ADR-014），`AIRY_SYS_REPLY` 操作必须是原子的——要么完全成功（接收方收到回复并恢复运行），要么完全失败（回复方阻塞等待接收方就绪）。这保证了 IPC 调用-回复的语义完整性，不会出现"回复已发送但接收方未恢复"的中间状态。

**原子性保证机制**：
1. Reply 操作在内核态持端点锁（`airy_ep->lock`）期间完成。
2. 内核将回复消息写入接收方地址空间，并恢复接收方上下文——两步在同一临界区内完成。
3. 若接收方未在 RecvWaiting 状态，回复方阻塞（转 SendWaiting），等待接收方 Recv。

```c
/* Reply 原子性：内核临界区内完成"写入回复 + 恢复接收方" */
static int airy_ipc_reply(struct airy_endpoint *ep,
                          const struct airy_ipc_msg_hdr *reply,
                          const void *payload, size_t len)
{
    spin_lock(&ep->lock);                    /* 进入临界区 */
    if (ep->state != AIRY_EP_RECV_WAITING) {
        spin_unlock(&ep->lock);
...
**与 seL4 的差异**：seL4 Reply 原子性通过形式化验证保证；agentrt-linux 通过自旋锁 + 临界区保证（非形式化验证），语义与 seL4 一致但实现独立。Reply 原子性语义在 [SS] 语义同源层与 agentrt 共享。
...
| `AIRY_IPC_FLAG_*` flags 位 | ZEROCOPY/CAP_CARRY/COMPRESS/ENCRYPT 标志位 | 消息处理 |
...
| `airy_ipc_register_ring()` | `airy_sys_ipc_register_ring()` | `(int ring_fd, uint32_t dst_pid) -> int` | 用户态 mmap 共享 vs 内核 io_uring_register |
