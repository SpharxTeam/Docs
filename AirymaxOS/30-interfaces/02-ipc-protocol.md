...
1. **定长消息头**: 128 字节定长，packed 紧凑布局（Layout C SSoT，`__attribute__((packed))`），128 字节 = 2 cache lines，便于零拷贝 IORING_OP_URING_CMD + registered buffer。
...
| **[IND] 完全独立层** | 完全独立 | agentrt POSIX MQ + mmap 传输 ↔ agentrt-linux io_uring + SQPOLL + IORING_OP_URING_CMD + registered buffer 零拷贝传输 |
