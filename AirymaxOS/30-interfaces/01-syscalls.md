...
- 编号在 MAJOR 版本内不可变更。
- v1.0.1 起，slot 513-515 重新分配给保留的 3 个控制原语（`airy_sys_rovol_ctl`/`sched_ctl`/`clt_notify`，原 0.1.1 的 519-521 重新编号为 513-515）；原 0.1.1 的 8 个 seL4 风格 IPC syscall（send/recv/nbsend/nbrecv/reply_recv/yield/reply/notify）全部移除，其原 slot 516-518/522-523 纳入预留段 516-535，调用返回 `-AIRY_ENOSYS`，并在 Doxygen 注释中标注 `@deprecated since v1.0.1, use IORING_OP_URING_CMD instead`。
