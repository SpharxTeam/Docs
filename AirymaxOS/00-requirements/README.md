...
> **正式全称**：agentrt-linux（AirymaxAgentOS，极境智能体操作系统，简称 AirymaxOS）\
...
1. **统一需求语言**：为 agentrt-linux 8 个子仓（airymaxos-kernel / services / security / memory / cognition / cloudnative / system / tests）提供一致的需求分类、编号与追溯框架。
...
| 子文档                                        | 关注维度    | 编号前缀 | 核心问题                    |
| ------------------------------------------ | ------- | ---- | ----------------------- |
| [业务需求](01-business-requirements.md)        | 用户与生态维度 | BR-  | agentrt-linux 为谁解决什么问题？ |
| [功能需求](02-functional-requirements.md)      | 能力与行为维度 | FR-  | agentrt-linux 提供哪些具体能力？ |
| [非功能需求](03-non-functional-requirements.md) | 质量属性维度  | NFR- | agentrt-linux 的能力要做到多好？ |
...
**版本演进规划**（战略决策 ADR-VERSION-001，2026-07-09 确立）：

| 版本阶段 | 内核基线 | 状态 | 核心目标 | 迁移路径 |
|---------|---------|------|---------|---------|
| **0.1.1**（文档体系完成） | Linux 6.6 | ✅ 已完成 | 仅仓库占位 + 设计草案 + Linux 内核基线参考声明，需求条目以"占位"形式列出，允许变更 | — |
| **1.x.x**（开发与生产） | **Linux 6.6 长期锁定** | 🔄 开发中 | 需求条目冻结为基线，进入实际开发，需求变更需通过 ADR（架构决策记录）评审。**1.x.x 全系列版本长期锁定 Linux 6.6 内核作为技术基线**，确保 ABI 稳定性与硬件兼容性 | 基线锁定，无迁移 |
| **2.x.x**（演进） | **Linux 7.1 长期锁定** | 📋 规划中 | 基于下一代内核基线（Linux 7.1）与未来 Linux 内核特性进行需求扩展。**2.x.x 全系列版本将升级至 Linux 7.1 内核作为新技术基线**，前瞻性预留设计见 `10-architecture/04-engineering-baseline.md` | 1.x.x→2.x.x 迁移路径见下 |
...
agentrt-linux 采用经典的**三层需求分层模型**，该模型借鉴 IEEE 830 与 ISO/IEC 25010 的需求工程思想，并结合 Airymax 的「五维正交 24 原则」中的「S-2 层次分解原则」与「E-8 可测试性原则」进行适配。
...
|                                                                       |
|   来源：8 子仓职责 / agentrt 同源模块 / agentrt-linux 治理组模块对齐        |
...
agentrt-linux 的需求来源遵循**四源汇聚**原则，每个来源对应一项设计支柱：

### 5.1 来源一：agentrt 同源（设计同源性）

| 同源维度   | agentrt 侧           | agentrt-linux 侧             | 同源红利      |
| ------ | ------------------- | --------------------------- | --------- |
| 调度语义   | MicroCoreRT 调度器     | SCHED\_AGENT 策略（sched\_ext） | 架构同源，无适配层 |
| IPC 协议 | AgentsIPC（128B 消息头） | IPC 子系统（io\_uring 零拷贝）      | 协议同源，低延迟  |
...
**同源带来的需求推导**：agentrt 的设计假设直接成为 agentrt-linux 的需求输入。例如，agentrt 的 MicroCoreRT 假设"任务调度延迟 < 100ms"，直接推导为 agentrt-linux 的 `NFR-P-001`。

### 5.2 来源二：Linux 企业级生态标准规范

agentrt-linux 全面参考 Linux 企业级生态标准，对齐其模块设计与技术规格：

| 内核版本基线                   | 对齐领域                                        | 对应 agentrt-linux 子仓               |
| ------------------------ | ------------------------------------------- | --------------------------------- |
...
| Linux 7.1（2.x.x 基线，ADR-013）  | 具身智能 Claw、超节点 OS                            | airymaxos-cognition / cloudnative |
| agentrt-linux Token 能效框架 | Token 能效优化                                  | airymaxos-cognition               |
| Linux 企业级生态国密            | SM2/SM3/SM4 算法                              | airymaxos-security                |

### 5.3 来源三：微内核设计思想

**微内核设计思想唯一来源：seL4**（ADR-014，2026-07-09 确立）。agentrt-linux 的微内核核心思想仅来源于 seL4，不引入 Zircon / Minix3 / 其他微内核架构，以避免工程思想在哲学和逻辑学层面的多元化冲突。对应设计原则中的「K-1 内核极简」「K-3 服务隔离」：

| 微内核机制           | 借鉴来源          | agentrt-linux 落地                  |
| --------------- | ------------- | --------------------------------- |
| capability 安全模型 | seL4          | airymaxos-security capability 子系统 |
| 消息传递 IPC        | seL4          | airymaxos-kernel io\_uring 消息传递   |
| 服务用户态化          | seL4（root task 模型） | airymaxos-services 12 daemons     |
| 形式化验证           | seL4          | airymaxos-tests-linux seL4 风格验证         |
| 最小特权态代码         | seL4（Liedtke minimality） | airymaxos-kernel 内核极简             |
| 接口契约 XML        | seL4（syscall.xml） | airymaxos-kernel 接口契约定义（R-01） |
| bitfield codegen | seL4（structures.bf） | airymaxos-kernel 位域结构体生成（R-02） |

> **设计决策说明**：seL4 是首个通过完整形式化验证的微内核，其 Liedtke minimality principle、capability 单一安全模型、消息传递 IPC、服务用户态化、接口契约 XML、bitfield codegen 等工程思想已形成完整自洽的体系。引入其他微内核架构（如 Zircon 的对象导向模型、Minix3 的自愈能力）会导致设计哲学冲突——例如 seL4 的"机制而非策略"原则与 Zircon 的"内核对象导向"存在根本性分歧。因此 agentrt-linux 的微内核思想来源单一化为 seL4，确保工程思想的一致性与可验证性。

### 5.4 来源四：Agent 工作负载

agentrt-linux 作为通用 AI Agent 运行底座，面向各行各业的 Agent 工作负载。Agent 工作负载按技术特征归纳为四类核心工作负载模式，每类模式对应多个行业的 Agent 应用：

| 工作负载模式 | 工作负载特性 | 覆盖行业 | 驱动的需求 |
| ---------- | --------- | ------- | --------- |
| 模式一：长序列知识密集型 | 长序列任务、知识图谱构建、文档分析 | 科研教育、法律合规、医疗健康、媒体内容、软件开发 | BR-001、FR-005、FR-013 |
| 模式二：高并发交互型 | 高并发对话、情感分析、多轮交互 | 零售电商、政务服务、金融服务、物流交通、智慧城市 | BR-002、FR-001、FR-031 |
| 模式三：实时控制型 | 实时控制、故障诊断、设备集成 | 工业制造、能源电力、农业科技 | BR-003、NFR-P-001、NFR-R-001 |
| 模式四：多模态感知型 | 传感器融合、运动控制、环境感知 | 具身智能、自动驾驶、智慧城市、农业科技 | BR-004、FR-022、FR-023 |

***

## 6. 与其他设计层次的链接

需求层是整个 agentrt-linux 文档体系的起点，向下驱动四个设计层次：
...
2. **讨论**：工程规范委员会组织讨论，评估变更对 8 子仓的影响。
3. **评审**：工程规范委员会投票决定是否采纳，需 2/3 多数通过。
...
| ---- | ---- | --------------------- |
| 新增需求 | 单个子仓 | 工程规范委员会评审               |
| 修改需求 | 跨子仓  | 工程规范委员会评审 + ADR         |
| 废弃需求 | 跨层次  | 工程规范委员会评审 + ADR + 兼容性评估 |
...
| 需求层次       | 验证方法               | 验证工具                               | 责任子仓                       |
| ---------- | ------------------ | ---------------------------------- | -------------------------- |
| 业务需求       | 用户场景验收测试（UAT）      | Python + pytest                    | airymaxos-tests-linux            |
| 功能需求       | 单元测试 + 集成测试 + 契约测试 | CUnit + CMock + 自定义框架              | airymaxos-tests-linux            |
| 非功能需求-性能   | 性能基准测试             | Locust + k6 + perf                 | airymaxos-tests-linux            |
| 非功能需求-安全   | 渗透测试 + 形式化验证       | seL4 风格验证 + 静态分析                   | airymaxos-tests-linux + security |
| 非功能需求-可靠性  | Soak Test + 混沌工程   | Chaos Mesh + agentrt-linux 系统级测试套件 | airymaxos-tests-linux            |
| 非功能需求-兼容性  | 兼容性测试矩阵            | agentrt-linux 集成测试框架               | airymaxos-tests-linux + system   |
| 非功能需求-可观测性 | 可观测性覆盖检查           | Prometheus + OpenTelemetry         | airymaxos-tests-linux            |

***

## 10. 与设计原则的映射

agentrt-linux 的需求体系与「五维正交 24 原则」保持紧密映射，每条需求都应能追溯到至少一条设计原则：
...
| 版本    | 日期         | 变更内容               | 变更人           |
| ----- | ---------- | ------------------ | ------------- |
| 0.1.1 | 2026-07-06 | 初始版本，建立需求分层模型与追溯框架 | 工程规范委员会 |
