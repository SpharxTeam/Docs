...
|---------|---------|------|---------|---------|
| **0.1.1**（文档体系） | Linux 6.6 | 文档 | 设计草案 + Linux 内核基线参考声明，需求条目以"占位"形式列出，允许变更 | — |
| **1.x.x**（开发与生产） | **Linux 6.6 长期锁定** | 开发 | 需求条目冻结为基线，进入实际开发，需求变更需通过 ADR（架构决策记录）评审。**1.x.x 全系列版本长期锁定 Linux 6.6 内核作为技术基线**，确保 ABI 稳定性与硬件兼容性 | 基线锁定，无迁移 |
| **2.x.x**（演进） | **Linux 7.1 长期锁定** | 规划 | 基于下一代内核基线（Linux 7.1）与未来 Linux 内核特性进行需求扩展。**2.x.x 全系列版本将升级至 Linux 7.1 内核作为新技术基线**，前瞻性预留设计见 `10-architecture/04-engineering-baseline.md` | 1.x.x→2.x.x 迁移路径见下 |
