...
| ------------------------ | ------------------------------------------- | --------------------------------- |
| Linux 6.6 内核基线           | 内核基线（Linux 6.6）、RPM 包格式、dnf、systemd、SELinux | kernel / system         |
| Linux 6.6 内核基线（SP3 增强）   | 认知循环系统、AI 原生特性                              | cognition               |
| Linux 7.1（2.x.x 基线，ADR-013）  | 具身智能 Claw、超节点 OS                            | cognition / cloudnative |
| agentrt-linux Token 能效框架 | Token 能效优化                                  | cognition               |
| Linux 企业级生态国密            | SM2/SM3/SM4 算法                              | security                |
...
| --------------- | ------------- | --------------------------------- |
| capability 安全模型 | seL4          | security capability 子系统 |
| 消息传递 IPC        | seL4          | kernel io\_uring 消息传递   |
| 服务用户态化          | seL4（root task 模型） | services 12 daemons     |
| 形式化验证           | seL4          | tests-linux seL4 风格验证         |
| 最小特权态代码         | seL4（Liedtke minimality） | kernel 内核极简             |
| 接口契约 XML        | seL4（syscall.xml） | kernel 接口契约定义（R-01） |
| bitfield codegen | seL4（structures.bf） | kernel 位域结构体生成（R-02） |
...
| 非功能需求-可靠性  | Soak Test + 混沌工程   | Chaos Mesh + agentrt-linux 系统级测试套件 | tests-linux            |
| 非功能需求-兼容性  | 兼容性测试矩阵            | agentrt-linux 集成测试框架               | tests-linux + system   |
