...
```c
/* include/linux/airy_cjk.h —— 内核态 CJK 宽度计算（禁用 wchar_t） */
#ifndef AIRY_CJK_H
#define AIRY_CJK_H
...
/* 字符宽度枚举 */
enum airy_char_width {
	AIRY_WIDTH_INVALID = -1,
	AIRY_WIDTH_ZERO     = 0,    /* 控制字符 */
	AIRY_WIDTH_NARROW   = 1,    /* ASCII、半角 */
	AIRY_WIDTH_WIDE     = 2,    /* CJK、全角 */
};

/* 判断码点是否为宽字符（CJK） */
static inline bool airy_cjk_is_wide(airy_unicode_t cp)
{
	/* CJK 统一表意文字（基本） */
...
static inline bool airy_cjk_is_zero_width(airy_unicode_t cp)
{
	if (cp == 0)
		return true;                       /* NUL */
...
static inline bool airy_cjk_is_ambiguous(airy_unicode_t cp)
{
...
/* 计算单字符显示宽度（基于 locale） */
static inline int airy_cjk_char_width(airy_unicode_t cp, bool cjk_locale)
{
	int w;

	if (airy_cjk_is_zero_width(cp))
		return AIRY_WIDTH_ZERO;
	if (airy_cjk_is_wide(cp))
		return AIRY_WIDTH_WIDE;
	if (airy_cjk_is_ambiguous(cp))
		return cjk_locale ? AIRY_WIDTH_WIDE : AIRY_WIDTH_NARROW;
	return AIRY_WIDTH_NARROW;
}

/* 计算字符串显示宽度（按 UTF-8 字节序列） */
static inline int airy_cjk_str_width(const char *s, size_t len,
				       bool cjk_locale)
{
	...
	airy_utf8_iter_init(&it, s, len);
	while ((n = airy_utf8_next(&it, &cp)) > 0) {
		int w = airy_cjk_char_width(cp, cjk_locale);
		if (w < 0)
			return w;
		width += w;
	}
...
/* 计算字符串显示宽度（用户态版本） */
int airy_user_str_width(const char *s, size_t len, bool cjk_locale)
{
	const char *p = s;
	...
	while (p < end) {
		airy_unicode_t cp;
		int n = airy_utf8_decode((const unsigned char *)p,
					   end - p, &cp);
		if (n < 0)
			return -1;
		total += airy_cjk_char_width(cp, cjk_locale);
		p += n;
	}
	return total;
}
...
int main(void)
{
	...
	printf("zh width = %d (expected %zu*1 + %zu*2)\n",
		airy_cjk_str_width(zh, strlen(zh), true),
		(size_t)0, strlen(zh));
	printf("ja width = %d\n",
		airy_cjk_str_width(ja, strlen(ja), true));
	printf("ko width = %d\n",
		airy_cjk_str_width(ko, strlen(ko), true));
	printf("mix width (CJK locale) = %d\n",
		airy_cjk_str_width(mix, strlen(mix), true));
...
int main(void)
{
	...
	/* 显示中文（按宽度对齐） */
	int zh_w = airy_cjk_str_width(zh_msg, strlen(zh_msg), true);
	mvprintw(row / 2 - 1, (col - zh_w) / 2, "%s", zh_msg);
	attron(COLOR_PAIR(1));
	attroff(COLOR_PAIR(1));

	/* 显示日文 */
	int ja_w = airy_cjk_str_width(ja_msg, strlen(ja_msg), true);
...
```c
/* 终端 CJK 字符串居中对齐 */
int airy_terminal_center_print(WINDOW *win, int row, int max_col,
				  const char *msg)
{
	int width;
	int col;

	width = airy_cjk_str_width(msg, strlen(msg), true);
	if (width < 0)
		return width;
...
int airy_sm3_hash_cjk(const char *input, size_t input_len,
			unsigned char *out, size_t *out_len)
{
	...
	/* 1. 校验 UTF-8 合法性 */
	if (airy_utf8_validate(input, input_len) < 0)
		return -EILSEQ;
...
| 错误码 | 数值 | 含义 |
|--------|------|------|
| AIRY_E_CJK_INVALID_CP | -911 | 无效 CJK 码点 |
| AIRY_E_CJK_FONT_MISSING | -912 | CJK 字体缺失 |
| AIRY_E_CJK_WIDTH_AMBIGUOUS | -913 | 字符宽度歧义 |
| AIRY_E_CJK_INPUT_METHOD | -914 | 输入法错误 |
| AIRY_E_CJK_TERMINAL | -915 | 终端 CJK 支持错误 |
...
|------|------------------------|------------------|------|
| CJK 宽度计算 | 内核 + 用户态 | 用户态 | `airy_cjk.h` 完全共享 |
| Unicode 范围表 | 编译期常量 | 编译期常量 | 范围定义 |
| 错误码 | AIRY_E_CJK_* | AIRY_E_CJK_* | `error.h` |
...
> **文档结束** | agentrt-linux（AirymaxOS）CJK 支持详细设计 
