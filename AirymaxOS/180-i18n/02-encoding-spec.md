...
本设计严格遵守 agentrt-linux 术语规范：agentrt（用户态）称为**微核心**（micro-core），agentrt-linux（OS 发行版）称为**微内核**（micro-kernel）。编码契约在 agentrt 与 agentrt-linux 之间属于 IRON-9 v3 [SC] 共享契约层。
