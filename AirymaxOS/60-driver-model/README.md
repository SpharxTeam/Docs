...
agentrt-linux 驱动模型是连接内核能力与硬件/虚拟设备的核心抽象层。它继承 Linux 内核 30+ 年沉淀的 device/driver/bus 三元组解耦哲学，并在其上扩展智能体工作负载所需的"软驱动"概念——Agent 行为契约作为一类虚拟设备参与驱动模型。
...
agentrt-linux 将 Agent 行为契约建模为虚拟设备：
