...
| **Macro-Supervisor** | 用户态 `macro_superv` daemon | 软约束温情裁决——超限可申诉、策略可热更新 | 配额策略下发、申诉仲裁、长期趋势分析 |
...
1. **全局配额**（Global Quota）：系统级上限，由 A-UCS 配置（`/etc/airymax/dev_quota.json`）
...
| `AIRY_QUOTA_ESCALATE` | 上报 macro_superv 仲裁 | 高优先级 Agent |
...
| CAP_DEV_QUOTA_BYPASS | macro_superv 仅 | 系统启动 | 永不撤销 |
