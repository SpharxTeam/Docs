...
| **[SS] 语义同源层** | API 签名同源，实现独立 | `platform_driver_register` 注册模式、probe/remove 生命周期、`devm_platform_ioremap_resource` 资源管理 |
...
agentrt daemons 的 `agentrt_agent_module_register()` 与内核 `platform_driver_register()` 同源——两者都遵循"注册 → platform 匹配（OF/SDK 接口精确匹配 → 名称回退）→ probe 探测 → remove 卸载"生命周期。[SS] 语义同源在此体现为：API 签名同源（注册/匹配/生命周期/资源映射四段式），实现独立（用户态 daemon 服务 vs 内核 platform bus）。
