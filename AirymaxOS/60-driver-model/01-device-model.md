...
# agentrt-liunx（AirymaxOS）驱动模型 — device/driver/bus 三元组详解

> **文档定位**: agentrt-liunx（AirymaxOS）驱动子系统 60 模块首篇——device/driver/bus/class 四元关系核心抽象
...
Linux 设备模型核心可一句话概括："**device 描述有什么，driver 描述如何做，bus 负责把两者绑在一起**"。agentrt-liunx 选择 Linux 6.6 内核基线作为驱动模型同源起点——这是经过最广泛硬件生态验证的解耦哲学。MicroCoreRT 在用户态重新实现智能体调度，AgentsIPC 在用户态承载进程间通信，而内核态硬件抽象层完整继承这套三元组。

本文档覆盖 device/driver/bus/class 四元关系、probe/remove 生命周期、name/OF/ACPI 三源匹配、sysfs 导出、kref/kobject 引用计数五大主题。IRON-9 v2 同源且部分代码共享体现为：内核态结构体与上游保持语义等价（确保可直接复用上游驱动代码）；用户态扩展（`agent_bus_type`、`devm_agentrt_*` 资源族）不进入上游内核，而是作为 agentrt-liunx 专属扩展通过 `agentrt/daemon/` 适配器接入。
...
| 角色 | 内核类型 | 头文件 | 职责 | agentrt-liunx 类比 |
|------|---------|--------|------|----------------|
...
> **OS-STD-010**: 任何 agentrt-liunx 内核态 driver 必须通过 `driver_register()` 注册，禁止绕过驱动核心直接调用 `probe`。
...
ACPI 匹配通过 `_HID`（Hardware ID）匹配 driver 的 `acpi_match_table`。同一 driver 可同时声明 OF 与 ACPI 匹配表，实现"一次编写，双源运行"。`platform_match` 会先尝试 OF，再尝试 ACPI，最后回退到名称匹配——这是 agentrt-liunx 实现"同一 driver 服务异构硬件平台"的基石，也是 E-4 跨平台一致性原则在内核态的体现。
...
每个 `struct device` 注册时在 `/sys/devices/` 下创建目录，每个 `struct device_driver` 在 `/sys/bus/<bus>/drivers/` 下创建目录。agentrt-liunx 的可观测性原则（E-2）要求所有 driver 暴露必要的属性用于诊断。
...
device/driver/bus 四元抽象在 agentrt-liunx 五维正交 24 原则上的映射：
...
device/driver/bus 三元组在 agentrt-liunx 用户态（agentrt）中的同源映射：
...
| 1.0.1 | 2026-07-06 | agentrt-liunx 驱动子系统组 | 开发版本，完成四元关系、生命周期、匹配、sysfs、kref 五章正文 |
...
> **文档结束** | agentrt-liunx 驱动模型 60 模块首篇 | IRON-9 v2 同源且部分代码共享 | Linux 6.6 内核基线
