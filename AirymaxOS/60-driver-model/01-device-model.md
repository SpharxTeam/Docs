...
本文档覆盖 device/driver/bus/class 四元关系、probe/remove 生命周期、name/OF/ACPI 三源匹配、sysfs 导出、kref/kobject 引用计数五大主题。IRON-9 v2 同源且部分代码共享体现为：内核态结构体与上游保持语义等价（确保可直接复用上游驱动代码）；用户态扩展（`agent_bus_type`、`devm_agentrt_*` 资源族）不进入上游内核，而是作为 agentrt-linux 专属扩展通过 `agentrt/daemon/` 适配器接入。
...
| `agent_bus_type` / `devm_agentrt_*` | agentrt-linux 自研 | 独立演进，不回传上游 |
...
| **[SS] 语义同源层** | API 签名同源，实现独立 | `device_register`/`driver_register` 注册模式、bus 匹配逻辑、probe/remove 生命周期、devm_ 资源管理 |
...
```c
/* include/airymax/sched.h —— IRON-9 v2 [SC] 间接依赖（节选，agent_id 用于 Agent 驱动匹配） */
...
struct airymax_task_descriptor {
    ...
    uint32_t agent_id;       /* Agent 实例 ID，用于 device_driver 匹配 */
...
|------|--------------------------|--------------------------------------|--------|
| 设备注册 | `agentrt_agent_instance_register()` | `device_register()` | 注册模式同源 |
| 驱动注册 | `agentrt_agent_module_register()` | `driver_register()` | 注册模式同源 |
| bus 匹配 | `agent_bus_type.match()` | `bus_type.match()` | 匹配逻辑同源 |
| probe 生命周期 | `agentrt_agent_module.probe()` | `device_driver.probe()` | 生命周期同源 |
| remove 生命周期 | `agentrt_agent_module.remove()` | `device_driver.remove()` | 卸载语义同源 |
| devm 资源管理 | `devm_agentrt_*` 托管族 | `devm_kzalloc` / `devm_*` | 资源托管语义同源（LIFO） |
| 引用计数 | `agentrt_handle` + refcount | `kobject` / `kref` | 引用计数语义同源 |

agentrt daemons 的 `agentrt_agent_module_register()` 与内核 `driver_register()` 同源——两者都遵循"注册 → bus 匹配 → probe 探测 → remove 卸载"生命周期。[SS] 语义同源在此体现为：API 签名同源（注册/匹配/生命周期四段式），实现独立（用户态 daemon 服务 vs 内核 device core）。
...
`agent_driver` 描述特定角色 Agent 的操作方式。`probe` 在 Agent 实例就绪时被 `agent_bus` 触发，完成 SDK 绑定与资源分配；`remove` 在 Agent 终止时触发，释放 `devm_agentrt_*` 托管资源。
 ...
 * show 回调必须用 sysfs_emit（OS-STD-012），返回字节数（OS-STD-013）。
 *
 * @name: 属性名（sysfs 文件名）
 */
