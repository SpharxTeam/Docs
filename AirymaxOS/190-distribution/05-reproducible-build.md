...
- Linux 6.6 内核基线构建（airymaxos-kernel）
...
/* airymaxos-tests-linux/reproducible/test_source_date_epoch.c */
#include <stdio.h>
#include <string.h>
...
```dockerfile
# airymaxos-cloudnative/oci/Containerfile.agent-base
# 可重现 Agent 基础镜像
...
```python
#!/usr/bin/env python3
# airymaxos-tests-linux/reproducible/verify_matrix.py
# 可重现构建验证矩阵
...
def main():
    ...
    products = [
        ...
        BuildProduct("oci-image", Path("agent-base-1.0.1.tar"),
                     "oci", "x86_64"),
        BuildProduct("python-wheel", Path("airymaxos_sdk-1.0.1-py3-none-any.whl"),
                     "wheel", "noarch"),
        BuildProduct("rust-crate", Path("airymaxos_sdk-1.0.1.crate"),
                     "crate", "noarch"),
