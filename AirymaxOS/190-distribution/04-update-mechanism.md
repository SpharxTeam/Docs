...
本设计严格遵守 agentrt-linux 术语规范：agentrt（用户态）称为**微核心**（micro-core），agentrt-linux（OS 发行版）称为**微内核**（micro-kernel）。所有外部 Linux 发行版统一表述为"主流 Linux 发行版"，禁止使用 openEuler/Euler 字样。更新机制与 agentrt 之间属于 IRON-9 v2 [IND] 完全独立层。
...
```c
/* airymaxos-kernel/livepatches/lp_sched_agent_fix.c */
...
#     Version: 1.0.0
#     Commit: 0a1b2c3d4e5f...
#     Subject: agentrt-linux 1.0.0 release

# 当前活跃版本（* 标记）为 1.0.2，可回滚到 1.0.1 或 1.0.0
```
...
layers:
  L1_raw:
    ...
    used_gb: 32.5
    format: zstd_v1
    schema_compatible: [1.0.0, 1.0.1, 1.0.2]
  L2_feat:
    capacity_gb: 64
    used_gb: 28.3
    format: hnsw_v1
...
> **文档结束** | agentrt-linux（AirymaxOS）更新机制设计详细规范 | 1.0.1 开发版本
