...
# agentrt-liunx（AirymaxOS）安装器设计

> **文档定位**: agentrt-liunx（AirymaxOS，极境智能体操作系统）系统安装器工程设计文档，覆盖分区方案、引导加载器配置、最小安装 vs 完整安装、kickstart 配置
...
agentrt-liunx（AirymaxOS）作为智能体操作系统发行版，其安装器是用户首次接触产品的入口。本文档定义安装器工程设计，目标是：
...
| 内核版本 | Linux 6.6 | agentrt-liunx 1.0.1 锁定基线 |
| 字符串安全 | `strscpy` | 安装器代码中字符串操作必须安全 |

### 1.3 术语规范

本文档严格遵循术语规范：agentrt（用户态运行时）= 微核心（micro-core）；agentrt-liunx（OS 发行版）= 微内核（micro-kernel）。安装器设计独立于 agentrt（[IND] 完全独立层），不与 agentrt 共享安装器代码。文中不使用 openEuler/Euler 字样，安装器实践对照一律表述为"主流 Linux 发行版"。
...
最小安装包含运行 agentrt-liunx 所需的最小包集：
...
agentrt-liunx 默认采用 Btrfs 子卷方案，支持快照回滚：
...
agentrt-liunx 使用 GRUB 2.06+ 作为引导加载器，支持 UEFI 与 BIOS 两种模式：
...
menuentry "agentrt-liunx (AirymaxOS) 1.0.1" {
...
agentrt-liunx 启用 Secure Boot，所有内核模块与 GRUB EFI 程序必须签名：
...
agentrt-liunx 在引导阶段启用 TPM 2.0 度量，记录引导链各阶段哈希：
...
# agentrt-liunx (AirymaxOS) 最小安装 kickstart 配置
...
echo "agentrt-liunx 最小安装完成" >> /var/log/airymaxos-install.log
%end
...
# agentrt-liunx (AirymaxOS) 完整安装 kickstart 配置
...
echo "agentrt-liunx 完整安装完成" >> /var/log/airymaxos-install.log
%end
...
```
2026-07-09 10:00:00 [INFO] agentrt-liunx 安装器启动
...
2026-07-09 10:09:00 [INFO] agentrt-liunx 完整安装完成
...
```bash
#!/bin/bash
# airymaxos-tests/install/test_install.sh [IND]
# 自动化安装测试
...
> **文档结束** | agentrt-liunx（AirymaxOS）安装器设计 v1.0.1
