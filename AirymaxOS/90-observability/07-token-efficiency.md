...
> **同源映射**：agentrt E-2 可观测性 + cogn_d daemon + A-ULS macro_superv 监管\
> **理论根基**：Linux 6.6 内核基线 + Airymax 五维正交 24 原则 + sched_tac 调度\
> **核心约束**：Token 效率监控是 agentrt-linux 专属 L8 层可观测性，A-ULS 模块 macro_superv 强制监管
...
1. **经济性约束**：Agent 调用 LLM 推理服务按 Token 计费，无约束的 Token 消耗会导致系统运行成本失控。Token 效率监控是 A-ULS 模块 macro_superv 执行预算监管的数据基础。
2. **行为诊断**：Token 消耗模式反映 Agent 决策质量——高 input/output 比率可能意味着提示工程低效，低 output/input 比率可能意味着模型输出冗余。Token 效率指标是 Agent 行为追踪的关键维度。
3. **sched_tac 协同**：SCHED_DEADLINE Agent 的 Token 预算必须与 runtime 预算匹配；Token 预算溢出会触发 macro_superv 降级 Agent 至 SCHED_FIFO，是sched_tac 调度类切换的触发条件之一。
...
| A-ULS 监管 | `daemons/macro_superv/token_superv.c` | 预算溢出告警与执法 |
| A-ULP 持久化 | `daemons/logger_daemon/token_persister.c` | 历史趋势落盘 |
...
| 危险 | 红 | output_input_ratio < 0.2 或 budget_ratio > 0.9 | macro_superv 降级或终止 |
...
### 4.1 A-ULS 模块 macro_superv 监管

A-ULS（Unified Supervision）模块的 macro_superv daemon 负责 Token 预算监管。监管策略分三层：
...
### 4.3 macro_superv 执法动作

macro_superv 收到 Token 预算告警后，执行对应执法动作：

| 告警等级 | macro_superv 动作 | 微观效果 |
|---------|------------------|---------|
...
### 6.1 logger_daemon 持久化

logger_daemon 定期将 Token 效率快照持久化至磁盘：
...
logger_daemon 提供趋势分析工具 `airy-token-trend`：
...
基于历史趋势，logger_daemon 可预测 Token 预算耗尽时间：
...
**OS-OBS-074: logger_daemon 必须每 5 分钟生成一次 Token 效率快照；快照保留至少 90 天，供趋势分析与容量规划使用。**
...
| **A-UCS** | 辅助 | Token 预算配置通过 A-UCS config_daemon 热重载 |
| **A-ULS** | **核心** | macro_superv 强制监管 Token 预算，执行降级/终止 |
...
| Daemon | Token 监控角色 |
|--------|--------------|
| macro_superv | **执行者**：预算溢出告警与执法 |
| logger_daemon | **持久化者**：历史趋势落盘 |
| config_daemon | **配置者**：预算阈值热重载 |
...
- [../20-modules/](../20-modules/README.md)：模块设计（cogn_d / macro_superv / logger_daemon）
...
| 版本 | 日期 | 变更 |
|------|------|------|
| v1.0.1 | 2026-07-18 | 初始版本：Token 消耗追踪机制、效率指标体系（output/input、effective、cache_hit）、预算溢出告警（L1/L2/L3 三层）、/proc/airy/agents/[id]/token_stats 接口、logger_daemon 历史趋势持久化、cogn_d daemon 协同 |
