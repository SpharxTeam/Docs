...
2. **资源配额约束**：记忆层级占用物理内存与磁盘空间，无约束的记忆增长会导致系统 OOM 或磁盘耗尽。记忆配额监控是 A-ULS 模块 macro_superv 执行记忆监管的数据基础。
...
| A-ULS 监管 | `daemons/macro_superv/mem_superv.c` | 记忆配额监管 |
| 持久化 | `daemons/logger_daemon/mem_persister.c` | 历史趋势落盘 |
...
4. **记忆配额执行**：执行 A-ULS macro_superv 下发的配额决策。
...
A-ULS 模块 macro_superv daemon 负责记忆配额监管。监管策略分三层：
...
每个 Agent 的记忆配额通过 A-UCS config_daemon 配置：
...
| **A-UCS** | 辅助 | 记忆配额配置通过 A-UCS config_daemon 热重载 |
| **A-ULS** | **核心** | macro_superv 强制监管记忆配额，执行降级/终止 |
...
| cogn_d | **上报者**：记忆访问事件上报 |
| macro_superv | **监管者**：配额溢出告警与执法 |
| logger_daemon | **持久化者**：历史趋势落盘 |
| config_daemon | **配置者**：配额阈值热重载 |
...
- [../20-modules/](../20-modules/README.md)：模块设计（mem_d / macro_superv）
