...
| 持久化 | `daemons/logger_daemon/trace_persister.c` | 追踪数据落盘 |
...
**OS-OBS-082: 生产环境默认启用粗粒度追踪；中/细粒度追踪必须经 macro_superv 批准，且设置自动关闭时间（≤ 30 分钟）。**
...
### 5.1 logger_daemon → Ring Buffer → 文件

Agent 行为追踪数据通过 logger_daemon 持久化至磁盘：

```mermaid
graph LR
    A[ftrace ring buffer] --> D[logger_daemon]
...
logger_daemon 通过 `trace_pipe` 流式消费追踪数据：

```bash
# logger_daemon 消费 ftrace 实例
cat /sys/kernel/tracing/instances/agent_trace/trace_pipe | \
    logger_daemon --trace-input --agent 42

# 消费 user_events
cat /sys/kernel/tracing/trace_pipe | \
    logger_daemon --user-events-input
```
...
| `SUPERVISE_SUSPEND` | 监管挂起 | macro_superv |
| `SUPERVISE_RESUME` | 监管恢复 | macro_superv |
| `SUPERVISE_TERMINATE` | 监管终止 | macro_superv |
| `BUDGET_EXCEEDED` | 预算耗尽 | macro_superv |
...
| **A-ULP** | **核心** | 追踪数据通过 logger_daemon 持久化至磁盘 |
| **A-UCS** | 辅助 | 追踪粒度配置通过 A-UCS config_daemon 热重载 |
...
| Daemon | 在行为追踪中的角色 |
|--------|------------------|
| macro_superv | **被追踪者**：监管执法事件被追踪 |
| logger_daemon | **追踪者**：消费并持久化追踪数据 |
...
| 版本 | 日期 | 变更 |
|------|------|------|
| v1.0.1 | 2026-07-18 | 初始版本：五大追踪维度（状态转换/调度延迟/IPC 交互/记忆访问/认知循环）、ftrace+user_events+eBPF 三工具栈、airy_trace_agent_call_graph() API、logger_daemon 持久化、perf report + 自定义脚本分析 |
