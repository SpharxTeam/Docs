...
# agentrt-liunx（AirymaxOS）eBPF 可编程探针

> **文档定位**: agentrt-liunx（AirymaxOS）可观测性体系 L2 层——可编程内核探针 eBPF 的工程规范
...
- [第 11 章 agentrt-liunx Token 能效监控与 Agent 行为追踪](#第-11-章-airymaxos-token-能效监控与-agent-行为追踪)
...
eBPF（extended Berkeley Packet Filter）是 Linux 6.6 内核基线提供的可编程探针框架。它允许在内核态运行经过验证器验证的字节码程序，无需修改内核源码或加载内核模块。agentrt-liunx 选择 eBPF 作为可观测性 L2 层，原因有三：
...
**OS-OBS-011: eBPF 是 agentrt-liunx 可观测性 L2 层的强制基线，所有动态可编程观测必须经 eBPF 实现，不得自行编写内核模块。**

**OS-KER-011: airymaxos-kernel 的 defconfig 必须开启 CONFIG_BPF、CONFIG_BPF_SYSCALL、CONFIG_BPF_JIT、CONFIG_BPF_EVENTS、CONFIG_DEBUG_INFO_BTF。**
...
**OS-KER-012: airymaxos-kernel 必须启用 CONFIG_BPF_JIT_ALWAYS_ON，确保 BPF 程序以本机码执行而非解释执行，避免性能损失。**

**OS-STD-010: BPF 程序复杂度（指令数）不得超过验证器上限（Linux 6.6 默认 100 万指令）；超限程序必须拆分为多个小程序。**
...
**OS-OBS-012: airymaxos-kernel 必须开启 CONFIG_DEBUG_INFO_BTF，并在构建产物中提供 `vmlinux.btf`；Agent 跟踪 BPF 程序必须基于 BTF 重定位（CO-RE），不得硬编码结构体偏移。**

**OS-KER-013: airymaxos-kernel 必须为 Agent 核心结构（`struct agent_descriptor`、`struct agent_decision`）导出 BTF，确保 eBPF 程序可安全访问。**
...
**OS-OBS-013: agentrt-liunx 自定义 kfunc 必须用 `__bpf_kfunc` 宏标注，并通过 `BTF_KFUNCS_START`/`BTF_KFUNCS_END` 注册；未注册的内核函数不得被 BPF 程序调用。**

**OS-KER-014: airymaxos-kernel 必须注册 `bpf_agent_decision_get`、`bpf_agent_token_usage_get` 两个 kfunc，供 BPF 程序查询 Agent 决策与 Token 消耗。**

### 4.3 kfunc 与 helper 区别

BPF helper 是早期内核函数调用接口，签名固定、有限集；kfunc 是新一代机制，签名通过 BTF 描述、可扩展。Linux 6.6 内核基线推荐新代码使用 kfunc。agentrt-liunx 遵循此约定：新观测 kfunc 优先，仅当无对应 kfunc 时退回 helper。
...
**OS-OBS-014: agentrt-liunx BPF 程序访问变长 Agent payload 必须使用 dynptr（`bpf_dynptr_from_skb`、`bpf_dynptr_slice`），不得直接对裸指针做算术运算。**

**OS-STD-011: BPF 程序中所有 dynptr 在退出前必须调用 `submit` 或 `discard`，验证器检测到未释放 dynptr 必须拒绝加载。**
...
加载失败时验证器返回详细日志，可通过 `bpf_attr.log_buf` 获取。验证器日志含程序名、失败指令、错误原因三要素。agentrt-liunx 规定所有 BPF 加载失败必须将验证器日志落审计至 `/var/log/airymaxos/bpf-load.log`，便于事后排查。
...
Linux 6.6 内核基线在 `include/uapi/linux/bpf.h` 定义了 30+ 种 BPF 程序类型。agentrt-liunx 主要使用以下七种：
...
| `BPF_PROG_TYPE_STRUCT_OPS` | struct_ops 成员 | SCHED_AGENT 调度回调 |
...
int trace_cognition(struct pt_regs *ctx)
{
    ...
    bpf_trace_printk("cognition pid=%u ts=%llu\\n", pid, ts);
    return 0;
}
...
kprobe 挂载于函数入口，kretprobe 挂载于函数返回。kretprobe 通过 `bpf_get_func_ip()` 与 `bpf_get_func_arg()` 获取返回值。agentrt-liunx Agent 行为追踪同时使用两者：kprobe 记录决策入口，kretprobe 记录决策耗时与结果。

**OS-OBS-016: Agent 决策追踪必须同时挂载 kprobe（入口）与 kretprobe（返回），形成完整闭环；仅挂载单一探针视为不完整观测。**

**OS-KER-016: airymaxos-kernel 必须保证 Agent 核心路径函数（`agentrt_cognition_process`、`agentrt_planner_dag_update`、`agentrt_scheduler_dispatch`、`agentrt_execution_unit_run`）支持 kprobe 挂载，不得使用 `__always_inline` 或 `notrace`。**

### 7.3 tracepoint 与 raw_tracepoint

tracepoint 程序读取 tracepoint 参数需经 BTF 重定位；raw_tracepoint 性能更高但需手动解析参数。agentrt-liunx 在高频路径（Agent 决策）使用 raw_tracepoint，低频路径（配置变更）使用 tracepoint。

### 7.4 struct_ops 与 tracing 程序

`BPF_PROG_TYPE_STRUCT_OPS` 程序作为子系统的操作表成员，通过 `BPF_MAP_TYPE_STRUCT_OPS` map 注册。agentrt-liunx SCHED_AGENT 调度器通过此类型集成（详见第 10 章）。`BPF_PROG_TYPE_TRACING` 程序通过 fentry/fexit 挂载到内核函数入口/出口，相比 kprobe 性能更高（无 int3 陷阱），但需 BTF 支持。
...
Linux 6.6 内核基线提供 28+ 种 BPF map 类型。agentrt-liunx 常用以下几种：
...
eBPF 程序从源码到运行经历五步：编译（clang -target bpf）、加载（bpf(BPF_PROG_LOAD)）、attach（kprobe/tracepoint/perf_event/struct_ops 挂载）、运行、卸载。agentrt-liunx 推荐使用 libbpf 框架管理生命周期：
...
struct bpf_link *link = bpf_program__attach_kprobe(
    ...
    false, "agentrt_cognition_process");
...
**OS-OBS-018: agentrt-liunx 必须为每个 BPF 程序实现显式卸载路径（`bpf_link__destroy`），不得依赖进程退出隐式卸载；Agent 重载策略时必须先卸载旧 BPF 程序再加载新版本。**

**OS-KER-017: airymaxos-kernel 必须支持 `bpf(BPF_LINK_CREATE)` 显式 link 创建，确保 BPF 程序生命周期可独立于加载进程管理。**

**OS-STD-014: BPF 程序 .o 文件必须随 airymaxos-kernel 发布包分发，存放于 `/usr/lib/airymaxos/bpf/`，便于 Agent 重载时引用。**

---

## 第 10 章 struct_ops 注册机制与 SCHED_AGENT 集成

### 10.1 struct_ops 设计哲学

struct_ops 是 Linux 6.6 内核基线提供的机制，允许 BPF 程序作为内核子系统的操作表（struct of function pointers）。agentrt-liunx 通过此机制实现 SCHED_AGENT 可插拔调度器和 Cupolas 动态安全策略。

**核心设计哲学**：

1. **结构体即接口**：内核子系统定义 `struct xxx_ops`（函数指针集合），BPF 程序填充此结构体
2. **双映像模型**：用户态映像（uvalue，含 prog fd）与内核态映像（kvalue，含真实函数指针）分离
3. **trampoline 桥接**：通过 trampoline 本机码桥接内核调用与 BPF 程序入口
4. **状态机管理**：INIT → INUSE → TOBEFREE 四状态生命周期
5. **CFI 安全**：cfi_stubs 提供 kCFI 桩函数，防止控制流劫持

**OS-OBS-021: agentrt-liunx 可插拔调度策略必须通过 struct_ops 机制集成，不得自行编写调度类内核模块；SCHED_AGENT 注册名为 `"sched_agent_ops"`。**

### 10.2 register_bpf_struct_ops 宏

struct_ops 类型通过 `register_bpf_struct_ops` 宏注册。此宏做三件事：(1) 定义 value 类型（common_value 头 + 真实 ops 结构体）；(2) 通过 `BTF_TYPE_EMIT` 确保 BTF 包含此类型；(3) 调用 `__register_bpf_struct_ops()` 注册到 BTF struct_ops 表。

```c
/* 注册模式示意 */
static struct bpf_struct_ops airymax_sched_agent_ops = {
    .reg          = sched_agent_reg,
    .unreg        = sched_agent_unreg,
    .check_member = sched_agent_check_member,
    .init_member  = sched_agent_init_member,
    .init         = sched_agent_init,
    .validate     = sched_agent_validate,
    .name         = "sched_agent_ops",
    .owner        = THIS_MODULE,
    .cfi_stubs   = &__airymax_ops_sched_agent_ops
};

/* 初始化时注册 */
ret = register_bpf_struct_ops(&airymax_sched_agent_ops, sched_agent_ops);
```

### 10.3 struct_ops 状态机

struct_ops map 维护四状态生命周期，agentrt 用户态策略引擎通过 [SC] 共享契约层可读取 `state` 字段判断调度器是否在线：

```mermaid
stateDiagram-v2
    [*] --> INIT: map_create
    INIT --> INUSE: map_update_elem + reg()
    INUSE --> INUSE: update() 原子替换
    INUSE --> TOBEFREE: map_free + unreg()
    TOBEFREE --> [*]: RCU grace period
```

**OS-KER-021: airymaxos-kernel 必须通过 `register_bpf_struct_ops()` 注册 `sched_agent_ops` 类型，并在 defconfig 开启 CONFIG_SCHED_CLASS_EXT。**

**OS-OBS-022: agentrt-liunx BPF 调度器加载后，策略守护进程必须通过 [SC] `airymax_struct_ops_state` 读取 struct_ops map 的 state 字段，确认 state==INUSE 后方可标记调度器在线。**

### 10.4 SCHED_AGENT 集成架构

SCHED_AGENT 通过 struct_ops 暴露完整调度接口（init/exit/enqueue/dispatch/select_cpu/runnable/running/stopping/quiescent 等 25+ 回调）。BPF 程序填充 `struct sched_agent_ops` 后，通过 `bpf(BPF_MAP_UPDATE_ELEM)` 写入 struct_ops map，触发 `reg()` 回调注册到调度核心。

```mermaid
graph TD
    A[BPF 程序填充 sched_agent_ops] --> B[bpf MAP_UPDATE_ELEM 写入 struct_ops map]
    B --> C[check_zero_holes 校验]
    C --> D[init_member 处理非函数指针字段]
    D --> E[prepare_trampoline 生成本机码]
    E --> F[reg 注册到调度核心]
...
SCHED_AGENT 沿用 sched_ext 的软可靠性设计——BPF 调度器失败/超时/SysRq 自动回退 EEVDF：

| 触发条件 | 回退动作 |
|---------|---------|
| BPF 主动 exit | `ops.exit()` 调用 |
| runnable 超时 | 任务等待超过 timeout_ms（默认 30s） |
| BPF 程序错误 | 验证器拒绝 / 运行时错误 |
| SysRq-S | 紧急回退快捷键 |
| 内存分配失败 | BPF 程序 OOM |

回退流程：所有任务迁入 BYPASS DSQ（FIFO）→ EEVDF 重新接管 → state INUSE→TOBEFREE→freed。

**OS-KER-022: airymaxos-kernel 必须实现 SCHED_AGENT fallback 机制，BPF 调度器异常时自动回退 EEVDF，保证系统不卡死。**

---

## 第 11 章 agentrt-liunx Token 能效监控与 Agent 行为追踪

### 11.1 Token 能效监控

agentrt-liunx 通过 eBPF 实现 Token 能效监控：kprobe 挂载 Agent 决策入口，kretprobe 挂载出口，两者时间差为决策耗时；同时通过 `bpf_agent_token_usage_get` kfunc 读取 Token 消耗，计算能效比（token/秒）：
...
SEC("kretprobe/agentrt_cognition_process")
...
**OS-KER-018: airymaxos-kernel 必须在 `/sys/kernel/agentrt/token_usage` 导出 per-Agent Token 累计值，与 BPF map 数据一致。**
...
SEC("kprobe/agentrt_scheduler_dispatch")
...
**OS-KER-019: airymaxos-kernel 必须保证 `agentrt_cognition_process`、`agentrt_planner_dag_update`、`agentrt_scheduler_dispatch`、`agentrt_execution_unit_run` 四个函数符号导出至 kallsyms，确保 BPF 程序可挂载。**

### 11.3 与 MicroCoreRT 协同

MicroCoreRT 极简内核契约要求所有 BPF 程序代码体积最小化。agentrt-liunx BPF 程序单文件指令数不得超过 1 万，编译后 .o 体积不得超过 64 KB。该约束体现 IRON-9 v2 同源且部分代码共享原则——与 agentrt 用户态观测通过 [SC] 共享 trace_event 头契约 + [SS] 语义同源（BPF map/ringbuf 模型），但 [IND] 内核态实现独立（BPF 字节码 vs 用户态进程）。

**OS-KER-020: agentrt-liunx BPF 程序编译后 .o 体积必须 < 64 KB，单程序指令数必须 < 1 万；超限视为违反 MicroCoreRT 极简契约。**
...
agentrt-liunx 在 Linux 6.6 内核基线上严格遵循五维正交 24 原则——每条 OS-OBS 规则都可追溯至至少一条五维原则。IRON-9 v2 同源且部分代码共享原则要求 eBPF 的内核态实现与 agentrt 用户态可观测性保持 [SC] 共享契约 + [SS] 语义同源 + [IND] 内核态实现独立。两端通过 MicroCoreRT 极简内核契约与 AgentsIPC 消息协议实现无适配层互操作。

---

## 第 13 章 IRON-9 v2 三层共享模型落地

### 13.1 三层共享模型

IRON-9 v2 将 agentrt 与 agentrt-liunx 的 eBPF 协作划分为三层：
...
`include/airymax/bpf_struct_ops.h` 定义 struct_ops 状态枚举与 common_value 结构，agentrt 用户态策略引擎通过此头解析 struct_ops map 的 `state` 字段，判断 BPF 调度器是否在线，无需访问内核私有数据结构：

```c
/* include/airymax/bpf_struct_ops.h —— IRON-9 v2 [SC] 共享契约层 */
enum airymax_struct_ops_state {
    AIRYMAX_STRUCT_OPS_STATE_INIT     = 0,  /* 已初始化，未注册 */
    AIRYMAX_STRUCT_OPS_STATE_INUSE     = 1,  /* 已注册，使用中 */
    AIRYMAX_STRUCT_OPS_STATE_TOBEFREE  = 2,  /* 待释放（RCU 宽限期内） */
    AIRYMAX_STRUCT_OPS_STATE_READY     = 3,  /* 就绪（仅模块 struct_ops） */
};

struct airymax_struct_ops_common_value {
    uint32_t refcnt;   /* 引用计数 */
    uint32_t state;    /* enum airymax_struct_ops_state */
    uint32_t reserved[2];
};
```

**OS-STD-015: agentrt-liunx eBPF 与 agentrt 共享 `include/airymax/bpf_struct_ops.h` 头文件，struct_ops state 枚举值（0/1/2/3）与字段语义两端必须一致；两端事件格式必须一致，便于跨态聚合分析。**

### 13.3 [SS] 语义同源层

| 维度 | agentrt 用户态 | agentrt-liunx 内核态 | 同源点 |
|------|----------------|------------------|--------|
...
agentrt 的 `commons/metrics` 模块定义了 `agentrt_metrics_record(name, value)`，与内核 BPF map 更新同源——两者都遵循"键值累加 + 周期聚合"模式。[SS] 语义同源在此体现为：语义同源（都是结构化指标采集），实现独立（用户态 metrics 库 vs 内核态 BPF map + verifier）。

### 13.4 [IND] 完全独立层

| 维度 | agentrt 用户态 | agentrt-liunx 内核态 |
|------|----------------|------------------|
| JIT 后端 | 不适用（用户态解释器/LLVM） | airymaxos-kernel x86_64/arm64 JIT |
...
graph LR
    ...
    B --> C[agentrt-liunx eBPF ringbuf]
    D[内核态 BPF kprobe] --> C
    C --> E[user_events / ringbuf 统一消费]
    F[agentrt 策略引擎] -->|读取 [SC] state 字段| G[agentrt-liunx struct_ops map]
    style C fill:#fde68a,stroke:#b45309
    style G fill:#bbf7d0,stroke:#15803d
```

MicroCoreRT 极简内核契约要求：内核态 BPF 程序不解析用户态事件 payload，仅按 trace_event 头透传；用户态守护进程负责跨态聚合与能效计算。agentrt 策略引擎通过 [SC] 共享契约层读取 struct_ops map 的 `state` 字段，判断 BPF 调度器在线状态，无需进入内核私有数据结构。
...
| 0.1.1 | 2026-07-07 | 修订：新增第 10 章 struct_ops 注册机制与 SCHED_AGENT 集成；更新 IRON-9 v2 三层共享模型（[SC]/[SS]/[IND]）；增强程序类型与 map 类型（struct_ops/task_storage）；新增 OS-OBS-021/022、OS-KER-021/022 规则 |
...
**维护责任**：文档负责人为 agentrt-liunx 可观测性工程组；代码负责人为 airymaxos-kernel BPF 维护者；每个 LTS 小版本发布前重新核对 BPF 接口与规则编号有效性。

---

> **文档结束** | agentrt-liunx eBPF 可编程探针 v0.1.1 / 1.0.1
