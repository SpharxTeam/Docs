...
**OS-KER-016: airymaxos-kernel 必须保证 Agent 核心路径函数（`airy_cognition_process`、`airy_planner_dag_update`、`airy_scheduler_dispatch`、`airy_execution_unit_run`）支持 kprobe 挂载，不得使用 `__always_inline` 或 `notrace`。**
...
```c
/* 注册模式示意 */
static struct bpf_struct_ops airy_sched_agent_ops = {
    .reg          = sched_agent_reg,
    ...
    .owner        = THIS_MODULE,
    .cfi_stubs   = &__airy_ops_sched_agent_ops
};

/* 初始化时注册 */
ret = register_bpf_struct_ops(&airy_sched_agent_ops, sched_agent_ops);
```
...
**OS-OBS-022: agentrt-linux BPF 调度器加载后，策略守护进程必须通过 [SC] `airy_struct_ops_state` 读取 struct_ops map 的 state 字段，确认 state==INUSE 后方可标记调度器在线。**
...
**OS-KER-019: airymaxos-kernel 必须保证 `airy_cognition_process`、`airy_planner_dag_update`、`airy_scheduler_dispatch`、`airy_execution_unit_run` 四个函数符号导出至 kallsyms，确保 BPF 程序可挂载。**
