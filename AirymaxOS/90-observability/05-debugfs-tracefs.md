...
3. **生产可关闭**：debugfs 默认不挂载，仅在调试构建或 `airy_debug=1` 启动参数下挂载，符合 A-ULS 模块 macro_superv 的"生产环境最小调试面"原则。
...
│   ├── logger_state         # logger_daemon 内部状态
...
**OS-STD-042: 所有 debugfs 文件权限必须为 0444（只读）；不得在 debugfs 中提供写接口，状态变更必须经 A-UCS config_daemon 的标准接口。**
...
|--------|------------|------------|
| macro_superv | `/sys/kernel/debug/airy/superv/macro_*` | 温情裁决历史 |
| logger_daemon | `/sys/kernel/debug/airy/log/logger_state` | 消费速率与 flush 延迟 |
| config_daemon | （无独立 debugfs） | 已由 procfs 覆盖 |
