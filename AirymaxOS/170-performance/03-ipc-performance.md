...
- AgentsIPC 128 字节统一消息头（magic=0x41524531，IRON-9 v3 [SC] 共享契约层）
...
本设计严格遵守 agentrt-linux 术语规范：agentrt（用户态）称为**微核心**（micro-core），agentrt-linux（OS 发行版）称为**微内核**（micro-kernel）。AgentsIPC 在 agentrt-linux 中基于内核 io_uring 固定 OP 码实现，在 agentrt 中基于用户态消息队列实现，两者属于 IRON-9 v3 [SS] 语义同源层。
...
AgentsIPC 128 字节统一消息头（定义于 `include/airymax/ipc.h`，IRON-9 v3 [SC] 共享契约层）：
