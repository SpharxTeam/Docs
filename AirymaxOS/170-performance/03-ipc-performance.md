...
# agentrt-liunx（AirymaxOS）IPC 性能工程设计

> **文档定位**: agentrt-liunx（AirymaxOS，极境智能体操作系统）进程间通信性能工程详细设计
...
agentrt-liunx（AirymaxOS）IPC 性能工程为 AgentsIPC（Agent 进程间通信）提供低延迟、高吞吐、零拷贝的传输能力。本设计聚焦三大目标：
...
本设计严格遵守 agentrt-liunx 术语规范：agentrt（用户态）称为**微核心**（micro-core），agentrt-liunx（OS 发行版）称为**微内核**（micro-kernel）。AgentsIPC 在 agentrt-liunx 中基于内核 io_uring 固定 OP 码实现，在 agentrt 中基于用户态消息队列实现，两者属于 IRON-9 v2 [SS] 语义同源层。
...
AgentsIPC 128 字节统一消息头（定义于 `include/airymax/bpf_struct_ops.h`，IRON-9 v2 [SC] 共享契约层）：

```c
/* include/airymax/ipc_msg_hdr.h —— 128B IPC 消息头（[SC] 共享契约层） */
#ifndef AIRYMAX_IPC_MSG_HDR_H
#define AIRYMAX_IPC_MSG_HDR_H

#include <linux/types.h>

#define AGENTRT_IPC_MAGIC       0x41524531U   /* "ARE1" ASCII */
#define AGENTRT_IPC_HDR_SIZE    128

#pragma pack(push, 1)
struct agentrt_ipc_msg_hdr {
	/* [0..3]   魔数（4B） */
	__u32 magic;                  /* = AGENTRT_IPC_MAGIC */

	/* [4..7]   版本 + 标志位 */
	__u16 version;                /* 协议版本 */
	__u16 flags;                  /* 标志位（零拷贝、批量等） */

	/* [8..11]  消息类型 + 优先级 */
	__u8  msg_type;               /* AGENTRT_IPC_MSG_* */
	__u8  priority;               /* 0-7（7 最高） */
	__u8  stage;                  /* CoreLoopThree 阶段 */
	__u8  reserved8;
	...
	/* [96..111] 内存区域描述（零拷贝） */
	__u64 iova;                   /* I/O 虚拟地址 */
	__u16 region_id;              /* 内存区域 ID */
	__u8  region_kind;            /* 0=DRAM, 1=CXL, 2=PMEM */
	__u8  reserved111[5];
...
enum agentrt_ipc_msg_type {
	AGENTRT_IPC_MSG_CTRL    = 0x01,   /* 控制消息 */
	AGENTRT_IPC_MSG_AGENT   = 0x02,   /* Agent 指令 */
	AGENTRT_IPC_MSG_MEMORY  = 0x03,   /* 记忆查询 */
	AGENTRT_IPC_MSG_EVENT   = 0x04,   /* 内核事件 */
	AGENTRT_IPC_MSG_TOKEN   = 0x05,   /* 流式 Token */
	AGENTRT_IPC_MSG_HEARTBEAT = 0x06, /* 心跳 */
};

#endif /* AIRYMAX_IPC_MSG_HDR_H */
```
...
1. **共享内存区域注册**：用户态 daemon 通过 `AGENTRT_IPC_OP_REGISTER_REGION` 注册内存区域，io_uring 通过 iova 直接访问
2. **io_uring 固定 OP 码**：`AGENTRT_IPC_OP_SEND_ZC`（零拷贝发送）、`AGENTRT_IPC_OP_RECV_ZC`（零拷贝接收）
3. **大页（hugepage）后端**：内存区域以 2MB 大页分配，减少 TLB miss

```c
/* 用户态：注册零拷贝内存区域 */
struct agentrt_ipc_region_desc {
...
int agentrt_ipc_register_region(int ring_fd,
				const struct agentrt_ipc_region_desc *desc)
{
	...
	io_uring_prep_rw(IORING_OP_AGENTRT_IPC_REGISTER_REGION, sqe,
			 ring_fd.ring_fd, NULL, 0, 0);
...
```c
/* airymaxos-services/common/agentrt_ipc_ring.c */
#include <liburing.h>
#include <airymax/ipc_msg_hdr.h>

#define AGENTRT_IPC_RING_DEPTH   1024
#define AGENTRT_IPC_BATCH_SIZE   64

struct agentrt_ipc_ring {
...
int agentrt_ipc_ring_init(struct agentrt_ipc_ring *r)
{
	...
	ret = io_uring_queue_init_mem(AGENTRT_IPC_RING_DEPTH,
				     &r->ring, &p, NULL);
	if (ret < 0)
		goto out_err;
...
out_err:
	pr_err("agentrt-liunx: io_uring init failed ret=%d\n", ret);
	return ret;
}
```
...
int agentrt_ipc_send_batch(struct agentrt_ipc_ring *r,
			   struct agentrt_ipc_msg_hdr **hdrs, int n)
{
	struct io_uring_sqe *sqe;
	int i, submitted;

	if (n > AGENTRT_IPC_BATCH_SIZE)
		n = AGENTRT_IPC_BATCH_SIZE;
	...
	for (i = 0; i < n; i++) {
		...
		io_uring_prep_rw(IORING_OP_AGENTRT_IPC_SEND_ZC, sqe,
				 r->ring_fd, hdrs[i],
				 AGENTRT_IPC_HDR_SIZE, 0);
		io_uring_sqe_set_data64(sqe, hdrs[i]->seq_no);
	...
	if (submitted < 0) {
		pr_warn("agentrt-liunx: io_uring submit failed %d\n",
			submitted);
		return submitted;
	}
...
/* 批量接收 CQE */
int agentrt_ipc_recv_batch(struct agentrt_ipc_ring *r,
...
static void fill_msg_hdr(struct agentrt_ipc_msg_hdr *h,
			 __u32 src, __u32 dst, __u64 seq)
{
	h->magic = AGENTRT_IPC_MAGIC;
	h->version = 0x0001;
	h->flags = 0;
	h->msg_type = AGENTRT_IPC_MSG_AGENT;
	h->priority = 5;
	...
	h->src_daemon = src;
	h->dst_daemon = dst;
	h->payload_len = AGENTRT_IPC_HDR_SIZE;
}

int main(int argc, char **argv)
{
	struct agentrt_ipc_ring r;
	struct agentrt_ipc_msg_hdr *send_batch[BATCH];
	struct io_uring_cqe cqes[BATCH];
	int i, ret, total = 0;

	ret = agentrt_ipc_ring_init(&r);
	if (ret < 0)
		return ret;
	...
	for (int iter = 0; iter < 1000000; iter++) {
		...
		/* 批量提交 SQE */
		ret = agentrt_ipc_send_batch(&r, send_batch, BATCH);
		if (ret < 0)
			goto out_free_batch;
		...
		ret = agentrt_ipc_recv_batch(&r, cqes, BATCH);
		/* 处理 CQE（这里仅统计成功） */
	}
...
```c
/* airymaxos-kernel/ipc/agentrt_kfifo_channel.c */
...
#define AGENTRT_KFIFO_SIZE  4096   /* 4096 个消息 */

struct agentrt_kfifo_chan {
	DECLARE_KFIFO_PTR(fifo, struct agentrt_ipc_msg_hdr);
...
/* 初始化 kfifo 通道 */
int agentrt_kfifo_chan_init(struct agentrt_kfifo_chan *c)
{
	int ret;
...
/* 发送：阻塞直到 FIFO 有空间 */
int agentrt_kfifo_chan_send(struct agentrt_kfifo_chan *c,
			   const struct agentrt_ipc_msg_hdr *msg)
{
	int ret;
...
int agentrt_kfifo_chan_recv_batch(struct agentrt_kfifo_chan *c,
				 struct agentrt_ipc_msg_hdr *out,
				 int max_n)
{
	int n, ret;
...
static int agentrt_ipc_rx_kthread(void *data)
{
	struct agentrt_kfifo_chan *chan = data;
	struct agentrt_ipc_msg_hdr batch[16];
	int n, i, ret;

	while (!kthread_should_stop()) {
		n = agentrt_kfifo_chan_recv_batch(chan, batch, 16);
		if (n == -ESHUTDOWN)
			break;
		if (n <= 0)
			continue;
		...
		/* 批量处理消息 */
		for (i = 0; i < n; i++) {
			ret = agentrt_ipc_dispatch(&batch[i]);
			if (ret < 0)
				pr_warn("agentrt-liunx: dispatch failed %d\n",
					ret);
		}
	}
	return 0;
...
/* 启动接收 kthread */
int agentrt_ipc_rx_kthread_start(struct agentrt_kfifo_chan *chan)
{
	struct task_struct *t;

	t = kthread_run(agentrt_ipc_rx_kthread, chan, "agentrt_ipc_rx");
	if (IS_ERR(t))
		return PTR_ERR(t);
	return 0;
}
...
static inline void emit_ipc_metric(const struct agentrt_ipc_metric *m)
{
...
IPC 子系统错误码纳入 agentrt-liunx 统一错误码体系（[SC] 共享契约层，-800 ~ -899 段）：

| 错误码 | 数值 | 含义 |
|--------|------|------|
| AGENTRT_E_IPC_MAGIC | -800 | 消息头魔数错误 |
| AGENTRT_E_IPC_CRC | -801 | CRC32 校验失败 |
| AGENTRT_E_IPC_OVF | -802 | 消息队列溢出 |
| AGENTRT_E_IPC_REGION | -803 | 内存区域未注册 |
| AGENTRT_E_IPC_TIMEOUT | -804 | IPC 超时 |
| AGENTRT_E_IPC_KFIFO_FULL | -805 | kfifo 队列满 |
| AGENTRT_E_IPC_RING_FULL | -806 | io_uring SQ 已满 |
| AGENTRT_E_IPC_BATCH_SIZE | -807 | 批量大小非法 |

集中错误处理示例：

```c
int agentrt_ipc_validate_hdr(const struct agentrt_ipc_msg_hdr *h)
{
	__u32 crc;

	if (h->magic != AGENTRT_IPC_MAGIC)
		return -AGENTRT_E_IPC_MAGIC;

	crc = agentrt_crc32(h, AGENTRT_IPC_HDR_SIZE - sizeof(h->crc32));
	if (crc != h->crc32)
		return -AGENTRT_E_IPC_CRC;

	if (h->payload_len < AGENTRT_IPC_HDR_SIZE)
		return -AGENTRT_EINVAL;

	if (h->payload_len > AGENTRT_IPC_MAX_PAYLOAD)
		return -AGENTRT_EINVAL;

	return 0;
}

int agentrt_ipc_send_validate(struct agentrt_ipc_ring *r,
			      const struct agentrt_ipc_msg_hdr *h)
{
	int ret;

	ret = agentrt_ipc_validate_hdr(h);
	if (ret < 0)
		goto out_err;

	ret = agentrt_ipc_send_batch(r, (struct agentrt_ipc_msg_hdr **)&h, 1);
	if (ret < 0)
		goto out_err;
...
out_err:
	pr_warn("agentrt-liunx: ipc send validate failed ret=%d\n", ret);
	return ret;
}
```
				...
				const struct agentrt_ipc_region_desc *desc)
{
	...
	/* 3. 大页分配并固定 */
	ret = agentrt_hugetlb_pin(desc->user_addr, desc->size);
	if (ret < 0)
		goto out_err;
...
out_deny:
	pr_warn("agentrt-liunx: ipc region register denied by cupolas\n");
	return -AGENTRT_E_IPC_REGION;
out_err:
	return ret;
}
```
    ...
    "syscalls": [
        {
            "names": [
                ...
                "agentrt_ipc_register_region", "agentrt_ipc_send_zc",
                "agentrt_ipc_recv_zc", "mmap", "munmap", "futex",
...
| 消息类型枚举 | AGENTRT_IPC_MSG_* | AGENTRT_IPC_MSG_* | `bpf_struct_ops.h` |
| TraceID | 16B UUID | 16B UUID | TraceID 体系 |
| 错误码 | AGENTRT_E_IPC_* | AGENTRT_E_IPC_* | `error.h` 错误码段 |
...
> **文档结束** | agentrt-liunx（AirymaxOS）IPC 性能工程设计 | 1.0.1 开发版本
