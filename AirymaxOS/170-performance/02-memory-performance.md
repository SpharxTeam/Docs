...
/* 分级内存节点描述 */
struct airy_memoryrov_node {
	enum airy_memoryrov_layer layer;
...
/* 分级内存管理器 */
struct airy_memoryrov_mgr {
	struct airy_memoryrov_node nodes[AIRY_MEMORYROV_LAYER_MAX];
...
/* 应用衰减映射到 MGLRU */
static int airy_mglru_apply_forgetting(struct airy_memoryrov_mgr *mgr,
...
struct airy_memoryrov_mglru_bind {
	enum airy_memoryrov_layer layer;
	u8 hot_gen_lo;     /* 热代下界 */
	u8 hot_gen_hi;     /* 热代上界 */
	u8 cold_gen_lo;    /* 冷代下界 */
...
static const struct airy_memoryrov_mglru_bind mglru_binds[] = {
...
struct airy_migrate_entry {
	enum airy_memoryrov_layer from;
	enum airy_memoryrov_layer to;
	__u64 page_addr;
	__u32 size_kb;
