...
本设计严格遵守 agentrt-linux 术语规范：agentrt（用户态）称为**微核心**（micro-core），agentrt-linux（OS 发行版）称为**微内核**（micro-kernel）。MemoryRovol 在 agentrt-linux 中基于 CXL + MGLRU 内核态实现，与 agentrt 基于 HeapStore 用户态实现属于 IRON-9 v2 [SS] 语义同源层。
...
> **文档结束** | agentrt-linux（AirymaxOS）内存性能工程设计 | 1.0.1 开发版本
