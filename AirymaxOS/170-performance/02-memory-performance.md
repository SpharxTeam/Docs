...
# agentrt-liunx（AirymaxOS）内存性能工程设计

> **文档定位**: agentrt-liunx（AirymaxOS，极境智能体操作系统）内存子系统性能工程详细设计
...
agentrt-liunx（AirymaxOS）内存性能工程为 MemoryRovol（记忆卷载）四层记忆架构提供硬件级加速支撑。本设计聚焦三大目标：
...
本设计严格遵守 agentrt-liunx 术语规范：agentrt（用户态）称为**微核心**（micro-core），agentrt-liunx（OS 发行版）称为**微内核**（micro-kernel）。MemoryRovol 在 agentrt-liunx 中基于 CXL + MGLRU 内核态实现，与 agentrt 基于 HeapStore 用户态实现属于 IRON-9 v2 [SS] 语义同源层。
...
```c
/* include/airymax/memory_types.h —— MemoryRovol 内存类型（[SC] 共享契约层） */
#ifndef AIRYMAX_MEMORY_TYPES_H
#define AIRYMAX_MEMORY_TYPES_H

#include <linux/types.h>
#include <airymax/airymax_q16.h>

/* GFP 掩码语义（与 agentrt 共享） */
#define AGENTRT_GFP_L1_RAW    (__GFP_HIGH | __GFP_MOVABLE)     /* DRAM 优先 */
#define AGENTRT_GFP_L2_FEAT   (__GFP_HIGHMEM | __GFP_MOVABLE) /* DRAM/CXL */
#define AGENTRT_GFP_L3_STR    (__GFP_KERNEL)                  /* DRAM 内核 */
#define AGENTRT_GFP_L4_PAT    (GFP_KERNEL | __GFP_HIGHMEM)  /* PMEM/CXL 通过 mempolicy 指定 NUMA 节点 */

/* 记忆层级枚举 */
enum agentrt_memoryrov_layer {
	AGENTRT_MEMORYROV_L1_RAW = 0,
	AGENTRT_MEMORYROV_L2_FEAT,
	AGENTRT_MEMORYROV_L3_STR,
	AGENTRT_MEMORYROV_L4_PAT,
	AGENTRT_MEMORYROV_LAYER_MAX,
};

/* 分级内存节点描述 */
struct agentrt_memoryrov_node {
	enum agentrt_memoryrov_layer layer;
	int numa_node;              /* 0=DRAM local, 1=CXL, 2=PMEM */
	__u32 capacity_mb;
	__u32 used_mb;
	airymax_q16_t hit_rate;     /* Q16.16 表示的命中率 */
	airymax_q16_t evict_rate;
};

/* 分级内存管理器 */
struct agentrt_memoryrov_mgr {
	struct agentrt_memoryrov_node nodes[AGENTRT_MEMORYROV_LAYER_MAX];
	struct kfifo *migrate_fifo;   /* 批量迁移队列 */
	wait_queue_head_t *migrate_wq;
	airymax_q16_t global_hit_rate;
};

#endif /* AIRYMAX_MEMORY_TYPES_H */
```
...
Linux 6.6 内核基线原生支持 MGLRU 多代 LRU（非 2.0 版本）。agentrt-liunx 调优后的默认参数：
...
# 最大代数（默认 8，agentrt-liunx 调整为 12 以匹配 MemoryRovol 四层 × 3 温度）
...
struct agentrt_mglru_forgetting_map {
	enum agentrt_forgetting_strategy strategy;
	u8 min_gen;       /* 最冷代数 */
	u8 max_gen;       /* 最热代数 */
	airymax_q16_t retention_rate;  /* Q16.16 保留率 */
};

/* 艾宾浩斯衰减映射表（基于 R(t) = e^(-t/τ)，τ = 7 天） */
static const struct agentrt_mglru_forgetting_map ebbinghaus_map[] = {
	/* 代数 | 保留率 | 含义 */
	{AGENTRT_FORGET_EBBINGHAUS, 0,  3, 0x10000},  /* 最热 4 代：1.0 */
	{AGENTRT_FORGET_EBBINGHAUS, 4,  7, 0x8000},   /* 中间 4 代：0.5 */
	{AGENTRT_FORGET_EBBINGHAUS, 8, 11, 0x2000},   /* 最冷 4 代：0.125 */
	{AGENTRT_FORGET_EBBINGHAUS, 12, 12, 0x0000},  /* 回收代：0 */
};

/* 应用衰减映射到 MGLRU */
static int agentrt_mglru_apply_forgetting(struct agentrt_memoryrov_mgr *mgr,
					  const struct agentrt_mglru_forgetting_map *m)
{
	u8 gen;
	int ret;

	for (gen = m->min_gen; gen <= m->max_gen; gen++) {
		ret = agentrt_mglru_set_gen_weight(gen, m->retention_rate);
		if (ret < 0)
			goto out_err;
	}
	return 0;
...
out_err:
	pr_warn("agentrt-liunx: mglru apply forgetting failed gen=%u ret=%d\n",
		gen, ret);
	return ret;
}
```
...
struct agentrt_memoryrov_mglru_bind {
	enum agentrt_memoryrov_layer layer;
	u8 hot_gen_lo;     /* 热代下界 */
	u8 hot_gen_hi;     /* 热代上界 */
	u8 cold_gen_lo;    /* 冷代下界 */
...
static const struct agentrt_memoryrov_mglru_bind mglru_binds[] = {
	/* L1_raw：热代 0-2，冷代 10-11（PMEM 持久化） */
	{AGENTRT_MEMORYROV_L1_RAW,  0,  2, 10, 11},
	/* L2_feat：热代 0-3，冷代 9-11（CXL 迁移） */
	{AGENTRT_MEMORYROV_L2_FEAT, 0,  3,  9, 11},
	/* L3_str：热代 0-4，冷代 8-11（DRAM 驻留） */
	{AGENTRT_MEMORYROV_L3_STR,  0,  4,  8, 11},
	/* L4_pat：热代 0-5，冷代 11-11（PMEM 持久） */
	{AGENTRT_MEMORYROV_L4_PAT,  0,  5, 11, 11},
};
```
...
```c
/* airymaxos-memory/agentrt_memory_tiering.c */
...
/* 迁移请求条目 */
struct agentrt_migrate_entry {
	enum agentrt_memoryrov_layer from;
	enum agentrt_memoryrov_layer to;
	__u64 page_addr;
	__u32 size_kb;
...
/* 批量迁移管理器 */
struct agentrt_migrate_mgr {
	DECLARE_KFIFO_PTR(fifo, struct agentrt_migrate_entry);
...
int agentrt_memory_tiering_submit(struct agentrt_migrate_mgr *mgr,
				 const struct agentrt_migrate_entry *e)
{
	unsigned long flags;
	int ret;
...
out_err:
	pr_warn("agentrt-liunx: migrate submit failed ret=%d\n", ret);
	return ret;
}

/* 迁移工作 kthread（批量处理迁移请求） */
static int agentrt_memory_tiering_thread(void *data)
{
	struct agentrt_migrate_mgr *mgr = data;
	struct agentrt_migrate_entry batch[16];
	int n, i, ret;
	...
	while (!kthread_should_stop()) {
		...
		for (i = 0; i < n; i++) {
			...
			ret = agentrt_memory_do_migrate(&batch[i]);
			if (ret < 0)
				pr_warn("agentrt-liunx: migrate failed i=%d\n", i);
		}
	}
	return 0;
}
...
int agentrt_memory_tiering_init(struct agentrt_migrate_mgr *mgr)
{
	int ret;
	...
	kthread_run(agentrt_memory_tiering_thread, mgr, "agentrt_tiering");
	if (ret < 0)
		goto out_err;
	return 0;

out_err:
	pr_err("agentrt-liunx: tiering init failed ret=%d\n", ret);
	return ret;
}
```
...
struct agentrt_tiering_ctx {
	DECLARE_KFIFO(fifo, struct agentrt_migrate_entry, 4096);
...
{
	struct agentrt_tiering_ctx *ctx = data;
	struct agentrt_migrate_entry e = {0};
	...
	while (!kthread_should_stop()) {
		...
		e.from = AGENTRT_MEMORYROV_L1_RAW;
		e.to = AGENTRT_MEMORYROV_L2_FEAT;
		e.reason = 1;  /* 冷降级 */
...
static int consumer_fn(void *data)
{
	...
	while (!kthread_should_stop()) {
		...
		for (i = 0; i < n; i++) {
			...
			agentrt_memory_do_migrate(&batch[i]);
		}
	}
	return 0;
}
...
| 指标 | 默认 MGLRU | agentrt-liunx 调优 | 提升 |
|------|-----------|----------------------|------|
| L1 命中率 | 0.72 | 0.85 | +18% |
| L2 命中率 | 0.58 | 0.72 | +24% |
...
内存子系统错误码纳入 agentrt-liunx 统一错误码体系（[SC] 共享契约层，-600 ~ -699 段）：

| 错误码 | 数值 | 含义 |
|--------|------|------|
| AGENTRT_E_MEMORY_NOMEM | -600 | 内存不足 |
| AGENTRT_E_MEMORY_TIER_FAIL | -601 | 跨层迁移失败 |
| AGENTRT_E_MEMORY_CXL_FAIL | -602 | CXL 操作失败 |
| AGENTRT_E_MEMORY_PMEM_FAIL | -603 | PMEM 持久化失败 |
| AGENTRT_E_MEMORY_MGLRU | -604 | MGLRU 调度错误 |
| AGENTRT_E_MEMORY_KFIFO_FULL | -605 | 迁移 kfifo 队列满 |
| AGENTRT_E_MEMORY_HASH_MISMATCH | -606 | SHA-256 哈希校验失败 |

集中错误处理示例：

```c
int agentrt_memory_do_migrate(const struct agentrt_migrate_entry *e)
{
	void *src, *dst;
	int ret;

	if (e->from >= AGENTRT_MEMORYROV_LAYER_MAX ||
	    e->to >= AGENTRT_MEMORYROV_LAYER_MAX) {
		ret = -AGENTRT_EINVAL;
		goto out_err;
	}

	src = agentrt_memory_map(e->from, e->page_addr, e->size_kb);
	if (!src) {
		ret = -AGENTRT_E_MEMORY_NOMEM;
		goto out_err;
	}

	dst = agentrt_memory_alloc(e->to, e->size_kb);
	if (!dst) {
		ret = -AGENTRT_E_MEMORY_NOMEM;
		goto out_free_src;
	}

	ret = agentrt_memory_copy(dst, src, e->size_kb);
	if (ret < 0) {
		ret = -AGENTRT_E_MEMORY_TIER_FAIL;
		goto out_free_dst;
	}

	if (e->to == AGENTRT_MEMORYROV_L4_PAT ||
	    e->to == AGENTRT_MEMORYROV_L1_RAW) {
		ret = agentrt_memory_persist(dst, e->size_kb);
		if (ret < 0) {
			ret = -AGENTRT_E_MEMORY_PMEM_FAIL;
			goto out_free_dst;
		}
	}

	agentrt_memory_unmap(src);
	agentrt_memory_unmap(dst);
	return 0;

out_free_dst:
	agentrt_memory_free(dst);
out_free_src:
	agentrt_memory_unmap(src);
out_err:
	return ret;
...
| GFP 掩码 | AGENTRT_GFP_L1_L4 | AGENTRT_GFP_L1_L4 | `memory_types.h` |
...
> **文档结束** | agentrt-liunx（AirymaxOS）内存性能工程设计 | 1.0.1 开发版本
