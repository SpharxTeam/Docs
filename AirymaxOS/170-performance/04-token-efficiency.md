...
> **IRON-9 v3 层次**：[SS] 语义同源层（Token 能效指标语义与 agentrt 同源）+ [IND] 完全独立层（内核态测量与优化为 agentrt-linux 专属）
...
- [15. 安全考量](#15-安全考量)
- [16. 性能约束](#16-性能约束)
- [17. IRON-9 v3 同源映射](#17-iron-9-v2-同源映射)
- [18. SDK 集成](#18-sdk-集成)
- [19. 使用示例](#19-使用示例)
- [20. 测试策略](#20-测试策略)
- [21. 合规声明](#21-合规声明)
...
agentrt-linux 需要在内核层面感知 Token 消耗与资源消耗的关联，通过 sched_tac 用户态调度器策略优化能效。本方案参考 Linux 6.6 perf 功耗剖析（`tools/perf/`）与 seL4 MCS（Mixed-Criticality System）时间隔离思想，建立 Token 能效工程体系。
 ...
 * 返回 0 表示成功
 */
int airy_measure_power(uint32_t agent_id,
...
```c
/**
 * airy_calc_token_per_watt - 计算 Token/Watt 能效
 * @tokens: Token 生成数量
 * @duration_s: 持续时间（秒）
 * @power_uj: 总功耗（微焦耳）
 *
 * 返回 Token/W·s 值
 */
double airy_calc_token_per_watt(uint64_t tokens,
                                    double duration_s,
                                    uint64_t power_uj)
{
...
} airy_token_latency_t;

/**
 * airy_measure_token_latency - 测量 Token 生成延迟
 * @agent_id: Agent ID
 * @latency: 输出延迟结构
 */
int airy_measure_token_latency(uint32_t agent_id,
                                  airy_token_latency_t *latency)
{
    /* 从 ftrace 缓冲区提取时间戳 */
...
```c
/**
 * airy_calc_token_per_latency - 计算 Token/Latency 能效
 * @latency: 延迟结构
 *
 * 返回 Token/ms 值
 */
double airy_calc_token_per_latency(const airy_token_latency_t *latency)
{
...
/**
 * airy_calc_token_cost - 计算 Token 成本
 * @tokens: Token 数量
 ...
 */
double airy_calc_token_cost(uint64_t tokens,
                                double gpu_hours,
                                double cpu_hours,
                                double memory_gb_h,
                                const airy_cost_model_t *model)
{
    double total_cost =
 ...
 */
int airy_budget_check_efficiency(uint32_t agent_id)
{
    struct airy_agent *agent = get_agent(agent_id);
    if (!agent) return -EINVAL;

    /* 计算 Agent 当前能效 */
    double token_per_watt = airy_calc_token_per_watt(
        agent->tokens_generated,
        agent->runtime_seconds,
        agent->energy_consumed_uj);

    double token_per_latency = airy_calc_token_per_latency(
        &agent->latency);
 ...
 */
void airy_sched_llm_phase(uint32_t agent_id,
                              enum airy_llm_phase phase)
{
    struct airy_agent *agent = get_agent(agent_id);

    switch (phase) {
    case AIRY_LLM_PREFILL:
        /* Prefill: 计算密集，提升 GPU 频率 */
        airy_dvfs_set_freq(agent->gpu_id, GPU_FREQ_MAX);
        airy_sched_set_weight(agent, SCHED_WEIGHT_HIGH);
        break;
    case AIRY_LLM_DECODE:
        /* Decode: 内存密集，降低 GPU 频率，提升内存带宽 */
        airy_dvfs_set_freq(agent->gpu_id, GPU_FREQ_MID);
        airy_mem_bw_reserve(agent->mem_bw_quota);
        break;
    case AIRY_LLM_SPECULATIVE:
        /* 投机解码: 双模型协作 */
        airy_sched_set_weight(agent, SCHED_WEIGHT_MID);
        break;
    }
}
```
...
```c
/**
 * airy_optimize_coreloop - 优化 CoreLoopThree 能效
 * @agent_id: Agent ID
 */
void airy_optimize_coreloop(uint32_t agent_id)
{
    struct airy_agent *agent = get_agent(agent_id);
    ...
    if (thinking_eff < perception_eff && thinking_eff < action_eff) {
        airy_sched_boost_phase(agent, THINKING);
    }
}
```
...
typedef struct airy_accel_efficiency {
    uint32_t accel_id;
    enum airy_accel_type type;  /* GPU/NPU/DPU */
    double token_per_watt;
    double token_per_latency;
    uint32_t current_agents;
    uint32_t max_agents;
} airy_accel_efficiency_t;

/**
 * airy_select_efficient_accel - 选择能效最高的加速器
 * @agent_id: Agent ID
 * @required_tokens: 需要的 Token 数
 *
 * 返回选中的加速器 ID
 */
uint32_t airy_select_efficient_accel(uint32_t agent_id,
                                         uint64_t required_tokens)
{
    airy_accel_efficiency_t accels[MAX_ACCELS];
...
} airy_efficiency_metrics_t;

/**
 * airy_collect_efficiency - 采集 Agent 能效指标
 * @agent_id: Agent ID
 *
 * 通过 perf + RAPL + ftrace 采集，导出至 OpenTelemetry
 */
int airy_collect_efficiency(uint32_t agent_id)
{
    airy_efficiency_metrics_t metrics = {0};
    ...
    /* 计算能效指标 */
    metrics.token_per_watt = airy_calc_token_per_watt(
        metrics.tokens_output,
        metrics.cpu_time_ns / 1e9 + metrics.gpu_time_ns / 1e9,
        metrics.energy_uj);
    metrics.token_per_latency = airy_calc_token_per_latency(
...
```c
/**
 * airy_auto_optimize_efficiency - 自动优化 Agent 能效
 * @agent_id: Agent ID
 *
 * 根据能效指标自动调整调度与资源分配
 */
int airy_auto_optimize_efficiency(uint32_t agent_id)
{
    airy_efficiency_metrics_t metrics;
    airy_collect_efficiency(agent_id, &metrics);
    ...
    if (metrics.token_per_latency < SLO_TOKEN_PER_LATENCY) {
        airy_enable_speculative_decode(agent_id);
    }
...
```c
/**
 * airy_detect_efficiency_regression - 检测能效回归
 * @agent_id: Agent ID
 * @baseline: 基线能效
 * @current: 当前能效
 *
 * 返回 1 表示检测到回归
 */
int airy_detect_efficiency_regression(uint32_t agent_id,
    const airy_efficiency_metrics_t *baseline,
    const airy_efficiency_metrics_t *current)
{
    /* Token/Watt 回归超过 10% */
...
- **IRON-9 v3 遵守**：CoreLoopThree 阶段枚举 [SC] 共享，测量与优化 [IND] 独立
...
> **文档结束** | Token 能效工程实现方案 | IRON-9 v3 [SC] + [SS] + [IND]
