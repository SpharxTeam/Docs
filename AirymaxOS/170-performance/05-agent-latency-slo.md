...
- [13. 安全考量](#13-安全考量)
- [14. 性能约束](#14-性能约束)
- [15. IRON-9 v2 同源映射](#15-iron-9-v2-同源映射)
- [16. SDK 集成](#16-sdk-集成)
- [17. 使用示例](#17-使用示例)
- [18. 测试策略](#18-测试策略)
- [19. 合规声明](#19-合规声明)
...
2. **可保障**：通过 SCHED_AGENT 策略与资源预留保障 SLO 达标
...
传统操作系统不感知这些阶段，无法针对性优化。agentrt-linux 通过 SCHED_AGENT 策略与 CoreLoopThree 阶段感知，实现端到端延迟 SLO 保障。

### 1.3 设计哲学

本方案参考 Linux 6.6 sched_ext + EEVDF 延迟调度（`kernel/sched/fair.c`）与 seL4 MCS（Mixed-Criticality System）时间隔离思想：
...
typedef struct agentrt_latency_slo {
    uint32_t agent_id;
...
} agentrt_latency_slo_t;

/* 默认 SLO 阈值 */
#define AGENTRT_SLO_L1_DEFAULT_NS  (100 * 1000 * 1000ULL)  /* 100ms */
#define AGENTRT_SLO_L2_DEFAULT_NS  (1 * 1000 * 1000 * 1000ULL)  /* 1s */
#define AGENTRT_SLO_L3_DEFAULT_NS  (10 * 1000 * 1000 * 1000ULL) /* 10s */
```
...
/**
 * agentrt_measure_cognition_latency - 测量认知延迟
 * @agent_id: Agent ID
 *
 * 返回认知延迟（纳秒）
 */
uint64_t agentrt_measure_cognition_latency(uint32_t agent_id)
{
...
/**
 * agentrt_check_l1_slo - 检查 L1 认知延迟 SLO
 * @agent_id: Agent ID
 *
 * 返回 0 表示达标，-EDELAY 表示违约
 */
int agentrt_check_l1_slo(uint32_t agent_id)
{
    uint64_t latency = agentrt_measure_cognition_latency(agent_id);
    struct agentrt_agent *agent = get_agent(agent_id);
...
```c
/**
 * agentrt_optimize_planning_latency - 优化规划延迟
 * @agent_id: Agent ID
 */
void agentrt_optimize_planning_latency(uint32_t agent_id)
{
    struct agentrt_agent *agent = get_agent(agent_id);
    uint64_t latency = agentrt_measure_planning_latency(agent_id);
    ...
    if (latency > agent->slo.l2_planning_max_ns * 0.8) {
        ...
        /* 策略 2: 启用工具缓存 */
        agentrt_enable_tool_cache(agent_id);

        /* 策略 3: 并行推理 */
        agentrt_enable_parallel_inference(agent_id);
    }
}
```
...
```c
/**
 * agentrt_guarantee_l3_slo - 保障 L3 执行延迟 SLO
 * @agent_id: Agent ID
 */
int agentrt_guarantee_l3_slo(uint32_t agent_id)
{
    struct agentrt_agent *agent = get_agent(agent_id);

    /* 预留执行时间预算 */
    uint64_t budget = agent->slo.l3_action_max_ns;
    if (!agentrt_reserve_time_budget(agent_id, budget)) {
    ...
    /* 提升调度优先级 */
    agentrt_sched_boost(agent_id, SCHED_BOOST_HIGH);

    /* 预热工具缓存 */
    agentrt_preheat_tool_cache(agent_id);
...
```c
/**
 * agentrt_monitor_coreloop_latency - 监控 CoreLoopThree 延迟
 * @agent_id: Agent ID
 */
void agentrt_monitor_coreloop_latency(uint32_t agent_id)
{
    struct agentrt_agent *agent = get_agent(agent_id);

    /* 记录各阶段延迟 */
    agentrt_record_phase_latency(agent_id, PERCEPTION,
        agent->trace.perception_end - agent->trace.perception_start);

    agentrt_record_phase_latency(agent_id, THINKING,
        agent->trace.thinking_end - agent->trace.thinking_start);

    agentrt_record_phase_latency(agent_id, ACTION,
        agent->trace.action_end - agent->trace.action_start);

    /* 检查 SLO */
    agentrt_check_l1_slo(agent_id);
    agentrt_check_l2_slo(agent_id);
    agentrt_check_l3_slo(agent_id);
}
```
...
/**
 * agentrt_slo_guard - SLO 保障守护
 * @agent_id: Agent ID
 *
 * 当 Agent 接近 SLO 阈值时，自动提升调度优先级
 */
void agentrt_slo_guard(uint32_t agent_id)
{
    struct agentrt_agent *agent = get_agent(agent_id);
    ...
    if (l1_ratio > 0.8) {
        ...
        /* 超过 60% 阈值，中量提升 */
        agentrt_sched_boost(agent_id, SCHED_BOOST_MID);
    }
}
```
...
```c
typedef struct agentrt_slo_metrics {
    uint32_t agent_id;
...
```c
/**
 * agentrt_handle_slo_violation - 处理 SLO 违约
 * @agent_id: Agent ID
 * @level: 违约级别（L1/L2/L3）
 */
void agentrt_handle_slo_violation(uint32_t agent_id, int level)
{
    struct agentrt_agent *agent = get_agent(agent_id);
    ...
    if (violations >= 3) {
        /* 连续 3 次违约，暂停 Agent */
        agentrt_suspend_agent(agent_id);
        agentrt_alert_admin("SLO critical violation", agent_id, level);
    } else if (violations >= 1) {
        /* 首次违约，降级 SLO 等级 */
        agentrt_downgrade_slo(agent_id);
    }
}
```
...
```c
typedef struct agentrt_latency_budget {
...
} agentrt_latency_budget_t;

/**
 * agentrt_consume_latency_budget - 消耗延迟预算
 * @agent_id: Agent ID
 * @phase: 阶段
 * @latency_ns: 消耗的延迟
 *
 ...
 */
int agentrt_consume_latency_budget(uint32_t agent_id,
                                     enum agentrt_coreloop_phase phase,
                                     uint64_t latency_ns)
{
    struct agentrt_agent *agent = get_agent(agent_id);
...
| 错误码 | 名称 | 含义 |
|--------|------|------|
| -EDELAY | AGENTRT_EDELAY | 延迟超过 SLO |
| -EBUDGET | AGENTRT_EBUDGET | 延迟预算耗尽 |
| -ESLOVIOLATION | AGENTRT_ESLOVIOLATION | SLO 违约 |
| -ENOSLO | AGENTRT_ENOSLO | SLO 未定义 |
...
- **IRON-9 v2 遵守**：SLO 数据结构 [SC] 共享，保障机制 [IND] 独立
- **seL4 唯一来源遵守**：MCS 时间隔离思想借鉴 seL4
- **Linux 6.6 基线遵守**：sched_ext + EEVDF 延迟调度对齐 Linux 6.6
...
> **文档结束** | Agent 延迟 SLO 实现方案 | IRON-9 v2 [SC] + [SS] + [IND]
