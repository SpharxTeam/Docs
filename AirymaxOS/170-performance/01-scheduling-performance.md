...
- SCHED_AGENT 策略（Agent 调度类，IRON-9 v2 [SS] 语义同源层）
...
agentrt-linux 将 sched_ext 框架从主线 6.12+ 向前移植到 Linux 6.6 内核基线。启用配置如下：
...
### 2.2 SCHED_AGENT 策略分层

SCHED_AGENT 策略（Agent 调度类）位于调度层次中的位置：

```
┌────────────────────────────────────────────┐
│  SCHED_FIFO / SCHED_RR（实时类， prio 0-99）│  ← 实时任务（保留）
├────────────────────────────────────────────┤
│  SCHED_AGENT（Agent 调度类，SCHED_EXT=7） │  ← agentrt-linux 专属
├────────────────────────────────────────────┤
...
通过 user_events 向 agentrt-linux 可观测性子系统（monit_d）上报指标：
...
void BPF_STRUCT_OPS(agent_exit, struct scx_exit_info *info)
{
	if (info->kind == SCX_EXIT_UNREG_BPF)
		bpf_printk("agentrt-linux: SCHED_AGENT BPF error %d\n",
			   info->exit_code);
	else if (info->kind == SCX_EXIT_WATCHDOG)
		bpf_printk("agentrt-linux: watchdog timeout, fallback to default\n");
}
```
...
> **文档结束** | agentrt-linux（AirymaxOS）调度性能工程设计 | 1.0.1 开发版本
