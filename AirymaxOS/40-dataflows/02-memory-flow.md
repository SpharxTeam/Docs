...
- **[SC] 共享契约层**（`include/airymax/memory_types.h`）：L1-L4 数据结构定义（`airy_l1_record_t` 等）、GFP 掩码语义、PMEM 持久化接口——agentrt 用户态与 agentrt-linux 内核态**完全共享代码**
...
>
> **[SC] 共享契约层方案**：`include/airymax/memory_types.h` 中所有浮点字段统一使用 Q16.16 定点数 `airy_q16_t`（int32_t），用户态需 float 展示时用 `AIRY_Q16_TO_F()` 转换。
...
L1 存储原始执行记录与感知数据，**仅追加（append-only），不可变**（FR-031，NFR-S-005 哈希链保护）。L1 数据结构 `airy_l1_record_t` 是 [SC] 共享契约层的核心——agentrt 用户态与 agentrt-linux 内核态通过 `include/airymax/memory_types.h` 共享此定义。
...
| 0.1.1 | 2026-07-07 | 修订：添加 Copyright 头、IRON-9 v2 三层共享模型、MGLRU 源码映射深度、[SC]/[SS]/[IND] 标注、agentrt 一致性检查 | 工程规范委员会 |
