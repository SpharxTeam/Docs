...
> **核心约束**: IRON-9 v2 同源且部分代码共享——与 agentrt 用户态 memoryrovol/heapstore 通过 [SC] 共享契约层 + [SS] 语义同源层协作，[IND] 内核态 CXL/PMEM/MGLRU 实现独立

---

## 1. 记忆卷载数据流概览

记忆卷载数据流是 agentrt-linux 区别于通用操作系统的核心特征之一，落地于 `airymaxos-memory` 子仓（同源 agentrt memoryrovol + heapstore 模块）。该数据流借鉴认知科学的记忆分层理论，将记忆从原始感知到抽象模式分为四层递进：
...
**IRON-9 v2 三层共享模型在记忆卷载的落地**：

- **[SC] 共享契约层**（`include/airymax/memory_types.h`）：L1-L4 数据结构定义（`agentrt_l1_record_t` 等）、GFP 掩码语义、PMEM 持久化接口——agentrt 用户态与 agentrt-linux 内核态**完全共享代码**
- **[SS] 语义同源层**：MGLRU aging/eviction API、`memory.reclaim` 主动回收、userfaultfd 记忆迁移——agentrt 用户态与 agentrt-linux 内核态**API 签名同源，实现独立**
...
L1 存储原始执行记录与感知数据，**仅追加（append-only），不可变**（FR-031，NFR-S-005 哈希链保护）。L1 数据结构 `agentrt_l1_record_t` 是 [SC] 共享契约层的核心——agentrt 用户态与 agentrt-linux 内核态通过 `include/airymax/memory_types.h` 共享此定义。
...
L2 从 L1 提取语义向量，支持相似度检索（FR-032）。向量维度 768（兼容主流 embedding 模型）。`agentrt_l2_feature_t` 同样是 [SC] 共享契约层。
...
L3 从 L2 向量构建关系图（知识图谱），支持图遍历（FR-033）。采用邻接表存储。`agentrt_l3_node_t` / `agentrt_l3_edge_t` 同为 [SC] 共享契约层。
...
L4 从 L3 图挖掘持久同调（persistent homology）模式，支持抽象推理（FR-034）。采用 barcodes 表示模式的 birth/death。`agentrt_l4_barcode_t` 同为 [SC] 共享契约层。
...
**MemoryRovol tier 对应**：MemoryRovol L2 特征的 `weight` 字段（衰减曲线 0.0-1.0）与 MGLRU tier 机制语义同源——都是基于"访问频次"的优先级调整。但 MemoryRovol weight 是连续值（0.0-1.0），MGLRU tier 是离散值（0-3），这是 MemoryRovol 的增强点。
...
| 层次 | 共享程度 | 内容 | 组织方式 |
|------|---------|------|---------|
| **[SC] 共享契约层** | 完全共享代码 | L1-L4 数据结构、GFP 掩码语义、PMEM 持久化接口 | `include/airymax/memory_types.h` 独立头文件 |
| **[SS] 语义同源层** | API 签名同源，实现独立 | MGLRU aging/eviction、`memory.reclaim`、userfaultfd | agentrt 用户态实现 + agentrt-linux 内核态实现 |
| **[IND] 完全独立层** | 完全独立 | CXL 驱动、PMEM nvdimm 驱动、Kbuild、内核 ABI 预留机制移除 | 各自独立仓库 |

### 10.2 [SC] 共享契约层 `include/airymax/memory_types.h`
...
#define AGENTRT_GFP_IO            0x0001u   /* 允许 I/O */
#define AGENTRT_GFP_FS             0x0002u   /* 允许文件系统操作 */
#define AGENTRT_GFP_RECLAIM        0x0004u   /* 允许直接回收 */
#define AGENTRT_GFP_KSWAPD         0x0008u   /* 唤醒 kswapd */
#define AGENTRT_GFP_HIGH           0x0010u   /* 高优先级 */
#define AGENTRT_GFP_NOWARN         0x0020u   /* 抑制失败警告 */
#define AGENTRT_GFP_ZERO           0x0040u   /* 返回清零内存 */
typedef uint32_t agentrt_gfp_t;

/* ========== MemoryRovol L1-L4 数据结构（见 §3） ========== */
typedef struct __attribute__((aligned(64))) agentrt_l1_record { ... } agentrt_l1_record_t;
typedef struct __attribute__((aligned(64))) agentrt_l2_feature { ... } agentrt_l2_feature_t;
typedef struct agentrt_l3_node { ... } agentrt_l3_node_t;
typedef struct agentrt_l3_edge { ... } agentrt_l3_edge_t;
typedef struct agentrt_l4_barcode { ... } agentrt_l4_barcode_t;

/* ========== PMEM 持久化语义 ========== */
typedef void (*agentrt_pmem_flush_fn)(void *addr, size_t size);

#endif /* AIRYMAX_MEMORY_TYPES_H */
```

> **OS-MM-001**: `include/airymax/memory_types.h` 是 MemoryRovol 数据结构的 [SC] 共享契约层——agentrt 用户态与 agentrt-linux 内核态通过此头文件共享 L1-L4 数据结构定义、GFP 掩码语义、PMEM 持久化接口，避免双份定义导致不一致。

### 10.3 [SS] 语义同源层

[SS] 语义同源层定义 agentrt 用户态与 agentrt-linux 内核态**API 签名同源、实现独立**的接口——MGLRU aging/eviction 语义、`memory.reclaim` 主动回收、userfaultfd 记忆迁移。

| Linux 6.6 内核基线 API | agentrt-linux 对应 | agentrt 用户态对应 | 同源说明 |
|------------------------|----------------|-------------------|----------|
| MGLRU `try_to_inc_max_seq()` | 内核态调用 | `agentrt_l2_extract()` | aging 语义：新数据进入 youngest gen |
| MGLRU `evict_folios()` | 内核态调用 | `agentrt_l2_forget()` | eviction 语义：老数据被驱逐 |
| `memory.reclaim` 写接口 | cgroup v2 接口 | `agentrt_mem_reclaim()` | 主动回收语义 |
| `userfaultfd` ioctl | 内核态 hook | `agentrt_uffd_*()` | 缺页处理语义 |
| `clwb + sfence` | 内核态调用 | `agentrt_pmem_flush()` | PMEM 持久化语义 |
| `kmem_cache_create()` | 内核态 SLUB | `agentrt_cache_create()` | 专用对象池语义 |
| `migrate_pages()` | 内核态调用 | `agentrt_migrate_pages()` | 页迁移语义（CXL demotion） |
...
| `lru_gen_memcg` 全局 memcg LRU | per-agent 隔离由 airymaxos-services daemon 管理 |
...
sequenceDiagram
    ...
    participant SC as [SC] include/airymax/memory_types.h
    ...
    AR->>SC: 1. 引用 agentrt_l1_record_t
    AR->>IPC: 2. 提交 L1 写入请求
    IPC->>KR: 3. 进入内核态
...
| **S-2 同源契约** | 同源代码必须共享 | [SC] 共享契约层 `include/airymax/memory_types.h`（§10.2） |
...
> **OS-MM-002**: 五维原则在记忆卷载数据流中的落地必须经过架构委员会审查，新增设计不得违反已落地的原则。
...
|--------|--------------|----------------|--------|------|
| L1-L4 数据结构 | 用户态定义 | [SC] 共享 | ✅ 一致 | `include/airymax/memory_types.h` |
| GFP 语义 | 用户态简化版 | [SC] 共享 | ✅ 一致 | `agentrt_gfp_t` |
| PMEM 持久化 | 用户态 memcpy + clwb | [SC] 共享 | ✅ 一致 | `agentrt_pmem_flush_fn` |
...
- [工程标准规范](../50-engineering-standards/README.md) §1.3 IRON-9 v2 三层共享模型
...
| 0.1.1 | 2026-07-07 | 修订：添加 Copyright 头、IRON-9 v2 三层共享模型、MGLRU 源码映射深度、[SC]/[SS]/[IND] 标注、agentrt 一致性检查 | Airymax 架构委员会 |
