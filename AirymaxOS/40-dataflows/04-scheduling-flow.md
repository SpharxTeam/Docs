...
> **设计约束**（BAN-361 + Linux 6.6 内核基线 验证）：调度类编号统一使用内核已有的 `SCHED_EXT`（值 7，定义于 `include/uapi/linux/sched.h:121`），**禁止定义 `SCHED_AGENT` 调度类编号宏**（与 `SCHED_EXT=7` 冲突）。`SCHED_AGENT` 仅作为 sched_ext 框架下的 **BPF 策略名称**（policy name），而非新的调度类编号。
 ...
 * @note Linux 6.6 内核基线 已内置 SCHED_EXT=7（include/uapi/linux/sched.h:121）
...
**同源红利**：agentrt 作为普通用户态应用（libc/POSIX）在 agentrt-linux 上天然更稳健；可选 `sched_setscheduler(p, SCHED_EXT, &param)` 享受内核原生调度优先级（SCHED_EXT=7，Linux 6.6 内核基线 已内置），无需任何转换/适配层。
