...
> **核心约束**： IRON-9 v2 同源且部分代码共享——[SC] bpf_struct_ops.h（sched_ext struct_ops 状态机 + common_value）落地于 include/airymax/，[SS] EEVDF + sched_ext + SCHED_AGENT 策略语义同源，[IND] sched_ext 内核驱动实现 + BPF 调度策略程序独立

---

## 1. 调度数据流概览

调度数据流是 agentrt-linux 内核的核心数据流，落地于 `airymaxos-kernel` 子仓（同源 agentrt atoms/corekern Task 模块）。该数据流基于 Linux 6.6 内核基线的三大调度能力：

1. **EEVDF 调度器**（FR-001, FR-008）：Linux 6.6 默认调度算法，替代传统 CFS。通过虚拟截止时间（virtual deadline）+ 资格判定（eligibility）实现更精确的延迟控制，抢占延迟 < 10μs。
2. **sched_ext**（FR-001, FR-002）：agentrt-linux 内核增强（主线 6.12+），基于 eBPF 的可插拔调度框架。agentrt-linux 通过 sched_ext 实现 SCHED_AGENT 策略，调度策略以 BPF 程序形式加载，支持运行时替换（FR-050）。
3. **SCHED_AGENT 策略**（FR-002）：agentrt-linux 自研调度类，专为 Agent 工作负载优化。优先级范围 0-139，支持抢占式调度与策略可插拔。
...
下图为 Agent 任务从提交到执行的完整调度数据流，包含 SCHED_AGENT 策略、EEVDF 选择、策略可插拔、DSQ 派发：

```mermaid
flowchart TD
    A[Agent 任务提交] --> B[syscall: airy_sys_call<br/>capability invocation]
    B --> C[SCHED_AGENT 策略<br/>sched_ext]
...
| DSQ 类型 | 标识 | 语义 | agentrt-linux SCHED_AGENT 使用 |
|---------|------|------|---------------------------|
 ...
 * @since 1.0.1
 * @note IRON-9 v2 [SS] 语义同源层：高层 API 语义同源（概念操作一致），签名因抽象层级不同而独立演进
 */
...
| kfunc | 语义 | 调用上下文 | agentrt-linux 使用 |
|-------|------|----------|---------------|
...
SCHED_AGENT 是 agentrt-linux 基于 sched_ext 框架实现的**自研 BPF 调度策略**（FR-002），专为 Agent 工作负载优化。同源 agentrt atoms/corekern 的 MicroCoreRT 调度器。

> **设计约束**（BAN-361 + OLK-6.6 验证）：调度类编号统一使用内核已有的 `SCHED_EXT`（值 7，定义于 `include/uapi/linux/sched.h:121`），**禁止定义 `SCHED_AGENT` 调度类编号宏**（与 `SCHED_EXT=7` 冲突）。`SCHED_AGENT` 仅作为 sched_ext 框架下的 **BPF 策略名称**（policy name），而非新的调度类编号。

### 6.1 SCHED_EXT 调度类与 SCHED_AGENT 策略定义

```c
/**
 * @brief 调度类编号——复用 SCHED_EXT（值 7）
 * @note OLK-6.6 已内置 SCHED_EXT=7（include/uapi/linux/sched.h:121）
 * @note 禁止定义 SCHED_AGENT 策略编号宏（BAN-361 + IRON-1）
 * @note IRON-9 v2 [SC] 共享契约层：agentrt 与 agentrt-linux 共享此约束
 * @see include/uapi/linux/sched.h（OLK-6.6）
 */
/* SCHED_EXT 已由内核定义，agentrt-linux 直接复用，不重复定义 */

/**
 * @brief SCHED_AGENT 策略名称（sched_ext BPF 策略标识，非调度类编号）
 * @since 1.0.1
 * @note SCHED_AGENT 是策略名称，调度类编号统一用 SCHED_EXT
 */
 ...
 * @note IRON-9 v2 [SC] 共享契约层：核心字段（magic/priority/vtime）同源
 * @note IRON-9 v2 [IND] 独立层：调度专用扩展字段（task_id/trace_id/cpu_affinity/role 等）各自独立
...
struct airy_task_desc {
    ...
    uint16_t priority;       /* 优先级 0-139（0 最高）— [SC] 同源 */
...
/* AIRY_SLICE_DFL 同源 sched_ext SCX_SLICE_DFL（20ms） */
 ...
 * @since 1.0.1
 * @see sched_ext BPF
 * @note IRON-9 v2 [SS] 语义同源层：语义同源 sched_ext_ops，实现独立
...
s32 BPF_STRUCT_OPS(agent_select_cpu, struct task_struct *p,
    ...
    if (is_idle)
        scx_bpf_dsq_insert(p, SCX_DSQ_LOCAL, AIRY_SLICE_DFL, 0);
    return cpu;
}
...
void BPF_STRUCT_OPS(agent_enqueue, struct task_struct *p, u64 enq_flags) {
    ...
    scx_bpf_dsq_insert_vtime(p, SHARED_DSQ, AIRY_SLICE_DFL, vtime, enq_flags);
}
...
void BPF_STRUCT_OPS(agent_stopping, struct task_struct *p, bool runnable) {
    p->scx.dsq_vtime += (AIRY_SLICE_DFL - p->scx.slice) * 100 / p->scx.weight;
}
 ...
 */
static inline u32 airy_prio_to_weight(u16 priority) {
    if (priority <= 49)
...
`atomic_long_t ops_state` 让 SCX 核心随时可靠判定任务是否可被 BPF 派发，从而放宽对 BPF 调度器的要求（BPF 可尝试派发任意状态的任务，SCX 核心拒绝无效派发）。agentrt-linux SCHED_AGENT 同源采用此机制实现**软可靠性**。

---

## 8. fallback_dsq 自动回退机制

agentrt-linux SCHED_AGENT 的核心安全网，确保 BPF 调度器失败时系统完整性不破坏。
...
agentrt-linux SCHED_AGENT 增强 sched_ext 的单一 30s 超时为分级超时：
...
调度策略可插拔（FR-050）是 agentrt-linux 的核心特征，基于 sched_ext 实现运行时策略替换，不影响运行任务。同源 agentrt coreloopthree 的策略可插拔机制。
 ...
 * @since 1.0.1
 * @see sched_ext BPF
 * @note IRON-9 v2 [SS] 语义同源层：语义同源 agentrt
 */
AIRY_API int airy_sched_load_policy(const char *bpf_obj,
...
| 掩码 | 上下文 | agentrt-linux SCHED_AGENT 使用 |
|------|-------|---------------------------|
...
**agentrt-linux 决策**：完全同源采用 kf_mask 机制，严格按回调上下文调用 kfunc。
...
| 标志位 | 值 | 语义 | agentrt-linux SCHED_AGENT 使用 |
|--------|-----|------|---------------------------|
...
**关键决策**：agentrt-linux SCHED_AGENT 默认**不设置** `SCX_OPS_SWITCH_PARTIAL`，即 BPF 加载后所有 SCHED_NORMAL/SCHED_BATCH/SCHED_IDLE/SCHED_EXT 任务均走 SCHED_AGENT，最大化 Agent 调度优先级。
...
agentrt-linux SCHED_AGENT 同源实现 fallback 机制，确保 BPF 调度器失败时回退到 EEVDF 默认调度。
...
| `agent_priority_threshold` | `/sys/fs/agentrt/sched/` | 100 | SCHED_AGENT 实时优先级阈值 |
| `agent_default_timeslice` | `/sys/fs/agentrt/sched/` | 100ms | SCHED_AGENT 默认时间片 |
| `max_queue_depth` | `/sys/fs/bpf/sched_ext/` | 1024 | sched_ext 队列深度 |
| `timeout_ms` | `/sys/kernel/sched_ext/` | 30000 | runnable 超时（30s） |
...
perf/ftrace 命令详见 §12.2 性能验证方法。sched_ext BPF 程序追踪：`cat /sys/kernel/debug/sched_ext/trace`。
...
|---------|-------|--------|------------------|
| 任务调度 | INFO | `atoms.corekern.sched` | task_id, priority, sched_class, policy, cpu, latency_ns, vruntime, deadline, dsq_id, ops_state |
| 策略切换 | WARN | `atoms.corekern.sched.policy` | old_policy, new_policy, trigger, initiator_pid, affected_running_tasks, switch_duration_ns, scx_ops_state_before/after |
| fallback 回退 | ERROR | `atoms.corekern.sched.fallback` | trigger, timeout_task_id, timeout_ms, migrated_tasks, fallback_dsq, scx_ops_state_before/after, exit_reason |
...
```json
{
  ...
  "context": { "task_id": "task_xyz789", "priority": 50,
    "sched_class": "SCHED_AGENT", "policy": "agent_realtime",
    "cpu": 7, "latency_ns": 4200000, "vruntime": 1234567890,
    "deadline": 1234568000, "dsq_id": "SCX_DSQ_LOCAL", "ops_state": "QUEUED" }
}
```
...
| 契约 | 内容 | 落地位置 |
|------|------|---------|
| 调度类编号 | `SCHED_EXT`（值 7，复用内核已定义） | `include/uapi/linux/sched.h`（内核）|
| 策略名称 | `SCHED_AGENT_NAME "sched_agent"` | `include/airymax/sched.h` |
| 任务描述符 magic | `0x41475453 'AGTS'` | `include/airymax/sched.h` |
| 任务描述符结构 | `struct airy_task_desc`（64 字节对齐） | `include/airymax/sched.h` |
| 默认时间片 | `AIRY_SLICE_DFL`（20ms，同源 SCX_SLICE_DFL） | `include/airymax/sched.h` |
...
| vtime 类型 | `airy_vtime_t`（u64） | `include/airymax/sched.h` |
| vtime 衰减公式 | `airy_vtime_decay(vtime, consumed, weight)` | `include/airymax/sched.h` |

### 14.2 [SS] 语义同源层（高层 API 语义同源（概念操作一致），签名因抽象层级不同而独立演进）

agentrt-linux SCHED_AGENT BPF 程序实现以下回调（签名与 `struct sched_ext_ops` 同源，但实现独立于 agentrt）：
...
agentrt 的 MicroCoreRT 不实现这些回调（用户态无 BPF），但消费同样的 `struct airy_task_desc` 契约，调度语义一致。
...
agentrt 用户态 MicroCoreRT 调度语义与 agentrt-linux SCHED_AGENT 策略、AgentsIPC 与内核原生 IPC 均共享 [SC] 契约层，运行于 agentrt-linux 上天然契合无需适配层：

| 维度 | agentrt（用户态） | agentrt-linux（内核态） | 同源标注 |
|------|------------------|----------------------|----------|
| 优先级范围 | 0-139 | SCHED_AGENT 0-139 | [SC] |
...
| 调度类编号 | 不涉及内核调度类 | `SCHED_EXT`（值 7，复用内核） | [SC] |
| IPC 消息头 | 128 字节定长 + magic `0x41524531 'ARE1'` | 内核原生支持同源 | [SC] |
| 调度 payload | `struct airy_task_desc` 直接承载 | 直接作为内核调度描述符 | [SC] |

**同源红利**：agentrt 作为普通用户态应用（libc/POSIX）在 agentrt-linux 上天然更稳健；可选 `sched_setscheduler(p, SCHED_EXT, &param)` 享受内核原生调度优先级（SCHED_EXT=7，OLK-6.6 已内置），无需任何转换/适配层。
...
|---|--------|------------------|---------------------|-----------|
| 1 | [SC] 调度类编号 | 不涉及（用户态无调度类编号） | `SCHED_EXT`（值 7，复用内核） | ✅ PASS 完全共享（复用内核 SCHED_EXT，禁止 SCHED_AGENT 宏） |
| 2 | [SC] 任务描述符 magic | `0x41475453 'AGTS'` | 同源 `0x41475453` | ✅ PASS 完全共享 |
| 3 | [SC] 任务描述符结构 | `struct airy_task_desc`（64 字节对齐） | 同源结构 | ✅ PASS 完全共享 |
| 4 | [SC] 默认时间片 | `AIRY_SLICE_DFL = 20ms` | 同源 | ✅ PASS 完全共享 |
| 5 | [SC] 权重范围 | `[1..10000]` | 同源 | ✅ PASS 完全共享 |
| 6 | [SC] vtime 类型 | `airy_vtime_t (u64)` | 同源 | ✅ PASS 完全共享 |
...
**结论**：15 项检查全部 PASS。调度数据流与 agentrt atoms/corekern 在 [SC] 共享契约层完全一致（调度类/magic/描述符/时间片/权重/vtime 7 项），[SS] 语义同源层语义等价，[IND] 独立层正确分离（内核态专属机制不污染 agentrt）。agentrt 设计无需修改，保持跨平台用户态；agentrt-linux 在 [IND] 独立层正确引入 sched_ext 内核加速路径，遵循 IRON-9 v2 同源且部分代码共享原则。
...
| 版本 | 日期 | 变更内容 | 变更人 |
|---|---|---|---|
| 0.1.1 | 2026-07-06 | 初始版本，定义 EEVDF + sched_ext + SCHED_AGENT 调度数据流 | 工程规范委员会 |
| 0.1.1 | 2026-07-07 | 增强：补充 DSQ 调度队列、状态机、fallback_dsq、kf_mask、IRON-9 v2 三层共享模型落地；修复 agentos→agentrt 命名 | 工程规范委员会 |
