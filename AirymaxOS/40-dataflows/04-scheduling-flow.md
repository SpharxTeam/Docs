 ...
 * @note IRON-9 v2 [SC] 共享契约层：核心字段（magic/prio/_pad/vtime）同源
 * @note IRON-9 v2 [IND] 独立层：调度专用扩展字段（version/task_id/trace_id/cpu_affinity/role 等）各自独立
...
airy_sched_scx_state{state="enabled"} 1
airy_sched_scx_fallback_total 0
airy_sched_scx_task_state{state="enabled"} 380
```
