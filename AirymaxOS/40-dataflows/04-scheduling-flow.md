...
# agentrt-liunx（AirymaxOS）调度数据流

> **文档定位**: agentrt-liunx（AirymaxOS）调度数据流的详细设计，刻画 EEVDF + sched_ext + SCHED_AGENT 三位一体调度体系
...
调度数据流是 agentrt-liunx 内核的核心数据流，落地于 `airymaxos-kernel` 子仓（同源 agentrt atoms/corekern Task 模块）。该数据流基于 Linux 6.6 内核基线的三大调度能力：

1. **EEVDF 调度器**（FR-001, FR-008）：Linux 6.6 默认调度算法，替代传统 CFS。通过虚拟截止时间（virtual deadline）+ 资格判定（eligibility）实现更精确的延迟控制，抢占延迟 < 10μs。
2. **sched_ext**（FR-001, FR-002）：agentrt-liunx 内核增强（主线 6.12+），基于 eBPF 的可插拔调度框架。agentrt-liunx 通过 sched_ext 实现 SCHED_AGENT 策略，调度策略以 BPF 程序形式加载，支持运行时替换（FR-050）。
3. **SCHED_AGENT 策略**（FR-002）：agentrt-liunx 自研调度类，专为 Agent 工作负载优化。优先级范围 0-139，支持抢占式调度与策略可插拔。
...
| DSQ 类型 | 标识 | 语义 | agentrt-liunx SCHED_AGENT 使用 |
|---------|------|------|---------------------------|
...
| kfunc | 语义 | 调用上下文 | agentrt-liunx 使用 |
|-------|------|----------|---------------|
...
SCHED_AGENT 是 agentrt-liunx 基于 sched_ext 框架实现的**自研 BPF 调度策略**（FR-002），专为 Agent 工作负载优化。同源 agentrt atoms/corekern 的 MicroCoreRT 调度器。
 ...
 * @note IRON-9 v2 [SC] 共享契约层：agentrt 与 agentrt-liunx 共享此约束
 * @see include/uapi/linux/sched.h（OLK-6.6）
 */
/* SCHED_EXT 已由内核定义，agentrt-liunx 直接复用，不重复定义 */
...
`atomic_long_t ops_state` 让 SCX 核心随时可靠判定任务是否可被 BPF 派发，从而放宽对 BPF 调度器的要求（BPF 可尝试派发任意状态的任务，SCX 核心拒绝无效派发）。agentrt-liunx SCHED_AGENT 同源采用此机制实现**软可靠性**。

---

## 8. fallback_dsq 自动回退机制

agentrt-liunx SCHED_AGENT 的核心安全网，确保 BPF 调度器失败时系统完整性不破坏。
...
agentrt-liunx SCHED_AGENT 增强 sched_ext 的单一 30s 超时为分级超时：
...
agentrt-liunx 超节点 OS（FR-048）支持跨节点任务迁移，基于 CXL + RDMA 实现迁移延迟 < 100ms。同源 agentrt atoms/corekern 的超节点扩展。
...
调度策略可插拔（FR-050）是 agentrt-liunx 的核心特征，基于 sched_ext 实现运行时策略替换，不影响运行任务。同源 agentrt coreloopthree 的策略可插拔机制。
...
| 掩码 | 上下文 | agentrt-liunx SCHED_AGENT 使用 |
|------|-------|---------------------------|
...
**agentrt-liunx 决策**：完全同源采用 kf_mask 机制，严格按回调上下文调用 kfunc。
...
| 标志位 | 值 | 语义 | agentrt-liunx SCHED_AGENT 使用 |
|--------|-----|------|---------------------------|
...
**关键决策**：agentrt-liunx SCHED_AGENT 默认**不设置** `SCX_OPS_SWITCH_PARTIAL`，即 BPF 加载后所有 SCHED_NORMAL/SCHED_BATCH/SCHED_IDLE/SCHED_EXT 任务均走 SCHED_AGENT，最大化 Agent 调度优先级。
...
agentrt-liunx SCHED_AGENT 同源实现 fallback 机制，确保 BPF 调度器失败时回退到 EEVDF 默认调度。
...
```json
{
  ...
  "function": "agentrt_sched_pick_next", "line": 412,
...
agentrt 与 agentrt-liunx 完全共享的契约定义：
...
agentrt-liunx SCHED_AGENT BPF 程序实现以下回调（签名与 `struct sched_ext_ops` 同源，但实现独立于 agentrt）：
...
agentrt 用户态 MicroCoreRT 调度语义与 agentrt-liunx SCHED_AGENT 策略、AgentsIPC 与内核原生 IPC 均共享 [SC] 契约层，运行于 agentrt-liunx 上天然契合无需适配层：

| 维度 | agentrt（用户态） | agentrt-liunx（内核态） | 同源标注 |
|------|------------------|----------------------|----------|
| 优先级范围 | 0-139 | SCHED_AGENT 0-139 | [SC] |
| 任务描述符 | `agentrt_task_desc_t` | 同源结构 + magic `0x41475453 'AGTS'` | [SC] |
| vtime 衰减 | `airymax_vtime_decay()` | 同源公式 | [SC] |
| 默认时间片 | `AIRYMAX_SLICE_DFL = 20ms` | 同源 | [SC] |
| 调度类编号 | 不涉及内核调度类 | `SCHED_EXT`（值 7，复用内核） | [SC] |
| IPC 消息头 | 128 字节定长 + magic `0x41524531 'ARE1'` | 内核原生支持同源 | [SC] |
| 调度 payload | `agentrt_task_desc_t` 直接承载 | 直接作为内核调度描述符 | [SC] |

**同源红利**：agentrt 作为普通用户态应用（libc/POSIX）在 agentrt-liunx 上天然更稳健；可选 `sched_setscheduler(p, SCHED_EXT, &param)` 享受内核原生调度优先级（SCHED_EXT=7，OLK-6.6 已内置），无需任何转换/适配层。
...
| 9 | [SS] 消费契约 | agentrt 消费 `agentrt_task_desc_t` | agentrt-liunx 同时消费同源契约 | ✅ PASS 语义同源 |
...
**结论**：15 项检查全部 PASS。调度数据流与 agentrt atoms/corekern 在 [SC] 共享契约层完全一致（调度类/magic/描述符/时间片/权重/vtime 7 项），[SS] 语义同源层签名一致，[IND] 独立层正确分离（内核态专属机制不污染 agentrt）。agentrt 设计无需修改，保持跨平台用户态；agentrt-liunx 在 [IND] 独立层正确引入 sched_ext 内核加速路径，遵循 IRON-9 v2 同源且部分代码共享原则。
