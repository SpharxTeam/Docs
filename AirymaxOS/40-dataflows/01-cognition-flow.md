...
# agentrt-liunx（AirymaxOS）认知循环数据流

> **文档定位**: agentrt-liunx（AirymaxOS）认知循环数据流的详细设计，刻画 System 1/2 双系统协同与 CoreLoopThree kthread 实现
...
认知循环数据流是 agentrt-liunx 区别于通用操作系统的核心特征，落地于 `airymaxos-cognition` 子仓（同源 agentrt coreloopthree 模块）。该数据流借鉴 Kahneman 双系统思维理论：
...
| API | agentrt（用户态） | agentrt-liunx（内核态） | 同源语义 |
|-----|------------------|---------------------|---------|
...
| 超节点沙箱 | 内核 capability + cgroup | agentrt-liunx 专属 |
| 具身智能 | 内核 sensorio + 工业总线 | agentrt-liunx 专属 |
| KVC-Gateway | 内核 LLMCache + Bifrost | agentrt-liunx 专属 |
...
**结论**：15 项检查全部 PASS。认知循环数据流与 agentrt coreloopthree 模块在 [SC] 共享契约层完全一致（阶段/模式/上下文/能效/GPU-NPU 5 项），[SS] 语义同源层 API 签名一致（5 项核心 API），[IND] 独立层正确分离（kthread/GPU-NPU 驱动/drm_sched/Wasm 内核态专属机制不污染 agentrt）。agentrt 设计无需修改，保持跨平台用户态；agentrt-liunx 在 [IND] 独立层正确引入 kthread + GPU/NPU + drm_sched 内核加速路径，遵循 IRON-9 v2 同源且部分代码共享原则。
