...
策略热更新通过 `airy_sys_sched_ctl` 系统调用触发，更新时重新评估所有活动任务（NFR-S-007）。
...
| API | agentrt（用户态） | agentrt-linux（内核态） | 同源语义 |
|-----|------------------|---------------------|---------|
| `airy_clt_run()` | 用户态事件循环 | 内核 kthread 主循环 | 三阶段循环 |
| `airy_clt_notify_phase()` | 同步通知 | 内核 `wake_up_process()` | 阶段切换通知 |
| `airy_thinkdual_switch()` | 协程切换 | kthread 优先级切换 | System 1/2 切换 |
| `airy_llm_scheduler_submit()` | 用户态队列 | io_uring 提交 | LLM 推理提交 |
| `airy_clt_query_phase()` | atomic 读取 | 内核 `atomic64_read()` | 当前阶段查询 |
| `airy_token_efficiency_record()` | 用户态 metric | 内核 perf_event | 能效指标记录 |
| `airy_gpu_npu_schedule()` | 用户态 ioctl | 内核 accel ops | GPU/NPU 调度 |
| `airy_wasm_runtime_instantiate()` | 用户态 Wasm | 内核 eBPF 验证器风格 | Wasm 实例化 |
...
| 超节点沙箱 | 内核 capability + cgroup | agentrt-linux 专属 |
| 具身智能 | 内核 sensorio + 工业总线 | agentrt-linux 专属 |
| KVC-Gateway | 内核 LLMCache + Bifrost | agentrt-linux 专属 |
...
| 6 | [SS] CLT_run 签名 | `airy_clt_run(ctx)` | 语义等价，实现独立 | PASS 语义同源 |
| 7 | [SS] notify_phase 签名 | `airy_clt_notify_phase(ctx, phase)` | 语义等价 | PASS 语义同源 |
| 8 | [SS] thinkdual_switch 签名 | `airy_thinkdual_switch(ctx, mode)` | 语义等价 | PASS 语义同源 |
| 9 | [SS] llm_scheduler_submit 签名 | `airy_llm_scheduler_submit(req)` | 语义等价 | PASS 语义同源 |
| 10 | [SS] query_phase 签名 | `airy_clt_query_phase(ctx)` | 语义等价 | PASS 语义同源 |
...
**结论**：15 项检查全部 PASS。认知循环数据流与 agentrt coreloopthree 模块在 [SC] 共享契约层完全一致（阶段/模式/上下文/能效/GPU-NPU 5 项），[SS] 语义同源层 API 语义等价（5 项核心 API），[IND] 独立层正确分离（kthread/GPU-NPU 驱动/drm_sched/Wasm 内核态专属机制不污染 agentrt）。agentrt 设计无需修改，保持跨平台用户态；agentrt-linux 在 [IND] 独立层正确引入 kthread + GPU/NPU + drm_sched 内核加速路径，遵循 IRON-9 v3 同源且部分代码共享原则。
...
| 版本 | 日期 | 变更内容 | 变更人 |
|---|---|---|---|
| 0.1.1 | 2026-07-06 | 初始版本，定义认知循环数据流 12 步与双系统切换 | 工程规范委员会 |
| 0.1.1 | 2026-07-07 | 新增 Copyright 头 + §9 IRON-9 v3 四层共享模型落地 + §10 agentrt 一致性检查（15 项全 PASS） | 工程规范委员会 |
