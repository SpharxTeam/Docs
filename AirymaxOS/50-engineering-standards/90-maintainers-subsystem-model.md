...
# agentrt-linux（AirymaxOS）MAINTAINERS 子系统维护者模式

> **文档定位**: agentrt-linux（AirymaxOS，极境智能体操作系统）的 MAINTAINERS 子系统维护者制度规范。基于 OLK-6.6 `MAINTAINERS` 文件格式（M/S/F/T/R/L/Q/B/C/P/N/K/X 等 13 类字段），制定 agentrt-linux 8 子仓的 MAINTAINERS 模板、维护者层级、CODEOWNERS 映射。
...
本文件研究 OLK-6.6 `MAINTAINERS` 文件（734KB，约 7000 行）的字段格式与维护者制度，制定 agentrt-linux 8 子仓的 MAINTAINERS 文件模板、维护者层级制度、以及与 GitHub/GitLab CODEOWNERS 的映射。
...
| 状态 | OLK-6.6 行号 | 含义 | agentrt-linux 适用 |
|------|------------|------|-------------------|
...
## 2. agentrt-linux 8 子仓划分

### 2.1 子仓清单

agentrt-linux 仓库划分为 8 个子系统（subsystem），每个子系统对应一个 MAINTAINERS 条目：
...
## 3. agentrt-linux MAINTAINERS 模板

### 3.1 完整 MAINTAINERS 文件

以下是 agentrt-linux 仓库根 `MAINTAINERS` 文件的完整模板：

```
List of maintainers
===================

agentrt-linux (AirymaxOS) subsystem maintainers. Field format
...
B:	https://gitlab.airymaxos.com/agentrt-linux/-/issues
T:	git git@gitlab.airymaxos.com:agentrt-linux.git
...
# (agentrt + agentrt-linux). See 120-cross-project-code-sharing.md.
...
B:	https://gitlab.airymaxos.com/agentrt-linux/-/issues?label=kernel-core
T:	git git@gitlab.airymaxos.com:agentrt-linux.git
...
B:	https://gitlab.airymaxos.com/agentrt-linux/-/issues?label=drivers
T:	git git@gitlab.airymaxos.com:agentrt-linux.git
...
B:	https://gitlab.airymaxos.com/agentrt-linux/-/issues?label=bpf
T:	git git@gitlab.airymaxos.com:agentrt-linux.git
...
B:	https://gitlab.airymaxos.com/agentrt-linux/-/issues?label=build
T:	git git@gitlab.airymaxos.com:agentrt-linux.git
F:	build/
F:	scripts/airymax/
F:	.clang-format
...
agentrt-linux 对 MAINTAINERS 字段的额外约定：
...
| `T:` | git 树统一指向 `git@gitlab.airymaxos.com:agentrt-linux.git` |
...
agentrt-linux 采用三级维护者层级，对应 Linux 内核的 maintainer / subsystem maintainer / Linus 模型：

| 层级 | 角色 | 职责 | agentrt-linux 实例 |
|------|------|------|------------------|
...
GitHub 与 GitLab 使用 `.CODEOWNERS` 文件（语法相似但路径规则不同）实现自动审查者指派。agentrt-linux 将 MAINTAINERS 的 `F:` + `M:` 映射为 CODEOWNERS。

### 5.2 CODEOWNERS 文件模板

以下是 agentrt-linux 仓库根 `.github/CODEOWNERS`（或 GitLab `CODEOWNERS`）的模板：

```
# CODEOWNERS for agentrt-linux (AirymaxOS)
...
为避免 MAINTAINERS 与 CODEOWNERS 不一致，agentrt-linux 提供 `make codeowners-sync` 目标自动同步：
...
OLK-6.6 提供 `scripts/get_maintainer.pl`，根据补丁修改的文件查询 MAINTAINERS，输出应 CC 的维护者邮箱列表。agentrt-linux 直接复用此脚本：
...
| 字段 | 含义 | agentrt-linux 用法 | OLK-6.6 行号 |
|------|------|------------------|------------|
...
| `T:` | git 树 | git@gitlab.airymaxos.com:agentrt-linux.git | `MAINTAINERS:31-32` |
