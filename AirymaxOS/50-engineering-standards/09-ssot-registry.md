...
**每个技术点只能有一个权威编号。** 本注册表是全部规则编号的唯一分配者；主题文档是规则详细说明的载体，但编号的分配权仅属于本注册表。本注册表覆盖三套同源但独立的编号体系：AirymaxOS `OS-*` 前缀规则（第 2-13 章）、agentrt 无 `OS-` 前缀规则（第 14 章）、跨切编号体系（§13.2.1 等，如 `C-S*` IPC fastpath 校验链）。
...
| **编号体系隔离** | AirymaxOS `OS-*` 编号、agentrt 无前缀编号、跨切编号（如 `C-S*`）属于三个不同编号空间，禁止混用（如禁止将 `OS-IRON-001`、`IRON-1`、`C-S9` 视为同一编号）。跨体系引用时须标注来源体系。 |
...
**跨切编号格式**（§13.2.1 等，无 `OS-` 前缀）：

```
<前缀>-<标识>
```

跨切编号体系用于同时被 AirymaxOS 内核态与 agentrt 用户态引用的规则，遵循 IRON-9 v3 四层共享模型——契约经 [SC] 共享，实现各自独立。当前登记的跨切编号体系：

| 前缀 | 编号区间    | 说明                                         | 登记章节  |
| --- | --------- | ------------------------------------------ | ----- |
| C-S | C-S0\~C-S12 | IPC Fastpath 校验链（13 项，含 Capability Folding C-S9） | §13.2.1 |
...
| OS-IFACE-009 | 系统调用语义映射遵循 IRON-9 v3 四层共享模型——agentrt 高层 API（JSON-RPC 统一入口）与 agentrt-linux 编号 syscall（548-571）在 \[SS] 语义同源层保持概念操作一致，签名因抽象层级不同而独立演进；SDK 层（AirymaxClient 4 语言）签名确实同源（同一份源码两端编译）；禁止在系统调用层引入签名转换层或 JSON-to-struct 适配层，签名同源仅限于 SDK 层 | 30-interfaces/05-syscall-semantic-mapping.md §8 L332 |
...
### 13.2.1 C-S IPC Fastpath 校验链（C-S0\~C-S12，跨切编号体系）

> **权威定义文档**：30-interfaces/07-ipc-fastpath.md §3.1
>
> **编号格式说明**：C-S 系列是跨切（cross-cutting）编号体系，不使用 `OS-` 前缀，因为 IPC fastpath 校验链同时被 AirymaxOS 内核态（`airy_uring_cmd` fastpath）与 agentrt 用户态（IPC 客户端校验）引用。C-S 编号属于第三编号空间，与 `OS-*`（第 2-13 章）和 agentrt 无前缀（第 14 章）并列，遵循 IRON-9 v3 四层共享模型——校验契约经 [SC] 共享，校验实现各自独立。
>
> **SSoT 声明**：07-ipc-fastpath.md §3.1 是 C-S0\~C-S12 校验链的唯一权威定义。本节登记编号分配，主题文档引用时须与本表一致。

| 编号    | 规则摘要                                                                  | 权威定义                    |
| ------- | ----------------------------------------------------------------------- | ------------------------- |
| C-S0    | `ring->frozen == false`（ring 未冻结检查）                                | 07-ipc-fastpath §3.1 L203 |
| C-S1    | IPC magic 校验（`magic == 0x41524531`）                                   | 07-ipc-fastpath §3.1      |
| C-S2    | opcode 合法性校验（在 7 种 opcode 内）                                     | 07-ipc-fastpath §3.1      |
| C-S3    | payload 大小校验（`payload_len <= AIRY_IPC_MAX_PAYLOAD`）                 | 07-ipc-fastpath §3.1      |
| C-S4    | 消息头大小 + reserved 零值校验（`hdr_size == 128` 且 `reserved[72]` 全零） | 07-ipc-fastpath §3.1      |
| C-S5    | src\_task / dst\_task 合法性校验                                          | 07-ipc-fastpath §3.1      |
| C-S6    | kfifo 可用空间校验（`kfifo_avail` 足够）                                  | 07-ipc-fastpath §3.1 L213 |
| C-S7    | reclaim flag 校验                                                        | 07-ipc-fastpath §3.1      |
| C-S8    | 上下文校验（`in_task()` 为真）                                            | 07-ipc-fastpath §3.1      |
| C-S9    | **Capability Folding Badge 校验**（Epoch + RandomTag + Perms 三重校验，~10ns 内联）—— H6: [DSL] 降级模式 `capability_badge=0` 跳过此校验 | 07-ipc-fastpath §3.1 L217 |
| C-S10   | flags 合法性校验（不含 `ENCRYPT`/`COMPRESS`/`RESERVED` 位）                | 07-ipc-fastpath §3.1 L213 |
