...
> **最后更新**： 2026-07-09\
> **同源映射**： agentrt `docs/AirymaxRT/00-architectural-principles.md`（五维正交 24 原则）+ IRON-9 v2 工程铁律（内部工程标准规范，17 类规则编号体系）\
...
agentrt-linux 工程标准与 agentrt 工程标准是**同源且部分代码共享**的关系（IRON-9 v2，2026-07-07 用户决策变更）：

#### 三层共享模型

| 层次                          | 共享程度          | 内容                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                              | 组织方式                             |
| --------------------------- | ------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------------------------------- |
| **共享契约层**（Shared-Contract）  | 完全共享代码        | **6 个 \[SC] 头文件清单**（`include/airymax/`）：(1) `bpf_struct_ops.h`——struct\_ops 状态机 + common\_value；(2) `memory_types.h`——MemoryRovol L1-L4 数据结构 + GFP 掩码语义 + PMEM 持久化接口；(3) `security_types.h`——POSIX capability 41 ID + LSM 钩子 252 ID + Cupolas blob 布局 + capability 派生模型 + Vault backend + 策略裁决 4 值枚举；(4) `cognition_types.h`——CoreLoopThree 阶段枚举 + Thinkdual 模式枚举 + LLM 推理阶段枚举 + 上下文结构 + Token 能效指标 + GPU/NPU 描述符；(5) `sched.h`——SCHED\_EXT 调度类编号约束（复用内核 SCHED\_EXT=7，禁止 SCHED\_AGENT 宏）+ 任务描述符（magic 0x41475453 'AGTS'）+ vtime 类型与衰减公式 + 优先级范围 + AIRYMAX\_SLICE\_DFL；(6) `ipc.h`——IPC magic（0x41524531 'ARE1'）+ 128B 消息头结构（`struct airy_ipc_msg_hdr`）+ SQE/CQE 操作码与标志位。另含 syscall 编号（`AIRY_SYS_*`）、capability 令牌格式、错误码（`airy_err_t`）、规则编号体系（IRON/BAN/STD/ACC）、五维正交 24 原则 | `include/airymax/` 独立头文件库，两端共同依赖 |
...
### 1.4 主流 Linux 发行版标准兼容性声明（2026-07-07 决策）
...
- **agentrt-linux ↔ Linux 6.6 基线**：**仅技术参考，不共享代码**——OLK-6.6 的 kthread.c/accel.c/sched\_main.c 等是 agentrt-linux \[IND] 独立层的实现参考
...
agentrt-linux 工程标准由 5 个主题章节（01-05）+ 2 个治理章节（06-07）+ 1 个合规性检查章节（08）+ 1 个许可证策略（09，已废弃）+ 6 个扩展专题文档（60/70/80/100/110/120）+ 5 个子目录（10-coding-style/20-contracts/30-runtime-interfaces/40-integration/50-project-erp）构成，共 52 个文档（含本 README）：
...
├── 02-code-format.md                  # 代码格式（缩进/行宽/大括号/空格/clang-format 规则映射）
...
├── 06-toolchain-and-automation.md     # 工具链与自动化（7 层验证框架/checkpatch/CI/CD）
...
├── 09-license-strategy.md             # 许可证策略（已废弃→合并至 110-spdx-license-compliance.md）
│
├── 10-coding-style/                   # 编码规范子目录（原 50-specifications/coding_standard/）
│   ├── README.md                      # 编码规范总览与导航
│   ├── C_coding_style_standard.md     # C 编码风格规范
│   ├── C_coding_style_standard_supplement.md  # C 编码风格补充（平台差异/兼容性）
│   ├── C_Cpp_secure_coding_standard.md # C/C++ 安全编码规范
│   ├── Cpp_coding_style_standard.md   # C++ 编码风格规范
│   ├── Rust_coding_style_standard.md  # Rust 编码风格规范
│   ├── Rust_secure_coding_standard.md # Rust 安全编码规范
│   ├── Go_coding_style_standard.md    # Go 编码风格规范
│   ├── Go_secure_coding_standard.md   # Go 安全编码规范
│   ├── Python_coding_style_standard.md # Python 编码风格规范
│   ├── JavaScript_coding_style_standard.md # JavaScript/TypeScript 编码规范
│   ├── Java_secure_coding_standard.md # Java 安全编码规范
│   ├── code_comment_template.md       # 代码注释模板（Doxygen/docstring）
│   ├── log_standard.md                # 日志打印规范（级别/内容/格式/质量）
│   ├── naming_conventions.md          # 命名规范（组件/文件/函数/类型/常量）
│   ├── security_design_standard.md    # 安全设计规范（D1-D4 四层防护）
│   └── config_audit_log.md            # 配置审计日志规范
│
├── 20-contracts/                      # 契约规范子目录（原 50-specifications/airy_contract/）
│   ├── README.md                      # 契约规范总览与导航
│   ├── ipc_protocol_contract.md       # IPC 协议契约
│   ├── logging_contract.md           # 日志契约
│   └── syscall_api_contract.md       # 系统调用 API 契约
│
├── 30-runtime-interfaces/             # 运行时接口子目录（原 50-specifications/are_standards/）
...
│
├── 40-integration/                    # 集成规范子目录（原 50-specifications/integration_standards/）
│   ├── README.md                      # 集成规范总览与导航
│   ├── agentrt_integration.md         # agentrt 集成规范
│   ├── integration_standard.md        # 集成交付标准
│   ├── standards_contribution.md      # 标准贡献指南
│   └── ecosystem_partnership.md       # 生态合作伙伴指南
│
├── 50-project-erp/                    # 项目工程管理子目录（原 50-specifications/project_erp/）
...
│
├── 60-checkpatch-rule-map.md          # checkpatch 规则映射表（OLK-6.6 checkpatch.pl → OS-STD-CODE-NNN）
├── 70-kernel-doc-standard.md          # kernel-doc 文档标准
├── 80-clang-format-enforcement.md     # clang-format 强制规范（格式化规则 + CI 检查集成）
├── 100-deprecated-api-registry.md     # 弃用接口注册表
├── 110-spdx-license-compliance.md     # SPDX 许可证合规
└── 120-cross-project-code-sharing.md   # 跨项目代码共享（IRON-9 v2 [SC] 共享契约层）
```

> **文件分层说明**：
> - **核心章节（00-09）**：8 主题 + 1 合规 + 1 许可证 = 10 文档，构成工程标准主干
> - **子目录（10-50）**：5 个专项规范子目录，由原 `50-specifications/` 迁移而来
> - **扩展文档（60-120）**：6 个专题深度文档，对核心章节的特定主题进行穷尽式展开

> **迁移说明**：`10-coding-style/` 至 `50-project-erp/` 五个子目录由原 `50-specifications/` 目录迁移而来（2026-07-08 完成迁移，原 `50-specifications/` 目录已删除）。`TERMINOLOGY.md` 已提升至 `docs/AirymaxOS/` 顶层。
...
| 09 许可证策略     | "代码归谁所有"——AGPL-3.0 + Apache-2.0 双许可证策略、SPDX 标识、第三方组件合规            | Airymax 双许可证模型 + Linux `license-rules.rst`                            |
| 60 checkpatch 规则映射 | "checkpatch 怎么落地"——OLK-6.6 `scripts/checkpatch.pl` ≥30 条规则 → OS-STD-CODE-NNN 映射 | OLK-6.6 `scripts/checkpatch.pl`（239KB，~7800 行）                       |
| 70 kernel-doc 标准 | "API 怎么文档化"——kernel-doc 注释格式、`make htmldocs` 验证、ABI 文档化           | Linux `kernel-doc.rst` + `Documentation/doc-guide/`                    |
| 80 clang-format 强制 | "格式怎么被工具强制"——`.clang-format` 完整 YAML（689 行 + 560 ForEachMacros）+ `make format-check` CI 门禁 | OLK-6.6 `.clang-format`（689 行）+ `clang-format.rst`                      |
| 100 弃用接口注册表  | "哪些接口不能再用了"——弃用接口动态清单 + 弃用流程（RFC + 6 个月宽限期）                      | Linux `deprecated.rst`（16575 字节）+ Airymax 弃用流程                       |
| 110 SPDX 许可证合规 | "许可证怎么标注"——SPDX 标识符规范、` SPDX-License-Identifier` 头部、双许可证选择策略      | Linux `license-rules.rst`（18024 字节）+ SPDX 规范                        |
...
| **K-4 可插拔策略**  | sched\_ext BPF 调度器 + LSM 钩子 + 模块签名                          | 03 代码风格                |
...
| **E-3 资源确定性**  | 引用计数强制 + devm\_ 资源管理 + 内存分配惯用法                              | 01 代码规范 + 03 代码风格      |
...
#### 2.3.1 代码格式 SSoT 设计（02 单一数据源）

> **SSoT 对齐（v0.3.1）**：原 `80-clang-format-enforcement.md` 已按 D-02 决策合并入 `02-code-format.md` §8/§9，消除"规则层"与"实现层"双文档重叠。02-code-format.md 现为代码格式规则的单一数据源（SSoT）。

| 文档 | 职责层 | 内容范围 | 关系 |
|------|--------|---------|------|
| **02-code-format.md** | 规则 + 实现层 | 格式规则（OS-KER-004~021）+ 规则到配置项映射表（§9）+ `.clang-format` 配置即代码（§8）+ CI 强制 `make format-check`（§8.5） | 代码格式 SSoT |

> **1.0.1 待补全**：完整 `.clang-format` YAML 文件、OLK-6.6 源码行号标注、Makefile `format-check` 目标定义与 CI 集成脚本在 1.0.1 开发阶段随代码实现补全。

#### 2.3.2 工具链双层设计（06 ↔ 60）

| 文档 | 职责层 | 内容范围 | 关系 |
|------|--------|---------|------|
| **06-toolchain-and-automation.md** | 框架层 | 7 层自动化验证体系（Layer 1-7）+ CI/CD 矩阵 + 覆盖率门槛 | 定义"在哪个阶段执行" |
| **60-checkpatch-rule-map.md** | 规则映射层 | OLK-6.6 `scripts/checkpatch.pl` ≥30 条规则 → OS-STD-CODE-NNN 映射 | 定义"checkpatch 检查什么" |

> **交叉引用约定**：06 Layer 3（预提交验证）与 Layer 4（CI 门禁）引用 60 的 checkpatch 规则与 02 的 clang-format 配置；60 覆盖语义检查（checkpatch），02 §8/§9 覆盖格式检查（clang-format）。

#### 2.3.3 维护者治理统一文档（07，原 90 已合并）

| 文档 | 职责层 | 内容范围 | 变更说明 |
|------|--------|---------|---------|
| **07-maintainers-and-governance.md** | 治理 SSoT | MAINTAINERS 字段定义（§2.1，含 OLK-6.6 源码行号）+ 新增字段（§2.2）+ Status 定义（§2.3）+ 文件分布与模板（§2.4）+ Lieutenant System（§3）+ 8 子仓分配（§4）+ DCO（§5）+ Reviewer's Statement（§6）+ 成熟度模型（§7）+ 审查礼仪（§8）+ 管理哲学（§9）+ 规则编号注册表（§10）+ 五维映射（§11） | 原 90-maintainers-subsystem-model.md 的 MAINTAINERS 模板、CODEOWNERS 同步、get_maintainer.pl 集成已于 2026-07-09 合并入 §2.4 |

> **合并说明**：原 `90-maintainers-subsystem-model.md` 因与 07 存在 MAINTAINERS 字段定义、维护者层级、8 子仓分配三处内容重叠，且其 §2 的 8 子系统分解（基于单仓子系统路径）与 v0.2.4 设计的 8 子仓独立仓库模型不一致，已于 2026-07-09 整体合并入 07，原文件删除。07 §2.4 新增"MAINTAINERS 文件分布与模板" subsection，包含两级 MAINTAINERS 模型、[SC] 审批流程、CODEOWNERS 自动同步、get_maintainer.pl 集成。
...
- **Agent 接口命名**：`airy_*` 前缀保留给 agentrt 同源 API，`airymaxos_*` 前缀用于 agentrt-linux 专属
...
- 完成 50-engineering-standards/ 全部 38 文档（含本 README + 00-09 核心 10 文档 + 60-120 扩展 6 文档 + 5 子目录 22 文档，含 08-compliance-checklist.md）
...
- `docs/AirymaxOS/50-engineering-standards/10-coding-style/`（编码规范子目录，含 C/Rust 风格与安全编码 4 文档 + README）
- `docs/AirymaxOS/TERMINOLOGY.md`（术语规范，已提升至顶层）
...
- **当前版本**: v1.2（2026-07-09，合并 90-maintainers-subsystem-model.md 到 07 + 添加 02↔80 / 06↔60↔80 交叉引用矩阵 + 更新文件树与章节定位表）
...
> **文档结束** | 共 38 文档（含本 README + 00-09 核心 10 文档 + 60-120 扩展 6 文档 + 5 子目录 22 文档）| 0.1.1 版本 P0 优先完成
