...
| 支柱                     | 思想来源                                        | 落地章节           |
| ---------------------- | ------------------------------------------- | -------------- |
| **Linux 内核工程思想**       | Linux 6.6 内核源代码（Linux 6.6 内核基线）深度研究提炼              | §3-§7 五大主题章节   |
...
│
├── 60-checkpatch-rule-map.md          # checkpatch 规则映射表（Linux 6.6 内核 checkpatch.pl → OS-STD-CODE-NNN）
...
> **迁移说明**：`10-coding-style/` 至 `50-project-erp/` 五个子目录由原 `50-specifications/` 目录迁移而来（原 `50-specifications/` 目录已删除）。`TERMINOLOGY.md` 已提升至 `docs/AirymaxOS/` 顶层。
...
| 60 checkpatch 规则映射 | "checkpatch 怎么落地"——Linux 6.6 内核基线 `scripts/checkpatch.pl` ≥30 条规则 → OS-STD-CODE-NNN 映射 | Linux 6.6 内核基线 `scripts/checkpatch.pl`（239KB，~7800 行）                       |
| 70 kernel-doc 标准 | "API 怎么文档化"——kernel-doc 注释格式、`make htmldocs` 验证、ABI 文档化           | Linux `kernel-doc.rst` + `Documentation/doc-guide/`                    |
| 80 clang-format 强制 | "格式怎么被工具强制"——`.clang-format` 完整 YAML（689 行 + 560 ForEachMacros）+ `make format-check` CI 门禁 | Linux 6.6 内核基线 `.clang-format`（689 行）+ `clang-format.rst`                      |
...
> **SSoT 对齐**：原 `80-clang-format-enforcement.md` 已合并入 `02-code-format.md` §8/§9，消除"规则层"与"实现层"双文档重叠。02-code-format.md 现为代码格式规则的单一数据源（SSoT）。

| 文档 | 职责层 | 内容范围 | 关系 |
|------|--------|---------|------|
| **02-code-format.md** | 规则 + 实现层 | 格式规则（OS-KER-004~021）+ 规则到配置项映射表（§9）+ `.clang-format` 配置即代码（§8）+ CI 强制 `make format-check`（§8.5） | 代码格式 SSoT |

> **待补全**：完整 `.clang-format` YAML 文件、Linux 6.6 内核基线 源码行号标注、Makefile `format-check` 目标定义与 CI 集成脚本随代码实现补全。
...
| **60-checkpatch-rule-map.md** | 规则映射层 | Linux 6.6 内核基线 `scripts/checkpatch.pl` ≥30 条规则 → OS-STD-CODE-NNN 映射 | 定义"checkpatch 检查什么" |
