...
- **互操作**：agentrt 在 agentrt-linux 上运行时，两端工程标准形成天然互补——agentrt 遵循其用户态运行时规范（如 `agentrt_log_write` 日志），agentrt-linux 遵循其内核发行版规范（如 `printk` 等级），两端通过**共享契约层代码**（如 `include/airymax/ipc.h` 的 IPC 消息头定义、`include/airymax/sched.h` 的任务描述符定义）实现无适配层互操作。
...
agentrt-linux 工程标准由 5 个主题章节（01-05）+ 2 个治理章节（06-07）+ 1 个合规性检查章节（08）+ 1 个许可证策略（09）+ 6 个扩展专题文档（60/70/80/100/110/120）+ 5 个子目录（10-coding-style/20-contracts/30-runtime-interfaces/40-integration/50-project-erp）构成，共 38 个文档（含本 README）：
...
│
├── 20-contracts/                      # 契约规范子目录（原 50-specifications/agentrt_contract/）
│   ├── README.md                      # 契约规范总览与导航
...
├── 80-clang-format-enforcement.md     # .clang-format 配置与 CI 门禁（完整 YAML + make format-check）
...
#### 2.3.1 代码格式双层设计（02 ↔ 80）

| 文档 | 职责层 | 内容范围 | 关系 |
|------|--------|---------|------|
| **02-code-format.md** | 规则层 | 定义格式规则（OS-KER-004~021）+ 规则到配置项的映射表（§9） | 定义"应当怎样" |
| **80-clang-format-enforcement.md** | 实现层 | 完整 `.clang-format` YAML（§2）+ OLK-6.6 源码行号标注（§1）+ `make format-check` CI 门禁流程（§3） | 定义"工具怎么强制" |

> **交叉引用约定**：02 §9 的"clang-format 关键配置项"表是规则到配置的速查映射；80 §1 的"关键配置项详解"是每个配置项的 OLK-6.6 源码行号与理由阐述。两者互补不重复。

#### 2.3.2 工具链三层设计（06 ↔ 60 ↔ 80）
...
| **80-clang-format-enforcement.md** | 工具配置层 | `.clang-format` 完整 YAML + `make format-check` 门禁 | 定义"clang-format 检查什么" |

> **交叉引用约定**：06 Layer 3（预提交验证）与 Layer 4（CI 门禁）引用 60 的 checkpatch 规则与 80 的 clang-format 配置；60 与 80 是两个独立的工具规则集，分别覆盖语义检查（checkpatch）与格式检查（clang-format）。
...
- **Agent 接口命名**：`agentrt_*` 前缀保留给 agentrt 同源 API，`airymaxos_*` 前缀用于 agentrt-linux 专属
...
- **维护者**: agentrt-linux 工程标准委员会（待成立，详见 07-maintainers-and-governance.md）
