...
agentrt-linux 是一个跨内核态与用户态、跨 8 个子仓、跨多语言栈（C/Rust/Python/TypeScript）的智能体操作系统发行版。当一个项目的代码量、贡献者数量、子系统边界同时增长时，"谁有权接受补丁""谁对某段代码的稳定性负责""补丁如何从作者流到主线"这三个问题若没有明确答案，项目会迅速退化为"无人负责的代码海洋"。
...
这套机制的关键数据：Linus 直接处理的补丁长期维持在约 1.3% 的比例——这不是因为 Linus 偷懒，而是因为信任链已经把 98.7% 的补丁在到达他之前过滤、审查、稳定过一遍。agentrt-linux 直接继承这套经过验证的治理模型。

### 1.3 agentrt-linux 维护者制度的三大支柱

agentrt-linux 维护者制度建立在三个不可拆分的支柱之上：
...
### 1.4 IRON-9 v2 同源且部分代码共享：与 agentrt 治理的关系
...
### 2.1 字段定义（Linux 内核 14 字段，OLK-6.6 源码行号）

agentrt-linux 沿用 Linux 6.6 内核基线 `MAINTAINERS` 文件的 14 字段（OLK-6.6 源码 `MAINTAINERS:7-60`），并按现代 SCM（git + GitHub）场景做适配。字段推荐顺序（OLK-6.6 `MAINTAINERS:4`）：`M: / R: / L: / S: / W: / Q: / B: / C: / P: / T: / F: / X: / N: / K:`。

| 字段 | 全称 | OLK-6.6 行号 | 含义 | agentrt-linux 适配 |
|------|------|------------|------|----------------|
| M: | Mail | `MAINTAINERS:7` | 主维护者邮箱 `FullName <address@domain>` | M: GitHub handle + email |
| R: | Reviewer | `MAINTAINERS:8` | 指定审查者（应被 CC） | R: Reviewers GitHub handle |
| L: | List | `MAINTAINERS:10` | 相关邮件列表 | L: GitHub Discussions 板块 |
| S: | Status | `MAINTAINERS:11-20` | 维护状态：Supported/Maintained/Odd Fixes/Orphan/Obsolete | S: 同 Linux + 新增 Experimental/Pilot/Stable/Deprecated |
| W: | Web | `MAINTAINERS:21` | 状态/信息网页 | W: 文档站点 URL |
| Q: | Patchwork | `MAINTAINERS:22` | Patchwork 补丁追踪网址 | Q: GitHub Projects URL |
| B: | Bug | `MAINTAINERS:23-24` | 缺陷追踪 URI | B: GitHub Issues URL |
| C: | Chat | `MAINTAINERS:25-26` | 聊天协议/服务器/频道 | C: Slack/Discord channel |
| P: | Profile | `MAINTAINERS:27-30` | 子系统提交者档案文档 URI | P: 同 Linux（子系统提交指南） |
| T: | Tree | `MAINTAINERS:31-32` | SCM 树类型与位置（git/hg/quilt/stgit/topgit） | T: GitHub repo URL + 分支 |
| F: | Files | `MAINTAINERS:33-38` | 文件/目录通配符（尾斜杠含子目录） | F: 同 Linux |
| X: | Excluded | `MAINTAINERS:39-44` | 排除的文件/目录（优先于 F:） | X: 同 Linux |
| N: | Regex | `MAINTAINERS:45-53` | 文件/目录正则模式（触发 git log 查询） | N: 同 Linux |
| K: | Keyword | `MAINTAINERS:54-60` | 内容正则（perl 扩展），匹配补丁/文件内容 | K: 同 Linux |

字段匹配优先级遵循 Linux 规则（OLK-6.6 `MAINTAINERS:39-44`）：`X:` 排除先于 `F:` 匹配；`N:` 正则匹配会触发 git log 历史回溯以通知历史签名者；`K:` 内容正则匹配补丁或文件内容。`F:` 模式尾随斜杠包含子目录，`F: drivers/net/*` 不含子目录下文件（OLK-6.6 `MAINTAINERS:33-38` 示例：3com vortex 子系统 `MAINTAINERS:71-75`、cfg80211 多字段示例 `MAINTAINERS:112-128`）。

### 2.2 agentrt-linux 新增字段

在 14 字段之外，agentrt-linux 新增 3 个字段以承载同源治理与代码所有权信息：

| 字段 | 含义 | 示例 |
|------|------|------|
| **O:** | Owners file（与 CODEOWNERS 格式兼容，供 GitHub CODEOWNERS bot 消费） | O: @airymaxos/kernel-team |
| **H:** | 同源 agentrt 模块（标注 IRON-9 v2 同源且部分代码共享的对应关系） | H: atoms/corekern |
| **V:** | 版本兼容性声明（标注该子系统对 agentrt-linux 主版本/内核基线的要求） | V: agentrt-linux 1.x / Linux 6.6 |

`O:` 字段是 `M:`/`R:` 的 CODEOWNERS 投影——`M:`/`R:` 给人看，`O:` 给机器看。两者必须保持一致，CI 会校验。`H:` 字段是 IRON-9 v2 同源且部分代码共享原则的落地：每个 agentrt-linux 子系统须声明其 agentrt 同源模块，以便在两端的语义漂移检测中自动对齐。

### 2.3 Status 状态定义

agentrt-linux 在 Linux 原有 5 种状态基础上，新增 4 种面向发行版生命周期的状态：
...
| **Experimental** | 实验性，API/ABI 可能剧烈变动 | agentrt-linux 0.1.x 阶段全部子仓 |
...
### 2.4 MAINTAINERS 文件分布与模板

#### 2.4.1 两级 MAINTAINERS 模型

agentrt-linux 采用 8 子仓独立 git 仓库模型（参见第 4 章），故 MAINTAINERS 文件分两级：
...
以下是伞仓（`agentrt-linux/`）MAINTAINERS 文件的简要模板，列出 8 个子仓及其仓库地址（详细模板见各子仓的 MAINTAINERS 文件）：

```
List of maintainers
===================

agentrt-linux (AirymaxOS) umbrella maintainers. Field format
...
Documentation/process/maintainers.rst for field semantics.
	...
	H: *Homologous* agentrt module (IRON-9 v2)
	V: *Version* baseline declaration

Preferred order: M: R: L: S: W: B: T: F: H: V:

----------------------------------------------------------------------

AIRYMAXOS-KERNEL SUBSYSTEM (L1.x / Linux 6.6 baseline)
...
AIRYMAXOS-SERVICES SUBSYSTEM
M:	SPHARX Engineering <eng@airymaxos.com>
R:	Services Lead <services-lead@airymaxos.com>
L:	airymax-dev@airymaxos.com
S:	Supported
...
AIRYMAXOS-SECURITY SUBSYSTEM (Cupolas LSM)
M:	SPHARX Engineering <eng@airymaxos.com>
R:	Security Lead <security-lead@airymaxos.com>
...
AIRYMAXOS-MEMORY SUBSYSTEM (MemoryRovol)
M:	SPHARX Engineering <eng@airymaxos.com>
...
AIRYMAXOS-COGNITION SUBSYSTEM (CoreLoopThree)
M:	SPHARX Engineering <eng@airymaxos.com>
R:	Cognition Lead <cognition-lead@airymaxos.com>
...
AIRYMAXOS-CLOUDNATIVE SUBSYSTEM
M:	SPHARX Engineering <eng@airymaxos.com>
R:	Cloud Native Lead <cloudnative-lead@airymaxos.com>
L:	airymax-cloudnative@airymaxos.com
S:	Maintained
...
AIRYMAXOS-SYSTEM SUBSYSTEM (commons)
M:	SPHARX Engineering <eng@airymaxos.com>
R:	System Lead <system-lead@airymaxos.com>
L:	airymax-system@airymaxos.com
S:	Supported
...
AIRYMAXOS-TESTS-LINUX SUBSYSTEM
M:	SPHARX Engineering <eng@airymaxos.com>
R:	Tests Lead <tests-lead@airymaxos.com>
L:	airymax-tests@airymaxos.com
S:	Maintained
...
[SC] 层（6 个头文件，物理宿主在 `kernel/include/airymax/`）的变更必须经过**双向 CI 校验 + 总维护者审批**：

```
开发者提交 [SC] 变更 MR
   ↓
agentrt-linux CI（本仓 CI）
   ↓ pass
...
#### 2.4.4 CODEOWNERS 自动同步机制

GitHub 与 GitLab 使用 `.github/CODEOWNERS`（或 `CODEOWNERS`）文件实现自动审查者指派。agentrt-linux 将 MAINTAINERS 的 `F:` + `M:` 映射为 CODEOWNERS，避免人工维护两份归属信息。

`O:` 字段（§2.2）是 `M:`/`R:` 的 CODEOWNERS 投影——`M:`/`R:` 给人看，`O:` 给机器看。两者必须保持一致，CI 会校验。agentrt-linux 提供 `make codeowners-sync` 目标自动同步：
...
#### 2.4.5 get_maintainer.pl 集成

OLK-6.6 提供 `scripts/get_maintainer.pl`（基于 MAINTAINERS），根据补丁修改的文件查询应 CC 的维护者邮箱列表。agentrt-linux 直接复用此脚本：
...
2. **链可任意长但实务不超过 2-3 级**：理论上信任链可无限延伸，但超过 3 级会显著增加补丁延迟与合并冲突风险。agentrt-linux 实务上限为 3 级（贡献者 → 子系统维护者 → 总维护者）。
3. **总维护者信任子系统维护者不送坏补丁上来**：这是信任链的根基。若子系统维护者反复送上未充分审查的补丁，总维护者会逐步降低对该子系统的信任权重，乃至收回合并授权。
4. **直接处理比例可作为信任链健康度指标**：参考 Linus 约 1.3% 直接处理率，agentrt-linux 总维护者的直接处理比例应维持在 5% 以下（项目规模较小时上限可放宽）。若该比例持续升高，意味着信任链在某一层断裂——下层维护者要么缺位、要么不作为。
...
agentrt-linux 由 8 个子仓构成，每个子仓对应一个 agentrt 同源模块域。0.1.1 版本下维护者席位待定（TBD），但归属关系与同源映射须先行固定：
...
| airymaxos-cloudnative | 极境云原生 | TBD | TBD | Experimental | gateway + sdk |
| airymaxos-system | 极境系统 | TBD | TBD | Experimental | commons |
| airymaxos-tests-linux | 极境测试 | TBD | TBD | Experimental | 全模块测试 |
...
agentrt-linux 沿用 Linux 内核的 Developer's Certificate of Origin 1.1。每一份贡献在提交时，作者须能自我认证以下声明：
...
agentrt-linux 使用 DCO bot 在 PR 层自动验证 SoB 链完整性：缺少 SoB 的 PR 无法合并。

### 5.3 审查标签体系

agentrt-linux 保留 Linux 内核的全部审查标签，语义不变：
...
`Reviewed-by:` 标签不是礼貌性点赞，而是审查者签署的一份正式声明。agentrt-linux 沿用 Linux 内核的 Reviewer's Statement of Oversight 语义：
...
### 6.4 agentrt-linux 等价物

agentrt-linux 的 `Reviewed-by:` 标签语义与 Linux 完全一致。在 PR 场景下：
...
agentrt-linux 采纳 Linux Foundation TAB 提出的贡献者成熟度模型，作为组织层面人才梯队建设的标准。该模型定义"组织允许/期望/鼓励工程师投入多少上游工作"的六级阶梯：

| Level | 标准 | agentrt-linux 期望 |
|-------|------|----------------|
| 0 | 工程师不允许贡献内核 | 不适用（agentrt-linux 拒绝与此类组织合作） |
| 1 | 允许贡献（业余或工作时间内） | 起步阶段——新加入的贡献者 |
| 2 | 期望贡献作为工作职责；支持参加 Linux 会议；贡献计入晋升评审 | **目标 Level**——所有 agentrt-linux 工程师底线 |
...
- 团队/组织的上游贡献数量
- 上游贡献者占内核开发者总数的比例
- 组织使用的 agentrt-linux 版本与上游基线的时间间隔（落后天数）
- 内核中的 out-of-tree commit 数量

### 7.2 agentrt-linux 适配

agentrt-linux 在 Linux 模型基础上增加两条适配：
...
agentrt-linux 总维护者承担 BDFL（Benevolent Dictator For Life）角色。该角色的管理哲学直接继承 Linux 内核 `management-style.rst`，并提炼为四条可操作原则。
...
| 编号 | 规则 | 分类 | 来源文档 |
|------|------|------|---------|
| OS-STD-047 | 非函数语句块开括号置于控制语句行末，闭括号置于下一行行首（K&R 风格） | 格式 | 02 代码格式（2026-07-09 新增，原 OS-KER-007 冲突消解） |
| OS-STD-048 | agentrt-linux 子目录禁止修改上游 `Makefile`（顶层），新增子目录通过 `TARGETS +=` 单行追加 | 构建/测试 | 80 测试（2026-07-09 新增，原 OS-KER-007 冲突消解） |
| OS-STD-049 | 禁止在 `Makefile.airymaxos` 中放置除版本四元组以外的任何变量；特性开关必须走 `Kconfig` | 构建 | 70 构建系统（2026-07-09 新增，原 OS-KER-007 冲突消解） |
| OS-STD-233 | 所有 PR 必须通过 GitHub Actions 全部检查才能合并；任一检查失败的 PR 禁止合并（即使审查者已 ACK） | 流程/CI | 05 开发流程（2026-07-09 新增注册，原"幽灵引用"消解——已在 05-development-process.md:525 本地定义但未注册 SSoT） |

> **plain OS-STD-NNN 分类标签体系**（2026-07-09 建立）：
>
> 经全量扫描，agentrt-linux 文档体系中实际使用 `OS-STD-NNN`（无子域）格式的规则达 130+ 条，远超审查报告 TS-P0-16 所述的"21 条"。为便于检索与未来全量注册，按规范内容归类为以下描述性标签：
>
> | 分类标签 | 涵盖范围 | 典型编号段 |
> |---------|---------|-----------|
...
| OS-KER-007 | 内核态禁止使用 float/double 类型（`-mno-80387`），改用 Q16.16 定点数 | 01 代码规范（2026-07-08 新增） |
| OS-KER-008 | bool 仅用于函数返回值与栈变量，禁止用于结构体成员（结构体成员用 `u8`/bitfield） | 01 代码规范（2026-07-08 新增） |
| OS-KER-053 | 共享数据必须用同步原语保护（spinlock/mutex/memory barrier/RCU），引用计数管理生命周期，两者不可互替 | 01 代码规范（2026-07-09 新增，原 OS-KER-007 冲突消解） |
| OS-KER-054 | ruleset 是策略载体，运行时不可变（Landlock 沙箱 K-4 可插拔策略） | 110 安全（2026-07-09 新增，原 OS-KER-007 冲突消解） |
| OS-KER-055 | Agent 跟踪模块必须使用 `trace_array_create()` 创建独立 instance，不得污染全局主缓冲 | 90 可观测性（2026-07-09 新增，原 OS-KER-007 冲突消解） |

> **编号冲突消解说明**（2026-07-08 SSoT 统一）：
...
> - 历史 `OS-KER-007` 曾在 7 份文档中指代 6 条不同规则（内核态禁 float / 共享数据同步原语 / K&R 开括号 / 子目录 Makefile / Makefile.airymaxos 版本四元组 / ruleset 策略载体 / trace_array 独立 instance），2026-07-09 消解：OS-KER-007 保留为"内核态禁 float"权威定义，其余 6 条分别拆分为 OS-KER-053 / OS-STD-047 / OS-STD-048 / OS-STD-049 / OS-KER-054 / OS-KER-055（见 10.2.5 与 10.3）
...
五维原则在治理层的映射具有正交性：S-3 关注"谁有权决策"（层级结构），A-3 关注"决策如何传达"（人际礼仪），A-4 关注"决策者如何培养"（成熟度梯队），E-6 关注"决策如何追溯"（DCO 链），IRON-9 关注"决策边界在哪"（同源独立）。五者互不重叠，共同构成 agentrt-linux 治理的完备规则集。
...
- `Linux 6.6 内核源码 MAINTAINERS`（734KB，维护者制度范本）
- `Linux 6.6 内核源码 Documentation/process/submitting-patches.rst`（DCO 章节）
- `Linux 6.6 内核源码 Documentation/process/management-style.rst`（管理哲学）
- `Linux 6.6 内核源码 Documentation/process/contribution-maturity-model.rst`（6 级成熟度模型）
- `Linux 6.6 内核源码 Documentation/process/6.Followthrough.rst`（审查礼仪）
...
> **文档结束** | agentrt-linux 工程标准第 7 篇 | 治理层规范 | 0.1.1 版本 P0 优先完成
