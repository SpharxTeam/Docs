...
**最新**: 2026-06-09
**状态**: 维护中
**路径**: OpenAirymax/docs/AirymaxOS/50-engineering-standards/10-coding-style/security_design_standard.md
---
...
> **跨平台合规说明**：本节代码中 `secure_memory_pool_t` 使用 `airy_mtx_t` 而非 `pthread_mutex_t`，遵循 [C编码规范 CROSS-01](./C_coding_style_standard.md) 的跨平台规则。直接使用 POSIX 线程 API（如 `pthread_mutex_t`）违反 CROSS-01，必须使用 `platform.h` 提供的 `airy_mtx_lock()`/`airy_mtx_unlock()` 抽象。
...
3. **Airymax 统一术语表**: [10-terminology.md](../10-terminology.md)
...
| **标准术语** | 8 个架构组件标准名称 | [10-terminology.md](../../TERMINOLOGY.md) |
