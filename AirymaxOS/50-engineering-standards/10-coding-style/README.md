...
> **说明**：用户态组件（Python、TypeScript、Rust 用户态）的编码规范由 agentrt 同源规范体系定义，遵循 IRON-9 v2 [SS] 语义同源层原则。本规范体系仅覆盖内核态 C 和内核模块 Rust——这是 agentrt-linux（AirymaxOS）作为 OS 发行版的专属责任域。
...
| 检查项 | [SC] | [SS] | [IND] |
|--------|------|------|-------|
| 前缀使用 | `AIRY_` 宏/常量 | `airy_` 函数 | `airymaxos_` 函数 |
