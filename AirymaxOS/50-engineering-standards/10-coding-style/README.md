...
|---------------|------|-------------|------|
| **[SC] 共享契约层** | 代码完全共享 | 共享 `include/airymax/` 6 个头文件（`bpf_struct_ops.h`/`memory_types.h`/`security_types.h`/`cognition_types.h`/`sched.h`/`ipc.h`）中的类型定义、常量、宏 | `AIRY_IPC_MAGIC`、`bpf_struct_ops.h` |
