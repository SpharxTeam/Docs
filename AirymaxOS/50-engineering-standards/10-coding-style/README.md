...
- **协同性**：agentrt-linux（AirymaxOS）内核与 agentrt 用户态运行时共享同源语义（IRON-9 v2），编码规范的一致性确保两端代码的无适配层互操作。
...
> **说明**：用户态组件（Python、TypeScript、Rust 用户态）的编码规范由 agentrt 同源规范体系定义，遵循 IRON-9 v2 [SS] 语义同源层原则。内核态 C 与内核模块 Rust 是 agentrt-linux（AirymaxOS）作为 OS 发行版的专属责任域；Go / Python / JavaScript / Java 编码规范为 Airymax 全栈体系的一部分，已合并入本子目录的 5 份合并文档。
...
agentrt-linux（AirymaxOS）编码规范与 agentrt 编码规范遵循 **IRON-9 v2 同源且部分代码共享** 原则：

| IRON-9 v2 层级 | 定义 | 编码规范关系 | 示例 |
|---------------|------|-------------|------|
| **[SC] 共享契约层** | 代码完全共享 | 共享 `include/airymax/` 6 个头文件（`syscalls.h`/`memory_types.h`/`security_types.h`/`cognition_types.h`/`sched.h`/`ipc.h`）中的类型定义、常量、宏 | `AIRY_IPC_MAGIC`、`syscalls.h` |
...
| 头文件 | 内容 | 共享方式 | 变更流程 |
|--------|------|----------|----------|
| `syscalls.h` | 12 核心 syscall 编号 + 12 预留槽位| 完全共享，双向同步 | 任一端变更需同步另一端 |
...
| `airy_task_submit()` | kthread + sched_ext | pthread + 用户态调度器 | 语义等价，调度模型同源 |
...
- [合规检查清单](../../50-engineering-standards/05-development-process.md) Part III：IRON-9 v2 合规检查清单
