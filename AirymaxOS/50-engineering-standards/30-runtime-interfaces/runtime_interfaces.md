...
| 错误码 | 值 | 含义 | 适用场景 |
|--------|-----|------|----------|
| `AIRY_EINVAL` | -1 | 参数无效 | 参数为 NULL、值超出范围、类型不匹配 |
| `AIRY_EPERM` | -4 | 权限不足 | 缺少必要的能力令牌 |
| `AIRY_ENOMEM` | -2 | 内存不足 | 内核或用户态内存不足 |
| `AIRY_ETIMEOUT` | -11 | 操作超时 | 阻塞操作超时 |
| `AIRY_EBUSY` | -9 | 资源忙 | 资源已被占用，无法立即获取 |
| `AIRY_EDEADLK` | -21 | 检测到死锁 | 互斥锁获取可能导致死锁 |
| `AIRY_EINTR` | -22 | 被信号中断 | 阻塞操作被信号中断 |
| `AIRY_ENOTSUP` | -10 | 不支持的操作 | 当前内核版本不支持该操作 |
| `AIRY_EOVERFLOW` | -14 | 数值溢出 | 算术运算溢出 |
| `AIRY_EFAULT` | -13 | 地址错误 | 用户空间指针无效 |
...
| 类别 | 范围 | 描述 | 示例 |
|------|------|------|------|
| 通用基础错误 | -1 至 -99 | 跨子系统的通用基础错误 | `AIRY_EINVAL`(-1), `AIRY_EPERM`(-4), `AIRY_ENOMEM`(-2), `AIRY_ETIMEOUT`(-11), `AIRY_EBUSY`(-9), `AIRY_EDEADLK`(-21), `AIRY_EINTR`(-22), `AIRY_ENOTSUP`(-10), `AIRY_EOVERFLOW`(-14), `AIRY_EFAULT`(-13) |
...
#define AIRY_EDEADLK           (-21)   /* 检测到死锁 */
#define AIRY_EINTR             (-22)   /* 被信号中断 */
#define AIRY_ENOTSUP           (-10)   /* 不支持的操作 */
#define AIRY_EOVERFLOW         (-14)   /* 数值溢出 */
#define AIRY_EFAULT            (-13)   /* 地址错误 */
