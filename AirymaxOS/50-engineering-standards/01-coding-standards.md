...
| OS-STD-005      | OS-STD-CODE-005   | `airy_*` 与 `airymaxos_*` 前缀隔离 | §1.5 | 3 段 → 4 段                            |
...
| OS-STD-009      | OS-STD-CODE-009   | EXPORT\_SYMBOL 紧跟右大括号         | §2.4 | 3 段 → 4 段                            |
...
| OS-STD-025      | OS-STD-CODE-025   | IS\_ENABLED 转 C 布尔            | §8.2 | 3 段 → 4 段                            |
...
- **跨文档对齐**：迁移后本卷与 60-checkpatch-rule-map.md（已使用 OS-STD-CODE-NNN）、10-coding-style/C\_coding\_style\_standard.md（已引用 OS-STD-CODE-010/012/014/015）完全对齐。
...
> **OS-STD-005**： `airy_*` 前缀保留给 agentrt 同源 API；`airymaxos_*` 前缀用于 agentrt-linux 内核 / 发行版专属 API。两者共享 Airymax 同源语义（MicroCoreRT / AgentsIPC / Cupolas / MemoryRovol / CoreLoopThree），但代码归属与 ABI 边界不同，前缀隔离确保无适配层互操作时不冲突。
...
| OS-STD-025      | 标准  | IS\_ENABLED 转 C 布尔                      |
