...
| **A-4 完美主义** | 配置即代码——格式规则必须可被工具 100% 强制 | OS-STD-201、OS-STD-FMT-029 |
...
| 原则 | 落地点 | 对应规则 |
|------|--------|----------|
| **A-1 极简主义** | 反过度抽象；反 inline 滥用；抽象只在必要时使用 | OS-KER-118、OS-KER-133、OS-KER-138 |
| **A-2 细节关注** | 分级标签按分配逆序释放；`sizeof(*p)` 跟随类型 | OS-KER-136、OS-KER-140 |
| **A-4 完美主义** | 零警告编译；多配置构建（allnoconfig + allmodconfig） | OS-KER-145、OS-STD-218 |
| **E-3 资源确定性** | 引用计数强制；锁不替代引用计数 | OS-KER-127、OS-KER-128 |
| **E-1 安全内生** | Rust 模块用于安全敏感路径；sparse 类型检查 | OS-KER-144、OS-KER-146 |
| **E-8 可测试性** | 错误路径必须测试；fault injection 强制 | OS-KER-134、OS-KER-148 |
| **K-2 接口契约化** | 4 层接口稳定性分级；用户 ABI 永久稳定 | OS-KER-132、OS-STD-214 |
| **K-3 服务隔离** | 策略外移至用户态守护进程 | OS-KER-121 |
| **K-4 可插拔策略** | 策略机制分离；sched_ext BPF 范式 | OS-KER-120、OS-KER-122 |
| **C-1 双系统协同** | 热路径 C / 慢路径 Rust / Agent Python+TS 分层 | OS-STD-213、OS-KER-143 |
