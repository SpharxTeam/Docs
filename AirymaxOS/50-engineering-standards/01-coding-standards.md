...
> **OS-STD-CODE-002**： 局部变量短小切题，循环计数器用 `i`，临时值用 `tmp`，缓冲区用 `buf`。需要长名才能区分说明函数职责过重，应拆分。
...
> **OS-STD-CODE-003**： 常量宏全大写；多个相关常量优先用 `enum` 而非散落 `#define`。
...
> **OS-STD-CODE-004**： 动作式 / 命令式函数返回错误码（0 = 成功，-Exxx = 失败）；谓词式函数返回 `bool`。混合两种约定是难调 Bug 的温床。
...
> **OS-STD-CODE-005**： `airy_*` 前缀保留给 agentrt 同源 API；`airymaxos_*` 前缀用于 agentrt-linux 内核 / 发行版专属 API。两者共享 Airymax 同源语义（MicroCoreRT / AgentsIPC / Cupolas / MemoryRovol / CoreLoopThree），但代码归属与 ABI 边界不同，前缀隔离确保无适配层互操作时不冲突。
...
| OS-STD-CODE-002 | 标准  | 局部变量短小精悍                                |
| OS-BAN-001      | 禁止  | 敏感术语 master/slave、blacklist/whitelist   |
| OS-STD-CODE-003 | 标准  | 宏常量全大写，相关常量优先 enum                      |
| OS-STD-CODE-004 | 标准  | 动作式返回错误码，谓词式返回 bool                     |
| OS-STD-CODE-005 | 标准  | `airy_*` 与 `airymaxos_*` 前缀隔离           |
...
| OS-KER-002      | 内核  | 内部 u8/u32，UAPI \_\_u32                  |
| OS-KER-008      | 内核  | bool 仅返回值与栈变量                           |
| OS-STD-CODE-021 | 标准  | 多布尔值合并 bitfield / flags                 |
| OS-KER-003      | 内核  | goto 集中出口模式                             |
| OS-KER-004      | 内核  | 分级标签按分配逆序释放                             |
| OS-BAN-005      | 禁止  | BUG()/BUG\_ON()，改 WARN\_ON\_ONCE        |
| OS-KER-005      | 内核  | sizeof(\*p)/kmalloc\_array/struct\_size |
