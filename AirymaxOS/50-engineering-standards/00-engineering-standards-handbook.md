...
| —  | [120-cross-project-code-sharing.md](./120-cross-project-code-sharing.md) | 跨项目代码共享（IRON-9 v2 [SC] 共享契约层） |
...
| **OS-IRON-008** | 共享契约层 6 核心头文件 + 2 补充共享头文件（bpf_struct_ops.h + error.h）双向 CI 验证——[SC] 头文件变更必须通过 agentrt + AirymaxOS CI 同时通过 | E-6 / E-8 | 120 跨项目代码共享 |
...
| **OS-IRON-014** | [SC] 共享契约层 6 核心头文件 + 2 补充共享头文件（bpf_struct_ops.h + error.h）单一数据源（禁止物理副本）——8 个头文件物理宿主在 kernel/include/airymax/，其他子仓通过 -I 引用 | E-7 | 120 跨项目代码共享 |
