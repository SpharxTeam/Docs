...
> **文档定位**： 交付成果 #3——工程标准规范手册。本文档是 agentrt-linux（AirymaxOS，极境智能体操作系统）工程标准的执行依据，汇总 7 主题文档的核心规范条目、SSoT 规则编号注册表、IRON-9 v2 三层共享模型、合规性禁词清单与乔布斯美学审查标准，作为开发的强制执行依据。\
> **版本**： 0.1.1\
> **最后更新**： 2026-07-08\
...
> **SSoT 声明**：OS-IRON 铁律编号的唯一权威来源（SSoT）为工程规范委员会规则编号注册表（14 条铁律）。本表为规则编号注册表的公开镜像，必须与之一一对应。原表中不属于 IRON 层级的规则已重分类：①"7 层自动化验证强制"→ OS-ACC-002（验收标准）；②"信任链分层不可越级提交"→ OS-STD-GOV-008；③"Reviewed-by 不可替代 Signed-off-by"→ 由 IRON-005（审查优先）+ IRON-007（DCO）共同涵盖；④"组织 6 级成熟度 Level 2"→ OS-STD-GOV-009。
...
> **SSoT 声明**：本章是工程规范委员会规则编号注册表的**公开镜像**。规则编号注册表定义了 6 类规则编号的权威定义：14 条 OS-IRON、7 条 OS-BAN（001-011，含间隔）、19 条 OS-STD（001-052，含间隔）、1 条 OS-ACC、3 条 OS-KER（001-003）、7 条 OS-FMT（001-007）。本章各表须与规则编号注册表一一对应；本章与规则编号注册表的任何冲突，以规则编号注册表为准。
>
> **已知差异**：公开文档历史演进中，OS-KER/OS-BAN/OS-ACC 三个类别出现了与规则编号注册表不同的编号定义与扩展（详见各表"SSoT 对齐"注释）。这是因公开子文档（01-07）各自独立编号累积所致，非设计意图。完整重编号消解计划见 [07-maintainers-and-governance.md §10.2.5](./07-maintainers-and-governance.md)。通过下方 §3.8 SSoT 交叉引用映射表建立"每个技术点→单一 SSoT 编号"的权威对应关系。
...
| OS-STD-GOV-008 | 信任链分层不可越级提交（重分类：原 OS-IRON-006 属治理标准而非工程铁律） |
| OS-STD-GOV-009 | 组织 6 级成熟度 Level 2 为底线目标（重分类：原 OS-IRON-010 属治理标准而非工程铁律） |

### 3.3 OS-KER 内核工程规则汇总（8 条）

> **SSoT 对齐**：工程规范委员会规则编号注册表定义 OS-KER 仅 3 条（001-003）：OS-KER-001=goto 集中出口模式、OS-KER-002=禁止 #if 桩函数模式、OS-KER-003=条件编译必须用 IS_ENABLED()。本表 8 条（001-008）是公开子文档历史演进的扩展，与 SSoT 编号定义不同。每条规则的 SSoT 权威编号见 §3.8 交叉引用映射表。

| 编号 | 规则 | Linux 6.6 内核基线 验证 |
|------|------|-------------|
...
> **SSoT 对齐**：工程规范委员会规则编号注册表定义 OS-BAN 共 7 条（001-005, 010-011）：001=敏感术语、002=禁止 BUG()、003=sizeof(*p)、004=禁止 strcpy、005=禁用 %p、010=开源文档禁词、011=禁词违规处罚。本表 6 条（001-006）是公开子文档的独立编号，与 SSoT 编号定义不同。例如本表 OS-BAN-001（匈牙利命名）在 SSoT 中无对应编号；本表 OS-BAN-002（敏感术语）对应 SSoT OS-BAN-001。每条规则的 SSoT 权威编号见 §3.8 交叉引用映射表。
...
> **SSoT 对齐**：本表与工程规范委员会规则编号注册表 OS-FMT（001-007）完全一致，无差异。此前公开文档将格式规则分散折叠至 OS-STD-FMT（仅 2 条）与 OS-KER-005，导致同一技术点（如"Tab 8 字符宽"）出现 3 个编号。OS-FMT 独立为完整 7 条表，作为格式规则的权威编号源；OS-STD-FMT-001/002 保留为历史子域编号，映射见 §3.8。

| 编号 | 规则 | SSoT 章节 |
|------|------|----------|
| **OS-FMT-001** | Tab 缩进，8 字符宽 | 规则编号注册表 §3.1 |
| **OS-FMT-002** | 80 列行宽 | 规则编号注册表 §3.2 |
| **OS-FMT-003** | K&R 大括号风格（开括号置于控制语句行末） | 规则编号注册表 §3.3 |
| **OS-FMT-004** | 即使单条语句也使用大括号 | 规则编号注册表 §3.3 |
| **OS-FMT-005** | 空格规则（关键字后空格、指针 `*` 贴名字） | 规则编号注册表 §3.4 |
| **OS-FMT-006** | 行尾禁止空白 | 规则编号注册表 §3.5 |
| **OS-FMT-007** | clang-format 强制（`.clang-format` 配置文件） | 规则编号注册表 §3.6 |

### 3.6 OS-ABI 接口稳定性规则（1 条）

> **SSoT 对齐**：工程规范委员会规则编号注册表未设独立的 OS-ABI 类别——4 层接口稳定性分级在 SSoT 中由 OS-IRON-001（L1 永久）+ OS-IRON-002（L3 可变）+ OS-STD-004（返回值约定）共同承载。本表保留 OS-ABI-001 作为公开文档的 4 层分级汇总视图。
...
> **SSoT 对齐**：工程规范委员会规则编号注册表定义 OS-ACC 仅 1 条：OS-ACC-001=覆盖率 ≥ 80%，关键路径 ≥ 90%。本表 5 条（001-005）是公开文档的扩展，其中 OS-ACC-002~005 分别映射至 OS-IRON-004（中点可编译）、OS-IRON-006（regression 零容忍）、OS-IRON-007（DCO 验证）、OS-STD-GOV-003（状态字段）。完整映射见 §3.8。
...
> **目的**：依据"每个技术点只能有一个规范标准"原则，本表为公开文档中所有与规则编号注册表编号不一致的规则建立"技术点 → SSoT 权威编号"的单一对应关系。重编号后本表将退役。

#### 3.8.1 OS-KER 技术点映射

| 技术点 | 公开文档当前编号 | SSoT 权威编号 | 处置 |
|--------|-----------------|-------------|------|
| goto 集中出口模式 | OS-STD-CODE-002 | OS-KER-001 | 统一为 OS-KER-001 |
| 禁止 #if 桩函数模式 | （由 IRON-1 涵盖） | OS-KER-002 | 补标 OS-KER-002 |
| 条件编译用 IS_ENABLED() | （散见于 02-code-format） | OS-KER-003 | 补标 OS-KER-003 |
| Linux 6.6 内核基线 | OS-KER-001（公开） | OS-IRON-010 | 改为 IRON-010 |
| u8/u16/u32/u64 类型 | OS-KER-002（公开） | OS-STD-010 | 改为 OS-STD-010 |
| typedef 限制 | OS-KER-003（公开） | OS-STD-010（扩展） | 并入 OS-STD-010 |
| 禁止 BUG()/BUG_ON() | OS-KER-004（公开） | OS-BAN-002 | 改为 OS-BAN-002 |
| Tab 8 字符宽 | OS-KER-005 / OS-STD-FMT-001 | OS-FMT-001 | 统一为 OS-FMT-001 |
| 内核内部 API 不稳定 | OS-KER-006（公开） | OS-IRON-002 | 改为 IRON-002 |
| 禁止 float/double | OS-KER-007（公开） | OS-STD-010（扩展） | 并入 OS-STD-010 |
| bool 仅用于返回值/栈变量 | OS-KER-008（公开） | OS-STD-011 | 改为 OS-STD-011 |

#### 3.8.2 OS-BAN 技术点映射

| 技术点 | 公开文档当前编号 | SSoT 权威编号 | 处置 |
|--------|-----------------|-------------|------|
| 敏感术语禁用 | OS-BAN-002（公开） | OS-BAN-001 | 统一为 OS-BAN-001 |
| 禁止 BUG() | OS-KER-004（公开） | OS-BAN-002 | 统一为 OS-BAN-002 |
| sizeof(*p) | （散见于 01-coding） | OS-BAN-003 | 补标 OS-BAN-003 |
| 禁止 strcpy/strncpy | OS-STD-CODE-003 | OS-BAN-004 | 统一为 OS-BAN-004 |
| 禁用 %p 默认输出 | OS-BAN-003（公开） | OS-BAN-005 | 统一为 OS-BAN-005 |
| 开源文档禁词 | （散见于 11-compliance） | OS-BAN-010 | 补标 OS-BAN-010 |
| 禁词违规处罚 | （散见于 11-compliance） | OS-BAN-011 | 补标 OS-BAN-011 |
| 禁止匈牙利命名法 | OS-BAN-001（公开） | （SSoT 无独立编号） | 新增 OS-STD-005 扩展或 OS-BAN-006 |
| 禁止越级提交 | OS-BAN-004（公开） | OS-STD-GOV-008 | 改为 OS-STD-GOV-008 |
| 禁止伪造 SoB | OS-BAN-005（公开） | OS-IRON-007（涵盖） | 并入 IRON-007 |
| 禁止打压审查 | OS-BAN-006（公开） | OS-STD-GOV（新增） | 新增 OS-STD-GOV-010 |

#### 3.8.3 OS-ACC 技术点映射

| 技术点 | 公开文档当前编号 | SSoT 权威编号 | 处置 |
|--------|-----------------|-------------|------|
| 覆盖率 ≥ 80% | （散见于 06-toolchain） | OS-ACC-001 | 补标 OS-ACC-001 |
| 7 层自动化验证 | OS-ACC-002（公开） | OS-STD-030 + 06 工具链 | 改为 OS-STD-030 |
| 中点可编译 | OS-ACC-003（公开） | OS-IRON-004 | 改为 IRON-004 |
| regression 72h 响应 | OS-ACC-004（公开） | OS-IRON-006 | 改为 IRON-006 |
| 状态字段一致 | OS-ACC-005（公开） | OS-STD-GOV-003 | 改为 OS-STD-GOV-003 |
...
> 基础 4 个头文件先确立；`sched.h` 和 `ipc.h` 于后续调度子系统与 IPC 子系统修订时新增，现共 6 个 [SC] 头文件。

| 头文件 | 共享内容 | IRON-9 v2 层次 |
|--------|---------|---------------|
| `include/airymax/syscalls.h` | Syscall 编号体系（12 核心 + 12 预留 = 24 槽位） | [SC] |
...
| `sched_ext（误标为 Linux 6.15+）` | `sched_ext（Linux 6.6 内核基线 内置）` | Linux 6.6 内核基线 已内置 SCHED_EXT=7；2.x.x 基线为 Linux 7.1（ADR-013） |
...
> **注**：10-terminology.md 已将 MicroCoreRT 标准名称修正为"微核心运行时"。
...
所有技术方案修正均对照 `/home/spharx/SpharxWorks/01Reference/kernel-Linux 6.6 内核基线/` 源码验证：

| 验证项 | 源码路径 | 验证内容 | 验证结果 |
|--------|---------|---------|---------|
| SCHED_EXT=7 | `include/uapi/linux/sched.h:121` | `#define SCHED_EXT 7` | Linux 6.6 内核基线 已内置 |
...
| 5 | IRON-9 v2 三层模型落地 | 验证通过 | 6 个 [SC] 共享头文件已定义 |
