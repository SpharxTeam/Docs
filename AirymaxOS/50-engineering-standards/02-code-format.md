...
| 当前编号（本卷正文） | 目标编号（OS-FMT） | 规则内容 | 所在章节 | 迁移说明 |
|---------------------|-------------------|----------|---------|---------|
| OS-KER-005（Tab 8 字符） | OS-FMT-001 | Tab 缩进，8 字符宽 | §1.1 | 消除双义：OS-KER-005 在 §1.1 表 Tab、在 §2.2 表字符串禁止断行 |
| OS-KER-004 | OS-FMT-002 | C 代码首选 80 列 | §2.1 | — |
...
| OS-KER-016 | OS-FMT-011 | fallthrough 必须显式标注 | §5.2 | — |
| OS-KER-017 | OS-FMT-012 | 函数之间用 1 个空行分隔 | §6.1 | — |
| OS-KER-018 | OS-FMT-013 | 禁止函数内多余空行 | §6.2 | — |
| OS-KER-019 | OS-FMT-004（部分） | 单行多语句禁令 | §7.1 | 与 §3.4 单语句大括号规则协同 |
| OS-KER-020 | OS-FMT-014 | 禁止用逗号逃避大括号 | §7.2 | — |
| OS-KER-021 | OS-FMT-014（扩展） | 禁止单行多赋值 | §7.3 | 与 OS-KER-020 合并为 OS-FMT-014 |
| OS-KER-003 | OS-FMT-007 | 行尾禁止空白 | §1.6 | — |
| OS-KER-006 | OS-FMT-015 | 后继行应大幅短于父行 | §2.4 | 新增 OS-FMT-015 |
| OS-STD-202 ~ OS-STD-211 | OS-FMT-020 ~ OS-FMT-029 | 多语言格式规则 + 配置即代码 | §1.2-§1.5、§8 | 多语言段保留 OS-STD-202~211 作为子域编号，不强制迁移 |
...
迁移后，`OS-KER` 前缀仅保留**内核态专属规则**（非格式规则），这些规则不在本卷定义，分布在 01/03/04 等卷：

| 保留编号 | 规则 | 所在卷 |
|---------|------|--------|
| OS-KER-001 | goto 集中出口 | 01-coding-standards / 10-coding-style/C* |
| OS-KER-002 | 禁止结构体 typedef | 01-coding-standards |
| OS-KER-007 | 内核态禁 float，强制 airy_q16_t | 10-coding-style/C_coding_style_standard_supplement |
| OS-KER-030 | IS_ENABLED 条件编译 | 03-code-style |
| OS-KER-031+ | bug 修复补丁规范等 | 04-engineering-philosophy |
...
### 1.1 C 内核代码：Tab，8 字符宽（OS-KER-005）

**OS-KER-005**：airymaxos-kernel 与 airymaxos-security 全部 C 代码使用 Tab 缩进，Tab 宽度 8 字符；禁止用空格缩进。理由有二：
...
```c
/* 正确：Tab 缩进，块边界清晰 */
int airymaxos_ipc_recv(struct airy_ipc_msg *msg)
{
...
```c
/* 正确：后继行与开括号对齐 */
int airymaxos_dispatcher_select(struct airy_task_desc *desc,
                                u32 capability_mask,
                                u64 deadline_ns)
{
	...
...
### 3.1 非函数语句块：开括号在行末（OS-STD-047）

**OS-STD-047**：所有非函数语句块（`if` / `switch` / `for` / `while` / `do`）开括号置于控制语句行末，闭括号置于下一行行首。
...
```c
int airymaxos_ipc_send(struct airy_ipc_msg *msg)
{
	return do_send(msg);
}
```
...
**OS-KER-021**：禁止一行内多个赋值；每个赋值独占一行。agentrt-linux 内核代码风格简单——避免技巧性表达。
...
**OS-STD-207**：airymaxos-kernel / airymaxos-security / airymaxos-services 仓库根目录必须维护 `.clang-format`（基于 Linux 6.6 内核基线 689 行版本 + ForEachMacros 列表 + agentrt-linux 专属宏扩展）。
...
| `IndentWidth` | `8` | OS-KER-005 | 缩进宽度 |
| `TabWidth` | `8` | OS-KER-005 | Tab 显示宽度 |
| `UseTab` | `Always` | OS-KER-005 | 强制 Tab |
| `ContinuationIndentWidth` | `8` | OS-KER-006 | 后继行缩进 |
| `BreakBeforeBraces` | `Custom` | OS-STD-047/OS-KER-008 | K&R 风格 |
| `BraceWrapping.AfterFunction` | `true` | OS-KER-008 | 函数开括号独占行 |
| `BraceWrapping.AfterControlStatement` | `false` | OS-STD-047 | 控制语句开括号行末 |
| `BraceWrapping.BeforeElse` | `false` | OS-STD-047 | else 紧贴闭括号 |
...
| 原则 | 落地点 | 对应规则 |
|------|--------|----------|
| **A-1 极简主义** | K&R 大括号最小化空行；连续空行最多 1 | OS-STD-047、OS-KER-017、OS-STD-206 |
...
| 1.0.1 | TBD | 与代码实现同步验证；ForEachMacros 扩展至包含 agentrt-linux 专属宏 |
