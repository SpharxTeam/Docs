...
# agentrt-liunx IPC 协议契约

> **文档定位**: agentrt-liunx（AirymaxOS）进程间通信协议的契约定义，涵盖 128B 消息头、payload 协议、io_uring 传输层、capability 传递
...
agentrt-liunx（AirymaxOS）IPC 协议与 agentrt AgentsIPC 同源，保留 128 字节定长消息头设计，并在底层升级为基于 io_uring 的零拷贝实现。协议设计遵循以下原则，与五维正交 24 原则对齐：
...
sequenceDiagram
    ...
    participant K as agentrt-liunx 内核 capability 模块
...
- 令牌传递必须在同一 agentrt-liunx 节点内（跨节点传递需通过安全网关）。
...
agentrt-liunx IPC 基于 Linux 6.6 内核基线的 io_uring 子系统，在标准 io_uring 之上注册以下 IPC 专用 OP（固定操作码扩展）：
...
agentrt-liunx IPC 与 agentrt AgentsIPC 通过 IRON-9 v2 三层契约共享：
...
agentrt 在 agentrt-liunx 上运行时，IPC 消息头布局完全兼容，无需协议转换层：

- agentrt 发出的 128B 消息头可直接被 agentrt-liunx io_uring 通道接收。
- agentrt-liunx 发出的消息头可被 agentrt 用户态消息队列接收。
...
- 每次 PR 运行 airymaxos-tests/benchmark/ipc-latency 与 ipc-throughput 微基准。
