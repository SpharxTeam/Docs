...
[SC] 层是 agentrt 与 agentrt-linux 代码字面一致的头文件库，位于 `include/airymax/` 目录。这 6 个头文件是 agentrt-linux OS 层契约的"共享基础"：
...
1. [SC] 层头文件由 agentrt 和 agentrt-linux 的维护者共同维护，变更需双方架构委员会批准。
...
| 同源兼容性测试 | agentrt ↔ agentrt-linux | `airymaxos-tests/homology/` | 每次发布 |
...
2. **[SS] 语义同源层协商**：语义冲突通过架构委员会协商，以 agentrt-linux OS 层实现可行性为底线。
...
**图 2**：agentrt-linux OS 层契约变更与版本管理流程。MAJOR 变更需架构委员会全票通过并记录 ADR，MINOR 变更由子仓维护者审批，PATCH 变更直接合并。
