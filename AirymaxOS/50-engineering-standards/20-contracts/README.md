...
# agentrt-liunx OS 层契约规范总览

> **文档定位**: agentrt-liunx（AirymaxOS）OS 层契约规范体系的入口与索引
> **版本**: 0.1.1（文档体系完成）/ 1.0.1（开发）
> **最后更新**: 2026-07-07
> **父文档**: [agentrt-liunx 设计文档](../../README.md)
...
agentrt-liunx（AirymaxOS）的 OS 层契约规范定义各组件间的交互协议，是 agentrt-liunx 内核态与用户态、进程与进程之间通信的"法律文书"。契约规范位于 agentrt-liunx 文档体系的 `50-engineering-standards/20-contracts/` 目录下，与 agentrt 用户态契约规范形成互补——前者定义 OS 层（内核级）契约，后者定义应用层（用户态运行时）契约。

### 1.2 与 agentrt 用户态契约的关系

agentrt-liunx 与 agentrt 遵循"同源且部分代码共享"原则（IRON-9 v2）。在契约层面，这一关系表现为三层结构：

| 契约层 | IRON-9 v2 分层 | 共享方式 | 涵盖内容 |
|--------|---------------|---------|---------|
| [SC] 共享契约层 | Shared Contract | 头文件完全共享，代码字面一致 | `bpf_struct_ops.h` / `memory_types.h` / `security_types.h` / `cognition_types.h` |
| [SS] 语义同源层 | Semantic Symbiosis | 语义一致，实现各自独立 | syscall API 语义 / IPC 128B 消息头 / 日志格式 |
| [IND] 完全独立层 | Independent | 完全独立，无共享约束 | 驱动接口 / 内核态具体实现 |

**关键区分**：

- agentrt 用户态契约：定义应用层组件间的交互协议（HTTP/WS/Stdio 网关、JSON-RPC 2.0、Agent 契约、Skill 契约），位于 `docs/AirymaxAgentRT/Capital_Specifications/agentrt_contract/`。
- agentrt-liunx OS 层契约：定义 OS 级组件间的交互协议（系统调用 API、IPC 协议、驱动接口、日志格式），位于本目录。
...
graph TB
    ...
    subgraph "agentrt-liunx（AirymaxOS）OS 层"
        subgraph "用户态服务层"
...
**图 1**：agentrt-liunx OS 层契约在整体架构中的位置。契约层位于用户态服务层与内核态之间，定义两者的交互边界。

---

## 2. IRON-9 v2 下的契约分层

### 2.1 [SC] 共享契约层：6 个共享头文件

[SC] 层是 agentrt 与 agentrt-liunx 代码字面一致的头文件库，位于 `include/airymax/` 目录。这 6 个头文件是 agentrt-liunx OS 层契约的"共享基础"：

| 头文件 | 全路径 | 定义内容 | 对应契约 |
|--------|--------|---------|---------|
| `bpf_struct_ops.h` | `include/airymax/bpf_struct_ops.h` | sched_ext BPF 调度器 struct_ops 状态机 + common_value | syscall API 契约 |
| `memory_types.h` | `include/airymax/memory_types.h` | MemoryRovol L1-L4 数据结构 + GFP 掩码语义 + PMEM 持久化接口 | syscall API 契约 |
| `security_types.h` | `include/airymax/security_types.h` | POSIX capability 38 ID 枚举 + LSM 钩子 254 ID 枚举 + Cupolas blob 布局 + capability 派生模型 + Vault backend 抽象 + 策略裁决 4 值枚举 | IPC 协议契约 |
...
| `sched.h` | `include/airymax/sched.h` | SCHED_EXT 调度类编号约束（复用内核 SCHED_EXT=7，禁止 SCHED_AGENT 宏）+ 任务描述符（magic 0x41475453 'AGTS'）+ vtime 衰减公式 + 优先级 0-139 + AIRYMAX_SLICE_DFL（20ms） | syscall API 契约 |
| `ipc.h` | `include/airymax/ipc.h` | IPC magic（0x41524531 'ARE1'）+ 128B 消息头结构（agentrt_ipc_msg_hdr_t）+ SQE/CQE 操作码与标志位 | IPC 协议契约 |

**共享规则**：

1. [SC] 层头文件由 agentrt 和 agentrt-liunx 的维护者共同维护，变更需双方架构委员会批准。
...
[SS] 层涵盖语义一致但实现独立的契约维度。agentrt-liunx 在 OS 层对这些契约进行内核级实现，agentrt 在用户态实现：

| 契约维度 | agentrt 用户态 | agentrt-liunx OS 层 | 同源语义 |
|---------|---------------|-------------------|---------|
| syscall API | 无（用户态运行时） | `AGENTRT_SYS_*` 系统调用 | 任务管理、调度、记忆卷载语义一致 |
...
| 错误码 | `agentrt_errno.h` | `agentrt_errno.h`（同源共享） | 错误码值、语义、`strerror()` 描述一致 |

### 2.3 [IND] 完全独立层

[IND] 层涵盖 agentrt-liunx 独有的 OS 级契约，agentrt 不涉及：
...
|---------|---------|---------|---------|---------|
| syscall API | kernel / cognition / memory / security | 用户态 → 内核态 | `agentrt_syscalls.h` + Doxygen | 编号不可变更、ABI 稳定、参数指针验证 |
...
|------|------------|---------|---------|---------|
| kernel | `AGENTRT_SYS_TASK_*` / `IPC_*` / `SCHED_*` | io_uring 内核侧 | 平台驱动 | log_write() |
| services | 通过 syscall 调用 | 128B 消息头用户态 | 用户态驱动框架 | 结构化日志 |
| security | `AGENTRT_SYS_CAP_*` / `LSM_*` | capability 携带 | 不适用 | 审计日志 |
| memory | `AGENTRT_SYS_ROVOL_*` / `CXL_*` | MemoryRovol 迁移 | 不适用 | 结构化日志 |
| cognition | `AGENTRT_SYS_CLT_*` / `WASM_*` | CoreLoopThree 通知 | 不适用 | 结构化日志 |
| cloudnative | 不直接暴露 | gRPC + CRD | 不适用 | 结构化日志 |
| system | sysctl / procfs | 不直接暴露 | 不适用 | 结构化日志 |
| tests | 不直接暴露 | 测试协议 | 不适用 | 结构化日志 |
...
agentrt-liunx OS 层契约采用语义化版本（Semantic Versioning），版本号格式为 `MAJOR.MINOR.PATCH`：

| 版本号 | 变更类型 | 示例 | 评审要求 |
|--------|---------|------|---------|
| MAJOR | 不兼容的契约变更 | 系统调用编号重分配、消息头布局变更 | 架构委员会全票通过 |
| MINOR | 向后兼容的契约新增 | 新增系统调用编号、新增 IPC payload 类型 | 架构委员会多数通过 |
...
agentrt-liunx OS 层契约在 MAJOR 版本内提供以下 ABI 兼容性保证：
...
- **向前兼容**：新版本 agentrt-liunx 内核必须能处理旧版本用户态服务发出的契约请求（通过 IPC 版本协商降级）。
- **向后兼容**：旧版本用户态服务可在新版本 agentrt-liunx 内核上运行（通过系统调用编号不变性保证）。
- **同源兼容**：agentrt 用户态运行时在 agentrt-liunx 上运行，IPC 消息头无需协议转换（[SS] 语义同源层保证）。
...
| 跨版本兼容性测试 | MAJOR 版本间 | `airymaxos-tests/compat/` | 每次发布 |
| 同源兼容性测试 | agentrt ↔ agentrt-liunx | `airymaxos-tests/homology/` | 每次发布 |
...
| 命名语义化 | E-5 | `AGENTRT_SYS_*` / `agentrt_ipc_*` / `log_write()` 命名规范 | 全文 |
| 错误可追溯 | E-6 | 错误码体系 `AGENTRT_E*` 与 `agentrt_strerror()` | syscall 契约 |
| 简约至上 | A-1 | 契约接口最小化，128B 定长消息头，6 类系统调用 | 全文 |

### 6.2 IRON-9 v2 同源且部分代码共享

本契约规范体系是 IRON-9 v2"同源且部分代码共享"原则在 OS 层契约的落地：[SC] 层 6 个头文件完全共享，[SS] 层语义一致但实现独立，[IND] 层完全独立。详见第 2 章 IRON-9 v2 契约分层。

---

## 7. 相关文档

- [agentrt-liunx 设计文档](../../README.md)
...
- [agentrt 用户态契约](../../../AirymaxAgentRT/Capital_Specifications/agentrt_contract/protocol_contract.md)
...
agentrt-liunx OS 层契约的治理遵循"分层负责、架构仲裁"模型：

| 治理层级 | 角色 | 职责 | 对应契约 |
|---------|------|------|---------|
| 架构委员会 | 最终仲裁 | 跨契约冲突裁决、MAJOR 版本变更审批 | 全部契约 |
...
当 agentrt 用户态契约与 agentrt-liunx OS 层契约出现语义冲突时，按以下优先级解决：

1. **[SC] 共享契约层优先**：共享头文件定义为准，双方必须一致。
2. **[SS] 语义同源层协商**：语义冲突通过架构委员会协商，以 agentrt-liunx OS 层实现可行性为底线。
3. **[IND] 完全独立层自主**：各自独立演进，不存在冲突。
4. **架构委员会最终仲裁**：无法协商解决的冲突，由架构委员会投票裁决。
...
| 术语 | agentrt-liunx OS 层含义 | 同源 agentrt 含义 | IRON-9 v2 分层 |
|------|------------------------|-------------------|---------------|
...
**图 2**：agentrt-liunx OS 层契约变更与版本管理流程。MAJOR 变更需架构委员会全票通过并记录 ADR，MINOR 变更由子仓维护者审批，PATCH 变更直接合并。
...
| 版本 | 日期 | 变更 |
|------|------|------|
| 0.1.1 | 2026-07-07 | 初始版本（agentrt-liunx OS 层契约规范总览，含 IRON-9 v2 三层契约分层、4 大契约规范索引、版本管理策略、兼容性保证） |
