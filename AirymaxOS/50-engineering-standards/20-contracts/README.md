...
| 内核态纯 C LSM（airy_lsm） | sched_tac 调度策略注册的用户态接口（struct airy_sched_ops） | syscall API 契约 |
...
契约版本 0.1.1
    │
    ├── 系统调用编号 548-571（4 核心 + 20 预留，v1.0.1 Capability Folding）
    ├── 128B 消息头 v0x0100 锁定
    ├── 日志格式 Schema v1.0 锁定
    │
    ▼
...
| 测试类型 | 覆盖范围 | 工具 | 频率 |
|---------|---------|------|------|
| 系统调用编号校验 | 全部 548-571 编号 | `scripts/check_syscall_numbers.sh` | 每次 PR |
