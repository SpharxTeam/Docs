...
| flags | 12 | 4 | uint32_t | 标志位（零拷贝/capability/压缩/加密） | 位定义见 Part I §2.3 |
...
| 标志位 | 值 | 语义 | 安全约束 |
|--------|-----|------|---------|
| AIRY_IPC_FLAG_ZEROCOPY | 0x00000001 | 使用 io_uring 零拷贝路径 | 发送方必须注册 buffer |
| AIRY_IPC_FLAG_CAP_CARRY | 0x00000002 | 消息携带 capability 令牌 | 内核校验令牌有效性 |
| AIRY_IPC_FLAG_COMPRESS | 0x00000004 | payload 已压缩（LZ4） | 接收方自动解压 |
| AIRY_IPC_FLAG_ENCRYPT | 0x00000008 | payload 已加密（SM4） | 密钥由 capability 携带 |
| AIRY_IPC_FLAG_URGENT | 0x00000010 | 紧急消息，优先投递 | 仅 SCHED_AGENT 实时类可用 |
| AIRY_IPC_FLAG_NOREPLY | 0x00000020 | 无需响应（单向通知） | 仅 EVENT 类型可用 |
...
| 超时处理 | 超过延迟预算的消息返回 AIRY_ETIMEOUT | 发送方设置 timeout_ms，内核超时返回 |
...
| 超时过期 | 令牌可设置有效期，超时自动过期 | 过期令牌返回 AIRY_ETIMEOUT |
...
| 4 | 调度 | `AIRY_SYS_SCHED_*` | 572-591 | sched_ext 策略设置、SCHED_AGENT 策略管理 | kernel | MicroCoreRT sub-scheduler |
...
| `AIRY_SYS_SCHED_*` | `airy_sys_sched_*` | SCHED_AGENT 调度策略 | `airy_sys_sched_set_policy` |
...
agentrt-linux 系统调用错误码对齐 `airy_errno.h`，与 agentrt 同源且部分代码共享（IRON-9 v2 [SC] 层）。错误码统一使用 `AIRY_E*` 前缀，负值返回：
...
| `AIRY_ETIMEOUT` | -11 | 超时 | 调度等待超时、IPC 接收超时 | 是（限制次数） |
...
| 编号 | 调用名 | 功能 | 关键参数 |
|------|--------|------|---------|
| 572 | `airy_sys_sched_set_policy` | 设置 sched_ext 调度策略 | `cgroup_path`（目标 cgroup）、`policy`（策略名称） |
| 573 | `airy_sys_sched_get_policy` | 查询当前调度策略 | `cgroup_path`、`policy_out`（输出） |
| 574 | `airy_sys_sched_register_bpf` | 注册 eBPF 调度器程序 | `bpf_fd`（eBPF 程序 fd）、`ops`（bpf_struct_ops 指针） |
...
| bpf_struct_ops | 不适用 | `syscalls.h`（[SC] 共享） | [SC] 代码字面一致 |
...
| 594 | `airy_sys_lsm_load_policy` | 加载 agent_lsm 策略 | `policy_buf`（策略数据）、`policy_len` |
...
| `security_types.h` | POSIX capability 41 ID 枚举 + LSM 钩子 252 ID 枚举 + Cupolas blob 布局 + capability 派生模型 + Vault backend 抽象 + 策略裁决 4 值枚举 | 安全（CAP） |
| `cognition_types.h` | CoreLoopThree 阶段枚举 + Thinkdual 模式枚举 + LLM 推理阶段枚举 + Token 能效指标 + GPU/NPU 能力描述符 | 认知（CLT） |
| `sched.h` | SCHED_EXT 调度类编号约束 + 任务描述符（magic 0x41475453 'AGTS'）+ vtime 衰减公式 + 优先级 0-139 + AIRY_SLICE_DFL（20ms） | 调度（SCHED） |
