...
sequenceDiagram
    ...
    S->>S: 申请 capability 令牌<br/>airy_sys_call(COMPILE_BADGE)
    S->>K: 发送 IPC 消息 flags = CAP_CARRY
    K->>K: 校验 cap_slot 属于 src_task
    K->>K: 派生受限 capability
    K->>R: 投递消息加派生令牌
    R->>R: 使用派生令牌执行受保护操作
    S->>S: 可选：撤销原令牌<br/>airy_sys_call(REVOKE_BADGE)
    K->>R: 递归撤销派生令牌
...
| 128B 消息延迟 | < 10 μs（P99） | io_uring IORING_OP_URING_CMD + CQE 往返 | NFR-P-002a |
...
agentrt-linux（AirymaxOS）在 Linux 6.6 内核基线的标准系统调用之上，新增 Agent 感知专用系统调用，统一使用 `airy_` 前缀。v1.0.1 采用 4 核心 syscall + 20 预留的 op-dispatch 模型（编号 548-571），IPC 数据面走 io_uring IORING_OP_URING_CMD（零 syscall），覆盖 8 子仓的内核入口：

| # | 核心调用 | 编号 | op-dispatch | 核心职责 | 覆盖子仓 | 同源 agentrt |
|---|---------|------|------------|---------|---------|--------------|
| 1 | `airy_sys_call` | 548 | COMPILE_BADGE / REVOKE_BADGE / LSM_CTL | Agent 任务提交、capability 编译/撤销、LSM 策略加载 | kernel / cognition / security | MicroCoreRT 调度 / Cupolas 权限 |
| 2 | `airy_sys_rovol_ctl` | 549 | AIRY_ROVOL_SNAPSHOT / RESTORE / MIGRATE / LIST / DELETE / DEMOTE / PROMOTE | 记忆卷载快照/恢复/迁移/列表/删除/降级/升级 | kernel / memory | MemoryRovol |
| 3 | `airy_sys_sched_ctl` | 550 | SET_POLICY | sched_tac 策略设置 | kernel | MicroCoreRT sub-scheduler |
| 4 | `airy_sys_clt_notify` | 551 | PHASE_NOTIFY | CoreLoopThree 阶段通知 | kernel / cognition | CoreLoopThree |
| - | 预留 | 552-571 | - | 20 个预留槽位，供未来扩展 | - | - |
| - | IPC 数据面 | io_uring | IORING_OP_URING_CMD / CQE | 进程间消息传递、零拷贝通道（零 syscall） | kernel / services | AgentsIPC |
...
| 安全内生 | E-1 | 所有安全敏感系统调用必须先通过 capability 守卫（`airy_sys_call`（COMPILE_BADGE）） |
...
graph TD
    ...
    subgraph "内核态 Kernel Space"
        subgraph "核心 syscall 548-551"
            K1[airy_sys_call 548<br/>COMPILE_BADGE / REVOKE_BADGE / LSM_CTL]
            K2[airy_sys_rovol_ctl 549<br/>SNAPSHOT / RESTORE / MIGRATE / ...]
            K3[airy_sys_sched_ctl 550<br/>SET_POLICY]
            K4[airy_sys_clt_notify 551<br/>PHASE_NOTIFY]
        end
        subgraph "预留 552-571"
            K5[20 个预留槽位]
        end
        subgraph "IPC 数据面 io_uring"
            K6[IORING_OP_URING_CMD<br/>CQE 零 syscall]
        end
    end
    ...
    style G2 fill:#fff3e0,stroke:#065100
    style G3 fill:#c8e6c9,stroke:#2e7d32
```

**图 1**：agentrt-linux 系统调用层级架构。所有用户态调用统一经过参数校验、capability 守卫、审计日志三关，再分发到 4 核心 syscall（IPC 数据面走 io_uring 零 syscall）。
...
agentrt-linux 专用系统调用编号从 `548` 起始分配，避开 Linux 6.6 标准 0-511 编号空间。v1.0.1 采用 4 核心 syscall + 20 预留的 op-dispatch 模型：

| 编号段 | 分类 | 数量 | 起始编号 | 终止编号 | 已分配 | 预留 |
|--------|------|------|---------|---------|--------|------|
| 548-571 | 核心 + 预留 | 24 | 548 | 571 | 4 | 20 |
...
4. **编号段扩展**：若 20 个预留槽位耗尽，需在下一 MAJOR 版本中扩展编号段，并创建 ADR 记录。
...
| 前缀 | C 符号形式 | 用途 | 示例 |
|------|-----------|------|------|
| `AIRY_SYS_CALL` | `airy_sys_call` | 通用 op-dispatch（任务/capability/LSM） | `airy_sys_call(COMPILE_BADGE)` |
| `AIRY_SYS_ROVOL_CTL` | `airy_sys_rovol_ctl` | MemoryRovol 记忆卷载 | `airy_sys_rovol_ctl(AIRY_ROVOL_SNAPSHOT)` |
| `AIRY_SYS_SCHED_CTL` | `airy_sys_sched_ctl` | sched_tac 调度策略 | `airy_sys_sched_ctl(SET_POLICY)` |
| `AIRY_SYS_CLT_NOTIFY` | `airy_sys_clt_notify` | CoreLoopThree 认知 | `airy_sys_clt_notify(PHASE_NOTIFY)` |
...
| 编号 | 调用名 | 功能 | 关键参数 |
|------|--------|------|---------|
| 550 | `airy_sys_sched_ctl`（SET_POLICY） | 设置 sched_tac 调度策略 | `cgroup_path`（目标 cgroup）、`policy`（策略名称） |
| 550 | `airy_sys_sched_ctl`（GET_POLICY） | 查询当前调度策略 | `cgroup_path`、`policy_out`（输出） |
| 550 | `airy_sys_sched_ctl`（SET_BUDGET） | 注入 sched_d 调度预算参数（SCHED_DEADLINE 语义，H5 纯 C 无 BPF 依赖） | `agent_id`、`budget`（struct airy_sched_budget：sched_runtime/deadline/period） |
| 550 | `airy_sys_sched_ctl`（GET_LATENCY） | 查询调度延迟统计 | `cgroup_path`、`stats_out`（延迟统计输出） |
...
| 编号 | 调用名 | 功能 | 关键参数 |
|------|--------|------|---------|
| 548 | `airy_sys_call`（COMPILE_BADGE） | 申请 capability 令牌 | `capability`（权限名称）、`resource`（资源路径） |
| 548 | `airy_sys_call`（REVOKE_BADGE） | 撤销 capability | `cap_slot`（capability 句柄） |
| 548 | `airy_sys_call`（LSM_CTL） | 加载 airy_lsm 策略 | `policy_buf`（策略数据）、`policy_len` |

#### 6.2 capability 令牌生命周期

```mermaid
stateDiagram-v2
    [*] --> Created: airy_sys_call(COMPILE_BADGE)
    Created --> Active: 令牌生成成功
    Created --> Denied: 权限拒绝（AIRY_EPERM）

    Active --> Derived: 通过 IPC 传递（CAP_CARRY）
    Active --> Used: 执行受保护操作
    Active --> Revoked: airy_sys_call(REVOKE_BADGE)
...
|------|--------|------|---------------------|
| 549 | `airy_sys_rovol_ctl`（AIRY_ROVOL_SNAPSHOT） | 创建进程记忆快照 | L1 原始卷 |
| 549 | `airy_sys_rovol_ctl`（AIRY_ROVOL_RESTORE） | 从快照恢复记忆 | L1 原始卷 |
| 549 | `airy_sys_rovol_ctl`（AIRY_ROVOL_MIGRATE） | 跨节点记忆迁移 | L1-L4 全部 |
| 549 | `airy_sys_rovol_ctl`（AIRY_ROVOL_TIER_SET） | CXL 内存分层策略 | L2-L4 索引层 |
| 549 | `airy_sys_rovol_ctl`（AIRY_ROVOL_MGLRU_CONFIG） | MGLRU（多代 LRU）配置 | L2-L4 索引层 |
...
| 编号 | 调用名 | 功能 | 对应认知阶段 |
|------|--------|------|-------------|
| 551 | `airy_sys_clt_notify`（PHASE_NOTIFY） | CoreLoopThree 阶段通知 | 全部三阶段 |
| 551 | `airy_sys_clt_notify`（REGISTER_KTHREAD） | 注册 CoreLoopThree kthread | 内核线程注册 |
| 551 | `airy_sys_clt_notify`（WASM_LOAD_MODULE） | 加载 Wasm 3.0 安全模块 | 行动阶段沙箱 |
...
| 核心调用 | agentrt 用户态 | agentrt-linux OS 层 | 同源语义 |
|---------|---------------|-------------------|---------|
| `airy_sys_call` | MicroCoreRT 用户态调度 / Cupolas 应用权限模型 | `AIRY_SYS_CALL` 内核 op-dispatch | 任务提交、capability 编译/撤销、LSM 策略加载语义一致 |
| `airy_sys_rovol_ctl` | MemoryRovol 用户态 API | `AIRY_SYS_ROVOL_CTL` 内核态 | 四层卷载、存用分离语义一致 |
| `airy_sys_sched_ctl` | MicroCoreRT 用户态调度 | `AIRY_SYS_SCHED_CTL` 内核态 | sched_tac 策略语义一致 |
| `airy_sys_clt_notify` | CoreLoopThree 用户态 | `AIRY_SYS_CLT_NOTIFY` kthread | 三阶段循环语义一致 |
| IPC 数据面 | AgentsIPC 用户态消息队列 | io_uring IORING_OP_URING_CMD 零拷贝 | 128B 消息头布局兼容 |
...
| 核心调用 | 典型延迟（P99） | 最大延迟（P99.9） | 测量方法 |
|---------|---------------|------------------|---------|
| `airy_sys_call` | < 1 μs | < 5 μs | `perf trace -e airy_sys_call` |
| `airy_sys_rovol_ctl` | < 5 μs（快照）/ < 100 ms（迁移） | < 10 μs / < 500 ms | 按操作类型分别测量 |
...
| IPC 数据面 | < 1 μs | < 5 μs | io_uring-bench（IORING_OP_URING_CMD） |
