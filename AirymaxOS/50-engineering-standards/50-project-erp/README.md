...
| **社区治理** | 子仓治理组之间协调不畅 | 低 | 架构委员会定期会议 + ADR 决策机制 |
...
| 沟通渠道 | 用途 | 参与者 | 频率 |
|----------|------|--------|------|
| 架构委员会会议 | 跨子仓架构决策、ADR 评审 | 各子仓治理组负责人 | 双周 |
...
| 跨子仓技术决策 | 架构委员会 | 架构委员会投票（2/3 多数） | ADR |
| 项目规范变更 | 架构委员会 | 架构委员会投票（2/3 多数） | 规范文档更新 + ADR |
| [SC] 层变更 | agentrt + agentrt-linux 架构委员会 | 双端联合审查 | 双端 ADR |
...
| 文档类型 | 存放位置 | 版本控制 | 审查要求 |
|----------|----------|----------|----------|
| 设计文档 | `docs/AirymaxAgentOS/` | Git | 子仓治理组审查 |
| 项目管理规范 | `docs/AirymaxAgentOS/50-engineering-standards/` | Git | 架构委员会审查 |
| 工程标准规范 | 内部闭源文档 | Git（闭源） | 架构委员会审查 |
| 代码注释 | 各子仓代码内 | Git | 代码审查 |
| API 文档 | Doxygen 生成 | Git（源码） | 代码审查 |
| 发布公告 | `docs/AirymaxAgentOS/releases/` | Git | 发布工程师审查 |
...
- [集成标准总览](../integration_standards/README.md)：agentrt-linux 集成标准
