...
> **关联规范**：IRON-9 v3 工程铁律（工程标准规范）\
> **SSoT 依赖声明**：本子目录的规则编号登记于 [09-ssot-registry.md §3](../09-ssot-registry.md)。项目管理规范引用 IRON-9 v3 工程铁律作为同源约束的权威。
...
| 挑战维度 | 说明 | 管理应对 |
|----------|------|----------|
| **多子仓协同** | 8 个子仓（kernel / services / security / memory / cognition / cloudnative / system / tests-linux）需同步演进 | 统一版本号 + 统一错误码 + 统一 SBOM |
...
| **同源约束** | 与 agentrt 共享 IRON-9 v3 四层（[SC] 共享契约层 / [SS] 语义同源层 / [IND] 完全独立层） | 项目管理规范与 agentrt 对齐 |
...
graph TD
    subgraph 项目管理规范体系
        A[project_erp/README.md] --> B[project_erp.md Part II: 错误码]
        A --> C[project_erp.md Part I: SBOM]
    end
...
| 文档 | 职责 | 受众 |
|------|------|------|
| **project_erp.md Part II** | 定义 agentrt-linux 全系统统一错误码体系，涵盖内核态与用户态双错误码空间、分段规划、与 agentrt 的映射关系、使用规范与维护流程 | 内核开发者、服务开发者、测试工程师 |
| **project_erp.md Part I** | 定义 agentrt-linux 软件物料清单的生成标准、组件追踪规则、许可证合规矩阵、漏洞扫描策略与自动生成流程 | 发布工程师、安全审计员、合规团队 |
...
| 错误码空间 | 表示形式 | 适用范围 | 说明 |
|------------|----------|----------|------|
| **内核态错误码** | 负整数（-1 ~ -899） | kernel | 内核态系统调用返回、内核模块内部错误 |
| **用户态错误码** | 十六进制（0xXXXX0000） | services 及用户态所有子仓 | 用户态守护进程、服务、SDK 返回 |
...
| 系统错误 | -100 ~ -199 | system | 系统级错误 |
| 调度错误 | -200 ~ -299 | kernel | 调度器相关错误 |
| IPC 错误 | -300 ~ -399 | kernel + services | 进程间通信错误 |
| 内存错误 | -400 ~ -499 | memory | 记忆子系统错误 |
| 安全错误 | -500 ~ -599 | security | 安全相关错误 |
| 认知错误 | -600 ~ -699 | cognition | 认知运行时错误 |
| 驱动错误 | -700 ~ -799 | kernel + services | 驱动框架错误 |
| 网络错误 | -800 ~ -899 | cloudnative | 网络与云原生错误 |

### 3.4 与 agentrt 错误码的映射关系

根据 IRON-9 v3 工程铁律，agentrt-linux 与 agentrt 共享契约层代码（[SC] 层）。错误码映射遵循以下规则：
...
详细错误码映射表见 [project_erp.md Part II](./project_erp.md)。
...
详细 SBOM 规范见 [project_erp.md Part I](./project_erp.md)。
...
| 铁律 | 内容 | 关联规范 |
|------|------|----------|
| **错误码统一** | 全系统错误码必须在 project_erp.md Part II 中注册，未注册的错误码不得使用 | E-6 错误可追溯原则 |
| **SBOM 强制** | 每次发布必须附带 SPDX 2.3 格式的 SBOM，未附带 SBOM 的发布视为无效 | E-1 安全内生原则 |
| **版本同步** | 项目管理文档版本号与代码版本号同步，不允许文档版本滞后 | E-7 文档即代码原则 |
| **同源对齐** | 错误码、SBOM 等项目管理规范必须与 agentrt 对齐（IRON-9 v3 约束） | IRON-9 v3 |
...
- [项目管理规范合集](./project_erp.md)：SBOM / 错误码 / 模块需求 / 资源管理
- [集成标准总览](../40-integration/README.md)：agentrt-linux 集成标准
- [集成标准合集](../40-integration/integration.md)：agentrt 集成 / 生态伙伴 / 配置集成 / 标准贡献
...
| 0.1.1 | 2026-07-13 | OLK-6.6 ES-OLK-1~13 + seL4 设计模式 + IRON-9 v3 SBOM/错误码同源对齐验证 |
