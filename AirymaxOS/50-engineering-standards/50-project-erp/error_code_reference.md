...
| **修改错误码语义** | 标记 @deprecated 旧定义，新增新定义 | 架构委员会审查 + ADR 记录 |
| **删除错误码** | 标记 @deprecated，保留至少 2 个版本周期 | 架构委员会审查 + 公告 |
| **调整分段区间** | 必须通过 ADR 评审 | 全架构委员会投票 |
| **修改 [SC] 层错误码** | 必须 agentrt + agentrt-linux 双向同步 | 双端架构委员会联合审查 |
