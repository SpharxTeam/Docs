...
# agentrt-linux（AirymaxOS）SPDX 许可证标识强制规范

> **文档定位**: agentrt-linux（AirymaxOS，极境智能体操作系统）的 SPDX 许可证标识强制规范。所有 `.c`/`.h`/`.md`/`.rst`/`.py`/`.sh`/`Makefile`/`Kconfig` 文件必须含 `SPDX-License-Identifier: AGPL-3.0-or-later OR Apache-2.0` 标识，含检查脚本与违规/合规示例。
...
本文件定义 agentrt-linux 仓库中所有源代码、文档、脚本文件的 SPDX 许可证标识强制要求。所有文件**必须**在首行（或首行注释内）含 `SPDX-License-Identifier: AGPL-3.0-or-later OR Apache-2.0` 标识。CI 通过扫描脚本强制校验，违规即拒绝合并。
...
agentrt-linux 仓库中以下文件类型**必须**在文件首行（或首行注释内）含 SPDX 标识：
...
agentrt-linux 仓库**统一**使用以下 SPDX 许可证字符串：
...
- **AGPL-3.0-or-later**：保障网络服务场景下的源代码披露义务，与 agentrt-linux 作为发行版的开放性一致
 ...
 *
 * Implements the kernel-side IPC channel for agentrt-linux.
...
#!/usr/bin/env python3
# SPDX-License-Identifier: AGPL-3.0-or-later OR Apache-2.0
"""agentrt-linux CI helper: validate SPDX tags across the repo."""
...
# SPDX-License-Identifier: AGPL-3.0-or-later OR Apache-2.0
#
# agentrt-linux build script: trigger kernel + userspace build.
...
# SPDX-License-Identifier: AGPL-3.0-or-later OR Apache-2.0
#
# agentrt-linux top-level Makefile.
...
# SPDX-License-Identifier: AGPL-3.0-or-later OR Apache-2.0
#
# CI configuration for agentrt-linux.
...
/* WRONG: agentrt-linux 统一用 AGPL-3.0-or-later OR Apache-2.0 */
...
agentrt-linux CI 使用以下脚本扫描所有文件，校验 SPDX 标识存在性与正确性：

```bash
#!/bin/bash
# SPDX-License-Identifier: AGPL-3.0-or-later OR Apache-2.0
#
# spdx-check.sh: Validate SPDX-License-Identifier tags in agentrt-linux repo.
...
agentrt-linux 在 `LICENSES/` 目录中需添加 `AGPL-3.0-or-later.txt` 与 `Apache-2.0.txt` 两个许可证文本文件，供 checkpatch 校验合法性。
