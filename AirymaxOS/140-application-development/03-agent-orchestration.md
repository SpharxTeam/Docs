...
enum airy_mac_mode {
	...
	AIRY_MAC_MODE_CONSENSUS = 2,    /* 共识：投票决策 */
	AIRY_MAC_MODE_DELEGATION = 3,   /* 委托：主从委托 */
};

enum airy_mac_consensus {
	AIRY_MAC_CONSENSUS_MAJORITY = 0,  /* 多数投票 */
	AIRY_MAC_CONSENSUS_UNANIMOUS = 1, /* 全票通过 */
	AIRY_MAC_CONSENSUS_WEIGHTED = 2,  /* 加权投票 */
	AIRY_MAC_CONSENSUS_LEADER_VETO = 3, /* 领导者否决 */
};
```
...
struct airy_task_node {
	...
	uint32_t dep_count;
	enum airy_task_status status;
	uint64_t trace_id;
};

enum airy_task_status {
	AIRY_TASK_PENDING   = 0,
	AIRY_TASK_READY     = 1,  /* 依赖全部完成 */
	AIRY_TASK_RUNNING   = 2,
	AIRY_TASK_COMPLETED = 3,
	AIRY_TASK_FAILED    = 4,
	AIRY_TASK_ROLLBACK  = 5,
};

struct airy_dag_workflow {
	uint32_t workflow_id;
	char name[64];
	struct airy_task_node nodes[256];
	uint32_t node_count;
...
```c
/**
 * airy_dag_schedule - 调度 DAG 工作流的下一超步
 * @wf: 工作流
 *
 * 算法：
 *   1. 扫描所有 PENDING 节点
 ...
 * 返回：本超步就绪节点数
 */
int airy_dag_schedule(struct airy_dag_workflow *wf)
{
	int ready_count = 0;
	int i, j;

	for (i = 0; i < wf->node_count; i++) {
		struct airy_task_node *node = &wf->nodes[i];
		bool deps_met = true;

		if (node->status != AIRY_TASK_PENDING)
			continue;

		/* 检查所有依赖 */
		for (j = 0; j < node->dep_count; j++) {
			uint32_t dep_id = node->dependencies[j];
			struct airy_task_node *dep =
				airy_dag_find_node(wf, dep_id);
			if (!dep || dep->status != AIRY_TASK_COMPLETED) {
				deps_met = false;
				break;
			}
		}
		...
		/* 依赖满足，置为 READY 并提交调度 */
		node->status = AIRY_TASK_READY;
		airy_sched_agent_submit_dag_node(node);
		ready_count++;
	}
...
```c
/* 示例：构造三 Agent 协作 DAG */
struct airy_dag_workflow research_wf = {
	.name = "research_workflow",
	.superstep = 0,
};

/* 节点定义 */
static struct airy_task_node nodes[] = {
	/* Agent A：认知阶段 */
	{.node_id = 1, .agent_id = 100, .method = "cognition.parse_intent",
	 .dep_count = 0, .status = AIRY_TASK_PENDING},
	{.node_id = 2, .agent_id = 100, .method = "cognition.decompose",
	 .dependencies = {1}, .dep_count = 1, .status = AIRY_TASK_PENDING},

	/* Agent B：检索阶段（并行三路） */
	{.node_id = 3, .agent_id = 200, .method = "memory.rovol_l1_search",
	 .dependencies = {2}, .dep_count = 1, .status = AIRY_TASK_PENDING},
	...
	{.node_id = 6, .agent_id = 200, .method = "memory.hybrid_fuse",
	 .dependencies = {3, 4, 5}, .dep_count = 3, .status = AIRY_TASK_PENDING},

	/* Agent C：综合阶段 */
	{.node_id = 7, .agent_id = 300, .method = "synthesis.aggregate",
	 .dependencies = {6}, .dep_count = 1, .status = AIRY_TASK_PENDING},
	{.node_id = 8, .agent_id = 300, .method = "synthesis.report",
	 .dependencies = {7}, .dep_count = 1, .status = AIRY_TASK_PENDING},
	{.node_id = 9, .agent_id = 300, .method = "synthesis.evaluate",
	 .dependencies = {8}, .dep_count = 1, .status = AIRY_TASK_PENDING},
};
```
 ...
 */
static int airy_taskflow_kthread(void *data)
{
	struct airy_dag_workflow *wf;
	...
	while (!kthread_should_stop()) {
		...
		/* 调度下一超步 */
		int ready = airy_dag_schedule(wf);
		if (ready < 0) {
...
void __init airy_taskflow_init(void)
{
	INIT_KFIFO(taskflow_queue);
	init_waitqueue_head(&taskflow_wq);
	taskflow_kthread = kthread_run(airy_taskflow_kthread, NULL,
				       "airy_taskflow");
}
```
...
```c
/**
 * airy_taskflow_checkpoint - 写入超步检查点
 * @wf: 工作流
 *
 * 将当前工作流状态序列化至 MemoryRovol L1 原始卷
 * 检查点格式：workflow_id + superstep + node_statuses
 */
int airy_taskflow_checkpoint(struct airy_dag_workflow *wf)
{
...
/**
 * airy_taskflow_rollback - 回滚至最近检查点
 * @wf: 工作流
 */
int airy_taskflow_rollback(struct airy_dag_workflow *wf)
{
	struct airy_checkpoint_record rec;
	int ret, i;
	...
	for (i = 0; i < wf->node_count; i++) {
		...
		if (wf->nodes[i].status != rec.node_statuses[i])
			wf->nodes[i].status = AIRY_TASK_PENDING;
	}
...
```c
struct airy_consensus_session {
	uint32_t session_id;
	enum airy_mac_consensus strategy;
	uint32_t *participants;   /* 参与 Agent ID 列表 */
	uint32_t participant_count;
	struct airy_vote_record *votes;
	uint32_t vote_count;
	uint64_t deadline_ns;    /* 投票截止时间 */
};

/**
 * airy_consensus_reach - 达成共识
 * @session: 共识会话
 *
 * 根据策略统计投票，返回共识结果
 */
int airy_consensus_reach(struct airy_consensus_session *session)
{
...
```c
/**
 * airy_delegation_dispatch - 主 Agent 委托子任务
 * @master: 主 Agent ID
 * @slave: 从 Agent ID
 * @task: 委托任务
 *
 ...
 */
int airy_delegation_dispatch(uint32_t master, uint32_t slave,
				struct airy_task_node *task)
{
