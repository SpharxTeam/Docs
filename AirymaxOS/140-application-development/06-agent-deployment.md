  ...
  "syscalls": [
    {
      "names": [
        ...
        "futex", "sched_yield",
        "airy_sys_call", "airy_sys_rovol_ctl",
...
```c
/* agentdeploy 在 Agent 启动前通过 airy_sys_call(COMPILE_BADGE) 编译 Capability Badge */
for (const char *cap_name : manifest.security.capabilities.request) {
    uint32_t cap_id = airy_cap_name_to_id(cap_name);
    __u64 badge;
    int ret = airy_sys_call(AIRY_OP_COMPILE_BADGE, cap_id, resource, &badge);
    if (ret < 0) {
        log_error("capability badge compile failed: %s", cap_name);
        goto failed;
    }
