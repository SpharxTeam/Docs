...
|------|--------|--------|------|--------|---------|------|
| 512 | `AIRY_SYS_TASK_SUBMIT` | `airy_sys_task_submit` | 提交 Agent 任务到 SCHED_AGENT 调度器 | 2 | 0.1.1 | 已分配 |
...
| 编号 | 宏定义 | C 符号 | 功能 | 参数数 | 引入版本 | 状态 |
|------|--------|--------|------|--------|---------|------|
| 572 | `AIRY_SYS_SCHED_SET_POLICY` | `airy_sys_sched_set_policy` | 设置 sched_ext 调度策略 | 2 | 0.1.1 | 已分配 |
...
| 594 | `AIRY_SYS_LSM_LOAD_POLICY` | `airy_sys_lsm_load_policy` | 加载 agent_lsm 策略 | 2 | 0.1.1 | 已分配 |
...
/* 任务操作 512-515 */
#define AIRY_SYS_TASK_SUBMIT     512  /* 提交 Agent 任务到 SCHED_AGENT */
...
#define AIRY_SYS_SCHED_SET_POLICY    572  /* 设置 sched_ext 调度策略 */
...
#define AIRY_SYS_LSM_LOAD_POLICY      594  /* 加载 agent_lsm 策略 */
...
| `security_types.h` | capability 41 ID 枚举 + LSM 252 钩子 + 派生模型 | 安全（CAP） |
