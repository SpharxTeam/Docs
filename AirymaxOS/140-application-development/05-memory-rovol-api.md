...
> **文档定位**：agentrt-linux（AirymaxOS）MemoryRovol 记忆卷载子系统的完整应用层 API 契约，通过单一 `airy_sys_rovol_ctl` (549) + op-dispatch 模型定义 10 项操作的签名、参数语义、状态机、数据流、错误处理与 SDK 集成\
> **文档版本**：v1.0.1\
> **最后更新**： 2026-07-22\
...
> **编号 SSoT**：完整的系统调用编号注册表见 [07-syscall-registry.md](07-syscall-registry.md) 第 3.3 节（ROVOL_CTL 549 + op-dispatch §4.3）\
> **v1.0.1 变更说明**：原 0.1.1 的 10 个独立 MemoryRovol syscall（552-561）已合并为单一 `airy_sys_rovol_ctl` (549) + op-dispatch 模型，详见 [07-syscall-registry.md](07-syscall-registry.md) §4.3
...
| 问题 | 当前状态 | 本契约解决方式 |
|------|---------|---------------|
| API 签名分散 | snapshot/restore 在 3 处文档，签名略有差异 | 统一为单一 `airy_sys_rovol_ctl` (549) + 10 项 op-dispatch 操作的权威签名 |
...
| [contracts.md](../50-engineering-standards/20-contracts/contracts.md) | 第 7 章编号定义的扩展——本契约以单一 `airy_sys_rovol_ctl` (549) + 10 个 op 码补全 |
| [07-syscall-registry.md](07-syscall-registry.md) | §4.3 ROVOL_CTL (549) op-dispatch 编号段的 API 层细化 |
| [30-interfaces/01-syscalls.md](../30-interfaces/01-syscalls.md) | snapshot/restore 签名的完整扩展（已迁移至 op-dispatch） |

### 1.3 设计目标

1. **存用分离**：存储（PMEM/CXL/DRAM tier）与使用（snapshot/restore/migrate）解耦，符合 K-3 服务隔离原则
2. **层级透明**：L1-L4 四层对应用透明，应用通过单一 API 操作记忆，层级迁移由内核 MemoryRovol 策略（MGLRU + demote/promote）驱动
...
|------|-------------------------|
| **K-1 内核极简** | 内核仅提供 `airy_sys_rovol_ctl` (549) op-dispatch 入口与 snapshot/restore/migrate 机制，遗忘策略在用户态 |
| **K-2 接口契约化** | 单一 `airy_sys_rovol_ctl` (549) + 10 个 op 码均通过 C 头文件 + Doxygen 给出显式契约 |
...
| **IRON-9 v3 同源且部分代码共享** | [SC] L1-L4 数据结构 + [SS] 6 项操作语义同源（签名独立演进） + [IND] 内核态实现 |
...
MemoryRovol API 在 v1.0.1 中采用 **Capability Folding 架构**——所有 MemoryRovol 操作通过单一系统调用 `airy_sys_rovol_ctl`（编号 549）承载，由 `op` 参数分派至具体操作。该调用隶属于 v1.0.1 的 4 核心 syscall 集合（548-551）。完整编号注册表见 [07-syscall-registry.md](07-syscall-registry.md) §4.3。

**统一入口签名**：

```c
/**
 * @brief MemoryRovol 操作统一入口（op-dispatch）
 * @param op        操作码（AIRY_ROVOL_*）
 * @param agent_id  目标 Agent ID（0 表示全局配置）
 * @param arg       操作特定参数（指针或立即数编码）
...
```mermaid
graph TD
    ENTRY[airy_sys_rovol_ctl<br/>编号 549<br/>op-dispatch 统一入口]

    subgraph "快照生命周期 Snapshot Lifecycle"
        S1[549 op=SNAPSHOT<br/>创建快照]
        S2[549 op=LIST<br/>列出快照]
        S3[549 op=DELETE<br/>删除快照]
        S4[549 op=RESTORE<br/>恢复快照]
    end
    subgraph "跨节点迁移 Migration"
        M1[549 op=MIGRATE<br/>跨节点迁移]
    end
    subgraph "CXL 内存分层 CXL Tiering"
        C1[549 op=TIER_SET<br/>设置分层策略]
        C2[549 op=TIER_GET<br/>查询分层策略]
    end
    subgraph "MGLRU 配置 MGLRU Config"
        G1[549 op=MGLRU_CONFIG<br/>配置多代 LRU]
    end
    subgraph "层级升降级 Tier Migration"
        T1[549 op=DEMOTE<br/>L1→L2→L3→L4 降级]
        T2[549 op=PROMOTE<br/>L4→L3→L2→L1 晋升]
    end
    ...
    style ENTRY fill:#fffde7,stroke:#f57f17
...
**图 1**：MemoryRovol API 分类。单一 `airy_sys_rovol_ctl` (549) 入口通过 op 码分派至 10 项操作，分为 5 组：快照生命周期（4 项）、跨节点迁移（1 项）、CXL 分层（2 项）、MGLRU 配置（1 项）、层级升降级（2 项）。
 ...
 * @since 1.0.1
 *
 * 此结构由内核填充，应用通过 airy_sys_rovol_ctl(AIRY_ROVOL_LIST) 获取数组。
...
### 4.2 `airy_sys_rovol_ctl(AIRY_ROVOL_SNAPSHOT)`（编号 549, op=SNAPSHOT）

创建指定 Agent 进程的记忆快照。借鉴 Linux `fork()` + COW（Copy-On-Write）机制——快照创建时不立即复制内存，而是在首次写入时按页复制。本操作通过统一入口 `airy_sys_rovol_ctl` 分派，`op=AIRY_ROVOL_SNAPSHOT`。

```c
/**
 * @brief 创建进程记忆快照（op-dispatch 入口）
 * @param op        必须为 AIRY_ROVOL_SNAPSHOT
 * @param agent_id  目标 Agent ID
 * @param arg       指向 airy_rovol_snapshot_info_t（调用方分配，options 编码至 layer_mask + flags）
 * @return 0 成功（info 已填充），<0 AIRY_E* 错误码
 *
 * @since v1.0.1
 * @par Example:
 * @code
 *   airy_rovol_snapshot_info_t info = { .size = sizeof(info), .version = 0x0100 };
 *   info.layer_mask = AIRY_ROVOL_LAYER_ALL;
 *   info.flags      = AIRY_ROVOL_FLAG_COMPRESS;
 *   int ret = airy_sys_rovol_ctl(AIRY_ROVOL_SNAPSHOT, agent_id,
...
| 参数 | 类型 | 语义 | 约束 |
|------|------|------|------|
| `op` | `uint32_t` | 必须为 `AIRY_ROVOL_SNAPSHOT` | 由 dispatch 表分派 |
| `agent_id` | `uint32_t` | 目标 Agent ID | 必须已注册且属于当前 capability 域 |
| `arg` | `uint64_t` | 指向 `airy_rovol_snapshot_info_t`（含 layer_mask + flags 编码的 options） | 调用方分配，`size` 字段必须已设置 |
...
### 4.3 `airy_sys_rovol_ctl(AIRY_ROVOL_LIST)`（编号 549, op=LIST）

列出指定 Agent 的所有快照。借鉴 Linux `getdents64()` 的"批量读取 + 偏移续读"模式。本操作通过统一入口 `airy_sys_rovol_ctl` 分派，`op=AIRY_ROVOL_LIST`。

```c
/**
 * @brief 列出 Agent 的所有快照（op-dispatch 入口）
 * @param op        必须为 AIRY_ROVOL_LIST
 * @param agent_id  目标 Agent ID
 * @param arg       指向 airy_rovol_list_args_t（infos_out + in_out_count）
 * @return 0 成功，<0 AIRY_E* 错误码
 *
 * @since v1.0.1
 * @par 借鉴来源:
...
typedef struct __attribute__((aligned(8))) airy_rovol_list_args {
    uint32_t size;                                  /* 结构体大小 */
    uint32_t version;
    airy_rovol_snapshot_info_t *infos_out;          /* 输出数组（调用方分配） */
    uint32_t *in_out_count;                         /* 输入：容量；输出：实际数量 */
    uint64_t reserved[2];
} airy_rovol_list_args_t;
```

### 4.4 `airy_sys_rovol_ctl(AIRY_ROVOL_DELETE)`（编号 549, op=DELETE）

删除指定快照。借鉴 seL4 `cteDelete()` 的"Zombie 中间态 → 最终清理"两阶段删除模式。本操作通过统一入口 `airy_sys_rovol_ctl` 分派，`op=AIRY_ROVOL_DELETE`。

```c
/**
 * @brief 删除指定快照（op-dispatch 入口）
 * @param op        必须为 AIRY_ROVOL_DELETE
 * @param agent_id  快照所属 Agent ID（用于 capability 校验）
 * @param arg       低 64 位为 snapshot_id（flags 当前保留，必须为 0）
 * @return 0 成功，<0 AIRY_E* 错误码
 *
 * @since v1.0.1
 * @par 借鉴来源:
...
### 5.1 `airy_sys_rovol_ctl(AIRY_ROVOL_RESTORE)`（编号 549, op=RESTORE）

从快照恢复 Agent 记忆。借鉴 Linux `mmap()` + `userfaultfd()` 的按需加载机制——恢复时不立即加载所有页，而是在访问时按需加载。本操作通过统一入口 `airy_sys_rovol_ctl` 分派，`op=AIRY_ROVOL_RESTORE`。

```c
/**
 * @brief 从快照恢复进程记忆（op-dispatch 入口）
 * @param op        必须为 AIRY_ROVOL_RESTORE
 * @param agent_id  目标 Agent ID（必须已存在）
 * @param arg       低 64 位为 snapshot_id（源快照 ID）
...
sequenceDiagram
    ...
    APP->>SNAP: 549 op=SNAPSHOT(agent_id=A) → S1
    ...
    APP->>DST1: 注册 Agent B
    APP->>DST1: 549 op=RESTORE(S1, agent_id=B)
    Note over DST1: Agent B 从 t=10s 状态启动
    APP->>DST1: Agent B 独立演化（不影响 A）

    APP->>DST2: 注册 Agent C
    APP->>DST2: 549 op=RESTORE(S1, agent_id=C)
...
### 6.1 `airy_sys_rovol_ctl(AIRY_ROVOL_MIGRATE)`（编号 549, op=MIGRATE）

跨节点迁移 Agent 记忆。借鉴 Linux CRIU + userfaultfd post-copy 迁移机制。本操作通过统一入口 `airy_sys_rovol_ctl` 分派，`op=AIRY_ROVOL_MIGRATE`。

```c
/**
 * @brief 跨节点迁移 Agent 记忆（op-dispatch 入口）
 * @param op        必须为 AIRY_ROVOL_MIGRATE
 * @param agent_id  目标 Agent ID
 * @param arg       指向 airy_migrate_args_t（target_node + policy）
...
typedef struct __attribute__((aligned(8))) airy_migrate_args {
    uint32_t size;                          /* 结构体大小 */
    uint32_t version;
    uint32_t target_node;                   /* 目标节点 ID */
    uint32_t reserved;
    airy_migrate_policy_t policy;           /* 迁移策略 */
...
### 7.1 `airy_sys_rovol_ctl(AIRY_ROVOL_TIER_SET)`（编号 549, op=TIER_SET）

设置 CXL 内存分层策略。借鉴 Linux 6.6 `mm/memory-tiers.c` 的分层抽象。本操作通过统一入口 `airy_sys_rovol_ctl` 分派，`op=AIRY_ROVOL_TIER_SET`。

```c
/**
 * @brief 设置 CXL 内存分层策略（op-dispatch 入口）
 * @param op        必须为 AIRY_ROVOL_TIER_SET
 * @param agent_id  目标 Agent ID
 * @param arg       指向 airy_cxl_tier_args_t（layer + policy）
 * @return 0 成功，<0 AIRY_E* 错误码
 *
 * @since v1.0.1
 * @par 借鉴来源:
 ...
 */
typedef struct __attribute__((aligned(8))) airy_cxl_tier_args {
    uint32_t size;                          /* 结构体大小 */
    uint32_t version;
    uint32_t layer;                         /* MemoryRovol 层级（1-4） */
    uint32_t reserved;
...
查询当前 CXL 分层策略。本操作通过统一入口 `airy_sys_rovol_ctl` 分派，`op=AIRY_ROVOL_TIER_GET`。

```c
/**
 * @brief 查询 CXL 分层策略（op-dispatch 入口）
 * @param op        必须为 AIRY_ROVOL_TIER_GET
 * @param agent_id  目标 Agent ID
 * @param arg       指向 airy_cxl_tier_policy_t *policy_out（输出指针）
 * @return 0 成功，<0 AIRY_E* 错误码
 *
 * @since v1.0.1
 * @par 借鉴来源:
...
### 8.1 `airy_sys_rovol_ctl(AIRY_ROVOL_MGLRU_CONFIG)`（编号 549, op=MGLRU_CONFIG）

配置 MGLRU 多代 LRU 回收策略。借鉴 Linux 6.6 `mm/vmscan.c` 的 MGLRU 参数。本操作通过统一入口 `airy_sys_rovol_ctl` 分派，`op=AIRY_ROVOL_MGLRU_CONFIG`。

```c
/**
 * @brief 配置 MGLRU 多代 LRU（op-dispatch 入口）
 * @param op        必须为 AIRY_ROVOL_MGLRU_CONFIG
 * @param agent_id  目标 Agent ID（0 = 全局配置）
 * @param arg       指向 airy_mglru_config_t（配置参数）
 * @return 0 成功，<0 AIRY_E* 错误码
 *
 * @since v1.0.1
 * @par 借鉴来源:
...
### 9.2 `airy_sys_rovol_ctl(AIRY_ROVOL_DEMOTE)`（编号 549, op=DEMOTE）

手动触发层级降级。借鉴 seL4 Untyped retype 的"降级为更原始类型"语义。本操作通过统一入口 `airy_sys_rovol_ctl` 分派，`op=AIRY_ROVOL_DEMOTE`。

```c
/**
 * @brief L1→L2→L3→L4 层级降级（op-dispatch 入口）
 * @param op        必须为 AIRY_ROVOL_DEMOTE
 * @param agent_id  目标 Agent ID（用于 capability 校验）
 * @param arg       指向 airy_rovol_tier_op_t（含 snapshot_id + from_layer + to_layer + decay_factor）
 * @return 0 成功，<0 AIRY_E* 错误码
 *
 * @since v1.0.1
 * @par 借鉴来源:
...
手动触发层级晋升。借鉴 seL4 capability mint 的"提升权限"语义。本操作通过统一入口 `airy_sys_rovol_ctl` 分派，`op=AIRY_ROVOL_PROMOTE`。

```c
/**
 * @brief L4→L3→L2→L1 层级晋升（op-dispatch 入口）
 * @param op        必须为 AIRY_ROVOL_PROMOTE
 * @param agent_id  目标 Agent ID（用于 capability 校验）
 * @param arg       指向 airy_rovol_tier_op_t（含 snapshot_id + from_layer + to_layer + decay_factor）
 * @return 0 成功，<0 AIRY_E* 错误码
 *
...
MemoryRovol API 的 6 项核心操作与 agentrt 用户态**操作模式同源（概念操作一致），函数签名因抽象层级不同而独立**。agentrt-linux 内核侧统一通过 `airy_sys_rovol_ctl` (549) + op-dispatch 入口承载：

| 序号 | agentrt 用户态 API | agentrt-linux 系统调用 | 语义 | 实现差异 |
|------|-------------------|----------------------|------|---------|
| 1 | `airy_rovol_snapshot()` | `airy_sys_rovol_ctl(AIRY_ROVOL_SNAPSHOT, ...)` | 创建快照 | 用户态 fork+COW / 内核 fork+userfaultfd |
| 2 | `airy_rovol_restore()` | `airy_sys_rovol_ctl(AIRY_ROVOL_RESTORE, ...)` | 恢复记忆 | 用户态 mmap / 内核 mmap+userfaultfd |
| 3 | `airy_rovol_migrate()` | `airy_sys_rovol_ctl(AIRY_ROVOL_MIGRATE, ...)` | 跨节点迁移 | 用户态迁移 / 内核 userfaultfd post-copy |
| 4 | `airy_rovol_compress()` | （封装在 549 op=SNAPSHOT 的 flags） | 压缩快照 | 用户态 zstd / 内核 zswap/zram |
| 5 | MGLRU aging | `airy_sys_rovol_ctl(AIRY_ROVOL_MGLRU_CONFIG, ...)` | 代际回收 | 用户态模拟 / 内核 `lru_gen_folio` |
| 6 | userfaultfd 处理 | （封装在 549 op=RESTORE/MIGRATE 内部） | 缺页处理 | 用户态 handler / 内核 uffd 框架 |
...
sequenceDiagram
    ...
    SDK->>SYSCALL: 549 airy_sys_rovol_ctl(AIRY_ROVOL_SNAPSHOT, ...)
    SYSCALL->>CAP: capability 守卫检查（按 op 查表）
    CAP-->>SYSCALL: AIRY_CAP_ROVOL_SNAPSHOT 通过
    SYSCALL->>ROVOL: op-dispatch 提交快照请求
...
from ctypes import c_uint32, c_uint64, POINTER, byref, cast, c_void_p
...
class MemoryRovolClient:
    ...
    def snapshot(self, agent_id: int, layers: int = 0x0F,
        ...
        info = SnapshotInfo()
        info._c.layer_mask = layers & 0xFFFF
        info._c.flags = flags & 0xFFFF
        ret = _libagentrt.airy_sys_rovol_ctl(
            c_uint32(AIRY_ROVOL_SNAPSHOT), c_uint32(agent_id),
            c_uint64(cast(byref(info._c), c_void_p).value))
        if ret < 0:
            raise AgentrtError(ret, f"snapshot failed for agent {agent_id}")
        return info

    def restore(self, snapshot_id: int, agent_id: int) -> None:
        """从快照恢复记忆（549, op=RESTORE）。"""
        ret = _libagentrt.airy_sys_rovol_ctl(
            c_uint32(AIRY_ROVOL_RESTORE), c_uint32(agent_id),
            c_uint64(snapshot_id))
        if ret < 0:
            raise AgentrtError(ret, f"restore failed for snapshot {snapshot_id}")

    def migrate(self, agent_id: int, target_node: int,
                policy: str = "hot") -> None:
        """跨节点迁移记忆（549, op=MIGRATE）。"""
        policy_map = {"cold": 0, "hot": 1, "increment": 2}
        if policy not in policy_map:
            raise ValueError(f"unknown policy: {policy}")
        args = self._make_migrate_args(target_node, policy_map[policy])
        ret = _libagentrt.airy_sys_rovol_ctl(
            c_uint32(AIRY_ROVOL_MIGRATE), c_uint32(agent_id),
            c_uint64(cast(byref(args), c_void_p).value))
        if ret < 0:
            raise AgentrtError(ret, f"migrate failed for agent {agent_id}")

    def list_snapshots(self, agent_id: int) -> list:
        """列出 Agent 的所有快照（549, op=LIST）。"""
        count = c_uint32(16)
        infos = (SnapshotInfo._c_type * 16)()
        list_args = self._make_list_args(infos, byref(count))
        ret = _libagentrt.airy_sys_rovol_ctl(
            c_uint32(AIRY_ROVOL_LIST), c_uint32(agent_id),
            c_uint64(cast(byref(list_args), c_void_p).value))
        if ret < 0:
    ...
    def delete(self, snapshot_id: int) -> None:
        """删除快照（549, op=DELETE）。"""
        ret = _libagentrt.airy_sys_rovol_ctl(
            c_uint32(AIRY_ROVOL_DELETE), c_uint32(0),
            c_uint64(snapshot_id))
        if ret < 0:
    ...
    def demote(self, snapshot_id: int, from_layer: int, to_layer: int,
        ...
        ret = _libagentrt.airy_sys_rovol_ctl(
            c_uint32(AIRY_ROVOL_DEMOTE), c_uint32(0),
            c_uint64(cast(byref(op), c_void_p).value))
        if ret < 0:
    ...
    def promote(self, snapshot_id: int, from_layer: int, to_layer: int) -> None:
        ...
        ret = _libagentrt.airy_sys_rovol_ctl(
            c_uint32(AIRY_ROVOL_PROMOTE), c_uint32(0),
            c_uint64(cast(byref(op), c_void_p).value))
        if ret < 0:
...
impl MemoryRovolClient {
    ...
    pub fn snapshot(agent_id: u32, layers: u32, compress: bool) -> Result<SnapshotInfo> {
        ...
        info.layer_mask = layers & 0xFFFF;
        info.flags = if compress { 0x01 } else { 0x00 };
        let ret = unsafe {
            airy_sys_rovol_ctl(AIRY_ROVOL_SNAPSHOT, agent_id,
    ...
    pub fn restore(snapshot_id: u64, agent_id: u32) -> Result<()> {
        let ret = unsafe {
            airy_sys_rovol_ctl(AIRY_ROVOL_RESTORE, agent_id, snapshot_id)
        };
        if ret < 0 {
            return Err(AgentrtError::from_errno(ret));
        }
    ...
    pub fn migrate(agent_id: u32, target_node: u32, policy: MigratePolicy) -> Result<()> {
        let mut args = MigrateArgs::new(target_node, policy);
        let ret = unsafe {
            airy_sys_rovol_ctl(AIRY_ROVOL_MIGRATE, agent_id,
...
// Snapshot 创建 Agent 记忆快照（549, op=SNAPSHOT）
func (c *MemoryRovolClient) Snapshot(agentID uint32, layers uint32, compress bool) (*SnapshotInfo, error) {
    ...
    info.version = 0x0100
    info.layer_mask = C.uint32_t(layers & 0xFFFF)
    if compress {
        info.flags = 0x01
    }
    ret := C.airy_sys_rovol_ctl(C.uint32_t(OpSnapshot),
        C.uint32_t(agentID), C.uint64_t(uintptr(unsafe.Pointer(&info))))
    if ret < 0 {
...
// Restore 从快照恢复记忆（549, op=RESTORE）
func (c *MemoryRovolClient) Restore(snapshotID uint64, agentID uint32) error {
    ret := C.airy_sys_rovol_ctl(C.uint32_t(OpRestore),
        C.uint32_t(agentID), C.uint64_t(snapshotID))
    if ret < 0 {
...
// Migrate 跨节点迁移记忆（549, op=MIGRATE）
func (c *MemoryRovolClient) Migrate(agentID, targetNode uint32, policy MigratePolicy) error {
    args := NewMigrateArgs(targetNode, policy)
    ret := C.airy_sys_rovol_ctl(C.uint32_t(OpMigrate),
        C.uint32_t(agentID), C.uint64_t(uintptr(unsafe.Pointer(&args))))
    if ret < 0 {
...
export class MemoryRovolClient {
    ...
    async snapshot(agentId: number, layers: number = 0x0F, compress: boolean = false): Promise<SnapshotInfo> {
        ...
        info.layer_mask = layers & 0xFFFF;
        info.flags = compress ? 0x01 : 0x00;
        const ret = await bindings.airy_sys_rovol_ctl(OP_SNAPSHOT, agentId, info.ptr());
        if (ret < 0) {
    ...
    async restore(snapshotId: number, agentId: number): Promise<void> {
        const ret = await bindings.airy_sys_rovol_ctl(OP_RESTORE, agentId, snapshotId);
        if (ret < 0) {
    ...
    async migrate(agentId: number, targetNode: number, policy: 'cold' | 'hot' | 'increment' = 'hot'): Promise<void> {
        ...
        const argsPtr = makeMigrateArgsPtr(targetNode, policyMap[policy]);
        const ret = await bindings.airy_sys_rovol_ctl(OP_MIGRATE, agentId, argsPtr);
        if (ret < 0) {
...
| 操作 | 目标延迟 | P99 延迟 | 测量方法 |
|------|---------|---------|---------|
| 549 op=SNAPSHOT（小，< 100MB） | < 50ms | < 100ms | `perf trace -e airy_sys_rovol_ctl` |
| 549 op=SNAPSHOT（大，~1GB） | < 500ms | < 1s | COW 延迟复制 |
| 549 op=RESTORE（首次） | < 100ms | < 200ms | 仅加载热页 |
| 549 op=RESTORE（冷页访问） | < 5ms | < 10ms | userfaultfd 缺页 |
| 549 op=MIGRATE（冷迁移，1GB） | < 30s | < 60s | 全量传输 |
| 549 op=MIGRATE（热迁移停顿） | < 100ms | < 200ms | 仅最终切换 |
| 549 op=TIER_SET | < 1ms | < 5ms | sysfs 写入 |
| 549 op=MGLRU_CONFIG | < 1ms | < 5ms | sysfs 写入 |
| 549 op=LIST | < 1ms | < 5ms | 红黑树遍历 |
| 549 op=DELETE（小） | < 10ms | < 50ms | 同步阶段 |
| 549 op=DELETE（大，> 1GB） | < 1s | < 5s | 异步回收 |
| 549 op=DEMOTE | < 100ms | < 500ms | 特征提取 |
| 549 op=PROMOTE | < 50ms | < 100ms | 权重重置 |
 ...
 * @brief 并发安全保证
 *
 * MemoryRovol API 提供以下并发保证（按 op 码区分）：
 *
 * 1. 快照创建（549 op=SNAPSHOT）：与 Agent 运行并发，COW 保证一致性
 * 2. 快照恢复（549 op=RESTORE）：与快照创建互斥（同一 Agent）
 * 3. 跨节点迁移（549 op=MIGRATE）：与快照操作互斥（同一 Agent）
 * 4. CXL 分层设置（549 op=TIER_SET）：与 MGLRU 配置互斥（同一 Agent）
 * 5. 列表/删除（549 op=LIST/DELETE）：只读操作可并发
 *
 * 互斥通过 per-agent 读写锁实现：
 * - 读操作（LIST/TIER_GET）：共享锁
...
int basic_snapshot_restore_example(uint32_t agent_id)
{
    ...
    info.layer_mask = AIRY_ROVOL_LAYER_ALL;
    info.flags      = AIRY_ROVOL_FLAG_COMPRESS;
    ret = airy_sys_rovol_ctl(AIRY_ROVOL_SNAPSHOT, agent_id,
                                 (uint64_t)(uintptr_t)&info);
    if (ret < 0) {
    ...
    uint32_t count = 16;
    airy_rovol_list_args_t list_args = {
        .size = sizeof(list_args),
        .version = 0x0100,
        .infos_out = infos,
        .in_out_count = &count,
    };
    ret = airy_sys_rovol_ctl(AIRY_ROVOL_LIST, agent_id,
                                 (uint64_t)(uintptr_t)&list_args);
    if (ret < 0) {
    ...
    ret = airy_sys_rovol_ctl(AIRY_ROVOL_RESTORE, new_agent_id,
                                 (uint64_t)info.snapshot_id);
    if (ret < 0) {
    ...
    /* 4. 删除快照 */
    ret = airy_sys_rovol_ctl(AIRY_ROVOL_DELETE, agent_id,
                                 (uint64_t)info.snapshot_id);
    if (ret < 0) {
...
{
    int ret;
    airy_migrate_args_t args = {
        .size = sizeof(args),
        .version = 0x0100,
        .target_node = 2,
        .policy = AIRY_MIGRATE_HOT,
    };
...
```c
int tier_migration_example(uint32_t agent_id, uint64_t snapshot_id)
{
    int ret;
    ...
    ret = airy_sys_rovol_ctl(AIRY_ROVOL_DEMOTE, agent_id,
    ...
    ret = airy_sys_rovol_ctl(AIRY_ROVOL_PROMOTE, agent_id,
                                 (uint64_t)(uintptr_t)&op);
    if (ret < 0) {
...
static void test_snapshot_basic(struct kunit *test)
{
    ...
    /* 清理 */
    airy_sys_rovol_ctl(AIRY_ROVOL_DELETE, agent_id,
                           (uint64_t)info.snapshot_id);
}
...
{
    /* agent_id 不存在 */
    int ret = airy_sys_rovol_ctl(AIRY_ROVOL_SNAPSHOT, 0xFFFF, 0ULL);
    KUNIT_EXPECT_EQ(test, ret, -AIRY_EINVAL);

    /* layer_mask 为 0 */
    airy_rovol_snapshot_info_t info = { .size = sizeof(info) };
    info.layer_mask = 0;
    ret = airy_sys_rovol_ctl(AIRY_ROVOL_SNAPSHOT, 1,
                                 (uint64_t)(uintptr_t)&info);
    KUNIT_EXPECT_EQ(test, ret, -AIRY_EINVAL);
}
...
static void test_snapshot_restore_roundtrip(struct kunit *test)
{
    ...
    airy_rovol_snapshot_info_t info = { .size = sizeof(info) };
    info.layer_mask = AIRY_ROVOL_LAYER_ALL;
    KUNIT_EXPECT_EQ(test, airy_sys_rovol_ctl(AIRY_ROVOL_SNAPSHOT,
        src_agent, (uint64_t)(uintptr_t)&info), 0);

    /* 恢复到目标 Agent */
    KUNIT_EXPECT_EQ(test, airy_sys_rovol_ctl(AIRY_ROVOL_RESTORE,
        dst_agent, (uint64_t)info.snapshot_id), 0);
...
# 1. 快照延迟测试（不同大小）—— op=SNAPSHOT 通过 perf trace 过滤
for size in 10MB 100MB 1GB 10GB; do
    echo "=== Snapshot latency (size=$size) ==="
    perf stat -e airy_sys_rovol_ctl \
...
| `AIRY_ROVOL_RESTORE` | `AIRY_CAP_ROVOL_RESTORE` | 恢复记忆到目标 Agent |
| `AIRY_ROVOL_MIGRATE` | `AIRY_CAP_ROVOL_MIGRATE` | 跨节点迁移 |
| `AIRY_ROVOL_TIER_SET` | `AIRY_CAP_CXL_TIER` | 修改 CXL 分层 |
| `AIRY_ROVOL_TIER_GET` | `AIRY_CAP_ROVOL_LIST` | 查询分层（较弱权限） |
| `AIRY_ROVOL_MGLRU_CONFIG` | `AIRY_CAP_MGLRU_CONFIG` | 修改 MGLRU 配置 |
| `AIRY_ROVOL_LIST` | `AIRY_CAP_ROVOL_LIST` | 列出快照（较弱权限） |
| `AIRY_ROVOL_DELETE` | `AIRY_CAP_ROVOL_DELETE` | 删除快照 |
| `AIRY_ROVOL_DEMOTE` | `AIRY_CAP_ROVOL_DEMOTE` | 层级降级 |
| `AIRY_ROVOL_PROMOTE` | `AIRY_CAP_ROVOL_PROMOTE` | 层级晋升 |
...
|------|-------------------------|
| **K-1 内核极简** | 内核仅提供 `airy_sys_rovol_ctl` (549) op-dispatch 入口与 snapshot/restore/migrate 机制，遗忘策略在用户态 |
| **K-2 接口契约化** | 单一 `airy_sys_rovol_ctl` (549) + 10 个 op 码通过 C 头文件 + Doxygen 给出显式契约 |
...
| v1.0.1 | 2026-07-22 | 采用 Capability Folding 架构：原 0.1.1 的 10 个独立 MemoryRovol syscall（552-561）合并为单一 `airy_sys_rovol_ctl` (549) + op-dispatch 模型；重写 §2.1/§4.2-§9 全部 API 签名为 op-dispatch 形式；同步更新 §2.2 分类图、§4.1 状态机、§5.2/§6.2/§11.4 序列图、§11.2 同源映射表、§12 四语言 SDK、§13 性能表、§14 使用示例、§15 测试、§16 capability 表 | 工程规范委员会 |

---

> **文档结束** | v1.0.1 实现方案文档 | IRON-9 v3 同源且部分代码共享 | 存用分离 + 零拷贝优先 + 可回滚 + 跨节点一致 + 遗忘可控
