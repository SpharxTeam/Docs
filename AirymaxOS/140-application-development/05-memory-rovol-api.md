...
MemoryRovol 是 agentrt-linux 区别于通用操作系统的核心特征——它将"记忆"作为一等资源进行卷载管理，而非传统的文件系统或数据库存储。MemoryRovol 的设计已分散在 [02-memory-flow.md](../40-dataflows/02-memory-flow.md)（数据流）、[04-memory.md](../20-modules/04-memory.md)（子仓设计）、[syscall_api_contract.md](../50-engineering-standards/20-contracts/syscall_api_contract.md) 第 7 章（系统调用编号）中。
...
| 快照生命周期不明 | 仅 syscall_api_contract.md 第 7.2 节提及"快照 ID"概念 | 定义完整 7 状态生命周期状态机 |
...
| 错误码未对齐 | memory-flow 与 syscalls 错误码命名混用 | 统一为 `AGENTRT_E*` 错误码表 |
...
| [syscall_api_contract.md](../50-engineering-standards/20-contracts/syscall_api_contract.md) | 第 7 章编号定义的扩展——本契约补全至 10 个调用 |
...
| **IRON-9 v2 同源且部分代码共享** | [SC] L1-L4 数据结构 + [SS] 6 项 API 签名 + [IND] 内核态实现 |
...
|------|--------|--------|------|--------|---------|
| 552 | `AGENTRT_SYS_ROVOL_SNAPSHOT` | `agentrt_sys_rovol_snapshot` | 创建进程记忆快照 | 2 | 0.1.1 |
| 553 | `AGENTRT_SYS_ROVOL_RESTORE` | `agentrt_sys_rovol_restore` | 从快照恢复记忆 | 2 | 0.1.1 |
| 554 | `AGENTRT_SYS_ROVOL_MIGRATE` | `agentrt_sys_rovol_migrate` | 跨节点记忆迁移 | 3 | 0.1.1 |
| 555 | `AGENTRT_SYS_CXL_TIER_SET` | `agentrt_sys_cxl_tier_set` | 设置 CXL 内存分层策略 | 3 | 0.1.1 |
| 556 | `AGENTRT_SYS_MGLRU_CONFIG` | `agentrt_sys_mglru_config` | 配置 MGLRU 多代 LRU | 2 | 0.1.1 |
| 557 | `AGENTRT_SYS_ROVOL_LIST` | `agentrt_sys_rovol_list` | 列出进程的所有快照 | 3 | 1.0.1 |
| 558 | `AGENTRT_SYS_ROVOL_DELETE` | `agentrt_sys_rovol_delete` | 删除指定快照 | 2 | 1.0.1 |
| 559 | `AGENTRT_SYS_ROVOL_DEMOTE` | `agentrt_sys_rovol_demote` | L1→L2→L3→L4 层级降级 | 3 | 1.0.1 |
| 560 | `AGENTRT_SYS_ROVOL_PROMOTE` | `agentrt_sys_rovol_promote` | L4→L3→L2→L1 层级晋升 | 3 | 1.0.1 |
| 561 | `AGENTRT_SYS_CXL_TIER_GET` | `agentrt_sys_cxl_tier_get` | 查询 CXL 分层策略 | 2 | 1.0.1 |
| 562-571 | — | — | 预留扩展空间 | — | — |
...
遵循 [syscall_api_contract.md](../50-engineering-standards/20-contracts/syscall_api_contract.md) 第 3 章的 System V AMD64 ABI 约定：

- 单个系统调用最多 6 个参数
- 超过 3 个参数时使用结构体指针封装
- 结构体首字段为 `uint32_t size`（用于版本协商）
- 所有指针参数经 `agentrt_validate_user_ptr()` 验证
...
typedef struct __attribute__((aligned(64))) agentrt_l2_feature {
    ...
    uint32_t dim;              /* 默认 768 */
    airymax_q16_t weight;      /* Q16.16 权重（受遗忘机制影响） */
    airymax_q16_t vector[768]; /* Q16.16 定点向量 */
...
typedef struct agentrt_l4_barcode {
    ...
    airymax_q16_t birth;      /* birth 阈值 */
    airymax_q16_t death;      /* death 阈值（INT32_MAX = 永生） */
    airymax_q16_t persistence;
 ...
 * 默认（值为 0）表示全部 4 层参与。
 */
#define AGENTRT_ROVOL_LAYER_L1   0x01u   /* 原始卷 */
#define AGENTRT_ROVOL_LAYER_L2   0x02u   /* 特征层 */
#define AGENTRT_ROVOL_LAYER_L3   0x04u   /* 结构层 */
#define AGENTRT_ROVOL_LAYER_L4   0x08u   /* 模式层 */
#define AGENTRT_ROVOL_LAYER_ALL  0x0Fu   /* 全部 4 层 */

/**
 * @brief 快照标志位
 * @since 1.0.1
 */
#define AGENTRT_ROVOL_FLAG_COMPRESS    0x01u   /* 启用 zstd 压缩 */
#define AGENTRT_ROVOL_FLAG_ENCRYPT     0x02u   /* 启用 TEE 加密 */
#define AGENTRT_ROVOL_FLAG_CHECKPOINT  0x04u   /* 标记为检查点（用于回滚） */
#define AGENTRT_ROVOL_FLAG_MIGRATABLE  0x08u   /* 允许后续跨节点迁移 */

/**
 * @brief 快照元数据——每次 snapshot 返回的描述符
 * @since 1.0.1
 *
 * 此结构由内核填充，应用通过 agentrt_sys_rovol_list 获取数组。
 * 借鉴 Linux 6.6 struct stat 的"不可变字段 + 保留字段"模式。
 */
typedef struct __attribute__((aligned(8))) agentrt_rovol_snapshot_info {
    ...
    uint32_t flags;             /* AGENTRT_ROVOL_FLAG_* 标志位 */
 ...
 */
typedef enum {
    AGENTRT_ROVOL_STATE_CREATING    = 0,   /* 创建中（fork + COW 进行中） */
    AGENTRT_ROVOL_STATE_ACTIVE      = 1,   /* 活跃（可用于 restore） */
    AGENTRT_ROVOL_STATE_MIGRATING   = 2,   /* 迁移中（跨节点传输） */
    AGENTRT_ROVOL_STATE_RESTORING   = 3,   /* 恢复中（mmap + userfaultfd） */
    AGENTRT_ROVOL_STATE_DELETING    = 4,   /* 删除中（资源回收） */
    AGENTRT_ROVOL_STATE_DELETED     = 5,   /* 已删除（终态） */
    AGENTRT_ROVOL_STATE_CORRUPTED   = 6,   /* 损坏（SHA-256 校验失败） */
} agentrt_rovol_state_t;
 ...
 */
typedef enum {
    AGENTRT_CXL_TIER_FAST     = 0,   /* 仅 DRAM（热数据） */
    AGENTRT_CXL_TIER_BALANCED = 1,   /* DRAM + CXL（温数据） */
    AGENTRT_CXL_TIER_COLD     = 2,   /* CXL + PMEM（冷数据） */
    AGENTRT_CXL_TIER_ARCHIVE  = 3,   /* PMEM + SSD（归档） */
} agentrt_cxl_tier_policy_t;

/**
 * @brief MGLRU 配置参数
 * @since 1.0.1
 */
typedef struct __attribute__((aligned(8))) agentrt_mglru_config {
    uint32_t size;              /* 结构体大小 */
    uint32_t version;
 ...
 */
typedef struct __attribute__((aligned(8))) agentrt_rovol_tier_op {
    uint32_t size;
    uint32_t version;
    uint64_t snapshot_id;       /* 目标快照 */
    uint32_t from_layer;       /* 源层级（1-4） */
    uint32_t to_layer;          /* 目标层级（1-4） */
    airymax_q16_t decay_factor; /* 衰减系数（Q16.16，艾宾浩斯曲线参数） */
    uint64_t reserved[3];
} agentrt_rovol_tier_op_t;
...
>
> **本契约方案**：所有浮点字段（`weight`、`decay_factor`、`birth`、`death`）统一使用 `airymax_q16_t`（int32_t）Q16.16 定点数。用户态需 float 展示时用 `AIRYMAX_Q16_TO_F()` 转换。

```c
/* Q16.16 转换辅助宏（仅用户态使用 TO_F） */
#define AIRYMAX_Q16_ONE      (1 << 16)                    /* 1.0 */
#define AIRYMAX_Q16_FLOAT(f) ((airymax_q16_t)((f) * AIRYMAX_Q16_ONE))
#define AIRYMAX_Q16_TO_F(x)  ((float)(x) / AIRYMAX_Q16_ONE)

/* 艾宾浩斯衰减系数常用值 */
#define AGENTRT_DECAY_EBBINGHAUS  0x8000    /* 0.5 Q16.16（默认艾宾浩斯系数） */
#define AGENTRT_DECAY_LINEAR      0x10000   /* 1.0 Q16.16（线性衰减） */
#define AGENTRT_DECAY_AGGRESSIVE  0x4000    /* 0.25 Q16.16（激进衰减） */
```
...
### 4.2 `agentrt_sys_rovol_snapshot`（编号 552）
 ...
 * @return 0 成功（info_out 已填充），<0 AGENTRT_E* 错误码
 *
 * @since 0.1.1
 * @par Example:
 * @code
 *   agentrt_rovol_snapshot_info_t info = { .size = sizeof(info), .version = 0x0100 };
 *   int ret = agentrt_sys_rovol_snapshot(agent_id,
 *       AGENTRT_ROVOL_LAYER_ALL | AGENTRT_ROVOL_FLAG_COMPRESS,
 *       &info);
 *   if (ret < 0) { ... }
 * @endcode
 *
 ...
 *
 * @par 实现细节:
 * 1. capability 守卫：检查 AGENTRT_CAP_ROVOL_SNAPSHOT 权限
 ...
 */
AGENTRT_API int agentrt_sys_rovol_snapshot(uint32_t agent_id,
                                           uint32_t options,
                                           agentrt_rovol_snapshot_info_t *info_out);
```
...
| `options` | `uint32_t` | 低 16 位为 `AGENTRT_ROVOL_LAYER_*`，高 16 位为 `AGENTRT_ROVOL_FLAG_*` | 至少包含一层 |
| `info_out` | `agentrt_rovol_snapshot_info_t *` | 快照元数据输出 | 调用方分配，`size` 字段必须已设置 |
...
| `-AGENTRT_EINVAL` | 参数无效 | `agent_id` 不存在、`options` 为 0、`info_out` 为 NULL |
| `-AGENTRT_EPERM` | 权限不足 | 缺少 `AGENTRT_CAP_ROVOL_SNAPSHOT` capability |
| `-AGENTRT_ECONFLICT` | 状态冲突 | Agent 不在 RUNNING/PAUSED 状态 |
| `-AGENTRT_ENOMEM` | 内存不足 | PMEM/CXL 池耗尽 |
| `-AGENTRT_EBUSY` | 资源繁忙 | Agent 正在迁移，无法快照 |

### 4.3 `agentrt_sys_rovol_list`（编号 557）
 ...
 *
 * @par 实现细节:
 * 1. capability 守卫：检查 AGENTRT_CAP_ROVOL_LIST 权限
 ...
 */
AGENTRT_API int agentrt_sys_rovol_list(uint32_t agent_id,
                                       agentrt_rovol_snapshot_info_t *infos_out,
                                       uint32_t *in_out_count);
```

### 4.4 `agentrt_sys_rovol_delete`（编号 558）
 ...
 *
 * @par 不可逆性:
 * 删除后 L1 原始卷也回收（除非快照标记了 AGENTRT_ROVOL_FLAG_CHECKPOINT）。
 * CHECKPOINT 快照的 L1 保留，可重新提取特征。
 */
AGENTRT_API int agentrt_sys_rovol_delete(uint64_t snapshot_id,
                                         uint32_t flags);
```
...
### 5.1 `agentrt_sys_rovol_restore`（编号 553）
 ...
 * @return 0 成功（Agent 进入 RESTORING → RUNNING），<0 AGENTRT_E* 错误码
 *
 * @since 0.1.1
 * @par 借鉴来源:
 ...
 *
 * @par 恢复流程:
 * 1. capability 守卫：检查 AGENTRT_CAP_ROVOL_RESTORE 权限
 ...
 * - 完整恢复：后台异步，不阻塞应用
 */
AGENTRT_API int agentrt_sys_rovol_restore(uint64_t snapshot_id,
                                          uint32_t agent_id);
```
...
### 6.1 `agentrt_sys_rovol_migrate`（编号 554）
 ...
 * @param policy        迁移策略
 * @return 0 成功（迁移在后台进行），<0 AGENTRT_E* 错误码
 *
 * @since 0.1.1
 * @par 借鉴来源:
 ...
 */
typedef enum {
    AGENTRT_MIGRATE_COLD     = 0,   /* 冷迁移：暂停 Agent，全量传输 */
    AGENTRT_MIGRATE_HOT      = 1,   /* 热迁移：post-copy，先切后传 */
    AGENTRT_MIGRATE_INCREMENT= 2,   /* 增量迁移：dirty page tracking */
} agentrt_migrate_policy_t;
/**
 * @par 8 步迁移协议（post-copy 热迁移）:
 * 1. capability 守卫：检查 AGENTRT_CAP_ROVOL_MIGRATE 权限
 ...
 */
AGENTRT_API int agentrt_sys_rovol_migrate(uint32_t agent_id,
                                          uint32_t target_node,
                                          agentrt_migrate_policy_t policy);
```
...
### 7.1 `agentrt_sys_cxl_tier_set`（编号 555）
 ...
 */
AGENTRT_API int agentrt_sys_cxl_tier_set(uint32_t agent_id,
                                         uint32_t layer,
                                         agentrt_cxl_tier_policy_t policy);
```

### 7.2 `agentrt_sys_cxl_tier_get`（编号 561）
 ...
 */
AGENTRT_API int agentrt_sys_cxl_tier_get(uint32_t agent_id,
                                         agentrt_cxl_tier_policy_t *policy_out);
```
...
### 8.1 `agentrt_sys_mglru_config`（编号 556）
 ...
 */
AGENTRT_API int agentrt_sys_mglru_config(uint32_t agent_id,
                                         const agentrt_mglru_config_t *config);
```
 ...
 * - AGENTRT_DECAY_EBBINGHAUS (0.5)：标准艾宾浩斯曲线
 * - AGENTRT_DECAY_LINEAR (1.0)：线性衰减（更快遗忘）
 * - AGENTRT_DECAY_AGGRESSIVE (0.25)：激进衰减（用于内存压力场景）
 */
AGENTRT_API int agentrt_sys_rovol_demote(uint64_t snapshot_id,
                                         const agentrt_rovol_tier_op_t *op,
                                         uint32_t flags);
```

### 9.3 `agentrt_sys_rovol_promote`（编号 560）
 ...
 * 相当于"重新学习"该记忆。
 */
AGENTRT_API int agentrt_sys_rovol_promote(uint64_t snapshot_id,
                                          const agentrt_rovol_tier_op_t *op,
                                          uint32_t flags);
```
...
MemoryRovol API 复用 [syscall_api_contract.md](../50-engineering-standards/20-contracts/syscall_api_contract.md) 第 4.2 节的通用错误码，并新增 3 个 MemoryRovol 专用错误码：

| 错误码 | 值 | 含义 | 典型触发场景 | 可重试 |
|--------|-----|------|-------------|--------|
| `AGENTRT_EOK` | 0 | 成功 | 调用成功 | - |
| `AGENTRT_EINVAL` | -1 | 参数无效 | `agent_id` 不存在、`options` 为 0 | 否 |
| `AGENTRT_ENOMEM` | -2 | 内存不足 | PMEM/CXL 池耗尽 | 是（等待后） |
| `AGENTRT_ENOSYS` | -3 | 未实现 | 编号未实现或已废弃 | 否 |
| `AGENTRT_EPERM` | -4 | 权限不足 | capability 令牌缺失 | 否 |
| `AGENTRT_ENOENT` | -5 | 资源不存在 | `snapshot_id` 不存在 | 否 |
| `AGENTRT_EAGAIN` | -6 | 暂时不可用 | CXL 带宽不足 | 是（立即） |
| `AGENTRT_EMSGSIZE` | -7 | 消息过大 | 快照超过最大限制 | 否 |
| `AGENTRT_EBUSY` | -9 | 资源繁忙 | Agent 正在迁移 | 是（延迟） |
| `AGENTRT_ENOTSUP` | -10 | 不支持 | 无 CXL 设备、PMEM 未配置 | 否 |
| `AGENTRT_ETIMEOUT` | -11 | 超时 | 迁移超时、userfaultfd 超时 | 是（限制次数） |
| `AGENTRT_ECONFLICT` | -12 | 状态冲突 | 快照非 ACTIVE 状态、Agent 非 RUNNING | 否 |
| `AGENTRT_EFAULT` | -13 | 地址错误 | 用户态指针非法 | 否 |
| **`AGENTRT_ECORRUPTED`** | **-15** | **快照损坏** | **SHA-256 校验失败** | **否** |
| **`AGENTRT_ETIER`** | **-16** | **层级非法** | **demote/promote 层级越界** | **否** |
| **`AGENTRT_EMIGRATE`** | **-17** | **迁移失败** | **目标节点不可达、CXL 池故障** | **是（限制次数）** |
...
/**
 * @brief 错误码转人类可读字符串
 * @param err 错误码（AGENTRT_E*）
 * @return 静态字符串指针（无需释放）
 *
 * @since 1.0.1
 */
AGENTRT_API const char *agentrt_strerror(int err);
```
...
3. **审计日志**：`-AGENTRT_EPERM`（权限不足）和 `-AGENTRT_ECORRUPTED`（损坏）必须记录审计日志
...
|--------------|-----------|------------------|
| `agentrt_l1_record_t` | `include/airymax/memory_types.h` | snapshot/restore 的 L1 数据结构 |
| `agentrt_l2_feature_t` | `include/airymax/memory_types.h` | snapshot/restore 的 L2 数据结构 |
| `agentrt_l3_node_t` / `agentrt_l3_edge_t` | `include/airymax/memory_types.h` | snapshot/restore 的 L3 数据结构 |
| `agentrt_l4_barcode_t` | `include/airymax/memory_types.h` | snapshot/restore 的 L4 数据结构 |
| `AGENTRT_GFP_*` 宏 | `include/airymax/memory_types.h` | 内核分配标志语义 |
| `agentrt_pmem_flush_fn` | `include/airymax/memory_types.h` | PMEM 持久化接口 |
| `airymax_q16_t` + 转换宏 | `include/airymax/memory_types.h` | 定点数算术（内核态禁用 float） |

### 11.2 [SS] 语义同源层

MemoryRovol API 的 6 项核心操作与 agentrt 用户态**API 签名同源，实现独立**：

| 序号 | agentrt 用户态 API | agentrt-linux 系统调用 | 语义 | 实现差异 |
|------|-------------------|----------------------|------|---------|
| 1 | `agentrt_rovol_snapshot()` | `agentrt_sys_rovol_snapshot` | 创建快照 | 用户态 fork+COW / 内核 fork+userfaultfd |
| 2 | `agentrt_rovol_restore()` | `agentrt_sys_rovol_restore` | 恢复记忆 | 用户态 mmap / 内核 mmap+userfaultfd |
| 3 | `agentrt_rovol_migrate()` | `agentrt_sys_rovol_migrate` | 跨节点迁移 | 用户态迁移 / 内核 userfaultfd post-copy |
| 4 | `agentrt_rovol_compress()` | （封装在 552 flags） | 压缩快照 | 用户态 zstd / 内核 zswap/zram |
| 5 | MGLRU aging | `agentrt_sys_mglru_config` | 代际回收 | 用户态模拟 / 内核 `lru_gen_folio` |
...
sequenceDiagram
    ...
    SDK->>SYSCALL: 552 agentrt_sys_rovol_snapshot()
    SYSCALL->>CAP: capability 守卫检查
    CAP-->>SYSCALL: AGENTRT_CAP_ROVOL_SNAPSHOT 通过
...
class MemoryRovolClient:
    ...
    def snapshot(self, agent_id: int, layers: int = 0x0F,
        ...
        :return: 快照元数据
        :raises AgentrtError: 失败时抛出，errno 为 AGENTRT_E*
        """
        flags = 0x01 if compress else 0x00
        options = (layers & 0xFFFF) | ((flags & 0xFFFF) << 16)
        info = SnapshotInfo()
        ret = _libagentrt.agentrt_sys_rovol_snapshot(
    ...
    def restore(self, snapshot_id: int, agent_id: int) -> None:
        """从快照恢复记忆（系统调用 553）。"""
        ret = _libagentrt.agentrt_sys_rovol_restore(
    ...
    def migrate(self, agent_id: int, target_node: int,
        ...
        ret = _libagentrt.agentrt_sys_rovol_migrate(
    ...
    def list_snapshots(self, agent_id: int) -> list:
        ...
        ret = _libagentrt.agentrt_sys_rovol_list(
    ...
    def delete(self, snapshot_id: int) -> None:
        """删除快照（系统调用 558）。"""
        ret = _libagentrt.agentrt_sys_rovol_delete(
    ...
    def demote(self, snapshot_id: int, from_layer: int, to_layer: int,
        ...
        ret = _libagentrt.agentrt_sys_rovol_demote(
    ...
    def promote(self, snapshot_id: int, from_layer: int, to_layer: int) -> None:
        ...
        ret = _libagentrt.agentrt_sys_rovol_promote(
            c_uint64(snapshot_id), byref(op), c_uint32(0))
        if ret < 0:
...
impl MemoryRovolClient {
    ...
    pub fn snapshot(agent_id: u32, layers: u32, compress: bool) -> Result<SnapshotInfo> {
        ...
        let ret = unsafe {
            agentrt_sys_rovol_snapshot(agent_id, options, &mut info as *mut _)
        };
        if ret < 0 {
    ...
    pub fn restore(snapshot_id: u64, agent_id: u32) -> Result<()> {
        let ret = unsafe {
            agentrt_sys_rovol_restore(snapshot_id, agent_id)
        };
        if ret < 0 {
            return Err(AgentrtError::from_errno(ret));
        }
    ...
    pub fn migrate(agent_id: u32, target_node: u32, policy: MigratePolicy) -> Result<()> {
        let ret = unsafe {
            agentrt_sys_rovol_migrate(agent_id, target_node, policy as u32)
        };
        if ret < 0 {
            return Err(AgentrtError::from_errno(ret));
        }
...
func (c *MemoryRovolClient) Snapshot(agentID uint32, layers uint32, compress bool) (*SnapshotInfo, error) {
    ...
    options := (layers & 0xFFFF) | ((flags & 0xFFFF) << 16)
    var info C.agentrt_rovol_snapshot_info_t
    info.size = C.uint32_t(unsafe.Sizeof(info))
    info.version = 0x0100
    ret := C.agentrt_sys_rovol_snapshot(C.uint32_t(agentID), C.uint32_t(options), &info)
    if ret < 0 {
...
func (c *MemoryRovolClient) Restore(snapshotID uint64, agentID uint32) error {
    ret := C.agentrt_sys_rovol_restore(C.uint64_t(snapshotID), C.uint32_t(agentID))
    if ret < 0 {
...
func (c *MemoryRovolClient) Migrate(agentID, targetNode uint32, policy MigratePolicy) error {
    ret := C.agentrt_sys_rovol_migrate(C.uint32_t(agentID), C.uint32_t(targetNode), C.uint32_t(policy))
    if ret < 0 {
...
export class MemoryRovolClient {
    ...
    async snapshot(agentId: number, layers: number = 0x0F, compress: boolean = false): Promise<SnapshotInfo> {
        ...
        const ret = await bindings.agentrt_sys_rovol_snapshot(agentId, options, info);
        if (ret < 0) {
    ...
    async restore(snapshotId: number, agentId: number): Promise<void> {
        const ret = await bindings.agentrt_sys_rovol_restore(snapshotId, agentId);
        if (ret < 0) {
    ...
    async migrate(agentId: number, targetNode: number, policy: 'cold' | 'hot' | 'increment' = 'hot'): Promise<void> {
        ...
        const ret = await bindings.agentrt_sys_rovol_migrate(agentId, targetNode, policyMap[policy]);
        if (ret < 0) {
...
| 操作 | 目标延迟 | P99 延迟 | 测量方法 |
|------|---------|---------|---------|
| 552 snapshot（小，< 100MB） | < 50ms | < 100ms | `perf trace -e agentrt_sys_rovol_snapshot` |
...
{
    int ret;
    agentrt_rovol_snapshot_info_t info = {
        .size = sizeof(info),
        .version = 0x0100,
    };

    /* 1. 创建快照（全部 4 层 + 压缩） */
    uint32_t options = AGENTRT_ROVOL_LAYER_ALL | (AGENTRT_ROVOL_FLAG_COMPRESS << 16);
    ret = agentrt_sys_rovol_snapshot(agent_id, options, &info);
    if (ret < 0) {
    ...
    /* 2. 列出快照 */
    agentrt_rovol_snapshot_info_t infos[16];
    uint32_t count = 16;
    ret = agentrt_sys_rovol_list(agent_id, infos, &count);
    if (ret < 0) {
    ...
    ret = agentrt_sys_rovol_restore(info.snapshot_id, new_agent_id);
    if (ret < 0) {
    ...
    /* 4. 删除快照 */
    ret = agentrt_sys_rovol_delete(info.snapshot_id, 0);
    if (ret < 0) {
...
int hot_migration_example(uint32_t agent_id)
{
    ...
    /* 热迁移：post-copy，先切后传 */
    ret = agentrt_sys_rovol_migrate(agent_id, /*target_node=*/2,
                                    AGENTRT_MIGRATE_HOT);
    if (ret == -AGENTRT_EMIGRATE) {
        /* 迁移失败，可重试 */
        printf("migration failed, retrying with cold policy...\n");
        ret = agentrt_sys_rovol_migrate(agent_id, 2, AGENTRT_MIGRATE_COLD);
    }
...
int tier_migration_example(uint64_t snapshot_id)
{
    ...
    op.from_layer = 2;
    op.to_layer = 3;
    op.decay_factor = AGENTRT_DECAY_EBBINGHAUS;  /* 0x8000 Q16.16 */
    ret = agentrt_sys_rovol_demote(snapshot_id, &op, 0);
    if (ret < 0) {
    ...
    op.from_layer = 3;
    op.to_layer = 2;
    op.decay_factor = AIRYMAX_Q16_ONE;  /* 0x10000 Q16.16 = 1.0 */
    ret = agentrt_sys_rovol_promote(snapshot_id, &op, 0);
    if (ret < 0) {
...
| 测试层 | 覆盖范围 | 工具 | 示例 |
|--------|---------|------|------|
| **单元测试** | API 签名、参数验证、错误码 | KUnit（内核）+ pytest（用户态） | snapshot 参数为 0 时返回 `-AGENTRT_EINVAL` |
...
static void test_snapshot_basic(struct kunit *test)
{
    ...
    /* 正常快照 */
    int ret = agentrt_sys_rovol_snapshot(agent_id,
        AGENTRT_ROVOL_LAYER_ALL, &info);
    KUNIT_EXPECT_EQ(test, ret, 0);
    KUNIT_EXPECT_NE(test, info.snapshot_id, 0);
    KUNIT_EXPECT_EQ(test, info.state, AGENTRT_ROVOL_STATE_ACTIVE);

    /* 清理 */
    agentrt_sys_rovol_delete(info.snapshot_id, 0);
}

static void test_snapshot_invalid_params(struct kunit *test)
{
    /* agent_id 不存在 */
    int ret = agentrt_sys_rovol_snapshot(0xFFFF, 0x0F, NULL);
    KUNIT_EXPECT_EQ(test, ret, -AGENTRT_EINVAL);

    /* options 为 0 */
    agentrt_rovol_snapshot_info_t info = { .size = sizeof(info) };
    ret = agentrt_sys_rovol_snapshot(1, 0, &info);
    KUNIT_EXPECT_EQ(test, ret, -AGENTRT_EINVAL);
}
...
static void test_snapshot_restore_roundtrip(struct kunit *test)
{
    ...
    /* 快照源 Agent */
    agentrt_rovol_snapshot_info_t info = { .size = sizeof(info) };
    KUNIT_EXPECT_EQ(test, agentrt_sys_rovol_snapshot(src_agent,
        AGENTRT_ROVOL_LAYER_ALL, &info), 0);

    /* 恢复到目标 Agent */
    KUNIT_EXPECT_EQ(test, agentrt_sys_rovol_restore(info.snapshot_id,
        dst_agent), 0);
...
|---------|---------------|------|
| 552 snapshot | `AGENTRT_CAP_ROVOL_SNAPSHOT` | 对目标 Agent 创建快照 |
| 553 restore | `AGENTRT_CAP_ROVOL_RESTORE` | 恢复记忆到目标 Agent |
| 554 migrate | `AGENTRT_CAP_ROVOL_MIGRATE` | 跨节点迁移 |
| 555 cxl_tier_set | `AGENTRT_CAP_CXL_TIER` | 修改 CXL 分层 |
| 556 mglru_config | `AGENTRT_CAP_MGLRU_CONFIG` | 修改 MGLRU 配置 |
| 557 list | `AGENTRT_CAP_ROVOL_LIST` | 列出快照（较弱权限） |
| 558 delete | `AGENTRT_CAP_ROVOL_DELETE` | 删除快照 |
| 559 demote | `AGENTRT_CAP_ROVOL_DEMOTE` | 层级降级 |
| 560 promote | `AGENTRT_CAP_ROVOL_PROMOTE` | 层级晋升 |
| 561 cxl_tier_get | `AGENTRT_CAP_ROVOL_LIST` | 查询分层（较弱权限） |
...
- `snapshot` 失败（`-AGENTRT_EPERM`）——可能的未授权访问
- `restore` 失败（`-AGENTRT_ECORRUPTED`）——可能的数据篡改
- `migrate` 失败（`-AGENTRT_EMIGRATE`）——可能的网络/CXL 故障
- `delete` 成功——记忆销毁审计
...
- [50-engineering-standards/20-contracts/syscall_api_contract.md](../50-engineering-standards/20-contracts/syscall_api_contract.md)——系统调用 API 契约
...
| 版本 | 日期 | 变更内容 | 变更人 |
|------|------|---------|--------|
| 0.1.1 | 2026-07-09 | 初始版本，定义 10 个系统调用（552-561）的完整 API 契约、快照状态机、迁移协议、层级升降级、SDK 集成、错误码统一、测试策略 | Airymax 架构委员会 |
