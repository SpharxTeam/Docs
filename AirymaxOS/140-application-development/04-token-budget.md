...
> **设计参考**: openEuler OLK-6.6 `kernel/sched/fair.c`（CFS 带宽控制 `sched_cfs_period_timer`）+ seL4 `src/object/schedcontext.c`（调度上下文预算消耗）
...
```c
/**
 * struct agentrt_token_budget - Token 预算状态
 *
...
```c
/**
 * struct agentrt_token_budget_config - Token 预算配置（版本化）
 *
 ...
 * @flags:             预算标志（AGENTRT_BUDGET_FLAG_*）
 *
 * 版本协商规则与 07-syscall-registry.md 第 8.2 节一致。
 */
struct agentrt_token_budget_config {
    uint32_t size;
    uint32_t version;
    uint32_t reserved;
...
/* 预算标志位 */
#define AGENTRT_BUDGET_FLAG_BORROW      0x0001  /* 允许预算借用 */
#define AGENTRT_BUDGET_FLAG_BURST       0x0002  /* 允许突发消耗 */
#define AGENTRT_BUDGET_FLAG_HARD_LIMIT  0x0004  /* 硬限制（禁止超限） */
#define AGENTRT_BUDGET_FLAG_AUTOSUSPEND 0x0008  /* 耗尽自动挂起 */
```
...
```c
/**
 * struct agentrt_token_usage - 单次 Token 消费记录
 *
 * @agent_id:     Agent ID
 * @task_id:      任务 ID
...
**层级规则**：
- 子级预算总和不超过父级预算上限
- 子级预算耗尽时，向父级借用（若 `AGENTRT_BUDGET_FLAG_BORROW` 已设置）
- 父级预算耗尽时，所有子级进入 SUSPENDED

### 4.2 初始预算分配

Agent 注册时通过 `agentrt_agent_config.initial_token_budget` 设置初始预算（详见 [01-agent-lifecycle.md](01-agent-lifecycle.md) 第 2.1 节）。
...
```c
/**
 * agentrt_budget_adaptive_refill - 自适应补充速率计算
 * @budget: Token 预算状态
 *
 * 基于过去 N 个补充周期的平均消耗率，动态调整补充速率。
 * 若 Agent 持续高消耗，提高补充速率；低消耗则降低。
 */
static uint32_t agentrt_budget_adaptive_refill(
    const struct agentrt_token_budget *budget)
{
    uint64_t avg_rate;
    uint32_t new_rate;
...
| 路径 | 机制 | 延迟 | 用途 |
|------|------|------|------|
| 同步上报 | LLM 推理完成后立即调用 `agentrt_budget_consume()` | < 1 μs | 精确计量 |
...
```c
/**
 * agentrt_budget_consume - 消费 Token 预算
 * @agent_id:  Agent ID
 * @tokens:   消费 Token 数
 * @usage:    消费记录（可选，NULL 则不记录详细日志）
 *
 * 返回: 0 成功，-AGENTRT_EBUDGET_EXHAUSTED 预算不足，
 *       -AGENTRT_ENOENT Agent 不存在
 *
 * 消费前先执行惰性补充（lazy refill），更新 current_tokens。
 * 若消费后 current_tokens 低于阈值，触发状态转换。
 */
int agentrt_budget_consume(uint32_t agent_id, uint32_t tokens,
                           const struct agentrt_token_usage *usage);
```
...
stateDiagram-v2
    ...
    SUSPENDED --> RUNNING: 预算补充恢复 + agentrt_sys_task_resume()
    SUSPENDED --> CRITICAL: current ≤ critical_threshold

    CRITICAL --> TERMINATING: 强制终止（资源回收）
    CRITICAL --> SUSPENDED: 手动注入预算（agentrt_sys_task_set_budget）
...
```c
/**
 * agentrt_budget_refill - 惰性补充令牌桶
 * @budget: Token 预算状态
 *
 * 基于时间差计算应补充的令牌数。
 * 不使用定时器，对齐 CFS update_curr() 惰性更新模式。
 */
static void agentrt_budget_refill(struct agentrt_token_budget *budget)
{
    uint64_t now, elapsed_ms, refill_amount;

    now = agentrt_clock_gettime_ns();  /* CLOCK_MONOTONIC */
...
```c
/* 手动注入 10000 Token */
struct agentrt_token_budget_config cfg = {
    .max_tokens = 100000,
    .refill_rate = 1000,
};
agentrt_sys_task_set_budget(agent_id, &cfg);
```

### 7.3 预算借用

启用 `AGENTRT_BUDGET_FLAG_BORROW` 时，子级预算可向父级借用：

```c
/**
 * agentrt_budget_borrow - 向父级借用预算
 * @child:  子级预算
 * @parent: 父级预算
 * @amount: 借用数量
 *
 * 借用规则:
 *   1. 子级必须设置 AGENTRT_BUDGET_FLAG_BORROW
 *   2. 父级剩余预算必须 > 借用后阈值
 *   3. 借用记录在审计日志中
 *   4. 父级预算补充时自动归还
 */
int agentrt_budget_borrow(struct agentrt_token_budget *child,
                          struct agentrt_token_budget *parent,
                          uint32_t amount);
```
...
| 编号 | 系统调用 | 功能 |
|------|---------|------|
| 523 | `agentrt_sys_task_set_budget` | 设置/更新 Agent Token 预算 |
| 524 | `agentrt_sys_task_get_budget` | 查询 Agent Token 预算状态 |

### 8.2 设置预算 C 接口

```c
/**
 * agentrt_sys_task_set_budget - 设置 Agent Token 预算
 * @agent_id: Agent ID
 * @config:   预算配置
 *
 * 返回: 0 成功，-AGENTRT_EINVAL 参数无效，
 *       -AGENTRT_ENOENT Agent 不存在，-AGENTRT_EPERM 权限不足
 *
 ...
 * @since 1.0.1
 */
AGENTRT_API int agentrt_sys_task_set_budget(
    uint32_t agent_id,
    const struct agentrt_token_budget_config *config);
```
...
```c
/**
 * agentrt_sys_task_get_budget - 查询 Agent Token 预算状态
 * @agent_id:   Agent ID
 * @budget_out: 预算状态输出指针
 *
 * 返回: 0 成功，-AGENTRT_ENOENT Agent 不存在
 *
 * 查询前执行惰性补充，返回最新状态。
 *
 * @since 1.0.1
 */
AGENTRT_API int agentrt_sys_task_get_budget(
    uint32_t agent_id,
    struct agentrt_token_budget *budget_out);
```
...
```c
/**
 * agentrt_budget_consume - 消费 Token 预算（内核内部接口）
 * @agent_id: Agent ID
 * @tokens:   消费数量
 * @usage:    消费记录（可选）
 *
...
static airymax_weight_t agentrt_budget_compute_token_factor(
    const struct agentrt_token_budget *budget)
{
    uint32_t pct;
...
try:
    ...
    if e.code == -401:  # AGENTRT_EBUDGET_EXHAUSTED
        print(f"Token 预算耗尽，已使用 {client.tokens_used}/{client.token_budget}")
        # 执行优雅降级：保存上下文、通知用户
    elif e.code == -701:  # AGENTRT_EPERM
        print("权限不足")
```
...
| 操作 | 延迟预算（P99） | 测量方法 |
|------|---------------|---------|
| `agentrt_budget_consume()` | < 1 μs | perf trace |
| `agentrt_budget_refill()` | < 500 ns | perf trace |
| `agentrt_sys_task_set_budget()` | < 1 μs | perf trace |
| `agentrt_sys_task_get_budget()` | < 1 μs | perf trace |
...
| 错误码 | 值 | 含义 | 触发场景 | 可重试 |
|--------|-----|------|---------|--------|
| `AGENTRT_EBUDGET_EXHAUSTED` | -15 | Token 预算耗尽 | current_tokens < 请求量 | 是（等待补充后） |
...
|-------------|--------------|---------|
| `-401` (SDK 旧) | `-15` (AGENTRT_EBUDGET_EXHAUSTED) | SDK 层通过 `agentrt_errno_to_app()` 转换 |
| `-701` (SDK 旧) | `-4` (AGENTRT_EPERM) | SDK 层通过 `agentrt_errno_to_app()` 转换 |

**统一约束**：SDK 层错误码是应用层错误码（`agentrt_app_*`），通过 `agentrt_errno_to_app()` 从系统调用错误码转换而来。系统调用层统一使用 `AGENTRT_E*` 前缀（值 -1 ~ -15）。

### 12.3 错误处理规范

```c
/* 正确：检查预算错误并优雅降级 */
int ret = agentrt_budget_consume(agent_id, tokens, &usage);
if (ret == -AGENTRT_EBUDGET_EXHAUSTED) {
    log_write(LOG_WARN, "token budget exhausted, agent=%u", agent_id);
    /* 触发 SUSPENDED：保存记忆快照 */
    agentrt_sys_task_pause(agent_id, AGENTRT_PAUSE_SAVE_MEMORY);
    return ret;
} else if (ret < 0) {
...
{
    struct agentrt_task_config task_cfg = {
    ...
    struct agentrt_token_budget_config budget_cfg = {
        ...
        .critical_pct = 5,         /* 5% 临界 */
        .flags = AGENTRT_BUDGET_FLAG_AUTOSUSPEND,
    };
    uint32_t agent_id;
    int ret;

    /* 1. 注册 Agent */
    ret = agentrt_sys_task_register(&task_cfg, &agent_id);
    if (ret < 0) return ret;

    /* 2. 设置预算 */
    ret = agentrt_sys_task_set_budget(agent_id, &budget_cfg);
    if (ret < 0) {
        agentrt_sys_task_stop(agent_id, 0);
        return ret;
    }
...
{
    struct agentrt_token_budget budget;

    if (agentrt_sys_task_get_budget(agent_id, &budget) == 0) {
...
{
    struct agentrt_token_budget_config new_budget = {
    ...
    /* 1. 手动注入新预算 */
    int ret = agentrt_sys_task_set_budget(agent_id, &new_budget);
    if (ret < 0) return ret;

    /* 2. 恢复 Agent（从 SUSPENDED 到 RUNNING） */
    ret = agentrt_sys_task_resume(agent_id);
    if (ret < 0) {
...
static void test_token_bucket_refill(struct kunit *test)
{
    ...
    /* 模拟 10ms 后补充 */
    budget.last_refill_ts = agentrt_clock_gettime_ns() - 10000000; /* 10ms 前 */
    old_ts = budget.last_refill_ts;

    agentrt_budget_refill(&budget);
...
static void test_budget_exhaustion(struct kunit *test)
{
    ...
    int ret = agentrt_budget_consume_internal(&budget, 850, NULL);
    ...
    ret = agentrt_budget_consume_internal(&budget, 60, NULL);
    KUNIT_EXPECT_EQ(test, ret, 0);
...
| 预算配置 | `agentrt_token_budget_config` | 同结构体 | [SC] 共享结构体 |
| 消费接口 | `agentrt_budget_consume()` | 内核 kfunc | 语义一致 |
| 耗尽处理 | 应用层 SUSPENDED | 内核态 SUSPENDED | 状态语义一致 |

### 15.2 [SC] 层共享

Token 预算的 `agentrt_token_budget` 结构体定义在 `include/airymax/sched.h`（[SC] 共享头文件）中，agentrt 与 agentrt-linux 代码字面共享。
...
**问题描述**：[02-sdk-integration.md](02-sdk-integration.md) 使用 `-401`（AGENTRT_EBUDGET_EXHAUSTED）和 `-701`（权限错误），而系统调用错误码表（[30-interfaces/01-syscalls.md](../30-interfaces/01-syscalls.md) 第 6 章）使用 `-15` 和 `-4`。

**严重程度**：P1（错误码一致性）

**修复指引**：SDK 层错误码是应用层错误码（`agentrt_app_*`），通过 `agentrt_errno_to_app()` 从系统调用错误码转换。需在 SDK 文档中明确标注转换关系。系统调用层新增 `AGENTRT_EBUDGET_EXHAUSTED = -15` 到错误码表。
...
| 版本 | 日期 | 变更 |
|------|------|------|
| 0.1.1 | 2026-07-09 | 初始版本。定义令牌桶算法、三级阈值设计、核心数据结构（`agentrt_token_budget` + `agentrt_token_budget_config` + `agentrt_token_usage`）、三级预算层级、消耗计量与审计、耗尽状态机、恢复机制（自动补充 + 手动注入 + 借用）、系统调用集成（523/524）、调度集成（token_factor Q16.16）、SDK 集成（四语言）、性能约束（Token/Watt/Latency）、错误码统一（AGENTRT_EBUDGET_EXHAUSTED = -15）、KUnit 测试 |
