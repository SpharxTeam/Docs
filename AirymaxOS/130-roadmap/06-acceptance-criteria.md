...
> **核心约束**：IRON-9 v3 同源且部分代码共享（agentrt 与 agentrt-linux 验收标准共享编号骨架，独立实施）
...
| OS-ACC-012 ★ | IRON-9 v3 同源且部分代码共享声明 | `grep -rE "IRON-9 v3 同源且部分代码共享" docs/AirymaxOS/50-engineering-standards/` | ≥3 处声明 | P0 | M0 |
...
| OS-ACC-025 | sched_tac 调度策略定义 | `grep -rE "stc_\|sched_tac" docs/AirymaxOS/` | ≥5 处 | P0 | M1 |
...
| **IRON-9 v3 同源且部分代码共享** | OS-ACC 继承 agentrt ACC 编号骨架，独立实施内核态专属验收 | §2.1 编号前缀 |
...
- IRON-9 v3 工程铁律 — 17 类规则编号体系（v28.0，含 IRON-9 + ACC-OS04 + 149 项 ACC）
...
> **文档结束** | 共 16 节 | Linux 6.6 内核基线 + 五维正交 24 原则 + IRON-9 v3 同源且部分代码共享 | 约 110 项 OS-ACC 验收标准 + 三层质量门禁 + 三级验收流程