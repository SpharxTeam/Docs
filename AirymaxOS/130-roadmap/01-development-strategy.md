...
**与 agentrt 工程标准的关系（IRON-9 v3 同源且部分代码共享）**:
...
|---|------|------|---------|--------------|
| 1 | kernel | 极境内核 | Linux 6.6 内核基线 + 微内核化改造（sched_tac + eBPF + io_uring + Rust） | atoms/corekern（MicroCoreRT） |
...
| 8 | tests-linux | 极境测试 | 单元测试 + 集成测试 + 形式化验证 + Soak + 混沌 | 全模块测试 |

**微内核化改造策略**（K-1 内核极简）:

| 改造方向 | 当前（Linux 6.6） | 目标（agentrt-linux 1.0.1） |
|---------|-------------------|------------------------|
| 调度类 | CFS + EEVDF | + sched_tac 用户态调度器（与 MicroCoreRT 同源） |
...
| 维度 | 内容 |
|------|------|
| **范围** | `50-engineering-standards/` 全部 23 文档（README + 顶层 8 + 10-coding-style 6 + 其余 4 子目录各 2，含 09-ssot-registry.md SSoT 注册表，2026-07-13 精简合并后） |
| **优先级** | P0 前置（无依赖） |
| **依赖** | 无 |
| **工时** | 240h |
| **里程碑** | M0（2 周） |
| **完成标准** | 23 文档完成 + OS 工程规则编号体系注册表建立 + 与 agentrt IRON-9 v3 同源且部分代码共享关系明确 |
...
**关键产出**:
- 系统架构设计（微内核策略 + 工程哲学）
- 8 子仓模块设计（kernel / services / security / memory / cognition / cloudnative / system / tests-linux）
...
- eBPF 可观测性探针（kfunc + dynamic pointer，非核心架构，H5 约束——仅用于 90-observability，airy_lsm 不依赖 BPF）
...
- 机密计算（TEE + 内存加密）
- 国密算法集成（SM2/SM3/SM4）
- 纯 C LSM 代码完整性验证（编译期校验 + 静态分析 + CBMC 形式化验证，无 BPF 依赖，对齐 H5）
...
|--------|-----------|--------------|-----------|
| 调度 | MicroCoreRT 调度 | sched_tac 调度策略 | 调度语义一致，无调度适配 |
...
|------|---------|-----------|---------|
| 0.1.1 | 完成奠基 + 29 仓拆分 + 生产就绪 | 文档体系完成（~79 文档，不含内核/OS 实施） | agentrt-linux 同步工程标准语义 |
...
### 5.1 IRON-9 v3 同源且部分代码共享

agentrt-linux 工程标准与 agentrt 工程标准遵循 IRON-9 v3 同源且部分代码共享原则：
...
| **K-4 可插拔策略** | sched_tac 用户态调度器 + LSM 钩子 + 模块签名 | Part 2 + Part 5 |
...
agentrt-linux 开发策略以三大支柱（工程标准 + 架构设计 + 开发治理）为骨架，以 9 个 Part（P0 七部分 + P1 两部分）为执行单元，以 9 个里程碑（M0-M8）为节奏节点，以五维正交 24 原则为设计哲学，以 IRON-9 v3 同源且部分代码共享原则处理与 agentrt 的协同关系。
...
- IRON-9 v3 工程铁律 — 17 类规则编号体系（v28.0，含 IRON-9 v3 同源且部分代码共享）
