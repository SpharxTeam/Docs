...
# agentrt-liunx（AirymaxOS）开发策略

> **文档定位**: agentrt-liunx（AirymaxOS，极境智能体操作系统）开发策略与三大支柱详解
...
agentrt-liunx 采用**"工程标准先行 + 模块化并行 + 渐进式发布"**的开发策略，与 agentrt 0.1.1 奠基版本并行（agentrt 在前，agentrt-liunx 在后）：
...
agentrt 0.1.1 先完成奠基（atoms 解耦、CoreKern 精简、taskflow 修复、SDK 4 语言、许可证合规、29 仓拆分、生产就绪），随后 agentrt-liunx 1.0.1 基于 agentrt 同源语义开始内核与 OS 开发，两端在 1.0.1 阶段协同验证。
...
agentrt-liunx 开发方案由三大支柱构成。三大支柱对应"标准 → 设计 → 治理"的递进关系。
...
- **独立**: agentrt-liunx 是 OS 发行版，工程标准必须独立处理内核 ABI 稳定性、内核内部 API 不稳定性、补丁生命周期、维护者层级制度等 agentrt 不涉及的领域；同时为内核态特有场景（驱动模型、构建系统、可观测性、安全 LSM）制定专门规范。
...
| 6 | clouds | 极境云原生 | K8s + containerd + OCI + agentctl + 超节点 OS | gateway + sdk |
| 7 | system | 极境系统 | 包管理 + 配置 + shell + 基础库 + DevStation | commons |
| 8 | airymaxos-tests | 极境测试 | 单元测试 + 集成测试 + 形式化验证 + Soak + 混沌 | 全模块测试 |

**微内核化改造策略**（K-1 内核极简）:

| 改造方向 | 当前（Linux 6.6） | 目标（agentrt-liunx 1.0.1） |
|---------|-------------------|------------------------|
...
agentrt-liunx 开发方案拆分为 9 个 Part。以下详述每个 Part 的范围、优先级、依赖与工时。
...
**关键产出**:
- 系统架构设计（微内核策略 + 工程哲学）
- 8 子仓模块设计（kernel / services / security / memory / cognition / clouds / system / tests）
...
agentrt-liunx 与 agentrt 是**同源**关系，两端通过同源 API 实现无适配层互操作：
...
graph TB
    ...
    style agentrt-liunx fill:#e3f2fd
```

### 4.2 同源红利

agentrt 的设计假设和 agentrt-liunx 的实现假设一致，agentrt 在 agentrt-liunx 上运行**无适配层**，天然契合：
...
| 阶段 | agentrt | agentrt-liunx | 协同内容 |
|------|---------|-----------|---------|
| 0.1.1 | 完成奠基 + 29 仓拆分 + 生产就绪 | 文档体系完成（~64 文档，不含内核/OS 实施） | agentrt-liunx 同步工程标准语义 |
| 1.0.1 | 与 agentrt-liunx 协同验证 | 完成 M0-M8 全部里程碑 | 同源 API 端到端验证 |
| 1.1.x | 在 agentrt-liunx 上天然适配运行 | 稳定运行 | 无适配层互操作 |
...
agentrt-liunx 工程标准与 agentrt 工程标准遵循 IRON-9 v2 同源且部分代码共享原则：

- **同源**: 共享五维正交 24 原则、17 类规则编号体系骨架、核心工程观（E-7 文档即代码、E-6 错误可追溯、E-1 安全内生）。
- **独立**: agentrt-liunx 独立处理内核 ABI 稳定性、补丁生命周期、维护者层级制度；新增 OS 专属编号前缀（OS-KER / OS-DRV / OS-BUILD / OS-TEST / OS-OBS / OS-SEC / OS-ABI）。
...
agentrt-liunx 开发策略是五维正交 24 原则在"开发方法"维度的具体落地：
...
agentrt-liunx 开发策略以三大支柱（工程标准 + 架构设计 + 开发治理）为骨架，以 9 个 Part（P0 七部分 + P1 两部分）为执行单元，以 9 个里程碑（M0-M8）为节奏节点，以五维正交 24 原则为设计哲学，以 IRON-9 v2 同源且部分代码共享原则处理与 agentrt 的协同关系。
