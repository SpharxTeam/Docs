...
- **缓解措施**：
  ...
  2. 兼容性测试——airymaxos-tests-linux 必须覆盖同源 API
...
- **缓解措施**：
  ...
  4. ABI 兼容性测试——airymaxos-tests-linux 必须覆盖 ABI 兼容性
...
- **风险描述**：airymaxos-tests-linux 若测试覆盖率不足（<80%），可能导致 regression 未被发现，违反 E-8 可测试性原则与 K-2 接口契约化原则。
- **触发条件**：
  1. 测试覆盖率 <80%
