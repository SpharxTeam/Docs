...
| 5 | **同源代码共享** | **IRON-9 v3 四层模型**：[SC] 共享契约层 + [SS] 语义同源层 + [IND] 独立实现层 + [DSL] 降级生存层 | （v2 三层模型升级为 v3 四层模型，新增 [DSL] 降级生存层） | CI 流水线强制校验 `include/airymax/*.h` 共 10 个 [SC] 头文件逐字节一致；`AIRY_SC_FALLBACK` 宏由构建系统在 [SC] 校验失败时自动注入（[DSL] 第四层） |
