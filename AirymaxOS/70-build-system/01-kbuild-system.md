...
构建系统层不属于 IRON-9 v2 的 6 个 [SC] 共享头文件清单（`bpf_struct_ops.h` / `memory_types.h` / `security_types.h` / `cognition_types.h` / `sched.h` / `ipc.h`）。构建系统是工程基础设施，其产物（`.ko` / `vmlinux` / 可执行文件）通过二进制契约解耦，而源码层无共享头文件依赖。这一约束确保 agentrt 用户态构建工具链演进时不会被动牵连 agentrt-linux Kbuild，反之亦然——构建系统层的演进由各自的 **OS-BUILD 评审** 独立裁决。
