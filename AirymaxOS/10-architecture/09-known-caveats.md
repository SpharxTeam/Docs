...
agentrt-linux 不引入 MCS（Mixed Criticality Systems）配置——此为 seL4 特有机制，agentrt-linux 通过 sched_tac + token budget 实现等价能力（seL4 MCS 语义映射到 SCHED\_DEADLINE 带宽配额，详见 [§3.2](#32-sched_ext-不可用约束与方案-c-prime-替代)）。
