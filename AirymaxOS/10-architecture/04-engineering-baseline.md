...
| sched_ext（SCHED_AGENT） | agentrt-linux 内核增强 | 主线 6.12+ | airymaxos-kernel 用户态调度 |
...
| QA 治理组 | airymaxos-tests-linux | 集成测试框架 + 形式化验证 |
...
**agentrt-linux 认知循环**（CoreLoopThree）是 airymaxos-cognition 子仓的核心能力，与 agentrt 同源：
...
**agentrt-linux 落地**：airymaxos-cognition 实现 agentrt-linux 认知循环，与 agentrt 的 CoreLoopThree 同源，作为内核态认知运行时。

### 5.2 超节点 OS

**agentrt-linux 超节点 OS** 是 airymaxos-cloudnative 子仓的核心能力，支持大规模 Agent 容器化部署：
...
**agentrt-linux 落地**：airymaxos-cloudnative 实现超节点 OS，支持大规模 Agent 容器化部署与跨节点低时延协同。

### 5.3 超节点沙箱

**agentrt-linux 超节点沙箱** 是 airymaxos-cognition 子仓的 Agent 快速冷启动能力：
...
**agentrt-linux 落地**：airymaxos-cognition 实现超节点沙箱，支持 Agent 快速冷启动，与 Wasm 3.0 沙箱运行时集成（ADR-008）。
...
**agentrt-linux 落地**：airymaxos-cognition 实现 Token 能效优化，让 Token 消耗走向边际递减，支撑 Agentic OS 的成本可控性。
...
**agentrt-linux 落地**：airymaxos-cognition 实现具身智能支持，由 Embedded 治理组协同推进。
...
>
> **参考来源**：Linux 7.1 内核技术检索报告（`docs-closed/agentrt-linux/_research_0.2.5/02-linux-7.1-forward-looking.md`），基于 kernel.org 官方发布表 + kernelnewbies.org + LWN + Phoronix 多方交叉验证。
...
> 遵循 IRON 规则——所有预留必须以「运行时能力探测 + 条件降级」实现，而非「假设未来内核一定有」。预留入口统一为 `airymax_kernel_cap_query(cap_id)` 框架（建议 10），避免散落各模块的 `#ifdef` 污染。

**建议 1：调度类复用 sched_ext，禁用 SCHED_AGENT 自定义类**

在调度接口契约中，将「调度策略」抽象为 BPF struct_ops 接口。当 7.1 cgroup 子调度器可用时启用 per-cgroup 策略，否则降级为单 sched_ext 实例。预留 `airymax_sched_ext_ops` 结构体，但 2.x.x 不默认启用 cgroup 子调度器（避免依赖未稳定 ABI）。
...
**建议 3：容器 mount 树构造抽象为 `airymax_mount_ns_create()` 接口**
...
在块 I/O 接口中，预留 `AIRYMAX_IO_URING_BPF` 与 `AIRYMAX_IO_UBLK_ZC` 两个能力位。策略裁决可通过 BPF struct_ops 注入 io_uring 主循环（7.1）。2.x.x 在 6.6 上降级为标准 io_uring（无 BPF 主循环）。zero-copy recv 在 6.15+ 可用，6.6 降级为标准 recv。

**建议 5：Landlock 沙箱覆盖 UNIX socket**

沙箱策略层预留 `AIRYMAX_LANDLOCK_SOCKET` 能力位。当 7.1 socket hook 可用时启用 pathname UNIX socket 访问控制，否则仅覆盖文件系统。同时利用 7.1 的 `user.* xattr on socket` 实现 IPC 协议标注，协议注册表应预留 socket xattr 查询接口。
...
在公共库层实现 `airymax_kernel_cap_query(cap_id)` 框架，集中管理所有 7.1/7.0/6.18/6.12/6.6 的能力探测（clone3 flags、FSMOUNT_NAMESPACE、Landlock socket hook、io_uring struct_ops、ublk zero-copy、ML-DSA、FRED、sched_ext cgroup sub-sched、PREEMPT_LAZY、KHO/liveupdate 等）。每个能力点配 `since` 标签与运行时探测函数 + 降级路径。这是 2.x.x 前瞻性预留的**总入口**，避免散落各模块的 `#ifdef` 污染。
...
| 兼容层先行（1.x.x→2.x.x 过渡） | 2027+ | 实现 `airymax_kernel_cap_query()` 框架 + 10 项能力探测 + 降级路径 + 接口契约前瞻性预留 + 在 6.6 上验证降级路径正确性 |
...
| 兼容层先行 | `airymax_kernel_cap_query()` 框架就绪 | 10 项能力探测全部实现 + 降级路径验证通过 |
