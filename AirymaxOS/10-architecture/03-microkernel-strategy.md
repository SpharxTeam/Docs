...
| ABI 稳定性      | syscall.xml + codegen | \[SC] 共享契约层 10 头文件                |
...
**agentrt-linux 适配**：io\_uring 的 SQ / CQ ring 提供等效的异步消息传递语义。IPC magic `0x41524531`（'ARE1'）通过 \[SC] `include/uapi/linux/airymax/ipc.h` 与 agentrt 共享。
...
| copyMRs              | 跨架构的消息复制实现                                       | io_uring 语义映射零拷贝（IORING_OP_URING_CMD）   |
...
**agentrt-linux 目标（v1.0.1 Capability Folding 后）**：4 核心 + 20 预留 = 24 槽位（`airy_sys_call` sec_d 专属管理入口 + `airy_sys_rovol_ctl` + `airy_sys_sched_ctl` + `airy_sys_clt_notify` 共 4 核心；IPC 数据面完全由 io_uring `IORING_OP_URING_CMD` 承载，零 syscall；原 8 个 seL4 风格 IPC 原语已全部移除）。新增 syscall 需 TSC（Technical Steering Committee）评审。
...
## 10.1 IRON-9 v3 四层共享模型

> **OS-ARCH-005**： seL4 微内核设计思想在 agentrt-linux 四层模型中分布落地——capability 模型经 \[SC] `security_types.h` 共享契约，IPC 消息传递经 \[SC] `ipc.h` + \[SS] 同源 API 双层落地，形式化验证经 \[IND] 各自独立实现，[SC] 损坏时由 \[DSL] 降级生存层保证最小可运行子集，禁止将 seL4 验证代码混入共享契约层。
...
| 调度原语   | 用户态协程           | sched\_tac（Linux 6.6 原生调度类组合 SCHED\_DEADLINE/SCHED\_FIFO/EEVDF） | 跨平台约束    |
...
- [工程基线](04-engineering-baseline.md)：Linux 6.6 / 7.1 双基线锁定（ADR-016）+ Linux 7.1 前瞻性预留设计（§8.4）
- [架构决策记录](05-adrs.md)：16 个核心 ADR（含 ADR-011\~016 新增决策）
...
- Linux 7.1 release notes（2.x.x 基线，ADR-016）
...
- IRON-9 v3 四层共享模型：`50-engineering-standards/README.md`
- \[SC] 共享契约层 10 头文件：`include/uapi/linux/airymax/{error,log_types,ipc,sched,memory_types,security_types,cognition_types,syscalls,uapi_compat,lsm_types}.h`
...
> **升级背景**：本次 v1.0.1 升级将本文档对齐 Airymax Unify Design（见 [10-unify-design.md](10-unify-design.md)）5 模块设计与sched_tac 技术选型，消解 v0.1.1 遗留的架构冲突点。升级内容如下。

### A.1 已融入 Airymax Unify Design 5 模块设计

本文档 §1.4 / §2.2 / §4 等章节描述的机制-策略分离、服务用户态化、IPC 消息传递等微内核思想，已纳入 Airymax Unify Design 5 模块统一架构：

- **A-UEF（统一错误码与故障定义）**：§3 capability 操作返回值统一为 `AIRY_E*` 负数错误码（[SC] `error.h`）
- **A-ULP（统一日志与打印系统）**：内核态日志统一为 `LOG_*` + 128B 记录（[SC] `log_types.h`），见 [05-ring-buffer-logging.md](../40-dataflows/05-ring-buffer-logging.md)
- **A-UCS（统一配置管理体系）**：Kconfig/sysctl 与用户态 JSON/TOML 语义映射，见 [11-unified-config.md](../20-modules/11-unified-config.md)
- **A-ULS（统一 Supervisor）**：§2.2 VFS 用户态化决策、调度策略由 A-ULS 模块统一管理
- **A-IPC（统一 IPC Fastpath）**：§4 IPC 消息传递对齐 A-IPC 模块，见 [07-ipc-fastpath.md](../30-interfaces/07-ipc-fastpath.md)

### A.2 已删除 sched_ext 依赖，采用sched_tac

v0.1.1 中 §1.4 / §2.1 / §4.5 / §9.1 多处将 `sched_ext` BPF 框架作为调度原语承载。鉴于 Linux 6.6 基线下 sched_ext 不可用（见 综合修正方案 §1 C-01 冲突点），v1.0.1 删除 sched_ext 依赖，采用**sched_tac**：

- 调度机制改为 `SCHED_DEADLINE` / `SCHED_FIFO` / `EEVDF` 三层组合，零内核调度器修改
- seL4 MCS 语义映射（SchedContext 捐赠）通过sched_tac 的优先级继承实现，不再依赖 sched_ext
- Agent 8 态生命周期调度承载由 A-ULS 模块管理，权威定义见 [10-sc-sched-extension.md](../30-interfaces/10-sc-sched-extension.md)

### A.3 已新增 [DSL] 降级生存层

v0.1.1 描述的 IRON-9 v3 四层共享模型（[SC] / [SS] / [IND]）已升级为 IRON-9 v3 四层模型，新增 **[DSL]（Degraded Survival Layer，降级生存层）** 作为第四层：

- [DSL] 确保 [SC] 头文件损坏/缺失时系统仍具备最小可运行子集
- 每个 [SC] 头文件底部包含 `#ifdef AIRY_SC_FALLBACK` 降级块
- 微内核策略在 [DSL] 降级模式下退化为：capability 仅 POSIX 校验、IPC 最简 128B 头、调度回退 EEVDF 默认
- 权威定义见 [06-iron9-shared-model.md](06-iron9-shared-model.md) §5

### A.4 VFS 用户态化决策已统一（A-ULS 模块管理）

v0.1.1 §2.2 已提及"VFS 用户态化决策（A-ULS 统一）"，v1.0.1 进一步明确该决策由 Airymax Unify Design 的 **A-ULS 模块**统一管理，消解 C-03 冲突点（VFS 是否完全下沉用户态的矛盾结论）：

- **统一决策**：内核保留 VFS 框架（路径解析、dentry cache），具体文件系统实现通过 FUSE 模型用户态化
- **权威源**：A-ULS 模块（[10-unify-design.md](10-unify-design.md) §5）是 VFS 用户态化决策的唯一权威源
- **禁止重复定义**：本文档及其他文档仅引用 A-ULS 决策，不得重新定义 VFS 下沉边界

***

> **文档结束** | agentrt-linux（AirymaxOS）微内核设计思想详解 v1.0.1 | 2026-07-22
> 微内核思想来源：seL4（唯一来源，ADR-014）| 技术路线：基于 Linux 改造（ADR-012）| 已融入 Airymax Unify Design 5 模块
