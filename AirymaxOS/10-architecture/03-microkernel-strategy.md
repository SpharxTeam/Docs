...
**agentrt-linux 目标（v1.1 Capability Folding 后）**：4 核心 + 20 预留 = 24 槽位（`airy_sys_call` sec_d 专属管理入口 + `airy_sys_rovol_ctl` + `airy_sys_sched_ctl` + `airy_sys_clt_notify` 共 4 核心；IPC 数据面完全由 io_uring `IORING_OP_URING_CMD` 承载，零 syscall；原 8 个 seL4 风格 IPC 原语已全部移除）。新增 syscall 需 TSC（Technical Steering Committee）评审。
...
> **升级背景**：本次 v1.0 升级将本文档对齐 Airymax Unify Design（见 [10-unify-design.md](10-unify-design.md)）5 模块设计与sched_tac 技术选型，消解 v0.1.1 遗留的架构冲突点。升级内容如下。
...
v0.1.1 §2.2 已提及"VFS 用户态化决策（A-ULS 统一）"，v1.0 进一步明确该决策由 Airymax Unify Design 的 **A-ULS 模块**统一管理，消解 C-03 冲突点（VFS 是否完全下沉用户态的矛盾结论）：
...
> **文档结束** | agentrt-linux（AirymaxOS）微内核设计思想详解 v1.0 | 2026-07-17
