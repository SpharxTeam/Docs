...
ALK-6.6
  ...
  + 新增 security/airy/ 纯 C LSM 模块（13 文件）
      ...
      内容：airy_lsm.c + airy_cap_array.c +
            airy_cap_derive.c + airy_cap_check.c + airy_cap_revoke.c +
            airy_cap_rotate.c + airy_superv_lsm.c + airy_ipc_freeze.c +
            airy_die_notify.c + airy_eventfd.c + airy_cap.h +
            Kbuild + Kconfig
  + 新增 kernel/kernel/superv/ Micro-Supervisor 内核模块（6 文件）
      物理位置：kernel/kernel/superv/（位于 Linux 源码根的 kernel/ 子目录下）
      内容：airy_cap_check_superv.c + airy_die_notify_superv.c +
            airy_eventfd_superv.c + airy_ipc_freeze_superv.c +
            airy_superv_lsm.c + Kbuild
  ...
  + 新增 kernel/kernel/corekern/ 微核心抽象层（10 子目录，22 文件）
  ...
  + 新增 kernel/kernel/ipc/ A-IPC 内核基础设施（2 文件 + Kbuild）
      ...
      内容：airy_ipc_capability.h + airy_ipc_capability.c（权威全局状态 + slowpath 包装）
...
│   │   ├── airy_lsm.c                              #   DEFINE_LSM(airy) 注册 + 5 钩子 + airy_cap_agent_caps_init() 调用
...
│   │
│   ├── kernel/kernel/superv/                       # ★ Micro-Supervisor（5 .c + Kbuild = 6 文件）
│   │   ├── Kbuild                                  #   构建配置
...
│   │   ├── Kbuild
│   │   ├── airy_ipc_capability.h                   #   引用 security/airy/airy_cap.h，声明 slowpath 包装
│   │   └── airy_ipc_capability.c                   #   权威定义 agent_caps[] + airy_cap_global_epoch + slowpath 包装
│   │
...
| 4  | `kernel/kernel/superv/`                       | 不存在             | 新增 6 文件 Micro-Supervisor    |        否        |
| 5  | `kernel/kernel/syscalls/airy_syscalls.c`      | 不存在             | 新增 4 syscall（编号 454-457）    |        否        |
| 6  | `kernel/kernel/corekern/`                     | 不存在             | 新增 10 子目录 22 文件微核心抽象层     |        否        |
| 7  | `kernel/kernel/log/`                          | 不存在             | 新增 4 文件 A-ULP 内核侧           |        否        |
| 8  | `kernel/kernel/ipc/`                          | 不存在             | 新增 4 文件 A-IPC 内核基础设施         |        否        |
...
| 6  | corekern 微核心抽象层  | 新增 10 子目录 22 文件                                 | `kernel/kernel/corekern/`                                                         |          否          | 改造点 1   |
| 7  | A-ULP 内核侧        | 新增 4 文件（3 .c + Kbuild）                          | `kernel/kernel/log/`                                                              |          否          | 改造点 1   |
| 8  | A-IPC 内核基础设施      | 新增 4 文件（2 .c + 1 .h + Kbuild）                   | `kernel/kernel/ipc/`                                                              |          否          | 改造点 1   |
...
| [10-unify-design.md](10-unify-design.md)               | Unify Design v1.0.1 | 统一设计总纲        |
...
> **本节定位**：v1.0.1 新增。本节展开 ALK-6.6 极境内核的**具体能力清单**，按物理载体（corekern/log/ipc/configs）逐项描述每个子系统的实现要点、关键函数、设计权衡。所有描述与 `agentrt-linux/kernel/` 实际源码一一对应，是设计-实现一致性审计的依据。

### 11.1 corekern 微核心抽象层（10 子目录，22 文件）
...
`kernel/kernel/ipc/` 提供 A-IPC 的内核侧 Capability Folding 权威全局状态与 slowpath 校验包装。本目录以 `obj-y` 总是编译，确保 `agent_caps[]` 和 `airy_cap_global_epoch` 在 `CONFIG_SECURITY_AIRY=n` 时仍可供 IPC 控制面引用（尽管 Badge 校验仅在 `CONFIG_SECURITY_AIRY=y` 时由 fastpath C-S9 执行）。

| 文件                          | 能力                                                              |
| --------------------------- | --------------------------------------------------------------- |
| `airy_ipc_capability.h`     | 声明 slowpath 包装函数 `airy_cap_badge_verify()` 和 `airy_cap_epoch_bump()`；通过 `#include "../../security/airy/airy_cap.h"` 引入 fastpath 内联 `airy_cap_badge_ok()`、`struct airy_cap_slot` 类型、`agent_caps[]` extern 声明、`airy_cap_global_epoch` extern 声明 |
| `airy_ipc_capability.c`     | **权威定义** `agent_caps[1024]`（`__aligned(64)`，`__ro_after_init` 指针）+ `airy_cap_global_epoch`（`atomic_t`，32 位）+ `airy_cap_agent_caps_init()` 初始化函数 + `airy_cap_badge_verify()` slowpath 包装（调用 `airy_cap_badge_ok()` 内联）+ `airy_cap_epoch_bump()` O(1) 撤销（`atomic_inc`） |

**关键设计**：

1. **权威全局状态单一源**：`agent_caps[]` 和 `airy_cap_global_epoch` 仅在 `airy_ipc_capability.c`（`obj-y`）中定义一次；`security/airy/` 通过 `extern` 引用，消除双重定义
2. **Capability Folding 双层校验**：fastpath 内联 `airy_cap_badge_ok()`（定义于 `security/airy/airy_cap.h`）仅 3 次 `READ_ONCE`（~10ns），slowpath `airy_cap_badge_verify()` 增加包装语义捕获 O(1) 撤销
3. **Badge 编码**：`Badge = Epoch<<48 | RandomTag<<16 | Perms`，16 位 Epoch（bits 48-63）、32 位 RandomTag（bits 16-47）、16 位 Perms（bits 0-15）
4. **O(1) 撤销**：单次 `atomic_inc` 使所有旧 epoch 的 badge 失效，无需遍历 slot 表
5. **`__cacheline_aligned`**：slot 表强制 128 字节 cacheline 对齐（`__aligned(64)` 填充），避免 fastpath 读取时的 false sharing
6. **syscall 实现独立**：4 个 `SYSCALL_DEFINE` 在 `kernel/syscalls/airy_syscalls.c`（`obj-$(CONFIG_AIRY_SYSCALL)`）中定义（编号 454-457）；M0 阶段返回 `-ENOSYS`，M1 阶段 `airy_sys_call` 将按 [01-syscalls.md §1.4](../30-interfaces/01-syscalls.md) 实现 `airy_decode_and_invoke()` cap-type dispatch
...
| 2  | corekern/sched   | `kernel/kernel/corekern/sched/` | 5   | sched_tac 4 策略 + seL4 MCS 映射 + stc_policy.h              |
...
| 12 | ipc              | `kernel/kernel/ipc/`          | 2   | Capability Folding 权威全局状态 + slowpath 包装 + cap header  |
| 13 | configs          | `kernel/configs/`             | 3   | defconfig / defconfig-agent / defconfig-embedded          |
| **合计** |              |                               | **30** |                                                          |

**注**：本表不含 Kbuild 文件（每个子目录另含 1 个 Kbuild，共 13 个 Kbuild，总文件数 30+13=43）。security/airy/（13 文件）和 kernel/superv/（6 文件）和 kernel/syscalls/（2 文件）另计，全内核 Airymax 专属文件合计 30+13+13+6+2=64。

### 11.6 性能关键路径

#### 11.6.1 IPC fastpath（~158ns，含 C-S9 Badge 校验）

```
用户态 io_uring SQE
  → IORING_OP_URING_CMD                    [Linux 6.6 io_uring 原生]
    → airy_uring_cmd()                     [corekern/ipc/airy_uring_cmd.c]
      → airy_ipc_validate() C-S0~C-S12     [corekern/ipc/airy_ipc_fastpath.c]
        → C-S9: airy_cap_badge_ok()        [security/airy/airy_cap.h, 内联]
            3 次 READ_ONCE(slot.badge/agent_id/perms)
            位运算提取 epoch/randtag/perms 与 slot 比对
        → C-S0: unlikely(READ_ONCE(ring->frozen))  [单次分支预测]
      → airy_ipc_deliver_fast()            [corekern/ipc/airy_ipc_fastpath.c]
        → airy_ipc_ring_post()             [corekern/ipc/airy_ipc_ring.c]
  → io_uring CQE → 用户态
```

> **设计说明**：v1.0.1 Capability Folding 后，IPC 数据传递完全走 io_uring 数据面（零 syscall），不经过 `airy_sys_call`（syscall 454）。`airy_sys_call` 仅用于 sec_d 专属管理操作（COMPILE_BADGE/REVOKE_BADGE/LSM_CTL/WASM_LOAD），详见 [01-syscalls.md §1.4](../30-interfaces/01-syscalls.md)。fastpath C-S9 Badge 校验内联在 `airy_cap_badge_ok()` 中（定义于 `security/airy/airy_cap.h`），读取 `agent_caps[]`（权威定义于 `kernel/ipc/airy_ipc_capability.c`）。

**性能保证**：

1. fastpath 内联：3 次 READ_ONCE（128 字节 cacheline-aligned slot，无 false sharing）
2. 单次 unlikely 分支：frozen 标志位检查（C-S0）
3. 无锁 SPSC Ring：生产者/消费者索引无需原子操作（单线程写入）
4. 零 syscall：IPC 数据传递完全走 io_uring `IORING_OP_URING_CMD`，不陷入 syscall

#### 11.6.2 Capability O(1) 撤销

```
airy_cap_epoch_bump()                          [kernel/ipc/airy_ipc_capability.c]
  atomic_inc(&airy_cap_global_epoch)           [单次原子操作，权威定义于本文件]
  → 所有旧 epoch 的 badge 在下一次 fastpath C-S9 airy_cap_badge_ok() 时失效
```
