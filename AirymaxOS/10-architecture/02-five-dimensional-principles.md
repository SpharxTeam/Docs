...
| 落地子仓/模块 | 守护进程 | 隔离机制 |
|---------------|----------|----------|
| services 12 daemons | cogn_d / gateway_d / audit_d / dev_d / sched_d / macro_d 等 | 独立地址空间 + systemd 单元 + capability 限制 |
| services VFS 用户态化 | vfs_d | 用户态文件系统（参考 seL4 服务用户态化，ADR-014） |
| services 网络栈用户态化 | net_d | DPDK / AF_XDP 用户态网络 |
| services 驱动框架用户态化 | dev_d | VFIO / libvfio 用户态驱动 |
...
| 1.0.1 | 2026-Q4 | 首个开发版本（与代码实现同步验证，0.1.1 奠基后直接过渡，IRON-8 硬约束） |
...
|--------|---------|-------------------|--------|
| `error.h` | E 工程观 | `AIRY_E*` 错误码 + `AIRY_FAULT_*` 故障码 + [DSL] 降级块（E-6 错误可追溯） | 全部子仓 |
| `log_types.h` | E 工程观 | `AIRY_LOG_MAGIC` + 128B 记录 + 5 级日志枚举 + printk 映射（E-2 可观测性） | kernel / services |
...
|--------|-------------|-------------------|---------|
| 调度原语 | 用户态协程 + event loop | sched_tac 用户态调度策略 | 跨平台约束（macOS/Windows 无 Linux 6.6 原生调度类组合） |
