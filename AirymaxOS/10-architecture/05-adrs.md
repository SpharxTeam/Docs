...
| ADR-002 | 微内核化改造策略（基于 Linux + sched\_tac + 纯 C LSM + io\_uring） | Accepted |
...
| eBPF kfunc + dynamic pointer | Linux 6.6 原生       | 主线（agentrt-linux 核心架构不使用，H5 约束）      |
...
3. **新特性完备**：Linux 6.6 原生提供 EEVDF 调度器、Rust 实验性支持、MGLRU、XFS 在线 fsck、eBPF kfunc（非核心架构，H5 约束），配合 sched\_tac（Linux 6.6 原生调度类组合 SCHED\_DEADLINE / SCHED\_FIFO / EEVDF + cgroup cpuset 隔离 + seL4 MCS 语义映射，详见 ADR-013），覆盖 agentrt-linux 全部技术需求
...
- 获得 4+ 年长期支持
- 获得 Linux 6.6 全部原生新特性（EEVDF / Rust 实验性 / MGLRU / XFS 在线 fsck / eBPF kfunc（非核心，H5））
...
## ADR-002: 微内核化改造策略（基于 Linux + sched\_tac + 纯 C LSM + io\_uring）
...
agentrt-linux 采用**微内核化改造策略**：基于 Linux 6.6 内核基线，利用 sched\_tac（SCHED\_DEADLINE / SCHED\_FIFO / EEVDF 调度类组合，详见 ADR-013）+ 纯 C LSM（airy\_lsm，H5 硬约束）+ io\_uring 实现微内核化改造，而非从零开发微内核。

具体改造路径：

| 阶段   | 版本     | 改造内容                                                                |
| ---- | ------ | ------------------------------------------------------------------- |
| 阶段 1 | 1.0.1  | 基于 Linux 6.6 + sched\_tac + io\_uring IPC + 纯 C LSM（H5） |
| 阶段 2 | 1.0.2+ | VFS 部分用户态化、网络栈部分用户态化（DPDK/AF\_XDP）、驱动框架用户态化（VFIO/libvfio）           |
| 阶段 3 | 2.0+   | 大部分系统服务用户态化、完整 capability 安全模型、形式化验证（部分核心模块）                        |

### 理由

1. **Liedtke minimality**：微内核思想的核心是"最小化特权态代码"，sched\_tac + 纯 C LSM + io\_uring 本身就是 Linux 内核微内核化方向（基于原生调度类组合的 Agent 感知调度、可编程内核扩展、零拷贝 IPC）
...
5. **能力足够**：sched\_tac（Linux 6.6 原生调度类组合，详见 ADR-013）+ 纯 C LSM（H5 硬约束）+ io\_uring（6.6 原生）已足够实现微内核化目标

### 影响

| 影响范围     | 描述                                                                         |
| -------- | -------------------------------------------------------------------------- |
| kernel   | 基于 sched\_tac 实现 Agent 感知调度策略，基于纯 C LSM（airy\_lsm）实现安全策略可插拔（H5 硬约束） |
| services | 基于 io\_uring 实现高性能 IPC，逐步将 VFS / 网络栈 / 驱动框架用户态化                            |
| security | 基于 airy\_lsm + ftrace/perf 实现安全观测                                                 |
...
- 通过 sched\_tac + 纯 C LSM（airy\_lsm）+ io\_uring 实现微内核化目标（H5）
- 渐进式改造降低风险
- 与 agentrt 同源天然契合
...
| 内核支持          | 纯 C LSM + agent_caps[1024] 静态数组实现 capability 检查（H5） |
...
6. **eBPF 支持**：Linux 6.6 原生的 纯 C LSM（airy\_lsm）+ agent_caps[1024] 为 capability 检查提供内核态实现（H5 硬约束）
...
| kernel      | 通过纯 C LSM（airy\_lsm）在内核态执行 capability 检查（H5）    |
...
3. **改造手段**：利用 Linux 原生新特性（sched\_tac、io\_uring、userfaultfd、Landlock、纯 C LSM）实现微内核化目标（H5 约束：不使用 sched_ext/eBPF 作为核心架构）
