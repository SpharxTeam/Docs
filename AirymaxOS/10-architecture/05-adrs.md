...
| ADR-003 | 8 子仓划分（基于微内核设计思想 + agentrt-liunx 工程基线 + Airymax 同源） | Accepted |
...
agentrt-liunx 需要选择一个稳定的 Linux 内核基线作为系统基础。内核基线的选择直接影响：
...
agentrt-liunx 1.0.1 采用 **Linux 6.6 内核基线** 作为内核基线。
...
| sched_ext（SCHED_AGENT） | agentrt-liunx 内核增强 | 主线 6.12+ |

### 理由

1. **生态兼容性**：Linux 6.6 内核基线继承企业级 Linux 生态（包管理、systemd、SELinux、国密等），具备广泛的企业级落地基础
2. **长期支持**：Linux 6.6 内核基线提供 4+ 年长期支持，符合 agentrt-liunx 1.0.1 的稳定运行需求
3. **新特性完备**：Linux 6.6 原生提供 EEVDF 调度器、Rust 实验性支持、MGLRU、XFS 在线 fsck、eBPF kfunc + dynamic pointer，配合 agentrt-liunx 内核增强的 sched_ext（主线 6.12+），覆盖 agentrt-liunx 全部技术需求
4. **多架构支持**：Linux 6.6 内核基线支持 x86 / ARM（鲲鹏/飞腾）/ RISC-V / LoongArch，符合 agentrt-liunx 多架构目标
...
| Linux 6.15+ 主线 | 更多新特性 | 稳定性不足、特性非必需 | agentrt-liunx 不需要 6.15+ 的特性 |
...
- 通过 agentrt-liunx 内核增强获得 sched_ext（主线 6.12+）支持

**负面后果**：
- 部分主线新特性（6.7+）需等待 agentrt-liunx 内核增强
- sched_ext 依赖 agentrt-liunx 内核增强而非主线原生
...
agentrt-liunx 的设计支柱之一是"微内核设计思想"（参考 seL4 / Zircon / Minix3）。但直接从零开发微内核会失去 Linux 30 年积累的硬件支持、驱动生态和稳定性。

需要在"从零开发微内核"和"完全基于 Linux 宏内核"之间找到平衡点。

### 决策

agentrt-liunx 采用**微内核化改造策略**：基于 Linux 6.6 内核基线，利用 sched_ext + eBPF kfunc + dynamic pointer + io_uring 实现微内核化改造，而非从零开发微内核。
...
5. **能力足够**：sched_ext（agentrt-liunx 内核增强，主线 6.12+）+ eBPF kfunc（6.6 原生）+ io_uring（6.6 原生）已足够实现微内核化目标
...
## ADR-003: 8 子仓划分（基于微内核设计思想 + agentrt-liunx 工程基线 + Airymax 同源）
...
agentrt-liunx 需要划分子仓结构以支持模块化开发。子仓划分需要满足：

1. 微内核设计思想（按能力域划分）
2. agentrt-liunx 治理组标准（采用 agentrt-liunx 子仓治理模式）
...
2. **agentrt-liunx 治理组对应**：8 子仓与 agentrt-liunx 子仓治理组（Kernel / Base Systems / Security / Cloud Native / QA 等）一一对应
...
| SIG 治理 | 每个子仓对应一个治理组，采用 agentrt-liunx 子仓治理模式 |
...
- 子仓按能力域清晰划分，无功能重叠
- 与 agentrt 同源对应，天然适配
- 与 agentrt-liunx 治理组一一对应，便于社区协作
- 每个子仓可独立测试和演进
...
agentrt-liunx 需要一个安全模型来控制资源访问。候选方案包括：
...
agentrt-liunx 在 airymaxos-security 子仓实现 **capability-based security 模型**（seL4 风格），与 SELinux 共存形成纵深防御。
...
5. **同源红利**：与 agentrt Cupolas 安全模型同源，agentrt 在 agentrt-liunx 上运行时权限检查天然兼容
...
agentrt-liunx 需要一个高性能 IPC 子系统支持服务间通信。需求包括：
...
agentrt-liunx 在 airymaxos-kernel + airymaxos-services 实现 **基于 io_uring 的 IPC 子系统**，采用与 agentrt AgentsIPC 同源的 128B 定长消息头。
...
1. **同源红利**：128B 消息头与 agentrt AgentsIPC 完全同源，agentrt 在 agentrt-liunx 上运行时 IPC 无需任何转换层
...
agentrt-liunx 需要在内核态实现认知循环以支持 Agent 的高性能认知决策。需求包括：
...
agentrt-liunx 在 airymaxos-cognition 子仓实现 **CoreLoopThree kthread 认知循环**，作为内核态认知运行时。
...
1. **同源红利**：CoreLoopThree 三层循环与 agentrt 同源，agentrt 在 agentrt-liunx 上运行时认知调度天然契合
...
agentrt-liunx 需要一个记忆系统支持 Agent 的长期记忆与知识提炼。需求包括：
...
agentrt-liunx 在 airymaxos-memory 子仓实现 **MemoryRovol 内核态实现**，采用 L1-L4 四层递进架构。
...
1. **同源红利**：L1-L4 四层卷载与 agentrt MemoryRovol 完全同源，agentrt 在 agentrt-liunx 上运行时记忆持久化无语义损失
...
agentrt-liunx 需要一个沙箱运行时支持 Agent 的安全执行。需求包括：
...
agentrt-liunx 在 airymaxos-cognition 子仓的 frameworks 模块实现 **Wasm 3.0 沙箱运行时**。
...
| 超节点沙箱 | 集成 agentrt-liunx 超节点沙箱（软硬协同） |
...
6. **agentrt-liunx 集成**：agentrt-liunx 超节点沙箱提供软硬协同优化
...
agentrt-liunx 需要云原生支持以实现大规模 Agent 部署。需求包括：

1. 容器化部署（Agent 容器化）
2. 集群编排（多节点 Agent 协同）
3. 与 agentrt-liunx 超节点 OS 对齐
4. 与 agentrt gateway + sdk 同源
5. 兼容主流云原生生态（K8s / OCI / CNI）

### 决策

agentrt-liunx 在 airymaxos-cloudnative 子仓实现 **K8s CRD + containerd shim 云原生方案**。
...
| 超节点 OS | 集成 agentrt-liunx 超节点 OS |
...
2. **超节点对齐**：agentrt-liunx 超节点 OS 提供大规模容器低时延通信，RPC 时延下降 20%
3. **同源红利**：与 agentrt gateway + sdk 同源，agentrt 应用可原生部署到 agentrt-liunx 集群
...
agentrt-liunx 与 agentrt 的关系是架构设计的核心问题。需要明确：

1. agentrt-liunx 是否共享 agentrt 代码？
2. agentrt 在 agentrt-liunx 上运行是否需要适配层？
3. 两者的演进如何协同？
4. 两者的边界在哪里？

### 决策

agentrt-liunx 与 agentrt 是 **同源且部分代码共享** 的关系（IRON-9 v2 工程铁律）：
...
| 关系 | 可独立运行于任何平台 | agentrt-liunx 是 agentrt 的最佳载体 |
| 适配层 | 无需适配层 | 无需适配层（天然契合） |

### 理由

1. **同源定义**：同源 = 共享设计理念 + 共享契约层代码（IRON-9 v2）。agentrt 和 agentrt-liunx 共享 MicroCoreRT/AgentsIPC/Cupolas/MemoryRovol/CoreLoopThree 的语义，并共享契约层代码（IPC 消息头结构、syscall 编号、capability 令牌格式、MemoryRovol L1-L4 数据结构、CoreLoopThree 接口定义、错误码、规则编号体系，统一存放于 `include/airymax/` 头文件库），实现层各自独立
2. **天然契合**：因为设计假设和实现假设一致，agentrt 在 agentrt-liunx 上运行**无适配层**，天然更稳健
3. **独立性**：agentrt 是跨平台用户态运行时，必须独立于任何特定 OS；agentrt-liunx 是 Linux 发行版，专注于 Linux 6.6 优化
4. **演进协同**：两者通过共享设计理念（ARCHITECTURAL_PRINCIPLES.md）与共享契约层代码（`include/airymax/`）协同演进，契约层变更须经 agentrt + agentrt-liunx 两端 CI 双向校验
5. **IRON-9 v2 约束**：工程标准规范手册 IRON-9 v2（2026-07-07 决策变更）明确规定"agentrt 和 agentrt-liunx 同源且部分代码共享"——共享契约层代码完全共享（`include/airymax/`），语义同源层 API 签名同源实现独立，完全独立层各自独立
6. **边界清晰**：agentrt 是用户态库 + 守护进程，agentrt-liunx 是 Linux 内核 + 用户态服务，边界清晰
...
| 测试验证 | airymaxos-tests 需验证 agentrt 在 agentrt-liunx 上的天然契合性 |
| 文档规范 | 所有文档明确标注"同源 agentrt"对应关系 |
| 部署模式 | agentrt 可独立运行，但在 agentrt-liunx 上运行获得同源红利 |
...
| agentrt 完全依赖 agentrt-liunx | 紧密集成 | 失去跨平台能力 | 违反 agentrt 跨平台定位 |
...
**正面后果**：
- agentrt 在 agentrt-liunx 上运行天然契合，无适配层
- 两者独立演进，互不制约
- 通过共享设计理念保证协同
- agentrt 保持跨平台能力
