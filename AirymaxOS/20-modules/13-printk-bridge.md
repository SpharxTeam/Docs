...
> **单一权威源声明**：本文件是 **printk 桥接设计** 的唯一权威源。`printk_hook` 注册机制、128B 记录格式化规则、Ring Buffer reserve/commit 写入路径、Panic 回退至 `printk_safe` 原生路径的切换条件、性能对比基线、v1.0.1 Capability Folding fastpath C-S9 Badge 校验协作均以本文件为唯一权威定义。
>
...
| Badge 校验透传 | v1.0.1 Capability Folding 下，桥接路径需透传 fastpath C-S9 Badge 校验失败事件至 Ring Buffer（不阻断 printk 主路径） |
...
## §4 v1.0.1 Capability Folding 集成

### 4.1 fastpath C-S9 Badge 校验协作

自 v1.0.1 起，printk 桥接路径与 fastpath C-S9 Badge 校验协作。printk 桥接本身不持有 Badge（printk 是内核原生路径，无 Agent 上下文），但其产出的 128B 记录被 `logger_d` 消费时，`logger_d` 会通过 `agent_caps[1024]` 静态数组反查 `caller_id` 对应的 Badge 状态：
...
`agent_caps[1024]` 静态数组（128KB，sec_d 唯一写者）在 printk 桥接中**不直接访问**——桥接路径不持有 Badge。但桥接产出的记录被 `logger_d` 消费时，`logger_d` 会通过 `agent_caps[rec->caller_id]` 反查 Badge 状态：
...
© 2025-2026 SPHARX Ltd. All Rights Reserved. | printk 桥接设计 | v1.0.1 | 2026-07-21
