...
8. **config_d [IND/SS]**：12 daemon 之一的统一配置管理守护进程，与 sec_d 协作加载 v1.0.1 Capability Folding 配置参数（`agent_caps[1024]` 容量、Badge Epoch 步进），通过 sysfs/procfs 与内核交互，systemd 集成 + 热重载机制，SSoT 见 `11-unified-config.md`。
...
├── config/                 # 配置工具 [IND]
├── config-d/          # config_d（12 daemon 之一，统一配置管理）[IND/SS]
...
### 3.2.1 config-d/（config_d，12 daemon 之一）[IND/SS]

config_d 是 agentrt-linux 12 daemon 之一，负责统一配置管理：
...
- `sec_d-bridge/`：与 sec_d 协作加载 v1.0.1 Capability Folding 配置参数 [SS]。
...
### 4.7 config_d（统一配置管理，12 daemon 之一）[IND/SS]

config_d 是 agentrt-linux 12 daemon 之一（完整名单见 §6.7），负责统一配置管理。SSoT 见 `11-unified-config.md`。

**配置路径**：
- 标准路径：`/etc/agentrt/`（**禁止**非 agentrt 命名空间的旧路径，SSoT 见 `11-unified-config.md`）。
- daemon 主配置：`/etc/agentrt/config_d.yaml`。
...
- v1.0.1 Capability Folding 配置参数通过 sysfs 暴露：
  - `/sys/fs/airymax/agent_caps_capacity` → `agent_caps[1024]` 容量（默认 1024，128KB）。
  - `/sys/fs/airymax/badge_epoch_step` → Badge Epoch 步进（默认 1，由 sec_d 推进）。
  - `/sys/fs/airymax/syscall_table_size` → v1.0.1 Syscall 24 槽位（4 核心 + 20 预留）。
...
**systemd 集成**：
- `config_d.service`：Type=notify，启动后通过 sd_notify 通知 systemd 就绪。
- 启动顺序：`config_d.service` 在 `sec_d.service` 之前启动（提供配置）。
...
系统模块的 [SC] 层通过 10 个核心头文件与 agentrt 共享（airymaxmon + DevStation + config_d 消费，SSoT 见 `50-engineering-standards/120-cross-project-code-sharing.md`）：
...
| `include/uapi/linux/airymax/syscalls.h` | v1.0.1 Syscall 24 槽位（4 核心 + 20 预留）：`airy_sys_call`(0)/`airy_sys_rovol_ctl`(1)/`airy_sys_sched_ctl`(2)/`airy_sys_clt_notify`(3) | airymaxmon 监控 syscall 调用统计 + config_d 推送配置 |
...
| 降级触发条件 | 降级后行为 | 恢复条件 |
| ------- | ------- | ------- |
| config_d 配置源不可达 | 降级为 `/usr/share/agentrt/defaults/` 内置默认配置 + 只读运行（拒绝写入） | 配置源恢复后强制全量 reload |
...
| sec_d 进程不可达 | config_d 缓存最近一次 capability 配置 + 暂停 Badge Epoch 推进 | sec_d 恢复后通过 `airy_sys_call(0)` 同步 Epoch |

\[DSL] 层保证系统工具在配置/LLM/sec_d 故障下仍能维持基础监控与运维能力，但拒绝写入操作以避免配置与 Epoch 倒流。

### 6.5 v1.0.1 Capability Folding 在系统场景的应用

v1.0.1 Capability Folding（ADR-014 seL4 唯一来源，参见 `docs/architecture/adr/ADR-014.md`）在系统模块下的核心约束：
...
| 序号 | daemon | 系统子仓协作内容 |
| -- | ------ | ---------- |
| 1 | sec\_d | config_d 推送 capability 配置参数 + sysfs 通道；airymaxmon 监控 sec_d 状态 |
| 2 | cogn\_d | DevStation 通过 io_uring 调用 LLM；airymaxmon 监控认知循环阶段 |
| 3 | mem\_d | airymaxmon 监控 MemoryRovol L1-L4 指标；config_d 推送内存配置 |
| 4 | gateway\_d | config_d 推送跨节点 Badge 一致性 gossip 配置（参考 `06-cloudnative.md` §6.6） |
| 5 | logger\_d | journald 集成；airymaxmon 日志聚合 |
| 6 | macro\_superv | airymaxmon 监控宏调度状态；config_d 推送调度策略 |
| 7 | audit\_d | airymaxmon 监控 `AIRY_FAULT_AUDIT_TAMPER = 0x100B` 上报 |
| 8 | sched\_d | airymaxmon 监控 sched_tac 统计；config_d 推送调度参数 |
| 9 | dev\_d | airymaxmon 监控设备状态；config_d 推送设备配置 |
| 10 | net\_d | airymaxmon 监控网络状态；config_d 推送网络配置 |
| 11 | vfs\_d | airymaxmon 监控文件系统状态；config_d 推送挂载配置 |
...
graph TD
    ...
    subgraph SS["[SS] 语义同源层"]
        ...
        CONFIG[配置管理<br/>agentrt config → config_d/sysctl/systemd]
    ...
    subgraph IND["[IND] 独立层"]
        ...
        CONFIGDAEMON[config_d<br/>YAML/TOML + 热重载 + sysfs 推送]
    end
    subgraph DSL["[DSL] 降级生存层"]
        CFG_FALL[config_d 内置默认配置只读]
        MON_FALL[airymaxmon Prometheus 拉取模式]
        DEV_FALL[DevStation 规则引擎模式]
    end
    subgraph CAP["v1.0.1 Capability Folding（系统场景应用）"]
...
| IRON-9 v3 四层合规 | [SC] 10 头文件 + [SS] 8 API + [IND] 12 项独立实现 + [DSL] 4 降级场景 + v1.0.1 Capability Folding sysfs 推送 + 12 daemon 完整列表 | PASS |
...
| config_d 完整性 | §1.2/§3.2.1/§4.7/§6.5/§6.6 均引用 config_d（路径 `/etc/agentrt/`，YAML/TOML，sysfs 推送，热重载，与 sec_d 协作） | PASS |
| 配置路径 SSoT | `/etc/agentrt/`（禁止非 agentrt 命名空间的旧路径），SSoT 见 `11-unified-config.md` | PASS |
| v1.0.1 Capability Folding | `agent_caps[1024]` + 64-bit Badge + O(1) 撤销 + fastpath C-S9 + slowpath airy_lsm（`LSM_ORDER_MUTABLE`）均在 §6.5 引用 | PASS |
| 12 daemon 完整列表 | sec_d/cogn_d/mem_d/gateway_d/logger_d/macro_d/audit_d/sched_d/dev_d/net_d/vfs_d/config_d 共 12 个均在 §6.6 列出 | PASS |
...
- [03-security.md](03-security.md)——安全模块（[SC] security_types.h + lsm_types.h，安全配置工具 + v1.0.1 Capability Folding）
