...
8. **config_d [IND/SS]**：12 daemon 之一的统一配置管理守护进程，与 sec_d 协作加载 v1.1 Capability Folding 配置参数（`agent_caps[1024]` 容量、Badge Epoch 步进），通过 sysfs/procfs 与内核交互，systemd 集成 + 热重载机制，SSoT 见 `11-unified-config.md`。
...
**config_d 与 sec_d 协作**（v1.1 Capability Folding）：
...
- **`agent_caps[1024]` 静态数组（16KB）**：config_d 通过 sysfs（`/sys/fs/airymax/agent_caps_capacity`）暴露容量配置；sec_d 作为唯一写者编译 Badge 写入数组。
...
- **v1.1 Syscall 24 槽位**：config_d 通过 sysfs（`/sys/fs/airymax/syscall_table_size`）暴露槽位数量；airymaxmon 监控 syscall 调用统计。
