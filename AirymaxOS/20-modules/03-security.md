...
| 数据流 | 安全切入点 | 同源标注 |
|--------|-----------|----------|
| 调度数据流 | task_create/task_kill 钩子 + capability 检查 | [SS] |
| IPC 数据流 | binder/ipc 钩子 + 消息端点 capability 校验 | [SS] |
| eBPF 数据流 | eBPF 程序签名验证 + BPF_LSM 钩子 | [SS] |
...
### 3.2 lsm/（agent_lsm）[SS]

与 Cupolas 同源的 LSM 实现，调度机制语义同源但实现独立：

- `agent_lsm.c`：LSM hook 注册（file_ops、net_ops、task_ops、ipc_ops）——`security_add_hooks()` [SS]。
...
| 15 | `bpf_lsm_##NAME` trampoline | BPF_LSM 钩子 | 用户态 eBPF | 内核 X-macro 生成 |
