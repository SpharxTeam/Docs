...
>
> **v1.0.1 Capability Folding 集成声明**（A-IPC 第一块基石）：自 v1.0.1 起，A-UCS 新增 A-IPC Capability Folding 相关配置项——IPC 消息头大小（128B Layout C v4）、magic 值（0x41524531 'ARE1'）、`agent_caps[]` 静态数组容量（1024）、Badge Epoch 位宽（16-bit）、fastpath C-S9 校验超时阈值等。这些配置项是 Capability Folding 单平面架构落地的运行时参数 SSoT。详见 §7 A-IPC Capability Folding 配置项。
>
 ...
 * v1.0.1: Layout C v4 定长 128B = 2 cache lines，含 capability_badge (offset 40-47)
...
/* 魔数（二进制标识，两端必须一致）
 * v1.0.1: AIRY_IPC_MAGIC 从 0x41495043 'AIPC' 变更为 0x41524531 'ARE1'
...
#define AIRY_IPC_MAGIC   0x41524531   /* 'ARE1' — Airymax Runtime Epoch（v1.0.1 Capability Folding） */
...
| 配置项 | 值 | 影响 | 热重载 |
|--------|-----|------|--------|
| `AIRY_IPC_HDR_SIZE` | 128B（v1.0.1: 64B→128B） | IPC 消息头结构大小（Layout C v4，含 capability_badge） | 否 |
...
| `AIRY_IPC_MAGIC` | 0x41524531 'ARE1'（v1.0.1: 'AIPC'→'ARE1'） | IPC 魔数（Capability Folding 单平面架构标识，发布即冻结） | 否 |

### 2.4 扩展错误码与故障码（[SC] 共享契约，v1.0.1 新增）

v1.0.1 Capability Folding 引入以下扩展错误码与故障码，均定义于 `include/uapi/linux/airymax/error.h`（[SC] 共享契约层），采用 `AIRY_E*` 前缀统一命名空间：
...
内核 sysctl 与用户态 YAML/TOML 的语义映射（v1.0.1: `cap.cache_ttl` 已废弃，替换为 Capability Folding 相关配置）：
...
| `kernel.airy.cap_epoch_bits` | `cap.epoch_bits` | v1.0.1: Badge Epoch 位宽（默认 16-bit） | 16 |
| `kernel.airy.cap_randtag_bits` | `cap.randtag_bits` | v1.0.1: Badge Random Tag 位宽（默认 32-bit） | 32 |
| `kernel.airy.cap_forge_threshold` | `cap.forge_threshold` | v1.0.1: Badge 伪造检测阈值（连续失败次数，触发警报） | 100 |
| `kernel.airy.ipc_fastpath_budget_ns` | `ipc.fastpath_budget_ns` | v1.0.1: fastpath C-S9 Badge 校验时间预算（ns） | 10 |
...
struct airy_config {
    ...
    /* v1.0.1: Capability Folding 配置（替代旧 cap_cache_ttl） */
...
## §7 A-IPC Capability Folding 配置项（v1.0.1 新增）
 ...
 *   仍保持编译时确定，不引入动态索引结构（v1.0.1 Capability Folding
 *   已确立 agent_caps[1024] 静态数组为唯一存储模型）
 */
#define AIRY_CAP_MAX_AGENTS_DEFAULT  1024

/* 静态数组大小 = 1024 × sizeof(struct airy_cap_slot) = 128KB
...
© 2025-2026 SPHARX Ltd. All Rights Reserved. | A-UCS 统一配置管理体系设计 | v1.0.1 | 2026-07-21
