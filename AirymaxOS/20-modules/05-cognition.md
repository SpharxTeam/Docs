...
| 6  | `macro_d`   | macro\_d 监管 cogn_d 生命周期（崩溃重启、资源限制违规冻结）                      | `AIRY_IPC_OP_FREEZE` / systemd lifecycle                          |
...
| 11 | `config_d`  | cogn_d 启动配置（kthread 优先级、令牌桶容量）由 config\_d 管理                    | io\_uring IPC → config\_d 配置读写                                    |
...
> **文档结束** | v1.0.1 | IRON-9 v3 同源且部分代码共享 | 认知循环贯穿 4 大数据流 | 0.1.1 = 文档体系完成
