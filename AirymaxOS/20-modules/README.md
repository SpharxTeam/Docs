...
| 13 | [13-printk-bridge.md](13-printk-bridge.md) | **A-ULP** | printk-bridge（内核 printk → 用户态 `logger_d` 桥接 + 等级映射 + `AIRY_FAC_*` facility） | v1.0.1 | 维护中 |
...
| 8 | `syscalls.h` | Syscall 编号体系（v1.0.1: 4 核心 + 20 预留 = 24 槽位，`AIRY_SYS_*` 前缀；`airy_sys_call` / `airy_sys_rovol_ctl` / `airy_sys_sched_ctl` / `airy_sys_clt_notify`） | **A-UCS** | kernel + 全部 |
