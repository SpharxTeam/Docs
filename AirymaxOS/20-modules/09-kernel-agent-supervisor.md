...
### 1.5.x seL4 Notification 借鉴

agentrt-linux 的事件通知机制（eventfd）借鉴 seL4 Notification 的位图语义（`src/object/notification.c`）：

| seL4 Notification | agentrt-linux 实现 | 说明 |
|-------------------|-------------------|------|
| `sendSignal()` | `eventfd_signal(ctx, n)` | 异步信号发送 |
| `receiveSignal()` | `eventfd_ctx_get()` + `wait_event` | 异步信号接收 |
| NtfnState_Idle/Waiting/Active | eventfd 计数器状态 | 3 态语义 |

**位图聚合借鉴**：seL4 Notification 使用 `word_t` 位图（每 bit 代表一个事件），agentrt-linux 在 eventfd 之上实现多事件聚合——`eventfd_signal(ctx, 1ULL << fault_type)` 按位编码 Fault 类型，Macro-Supervisor 通过 `eventfd_ctx_get()` 读取后解码。
...
```c
/* kernel/kernel/superv/airy_ipc_freeze.c —— IPC 队列冻结 */
...
**v1.0.1 新增**：A-IPC Capability Folding 引入 `FREEZE` opcode（0x0005），允许 Micro-Supervisor 或 Macro-Supervisor 通过 IPC 消息主动冻结目标 Ring，无需独立 syscall。这是 Capability Folding 单平面架构的体现——Ring 冻结也通过 io_uring 数据面完成，不引入额外控制面 syscall。
...
```c
/* kernel/kernel/superv/airy_die_notify.c —— die_notifier 注册 */
...
```c
/* kernel/kernel/superv/airy_eventfd.c —— eventfd 故障通知 */
...
```c
/* kernel/kernel/superv/airy_cap_check.c —— 冷酷执法（v1.0.1） */
...
### §7.5 Double Fault 处理（借鉴 seL4）

Micro-Supervisor 的正常故障处理路径（Agent 故障 → 冷酷执法 → Macro-Supervisor 裁决）依赖于 Macro-Supervisor（macro_d）的正常运行。当 Macro-Supervisor 自身故障时，需要 Double Fault 处理机制。

**借鉴 seL4 Double Fault 流程**（`src/api/faulthandler.c` `handleDoubleFault()`）：
1. **检测**：macro_d 心跳超时（systemd watchdog 30s 无响应）
2. **兜底**：内核直接执行预定义处置策略（不依赖用户态 macro_d）
   - 冻结所有 Agent 的 IPC Ring（`ring->frozen = true`）
   - 降级至 [DSL] 模式（`#ifdef AIRY_SC_FALLBACK`）
   - 通过 `printk_safe` 输出最终日志
3. **恢复**：systemd 重启 macro_d，macro_d 从 `/var/lib/airy/sec_d_state.json` 恢复状态
4. **审计**：Double Fault 事件写入 `AIRY_FAULT_DOUBLE_FAULT`（预留 0x100C）
...
© 2025-2026 SPHARX Ltd. All Rights Reserved. | Micro-Supervisor 内核模块设计 | v1.0.1 | 2026-07-21
