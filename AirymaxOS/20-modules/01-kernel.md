...
> **核心约束**：IRON-9 v3 同源且部分代码共享——与 agentrt 用户态 atoms/corekern 通过 \[SC] 共享契约层（10 头文件）+ \[SS] 语义同源层 + \[DSL] 降级生存层协作，\[IND] 内核态 sched_tac（基于 Linux 6.6 原生调度类组合 SCHED_DEADLINE/SCHED_FIFO/EEVDF + cgroup cpuset 隔离 + seL4 MCS 语义映射的策略框架，非用户态调度器，非 sched_ext）/io\_uring/eBPF/Rust 实现独立。**Syscall 架构**：12 核心 + 12 预留 = 24 槽位（Capability Invocation 统一入口 + IPC 数据面/控制面分离）。**命名统一**：所有 \[SC] 共享标识符使用 `AIRY_*` 前缀，C 函数使用 `airy_sys_*` 前缀。\
> **子仓编号**：01\
...
7. **\[SC] 共享契约层物理宿主 \[IND]**：IRON-9 v3 全部 10 个 \[SC] 共享契约头文件物理宿主在 `kernel/include/airymax/`，其他子仓通过 `-I` 引用确保单一数据源。
...
| ---------------- | ------------------ | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | --------------------------------- |
| **\[SC] 共享契约层**  | 完全共享代码             | 10 个头文件（详见 §10.1）：error.h / log_types.h / ipc.h / sched.h / memory_types.h / security_types.h / cognition_types.h / syscalls.h / uapi_compat.h / lsm_types.h                                                                                                                                                                                | `kernel/include/airymax/`（单一物理宿主，每个头文件底部含 \[DSL] 降级块） |
...
| IPC            | 用户态消息队列                         | io\_uring 零拷贝 IPC（数据面）+ 8 IPC 原语 syscall（控制面） | \[SS]  |
...
这消除了独立 syscall 的需求（task_submit/cancel/status/capability_request/revoke/lsm_ctl/wasm_load），syscall 数量保持 12 核心（airy_sys_call 统一 capability 调用入口消除了独立 syscall 的需求，但 airy_sys_call 本身即为 12 核心 syscall 之一），不缩减。
...
| 平面      | 路径                                | 延迟      | 通信方式               | 用途                                |
| ------- | --------------------------------- | ------- | ------------------ | --------------------------------- |
| **控制面** | 用户态 → syscall → 内核 → 返回           | \~1 μs  | 8 个 IPC 原语 syscall | capability invocation、同步 IPC、策略设置 |
...
错误码统一使用 `AIRY_E*` 前缀，SSoT 定义于 `include/airymax/error.h`：
