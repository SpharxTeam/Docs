...
> **核心约束**：IRON-9 v3 四层共享模型（\[SC] 共享契约层 + \[SS] 语义同源层 + \[IND] 完全独立层 + \[DSL] 降级生存层）——与 agentrt 用户态 atoms/corekern 通过 \[SC] 共享契约层（10 头文件）+ \[SS] 语义同源层 + \[DSL] 降级生存层协作，\[IND] 完全独立层承载内核态 sched_tac（基于 Linux 6.6 原生调度类组合 SCHED_DEADLINE/SCHED_FIFO/EEVDF + cgroup cpuset 隔离 + seL4 MCS 语义映射的策略框架，非用户态调度器，非 sched_ext）/io\_uring/eBPF/Rust 实现。**Syscall 架构**（v1.0.1 Capability Folding 后）：4 核心 + 20 预留 = 24 槽位（`airy_sys_call` sec_d 专属管理入口 + 3 控制原语，IPC 数据面完全由 io\_uring `IORING_OP_URING_CMD` 承载，fastpath C-S9 内联 Badge 校验）。**命名统一**：所有 \[SC] 共享标识符使用 `AIRY_*` 前缀，C 函数使用 `airy_sys_*` 前缀。\
> **子仓编号**：01\
...
**v1.0.1 目标**：**4 核心 + 20 预留 = 24 槽位**（Capability Folding 后，见 [01-syscalls.md](../30-interfaces/01-syscalls.md)），SSoT 定义于 `syscalls.h`：
...
**数据面 I/O 完全由 io\_uring 处理（零 syscall）**。v1.0.1 Capability Folding 后，8 个 seL4 风格 IPC 原语（send/recv/nbsend/nbrecv/reply_recv/yield/reply/notify）全部移除，IPC 数据传递完全由 io\_uring `IORING_OP_URING_CMD` 承载。`airy_sys_call` 语义收窄为 sec_d 专属管理入口（Badge 编译/撤销/LSM_ctl/Wasm_load）。详见 [01-syscalls.md](../30-interfaces/01-syscalls.md) §2.2。
...
| IPC 数据流（控制面） | v1.0.1 Capability Folding 后控制面已折叠：4 核心 syscall（`airy_sys_call` sec_d 专属管理 + 3 控制原语），原 8 个 seL4 风格 IPC 原语已全部移除 | \[SS]  |
...
│   │   ├── stc_policy.c        # stc_realtime/interactive/agent/batch 4 类策略定义 [SS]
│   │   ├── stc_policy.h        # 策略枚举与回调签名（与 [SC] sched.h 对齐）[SS]
│   │   ├── stc_dispatch.c      # sched_tac → 原生 SCHED_DEADLINE/FIFO/EEVDF 派发 [SS]
...
│   ├── bpf/                    # eBPF struct_ops 扩展 [SS]
│   │   ├── airy_struct_ops.c   # struct_ops 值类型 + 状态机（INIT/REGISTERED/ACTIVE/DRAINING）[SS]
│   │   └── airy_bpf_probe.c    # BPF 可观测性探针（printk ring 缓冲）[SS]
│   ├── object/                 # 内核对象管理 [IND]
│   │   └── airy_object.c       # DEFINE_SPINLOCK + LIST_HEAD 内核对象管理 [IND]
│   │      # 注：seL4 风格 cte/cspace/mdb/cnode_ops 为设计溯源参考（ES-SEL4-05~09），
│   │      # v1.0.1 Capability Folding 后已简化为 agent_caps[] 静态数组 + Badge 64-bit 编码，
│   │      # 实际实现见 security/airy/（airy_cap_derive.c 等 7 操作）
│   ├── taskflow/               # 任务流原语 [SS]
...
- `stc_policy.c`：4 类策略枚举定义（`stc_realtime` / `stc_interactive` / `stc_agent` / `stc_batch`），数值与 \[SC] `AIRY_SCHED_POLICY_*` 宏对齐 \[SS]。
- `stc_policy.h`：策略枚举与回调签名内部头文件（与 \[SC] `sched.h` 对齐）\[SS]。
- `stc_dispatch.c`：sched_tac → 原生 Linux 调度类派发（SCHED_DEADLINE/SCHED_FIFO/SCHED_NORMAL(EEVDF)/SCHED_BATCH），记录派发日志并更新统计 \[SS]。
- `stc_mcs_map.c`：seL4 MCS（Mixed-Criticality Systems）关键性标志 → Linux 调度策略 + GFP 分配提示映射（低 16 位 SCHED_\* 策略 + 高 16 位 GFP mask）\[SS]。
- `stc_stats.c`：sched_tac 派发统计（全局 atomic 计数器 + 按策略分解，通过 `stc_stats_read()` 查询）\[SS]。
...
### 3.3 corekern/bpf — eBPF struct\_ops 扩展 \[SS]

eBPF struct\_ops 扩展，struct\_ops 状态机与 common\_value \[SC] 与 agentrt 共享：

- `airy_struct_ops.c`：Airymax 专属 struct\_ops 值类型定义 + 状态机（INIT/REGISTERED/ACTIVE/DRAINING 4 态），仅在 `CONFIG_BPF=y` 时编译，实际 `bpf_struct_ops` 注册延迟到集成层 \[SS]。
- `airy_bpf_probe.c`：BPF 可观测性探针，提供 `airy_bpf_probe_trace()` 轻量级 tracing（基于内核 printk ring 缓冲），仅在 `CONFIG_BPF=y` 时编译 \[SS]。
- 与 `bpf_struct_ops.h`（补充共享文件，非 \[SC] 核心头文件）协作。
- 自定义 kfunc 导出（`bpf_agent_decision_get` 等）与 ringbuf 事件格式（与 agentrt AgentsIPC 128B 消息头同源）由集成层承载。
...
>
> 详见 §5.0 v1.0.1 Capability Folding 实现概览。SSoT：[03-capability-model.md](../110-security/03-capability-model.md) + [07-ipc-fastpath.md](../30-interfaces/07-ipc-fastpath.md)。
...
### 5.0 v1.0.1 Capability Folding 实现概览

v1.0.1 Capability Folding 决策对 seL4 风格 CSpace/MDB/radix tree 进行了工程简化，**保留设计语义、简化实现路径**。以下为 v1.0.1 实现概览（SSoT：[03-capability-model.md](../110-security/03-capability-model.md) + [07-ipc-fastpath.md](../30-interfaces/07-ipc-fastpath.md)）：
...
**关键数据结构**（`security/airy/` + `include/uapi/linux/airymax/lsm_types.h`）：
...
```c
/* security/airy/airy_cap.h: io_uring fastpath C-S9 Badge 内联校验
 * 3 次 READ_ONCE + 位操作 + 比较，__always_inline 强制内联 */
...
**与 seL4 设计溯源对照**：seL4 CSpace radix tree、MDB 派生树、CTE 双向链表等概念在 v1.0.1 保留为**设计溯源参考**（§5.1-§5.4 详述 seL4 原始模型），但工程实现简化为静态数组 + Badge 编码 + epoch 撤销。这一简化符合"对 Linux 6.6 进行 seL4 思想借鉴的微内核化改造"定位（ADR-012 + ADR-014），不引入 seL4 完整 CSpace/MDB 实现复杂度。
...
这消除了独立 syscall 的需求（task_submit/cancel/status/capability_request/revoke/lsm_ctl/wasm_load），v1.0.1 Capability Folding 后，airy_sys_call 收窄为 sec_d 专属管理入口（Badge 编译/撤销 + LSM_ctl + Wasm_load），IPC 数据传递由 io_uring fastpath C-S9 内联 Badge 校验承载，syscall 总数从 12 精简为 4。
...
> **v1.0.1 Capability Folding 说明**：原 8 个 seL4 风格 IPC 原语 syscall（`airy_sys_send`/`recv`/`nbsend`/`nbrecv`/`reply_recv`/`yield`/`reply`/`notify`）在 v1.0.1 全部移除，IPC 数据传递完全由 io\_uring `IORING_OP_URING_CMD` 承载（见下方数据面表）。能力校验折叠到数据面 fastpath C-S9 内联 Badge 校验，`airy_sys_call` 语义收窄为 sec_d 专属管理入口。seL4 借鉴映射保留作设计溯源参考。
...
### 6.7 v1.0.1 Capability Folding 后的 fastpath C-S9 Badge 校验路径

v1.0.1 Capability Folding 将 capability 校验从控制面 syscall 折叠到数据面 io\_uring fastpath，**消除每条 IPC 消息的 syscall 开销**。SSoT：[07-ipc-fastpath.md](../30-interfaces/07-ipc-fastpath.md)。
...
### 14.2 12 daemon 内核切入点（v1.0.1 Capability Folding 后）
...
- sec_d 是 v1.0.1 Capability Folding 的**核心枢纽**：唯一 `agent_caps[1024]` 写者，所有 Badge 编译/撤销请求通过 `airy_sys_call` 串行化处理（令牌桶限流 + 50ms SLO）。
