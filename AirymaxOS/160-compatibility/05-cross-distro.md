...
1. **Agent 应用一次编写，多发行版运行**：Agent 应用编译一次，可在 Ubuntu/Debian/Fedora/RHEL/主流 Linux 发行版 等主流发行版上运行
...
5. **主流 Linux 发行版标准兼容**：agentrt-linux 的工程思想与实现方式与 主流 Linux 发行版标准保持一致性
...
- **内核版本**：各发行版内核版本不一（Ubuntu 22.04 5.15/6.8、Fedora 40 6.8、RHEL 9 5.14 等），agentrt-linux 以 **6.6 为唯一原生基线**，其余版本仅作 DKMS 兼容目标
- **包管理器**：Ubuntu/Debian 使用 apt/dpkg，Fedora/RHEL/主流 Linux 发行版 使用 dnf/rpm
...
本方案遵循 Linux 6.6 发行版兼容性哲学（参考 主流 Linux 发行版 兼容性规范）与 seL4 接口最小化思想：
...
|--------|------|---------|-----------|--------|---------|------|
| **agentrt-linux** | 1.0.1 | 6.6 | 2.38 | dnf/rpm | 原生 | 完整支持（唯一原生基线） |
...
| Fedora | 40 | 6.8 | 2.39 | dnf/rpm | L1+L3+L4+L5 | 内核模块 DKMS 可加载 |
| Fedora | 41 | 6.10 | 2.40 | dnf/rpm | L1+L3+L4+L5 | 内核模块 DKMS 可加载 |
| RHEL | 9 | 5.14 | 2.34 | dnf/rpm | L1+L4+L5 | Agent 应用二进制兼容 |
| 主流 Linux 发行版 | 24.03 LTS | 6.6 | 2.38 | dnf/rpm | L1+L3+L4+L5 | 完整兼容（同源 6.6 基线） |
| 主流 Linux 发行版 | 26.03 | 6.6+ | 2.39+ | dnf/rpm | L1+L3+L4+L5 | 完整兼容 |
...
```bash
# RPM 格式（Fedora/RHEL/主流 Linux 发行版/agentrt-linux）
...
| 主流 Linux 发行版 | /etc/sysconfig/ | 配置文件兼容 |
...
| 主流 Linux 发行版 24.03 | 252 | 完整功能 |

---

## 11. 主流 Linux 发行版标准兼容性

### 11.1 工程思想一致性

agentrt-linux 的工程思想与实现方式与 主流 Linux 发行版 保持一致性（参考 主流 Linux 发行版 LTS）：
...
| 测试维度 | 测试内容 | 发行版 |
|---------|---------|--------|
| 二进制兼容 | Agent 应用运行 | Ubuntu/Debian/Fedora/RHEL/主流 Linux 发行版 |
| 内核模块 | 模块加载与卸载 | Fedora/主流 Linux 发行版 |
