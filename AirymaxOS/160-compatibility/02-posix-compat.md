...
agentrt-linux 提供两种进程创建接口：标准 POSIX `fork()` 与 Agent 专属 `agentrt_agent_fork()`。下表详细对比两者语义差异：

| 维度 | `fork()`（POSIX） | `agentrt_agent_fork()`（agentrt-linux 扩展） |
|------|-------------------|-----------------------------------------------|
...
| `sched_setscheduler(pid, SCHED_EXT, &param)` | agentrt-linux 扩展（复用 SCHED_EXT=7） |
