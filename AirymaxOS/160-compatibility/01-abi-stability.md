...
> **同源映射**: agentrt ABI 稳定性（IRON-9 v2 [SC] 共享契约层共享 syscall 编号 + [SS] 语义同源层 API 签名）
> **IRON-9 v2 层次**: [SC] 共享契约层（syscall 编号、错误码、消息头结构）+ [SS] 语义同源层（API 签名）+ [IND] 完全独立层（内部实现）
...
- agentrt ABI 稳定性规范（IRON-9 v2 [SC] 同源）
