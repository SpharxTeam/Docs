...
3. **统一接入**：所有监控数据通过 A-ULS（Unified Supervision）模块 macro_superv 汇聚，所有日志事件通过 A-ULP（Unified Log Processing System）模块 logger_daemon 采集，避免多通道重复上报。
...
| 通道 | 模块 | 数据类型 | 触发模式 | 延迟 |
|------|------|----------|----------|------|
| 指标通道 | A-ULS / macro_superv | 数值型指标（CPU、内存、Token 等） | 周期轮询 + 异常推送 | ≤100ms |
| 日志通道 | A-ULP / logger_daemon | 文本事件（崩溃、警告、错误） | 事件驱动 | ≤10ms |
...
|--------|------|-----------|----------|
| macro_superv | 统一监管 | 监管的子 daemon 数、重启次数、心跳超时数 | A-ULS 内部计数器 |
| logger_daemon | 统一日志 | Ring Buffer 占用率、丢弃日志数、写入 QPS | A-ULP 内部计数器 |
| config_daemon | 统一配置 | 配置加载耗时、热更新次数、校验失败数 | A-UCS 内部计数器 |
...
- `liveness`：进程是否存活（0/1），由 systemd WatchdogSec 与 macro_superv 心跳共同判定。
...
- **写语义**：除 `health_score` 由 macro_superv 周期更新外，其余文件对用户态只读（0444 权限）。
...
| 组件 | 角色 | 实现位置 | 数据通路 |
|------|------|----------|----------|
| ftrace 采集器 | 内核 tracepoint 抓取 | 内核模块 `airy_mon_ftrace.ko` | `trace_pipe` → macro_superv |
| sysfs 采集器 | sysfs 周期轮询 | macro_superv 用户态线程 | 读 `/sys/airy/monitor/**` |
| procfs 采集器 | procfs 周期轮询 | macro_superv 用户态线程 | 读 `/proc/**` |
| daemon 埋点 | 业务指标上报 | 12 daemon 各自实现 | io_uring CMD → logger_daemon |
| LSM hook 计数器 | 安全事件计数 | 内核 LSM 钩子 | `/sys/airy/security/` |

### 5.2 ftrace 采集

macro_superv 启动后通过 `tracefs` 启用以下 tracepoint：
...
macro_superv 内置采集线程池（默认 4 线程），按如下周期轮询：
...
底层实现复用 io_uring 的 `IORING_OP_URING_CMD`，将指标批量提交到 logger_daemon 的 ring buffer，避免 syscall 开销。
...
- **清理**：超过 30 天的数据自动删除，由 cron-like 任务（由 macro_superv 周期触发 audit_d）执行。
...
告警阈值通过 A-UCS（Unified Configuration Subsystem）模块 config_daemon 集中管理，配置文件位于 `/etc/airy/monitor/thresholds.yaml`：
...
macro_superv 周期比对采集值与阈值，命中后通过以下通道通知：

| 通道 | 目的 | 延迟 |
|------|------|------|
| `/dev/airy_alert` 字符设备 | 内核态订阅者（如 sec_d） | <1ms |
| `sd_notify(STATUS=...)` | systemd 单元状态更新 | <5ms |
| `logger_daemon` Ring Buffer | 持久化 + 用户态订阅 | <10ms |
...
logger_daemon 在监控体系中承担"事件通道"角色：

- 所有 daemon 崩溃、重启、错误日志通过 A-ULP 上报。
- macro_superv 订阅 logger_daemon 的实时流，将日志事件转化为监控指标（如 `airy_daemon_error_total`）。
- audit_d 同时消费指标流和日志流，进行关联分析。

### 8.2 与 A-ULS（统一监管）的关系

macro_superv 在监控体系中承担"指标通道"角色：
...
- Agent 迁移场景下，源节点与目标节点的 macro_superv 通过 RPC 交换 Agent 监控快照。
...
| logger_daemon 事件流 | 0.2% CPU | 13% |
| 其他 | 0.1% CPU | 7% |
| **总计** | **1.5% CPU** | 100% |

内存占用：audit_d 进程常驻 80MB，macro_superv 采集缓冲 20MB。
...
- 分布式采集：每 100 Agent 分配一个 macro_superv 副本。
- 联邦聚合：多节点 audit_d 通过 gRPC 互查。

### 9.3 失败处理

- audit_d 崩溃：macro_superv 在 5s 内重启，期间短期缓冲保留在 logger_daemon Ring Buffer，不丢失数据。
- macro_superv 崩溃：systemd 在 10s 内（WatchdogSec）重启，期间 sysfs 数据仍可由用户态直读。
...
参数热更新通过 `config_daemon` 发送 SIGHUP 至 macro_superv 与 audit_d 完成。
...
- 日志事件中的敏感字段（如密钥、PII）由 logger_daemon 在写入前脱敏。
...
- 支持分布式 macro_superv 副本（1000+ Agent 场景）。
