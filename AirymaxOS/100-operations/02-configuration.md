...
**OS-KER-202**：任何 agentrt-linux 自定义内核参数（`kernel.airymaxos_*`）必须在 `Documentation/admin-guide/sysctl/kernel.rst` 等价文档中登记，并遵循 Linux sysctl 文档规范（E-7 文档即代码）。
...
`/etc/agentrt/` 是 agentrt-linux 12 daemons 的系统级配置根目录，与 agentrt 用户态运行时同源（IRON-9 v2 同源且部分代码共享）。目录包含：`agentrt.conf`（全局配置，日志等级/IPC 端口/运行模式）、12 个 daemon 配置（`gateway.conf`/`llm.conf`/`tool.conf`/`sched.conf`/`market.conf`/`monit.conf`/`channel.conf`/`info.conf`/`notify.conf`/`observe.conf`/`hook.conf`/`plugin.conf`）、`agentsipc.conf`（AgentsIPC 协议参数）、`microcorert.conf`（MicroCoreRT 用户态适配参数）、`keys/`（密钥目录，`0600`）与 `conf.d/`（drop-in 覆盖目录）。
...
# agentrt-linux（AirymaxOS）llm_d 资源限制覆盖（本地定制）
[Service]
MemoryMax=8G
TasksMax=2048
# LLM 推理需要更大的 fd 上限
...
| VALIDATING | VALIDATION_FAILED | 校验返回 `CONFIG_E_PARSE` / `CONFIG_E_RANGE` / `CONFIG_E_HEADER_SIZE` 等错误码 | 记录失败规则编号（如 `OS-STD-210 violated`），告警路由到 monit_d（OS-OPS-115） |
...
agentrt-linux 配置管理与 agentrt 遵循 IRON-9 v2 同源且部分代码共享原则：**同源**——`/etc/agentrt/` 配置目录与 agentrt 用户态 `~/.agentrt/` 共享配置文件名与语义（`gateway.conf`、`llm.conf` 等），AgentsIPC 128B 消息头协议参数两端共享，MicroCoreRT 用户态适配参数两端共享；**独立**——agentrt-linux 独立承担内核 sysctl 配置（agentrt 无权调整内核参数）、systemd 单元配置、系统级强制白名单，这些是发行版的固有责任；**互操作**——agentrt SDK 通过 `agentrt_config_load()` 注入 Agent 级配置，agentrt-linux 系统级加载器校验并记录，两端通过同源配置语义实现无适配层互操作。
...
**OS-OPS-121**：agentrt-linux 系统级配置的 `agentsipc.header_size` 必须与同版本 agentrt 用户态 SDK 的协议版本一致；版本不一致时 `airymaxos-config-check` 报错并阻止 daemon 启动（IRON-9 v2 同源且部分代码共享在配置层的强制）。
...
```c
/* include/airymax/sched.h — 调度参数配置结构 */
#define AGENT_SCHED_PRIO_MIN   0     /* 最低优先级（实时） */
#define AGENT_SCHED_PRIO_MAX   139   /* 最高优先级（idle） */
#define AGENT_SCHED_VTIME_DEF  1000000ULL  /* 默认虚拟时间片 */

struct agent_sched_config {
    __u32 prio;            /* 优先级 0-139，超出范围拒绝加载 */
    __u64 vtime;           /* 虚拟时间，0 表示使用默认值 */
    __u32 time_slice_us;   /* 时间片（微秒），最小 100us */
    __u32 weight;          /* 调度权重 1-10000 */
};
```

```c
/* include/airymax/ipc.h — IPC 配置参数结构 */
#define AGENTSIPC_RING_MIN     64      /* ring 最小条目数 */
#define AGENTSIPC_RING_MAX     4096    /* ring 最大条目数 */
#define AGENTSIPC_OPCODE_MAX   256     /* 操作码上限 */

struct agentsipc_config {
    __u32 ring_entries;    /* ring 条目数，须为 2 的幂 */
    __u16 opcode_base;     /* 操作码基址 */
    __u16 opcode_count;    /* 操作码数量 */
    __u32 header_size;     /* 消息头大小，固定 128（AGENTSIPC_HDR_SIZE） */
    __u32 timeout_ms;      /* 操作超时（毫秒） */
};
```
...
两端在配置语义上完全同源——文件名（`gateway.conf`、`llm.conf` 等）、字段名、层级覆盖优先级、热更新触发的概念模型一致——但 agentrt-linux 将配置语义落地为系统级强制（`/etc/agentrt/` 系统目录、sysctl 内核接口、白名单校验），而 agentrt 用户态运行时保持 `~/.agentrt/` 用户目录的轻量实现。这种语义同源使得 Agent 配置在两端迁移时无需字段级转换，仅路径前缀从 `~/.agentrt/` 变为 `/etc/agentrt/`。
...
配置协作流：agentrt 用户态使用 TOML 格式配置 daemons（`gateway.conf`、`llm.conf` 等），配置参数的字段名与取值范围由 [SC] 层的 `sched.h`（调度参数）和 `ipc.h`（IPC 参数）头文件约束。agentrt-linux 在 `/etc/agentrt/` 系统级目录加载同源配置文件（[SS] 语义同源——文件名与字段名两端一致），通过 `airymaxos-config-check` 校验 [SC] 参数边界后，将内核态参数映射到 sysctl 接口（`kernel.agentrt.*`）实现运行时热更新（[IND] 完全独立——sysctl/procfs/审计日志为发行版固有责任）。两端通过同源配置语义实现无适配层互操作。
 ...
 * 每个配置项必须有对应的 schema 条目（OS-STD-209）。
 *
 * 对齐 Linux 6.6 内核基线 sysctl 模式 + openEuler 配置校验规范
 * 对齐 agentrt-linux OS-STD-201 / OS-STD-213
 */
struct config_schema {
    ...
    const char *related_rule;     /* @field: 关联规则编号（如 "OS-STD-210"） */
};
```
 ...
 * 对齐 agentrt-linux OS-KER-201 / OS-STD-202
 */
struct sysctl_entry {
    ...
    bool        affects_microcorert; /* @field: 是否影响 MicroCoreRT 内核态行为（OS-KER-201） */
    bool        affects_agentsipc; /* @field: 是否影响 AgentsIPC 协议行为（OS-STD-210） */
 ...
 * 系统级强制 > 用户级偏好 > Agent 级默认（OS-STD-215）。
 *
...
#### A.1.4 agentrt_config — /etc/agentrt/ 主配置

```c
/**
 * struct agentrt_config - /etc/agentrt/ 主配置（agentrt-linux 专属）
 *
...
struct agentrt_config {
    ...
    /* AgentsIPC 协议配置（agentsipc.conf） */
    struct {
        uint16_t    version;      /* @field: 协议版本（默认 1） */
        uint16_t    header_size;  /* @field: 消息头大小（固定 128，OS-STD-210） */
...
struct config_validation_result {
    ...
    /* 校验条目列表 */
    struct {
        const char *rule_id;       /* @field: 关联规则编号（如 "OS-STD-210"） */
 ...
 *         (4) AgentsIPC header_size==128（OS-STD-210）；
 ...
 */
struct config_validation_result *
config_validate(const struct agentrt_config *config,
 ...
 */
int config_apply(const struct agentrt_config *config,
                 bool dry_run,
                 bool restart);
```
 ...
 */
int config_reload(const struct agentrt_config *config);
```
 ...
 *
 * 通过 /proc/sys/ 文件接口写入（OS-STD-201），禁止直接操作内核数据结构。
 * 若 persist=true，同时写入 /etc/sysctl.d/99-local.conf（OS-STD-202）。
 ...
 * @count:     层数（固定 3）
 * @out:       输出合并后的 agentrt_config 结构
 *
 * 合并规则：系统级 > 用户级 > Agent 级（OS-STD-215）。
...
#define CONFIG_E_DUPLICATE     (-306)   /* sysctl 参数在多个文件中重复声明（OS-STD-203） */
#define CONFIG_E_HEADER_SIZE   (-307)   /* AgentsIPC header_size != 128（OS-STD-210） */
...
#define SYSCTL_AGENTSIPC_HEADER_SIZE 128      /* AgentsIPC 消息头固定大小（OS-STD-210） */
```
...
/* /etc/agentrt/ 配置目录 */
#define AGENTRT_CONF_DIR             "/etc/agentrt"
#define AGENTRT_CONF_MAIN            "/etc/agentrt/agentrt.conf"
#define AGENTRT_CONF_AGENTSIPC       "/etc/agentrt/agentsipc.conf"
#define AGENTRT_CONF_MICROCORERT     "/etc/agentrt/microcorert.conf"
#define AGENTRT_CONF_DROPIN_DIR      "/etc/agentrt/conf.d"
#define AGENTRT_ENV_FILE             "/etc/agentrt/agentrt.env"
#define AGENTRT_KEYS_DIR             "/etc/agentrt/keys"
...
/* 用户级配置目录 */
#define AGENTRT_USER_CONF_DIR        "~/.config/agentrt"
#define AGENTRT_USER_CONF_MAIN       "~/.config/agentrt/user.conf"

/* 配置文件权限 */
#define CONFIG_FILE_MODE             0640  /* /etc/agentrt/*.conf 权限（OS-OPS-107） */
#define CONFIG_KEY_FILE_MODE         0600  /* 密钥文件权限（OS-STD-206） */
#define CONFIG_ENV_FILE_MODE         0640  /* agentrt.env 权限 */

/* 配置仓库目录结构（配置即代码，OS-OPS-116） */
#define CONFIG_REPO_SYSCTL_DIR       "sysctl/"
#define CONFIG_REPO_AGENTRT_DIR      "agentrt/"
