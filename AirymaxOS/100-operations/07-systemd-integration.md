...
systemd 与 A-ULS（Unified Supervision）模块 macro_superv 分工：

| 职责 | systemd | macro_superv |
|------|---------|--------------|
...
systemd 提供基础的进程级监管，macro_superv 提供业务级监管，两者互为补充。macro_superv 自身也是 systemd 单元，由 systemd 保证其活性。
...
| `macro_superv.service` | service | 统一监管 | 1 |
| `logger_daemon.service` | service | 统一日志 | 2 |
| `config_daemon.service` | service | 统一配置 | 2 |
...
## 3. macro_superv.service

### 3.1 单元文件

```ini
# /usr/lib/systemd/system/macro_superv.service
[Unit]
Description=AirymaxOS Unified Supervision Daemon (A-ULS)
Documentation=man:macro_superv(8)
After=systemd-journald.service
Before=logger_daemon.service config_daemon.service
...
Type=notify
NotifyAccess=main
ExecStart=/usr/sbin/macro_superv --config=/etc/airy/daemons/macro_superv.conf
...
- **Type=notify**：macro_superv 启动后通过 `sd_notify("READY=1")` 通知 systemd 就绪，systemd 在 `TimeoutStartSec=30` 内未收到通知则视为启动失败。
...
- **WatchdogSec=10**：macro_superv 每 5 秒发送 `sd_notify("WATCHDOG=1")`，超过 10 秒未收到则 systemd 强制重启。
...
macro_superv 在以下事件中调用 sd_notify：
...
## 4. logger_daemon.service

### 4.1 单元文件

```ini
# /usr/lib/systemd/system/logger_daemon.service
[Unit]
Description=AirymaxOS Unified Log Processing System (A-ULP)
Documentation=man:logger_daemon(8)
After=macro_superv.service
Requires=macro_superv.service
PartOf=airy-os.target

[Service]
Type=notify
NotifyAccess=main
ExecStart=/usr/sbin/logger_daemon --config=/etc/airy/daemons/logger_daemon.conf
ExecReload=/bin/kill -HUP $MAINPID
Restart=always
RestartSec=1s
StartLimitBurst=5
...
SyslogIdentifier=logger_daemon
...
- **MemoryMax=1G**：Ring Buffer 占用较大，内存上限高于 macro_superv。
- **LimitNOFILE=131072**：支持大量日志文件并发写入。
- **ReadWritePaths**：仅 `/var/log/airy` 与 `/var/lib/airy/log` 可写。
- **CapabilityBoundingSet=**（空）：logger_daemon 不需要任何 capability，最小权限原则。

---

## 5. config_daemon.service

### 5.1 单元文件

```ini
# /usr/lib/systemd/system/config_daemon.service
[Unit]
Description=AirymaxOS Unified Configuration Subsystem (A-UCS)
Documentation=man:config_daemon(8)
After=macro_superv.service
Requires=macro_superv.service
...
Type=oneshot
RemainAfterExit=yes
ExecStart=/usr/sbin/config_daemon --load --config=/etc/airy/airy.conf
ExecReload=/usr/sbin/config_daemon --reload
ExecStop=/bin/true
...
SyslogIdentifier=config_daemon
...
- **Type=oneshot**：config_daemon 是一次性任务，加载配置后退出。
- **RemainAfterExit=yes**：退出后单元仍视为 active，便于其他 daemon 声明依赖。
- **Before=**：所有业务 daemon 在 config_daemon 之后启动，确保配置已加载。
- **ExecReload**：热更新通过 `systemctl reload config_daemon` 触发。
...
After=macro_superv.service logger_daemon.service config_daemon.service
Requires=macro_superv.service logger_daemon.service config_daemon.service
PartOf=airy-os.target
...
```
                    systemd-journald.service
                              │
                              ▼
                       macro_superv.service
                       │           │
              ┌────────┘           └────────┐
              ▼                              ▼
       logger_daemon.service        config_daemon.service
              │                              │
              └──────────────┬───────────────┘
                             │
       ┌─────────────────────┼─────────────────────┐
...
|------|----------|-------|--------|
| macro_superv | systemd-journald | systemd-journald | logger_daemon, config_daemon |
| logger_daemon | macro_superv | macro_superv | (业务 daemon) |
| config_daemon | macro_superv | macro_superv | (业务 daemon) |
| sec_d | macro_superv, logger_daemon, config_daemon | macro_superv, logger_daemon, config_daemon | vfs_d, mem_d |
| sched_d | macro_superv, logger_daemon, config_daemon | macro_superv, logger_daemon, config_daemon | mem_d, cogn_d |
| net_d | macro_superv, logger_daemon, config_daemon | macro_superv, logger_daemon, config_daemon | gateway_d |
...
Requires=macro_superv.service logger_daemon.service config_daemon.service gateway_d.service sched_d.service vfs_d.service net_d.service mem_d.service cogn_d.service sec_d.service audit_d.service dev_d.service
...
Requires=macro_superv.service logger_daemon.service config_daemon.service sec_d.service audit_d.service mem_d.service
...
- macro_superv 监听 target 切换事件，触发相应的业务级降级/恢复逻辑。
...
|--------|-----------|------------|----------|----------|-----------|
| macro_superv | 512M | 400M | 200% | 512 | realtime:4 |
| logger_daemon | 1G | 800M | 100% | 256 | best-effort:2 |
| config_daemon | 256M | 200M | 50% | 128 | best-effort:4 |
...
- **IO 优先级**：realtime 优先级用于关键 daemon（macro_superv/sched_d/sec_d），best-effort 用于其他。
...
3. macro_superv 监听 OOM 事件（通过 `sd_notify("ERRNO=...")` 或 journald），触发 A-ULS 重启策略。
...
|--------|---------|------------|-----------------|----------------------|-----------|
| macro_superv | always | 1s | 3 | 300s | DSL 模式 |
| logger_daemon | always | 1s | 5 | 300s | DSL 模式 |
| config_daemon | on-failure | 5s | 3 | 300s | DSL 模式 |
...
```bash
# 模拟 macro_superv 崩溃
airyctl fault inject --target macro_superv --action crash
...
### 11.3 与 logger_daemon 的关系

logger_daemon 接收所有 daemon 的结构化日志（通过 io_uring），journald 接收非结构化的 stdout/stderr。两者分工：

| 维度 | logger_daemon | journald |
|------|---------------|----------|
