...
- **消费者**：消费 macro_superv、logger_daemon、audit_d 提供的监控与日志数据。
...
|--------|------------------------|
| macro_superv | 提供 daemon 与 Agent 状态数据 |
| logger_daemon | 提供实时日志流与历史日志 |
| audit_d | 提供监控指标、告警、事件数据 |
| sched_d | 提供调度性能数据，接收调优指令 |
| mem_d | 提供记忆层级数据，接收记忆操作 |
| config_daemon | 提供配置查看与修改接口 |
| sec_d | 提供 mTLS 证书与权限校验 |
| cogn_d | 提供 Token 预算与消耗数据 |

### 1.4 与技术选型的契合

- **IORING_OP_URING_CMD**：devstation 通过 io_uring 订阅 logger_daemon 实时日志流，避免轮询。
- **纯 C LSM**：devstation 的所有操作受 LSM 控制，无特权操作。
- **IRON-9 v3 [IND] 层**：记忆检索复用 [IND] 层索引，支持快速搜索。
- **A-ULS 模块**：devstation 通过 macro_superv RPC 接口操作 Agent，与 A-ULS 联动。
...
│  │ macro_superv  ●  3.2%  │  │ ID  Name              CPU%   Mem       │ │
│  │ logger_daemon ●  1.8%  │  │ 42  customer-bot      45.2   256MB     │ │
│  │ config_daemon ●  0.0%  │  │ 43  support-bot       38.1   512MB     │ │
...
│                              └────────────────────────────────────────┘ │
├─────────────────────────────────────────────────────────────────────────┤
│ Status: Connected to macro_superv   Refresh: 1s   Local time: 14:23:11  │
└─────────────────────────────────────────────────────────────────────────┘
```
...
- **daemon 状态**：通过 RPC 从 macro_superv 获取。
- **Agent 列表**：通过 RPC 从 macro_superv 获取。
...
| 字段 | 说明 | 数据源 |
|------|------|--------|
| 名称 | daemon 名称 | macro_superv |
| 状态 | ●(alive)/○(dead) | macro_superv |
...
日志查看器展示来自 logger_daemon 的实时日志流与历史日志：

- **实时流**：订阅 logger_daemon Ring Buffer，<100ms 延迟展示新日志。
...
├─────────────────────────────────────────────────────────────────────────┤
│ 14:23:11.123  CRIT  macro_superv  daemon gateway_d heartbeat timeout     │
...
- **来源过滤**：`source=macro_superv` 仅显示 macro_superv 日志。
...
所有操作通过 RPC 调用 macro_superv，由 macro_superv 实际执行。
...
热更新通过 config_daemon 触发 SIGHUP 至相关 daemon。
