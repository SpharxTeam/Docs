...
| 子系统 | 在 Agent 运维中的角色 |
|--------|-----------------------|
| macro_superv (A-ULS) | Agent 生命周期监管、健康检查、自动重启 |
| sched_d | Agent 调度参数调整、资源配额 |
| cogn_d | Agent Token 消耗监控、推理预算 |
| mem_d | Agent 记忆层级管理 |
| logger_daemon (A-ULP) | Agent 日志收集与持久化 |
| config_daemon (A-UCS) | Agent 配置管理 |
...
2. **校验**：macro_superv 校验请求参数（名称、模板、预算等）。
3. **分配 ID**：macro_superv 分配全局唯一 Agent ID。
4. **初始化元数据**：写入 `/var/lib/airy/agents/<id>/meta.json`。
5. **加载模板**：config_daemon 加载 Agent 模板配置。
...
2. **fork 进程**：macro_superv 通过 systemd-run 或直接 fork 创建 Agent 进程。
...
7. **状态切换**：macro_superv 将状态改为 RUNNING。
...
2. **优雅停止**：macro_superv 向 Agent 发送 SIGTERM，等待 30s。
...
macro_superv 对每个 RUNNING 状态的 Agent 执行定期心跳检测：
...
macro_superv 综合以下因素计算 Agent 健康分（0-100）：
...
|--------|----------|
| 80-100 | 无 |
| 50-79 | 触发 WARNING 告警，macro_superv 评估是否需要调优 |
| 30-49 | 触发 WARNING 告警，sched_d 降低 Agent 优先级 |
| 10-29 | 触发 CRITICAL 告警，macro_superv 暂停 Agent |
| <10 | 触发 CRITICAL 告警，macro_superv 重启 Agent |
| 0（连续 3 次心跳超时） | 触发 P0 事件，重启或迁移 Agent |

### 3.4 深度健康检查

除了心跳检测，macro_superv 周期执行深度健康检查（默认 30 秒一次）：
...
1. **预迁移**：macro_superv 在目标节点预创建 Agent（CREATED 状态）。
2. **状态同步**：
...
1. **检测**：macro_superv 检测到源节点故障（心跳超时）。
...
1. **准备**：config_daemon 加载新版本配置到 `/etc/airy/agents/<id>.new/`。
2. **校验**：校验配置完整性与兼容性。
3. **原子切换**：通过 `renameat2(ATOMIC_REPLACE)` 原子替换配置目录。
4. **通知**：macro_superv 发送 SIGHUP 给 Agent。
...
2. macro_superv 立即暂停所有受影响 Agent。
3. 应用补丁并重启。
4. 验证后恢复。
...
Agent 日志通过 A-ULP 模块 logger_daemon 统一收集：

```
[Agent 进程] ──── io_uring CMD ────> [logger_daemon Ring Buffer]
                                              │
                                  ┌───────────┼───────────┐
                                  ▼           ▼           ▼
                            [实时流]    [journal]    [持久化文件]
...
底层复用 io_uring CMD，批量提交到 logger_daemon 的 Ring Buffer。
...
- **短期**：logger_daemon Ring Buffer（内存，1 小时）。
...
- Agent 日志中的敏感字段由 logger_daemon 脱敏。
