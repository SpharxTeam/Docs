...
2. **不丢不重**：告警传输基于 A-ULP 模块 logger_daemon 的 Ring Buffer，保证至少一次（at-least-once）语义。
...
| 主要模块 | macro_superv + audit_d | macro_superv + audit_d + logger_daemon |
...
告警级别由 macro_superv 根据以下规则综合判定：

1. **阈值规则**：指标超过 `critical` 阈值 → CRITICAL；超过 `warning` 阈值 → WARNING。
2. **持续窗口**：阈值命中后须持续 `window` 时间，否则降一级（CRITICAL → WARNING，WARNING → INFO）。
3. **影响范围**：影响 ≥3 个 Agent 或 ≥1 个核心 daemon（macro_superv/logger_daemon/config_daemon/sched_d/sec_d）的告警强制升级为 CRITICAL。
...
| 场景 | 默认级别 | 触发条件 |
|------|----------|----------|
| macro_superv 进程崩溃 | CRITICAL | systemd WatchdogSec 超时 |
| logger_daemon Ring Buffer 满 | WARNING | 占用率 >90% 持续 30s |
...
| config_daemon 配置校验失败 | CRITICAL | 热更新校验失败 |
...
| 告警源 | 类型 | 主要告警场景 | 上报方式 |
|--------|------|--------------|----------|
| macro_superv | 进程监管 | daemon 崩溃、重启、心跳超时、健康分异常 | 直接调用 `airy_alert_emit()` |
| sched_d | 调度监管 | SCHED_DEADLINE miss、调度延迟超阈值、CPU 饥饿 | io_uring CMD → logger_daemon |
| net_d | 网络监管 | 链路中断、重传率高、连接跟踪表满 | io_uring CMD → logger_daemon |
| mem_d | 内存监管 | L1-L4 溢出、defrag 失败、OOM 临近 | io_uring CMD → logger_daemon |
| sec_d | 安全监管 | LSM hook 命中、密钥访问异常、能力越权 | LSM hook 直采 → `/dev/airy_alert` |
| cogn_d | 认知监管 | Token 预算耗尽、推理队列堆积、模型不可用 | io_uring CMD → logger_daemon |
| vfs_d | 文件系统监管 | fsync 延迟超阈值、inode 耗尽、只读挂载 | io_uring CMD → logger_daemon |
| gateway_d | 网关监管 | 握手失败率、请求超时、证书过期 | io_uring CMD → logger_daemon |
| config_daemon | 配置监管 | 校验失败、热更新冲突、版本回退 | 直接调用 `airy_alert_emit()` |
| audit_d | 聚合监管 | 自身存储满、聚合延迟、关联分析异常 | 直接调用 `airy_alert_emit()` |
| 内核 LSM hook | 安全钩子 | 进程越权、文件非法访问、网络异常连接 | `/dev/airy_alert` 字符设备 |
| systemd | init 系统 | 单元失败、WatchdogSec 超时、依赖循环 | `sd_notify` → macro_superv |

### 3.2 告警源详细说明

#### 3.2.1 macro_superv（进程监管）

macro_superv 作为 A-ULS 模块的核心，负责 12 daemon 的生命周期监管。其告警场景：
...
[告警源]                                                       [订阅方]
   ...
   ▼                                                              │
┌─────────────────────────────────────────────────────────────────┤
│  logger_daemon Ring Buffer（统一入口）                          │
...
### 4.2 logger_daemon Ring Buffer

Ring Buffer 是告警传输的统一入口，所有告警源必须经过此通道。设计要点：

- **容量**：默认 16MB，可由 `airy_alert.ring_buf_mb` 配置。
- **背压**：当占用率 >90% 时，logger_daemon 拒绝写入新的 INFO 级告警，返回 `-EAGAIN`；CRITICAL/WARNING 强制写入，丢弃最旧的 INFO。
...
struct airy_alert_record {
    ...
    uint8_t  source_id;      /* 告警源 ID（macro_superv=1, sched_d=2, ...） */
...
- macro_superv：接收跨 daemon 告警，触发 A-ULS 自动重启。
...
| 现象告警 | 推断根因 | 聚合规则 |
|----------|----------|----------|
| 多个 daemon 心跳超时 | macro_superv 自身异常 | 合并为 1 条主告警（macro_superv CRITICAL） |
...
| 参数 | 默认值 | 说明 |
|------|--------|------|
| `airy_alert.ring_buf_mb` | 16 | logger_daemon 告警 Ring Buffer 大小（MB） |
...
参数热更新通过 config_daemon 发送 SIGHUP 至 audit_d 完成。
...
- logger_daemon Ring Buffer 仅接受 root 或 `airy-alert` 组进程写入。
...
- logger_daemon 内置脱敏规则（正则匹配），命中后替换为 `***`。
...
- 抑制规则变更记录到 journal，由 config_daemon 上报。
...
- macro_superv 既是告警源，也是告警订阅方（接收跨 daemon 告警触发自动重启）。
- A-ULS 重启事件产生 INFO 告警，记录到 audit_d 用于复盘。

### 11.4 与 A-ULP（统一日志）

- 所有告警同时写入 A-ULP 日志流，与 ERROR/WARN/INFO 级别对齐。
- logger_daemon 的 Ring Buffer 复用为告警传输通道，避免重复基础设施。
