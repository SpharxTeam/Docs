...
%description
agentrt-linux 12 daemons userspace core, IRON-9 v2 同源且部分代码共享于 agentrt daemons.
...
agentrt-linux 部署体系与 agentrt 遵循 IRON-9 v2 同源且部分代码共享原则：**同源**——12 daemons 二进制名（`*_d`）与 agentrt 完全一致，AgentsIPC 128B 消息头协议两端共享，MicroCoreRT 极简内核契约两端共享，配置目录 `/etc/agentrt/` 两端共享语义；**独立**——agentrt-linux 独立承担发行版部署责任（RPM 打包、dnf 仓库、ISO 制作、Kickstart、PXE、systemd 集成、根分区快照回滚），这些都是 agentrt 用户态运行时不涉及的领域；**互操作**——agentrt 遵循其用户态部署（pip/npm 安装），agentrt-linux 遵循其发行版部署（RPM/dnf），两端通过同源二进制名与 AgentsIPC 协议实现无适配层互操作。
...
**OS-OPS-024**：agentrt-linux 部署的 12 daemons 必须与同版本 agentrt 用户态 SDK 的 AgentsIPC 协议版本一致；版本不一致时 daemon 拒绝接受 SDK 连接并输出诊断码（IRON-9 v2 同源且部分代码共享在协议层的强制）。

### 13.3 IRON-9 v2 三层共享模型

IRON-9 v2 三层共享模型将 agentrt（用户态运行时）与 agentrt-linux（发行版部署）之间的同源关系细分为三个正交层次：[SC] 共享契约层（头文件级代码共享）、[SS] 语义同源层（语义两端一致但实现独立）、[IND] 完全独立层（发行版固有责任）。本节聚焦部署体系的三层映射。
...
```c
/* include/airymax/sched.h — SCHED_EXT 调度类 + 任务描述符 */
#define AGENT_TASK_MAGIC    0x41475453  /* 'AGTS' — Agent 任务描述符魔数 */

struct agent_task_desc {
    __u32 magic;           /* AGENT_TASK_MAGIC，校验任务描述符完整性 */
    __u32 prio;            /* 优先级 0-139，SCHED_EXT 调度类 */
    __u64 vtime;           /* 虚拟时间，公平调度基准 */
    __u32 time_slice_us;   /* 时间片（微秒） */
    __u32 cpu_affinity;    /* CPU 亲和性掩码 */
    char  daemon_name[16]; /* daemon 名（如 "gateway_d"） */
};
```

```c
/* include/airymax/ipc.h — IPC magic + 128B 消息头 + 操作码 */
#define AGENTSIPC_MAGIC    0x41524531  /* 'ARE1' — AgentsIPC 协议魔数 */
#define AGENTSIPC_HDR_SIZE 128         /* 消息头固定 128 字节 */

struct agentsipc_hdr {
    __u32 magic;           /* AGENTSIPC_MAGIC */
    __u16 opcode;          /* SQE/CQE 操作码 */
    __u16 flags;           /* 标志位 */
    __u32 src_daemon_id;   /* 源 daemon ID */
    __u32 dst_daemon_id;   /* 目标 daemon ID */
    __u64 seq;             /* 序列号 */
    __u64 timestamp_ns;   /* 时间戳（纳秒） */
    __u32 payload_len;     /* 负载长度 */
    __u32 reserved;        /* 保留对齐 */
    __u8  padding[80];     /* 填充至 128B */
};
```
...
graph LR
    ...
    subgraph SC ["[SC] 共享契约层"]
        S1["sched.h<br/>AGENT_TASK_MAGIC 0x41475453"]
        S2["ipc.h<br/>AGENTSIPC_MAGIC 0x41524531<br/>128B 消息头"]
    end
...
struct rpm_spec {
    ...
    const char **changelog_refs; /* @field: %changelog 引用的 Fixes:/Closes: 提交哈希（OS-STD-101） */
    int          changelog_count;/* @field: changelog 条目数 */
    uint32_t     file_mode_bin;  /* @field: 二进制文件权限（必须 0755，OS-STD-104） */
    uint32_t     file_mode_conf; /* @field: 配置文件权限（必须 0644，OS-STD-104） */
    uint32_t     file_mode_key;  /* @field: 密钥文件权限（必须 0600，OS-STD-104） */
    bool         config_noreplace; /* @field: 是否使用 %config(noreplace)（OS-STD-103） */
 ...
 * 对齐 agentrt-linux OS-STD-109 / OS-OPS-014
 */
struct systemd_unit {
    ...
    uint64_t    memory_max;       /* @field: [Service] MemoryMax= 字节（OS-STD-110） */
    uint64_t    tasks_max;        /* @field: [Service] TasksMax= 任务上限（OS-STD-110） */
 ...
 *
 * agentrt-linux 专属（IRON-9 v2 同源，对齐 12 daemons）
 */
struct daemon_config {
    ...
    uint32_t    ipc_port;          /* @field: AgentsIPC 端口（AGENTSIPC_BASE_PORT + offset） */
    uint16_t    ipc_queue_depth;   /* @field: IPC ring 队列深度（默认 1024） */
    uint16_t    header_size;       /* @field: AgentsIPC 消息头大小（固定 128，OS-STD-210） */
    ...
    const char *tls_key_file;      /* @field: TLS 密钥文件路径（引用形式，OS-STD-206） */
};
...
struct iso_image_spec {
    ...
    const char *repo_baseurl;      /* @field: 仓库 base URL（固定快照，OS-STD-106） */
    ...
    bool        uefi_support;      /* @field: 是否支持 UEFI 引导（OS-STD-107） */
    bool        include_pxeboot;   /* @field: 是否包含 images/pxeboot/（OS-STD-107） */
    bool        include_efiboot;   /* @field: 是否包含 images/efiboot.img（OS-STD-107） */
    const char *microcorert_probe; /* @field: MicroCoreRT 早期探测模块路径（OS-KER-101） */
...
struct kickstart_config {
    ...
    struct {
        ...
        bool        independent;   /* @field: 是否独立分区（/var/lib/agentrt 必须独立，OS-STD-108） */
    } *partitions;
 ...
 *
 * 将单元文件写入 path_prefix，按 OS-STD-104 设置文件权限（0644）。
 ...
 *           (2) AgentsIPC header_size==128（OS-STD-210）；
 *           (3) sysinit.target 已完成（OS-STD-109）。
 ...
 *           (3) 写入 .discinfo（含仓库快照哈希，OS-STD-106）；
 *           (4) 确保 images/pxeboot/ + images/efiboot.img（OS-STD-107）；
...
#define DAEMON_E_DEPS           (-2)    /* 依赖未满足（如 sysinit.target 未完成，OS-STD-109） */
#define DAEMON_E_PERM           (-3)    /* 权限不足（配置文件权限 != 0640，OS-OPS-107） */
#define DAEMON_E_IPC            (-4)    /* AgentsIPC 契约违反（header_size != 128，OS-STD-210） */
#define DAEMON_E_GPG            (-5)    /* GPG 签名验证失败（OS-OPS-004） */
#define DAEMON_E_WATCHDOG      (-6)    /* watchdog 心跳超时（30s 无心跳，OS-OPS-014） */
#define DAEMON_E_OOM           (-7)    /* OOM kill 触发（超出 MemoryMax，OS-STD-110） */
#define DAEMON_E_DEPENDENCY_CYCLE (-8) /* 循环依赖检测到（OS-STD-109） */
...
#define RPM_E_FILE_MODE         (-105)   /* 文件权限不符（非 0755/0644/0600，OS-STD-104） */
#define RPM_E_CHANGELOG         (-106)   /* %changelog 缺少 Fixes:/Closes: 引用（OS-STD-101） */
#define RPM_E_CONFIG_NOREPLACE  (-107)   /* /etc/agentrt/ 未使用 %config(noreplace)（OS-STD-103） */
```
