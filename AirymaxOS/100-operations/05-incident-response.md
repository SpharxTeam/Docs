...
| 主导模块 | audit_d + logger_daemon | macro_superv + sec_d + sched_d |
| 触发模式 | 事件驱动 | 告警订阅 + 自动响应 |
...
- **A-ULS 模块**：macro_superv 提供统一的 daemon 重启能力，所有自动重启动作通过 A-ULS 执行。
...
- **systemd**：单元失败、WatchdogSec 超时通过 `OnFailure=` 触发 macro_superv。
...
- **自动响应**：macro_superv / sec_d / sched_d 根据 incident 类别自动执行。
...
- **响应策略**：
  ...
  3. 重启后 macro_superv 检测到非正常重启，自动创建 P0 事件。
  4. 加载 [DSL] 降级模式启动（详见 4.3）。
  5. 通知运维 on-call（Email + SMS）。

#### 3.2.2 daemon 崩溃

- **触发条件**：核心 daemon（macro_superv/logger_daemon/config_daemon/sched_d/sec_d）进程崩溃。
- **响应策略**：
  1. systemd 自动重启（Restart=always）。
  2. macro_superv 监管检测到重启事件，评估重启次数：
...
- macro_superv 自身崩溃：systemd 重启，依赖 audit_d 检测并创建事件。
...
- **响应策略**：
  1. macro_superv 自动重启 Agent（保留 Agent ID 与状态）。
...
- **响应策略**：
  ...
  2. macro_superv 重启阻塞方（若是 daemon）或暂停 Agent（若是 Agent）。
  3. 通知运维，提供 IPC 诊断报告。
...
- **响应策略**：
  ...
  3. 仍不足：macro_superv 暂停最低优先级 Agent。
...
A-ULS 模块 macro_superv 提供统一的自动重启能力，是事件响应的首选策略。
...
1. **检测**：macro_superv 通过 SIGCHLD 或心跳超时检测到进程异常。
...
| dev_d | 禁用非必要设备 |
| logger_daemon | 降低日志级别至 WARN |
| config_daemon | 加载降级模式专用配置 `airy-dsl.conf` |
...
1. macro_superv 逐步恢复非关键 Agent（按优先级）。
2. 各 daemon 切换回正常配置。
...
```json
{
  ...
  "root_cause": "config_daemon pushed invalid SCHED_DEADLINE parameters",
  "timeline": [
    {
      "ts": "2026-07-18T14:23:11.123Z",
      "event": "detected",
      "actor": "audit_d",
      "detail": "macro_superv heartbeat timeout (8s)"
    },
    {
    ...
    },
    {
      ...
      "event": "recovered",
      "actor": "ops_alice",
      "detail": "config rollback, macro_superv health restored"
    }
  ],
  "alerts": ["ALT-20260718-001", "ALT-20260718-002"],
  "affected_agents": [42, 43, 44],
  "affected_daemons": ["macro_superv"],
  "response_actions": [
    {
      "action": "dsl_activate",
      "actor": "macro_superv",
...
| 恶意 Agent | sec_d + macro_superv | Agent 终止 + 状态冻结 |
...
macro_superv 支持的自动响应动作：

| 动作 | 触发场景 | 执行模块 |
|------|----------|----------|
| `daemon_restart` | daemon 崩溃 | macro_superv + systemd |
| `agent_restart` | Agent 异常退出 | macro_superv |
| `agent_pause` | Agent 资源超限 | macro_superv + sched_d |
| `agent_migrate` | 节点故障 | macro_superv + 150-cloudnative |
| `dsl_activate` | P0 系统故障 | macro_superv + 全 daemon |
| `config_rollback` | 配置热更新失败 | config_daemon |
...
|------|------|------|
| T+0s  | 检测到异常 | audit_d |
| T+2s  | 自动响应启动 | macro_superv |
| T+10s | DSL 模式激活 | macro_superv |
| ...   | ...           | ...   |
...
- 自动响应动作仅限 macro_superv / sec_d / sched_d 等核心 daemon 执行。
