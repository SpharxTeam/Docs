...
### 13.4 Random Tag 碰撞概率分析（v1.1 新增）
...
### 13.5 NUMA 场景 agent_caps[] 访问延迟分析与优化（v1.1 新增）
...
```c
/* kernel/security/airy/capability.c —— NUMA 本地分配（v1.1 新增）
 *
...
```c
/* kernel/ipc/airy_uring_cmd.c —— fastpath prefetch 优化（v1.1 新增）
 *
...
static int airy_ipc_validate(struct airy_ipc_cmd *cmd)
{
    ...
    /* === v1.1 新增: prefetch agent_caps[src_task]（在 C-S2~C-S8 期间加载） === */
