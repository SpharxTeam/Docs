...
```c
/**
 * airy_sys_call - Capability Invocation 系统调用（v1.0.1 唯一保留的 capability syscall）
 ...
 *   -AIRY_EINVAL       (-22)  参数无效
 *
 * v1.0.1 变更: 原 592-600 共 9 个 capability syscall 全部收敛到此入口。
...
```c
/* v1.0.1: A-IPC fastpath 内联 Badge 校验，无独立守卫 */
...
```c
/**
 * capability 守卫宏（v1.0.1: 仅用于非 IPC 路径的 LSM 钩子）
...
#define AIRY_CAP_GUARD_END(cap_slot)  \
    /* v1.0.1: 非 IPC 路径无需撤销令牌（slowpath 不持有 Badge）*/
```
...
- [10-architecture/10-unify-design.md](../10-architecture/10-unify-design.md) — Airymax Unify Design 总纲（v1.0.1 §8 A-IPC Capability Folding）
...
### A.1 v1.0.1 重构：离线缓存校验机制 → fastpath C-S9 内联 Badge 校验
