...
> **设计参考**: seL4 `src/object/cnode.c`（CNode mint/mintcopy/move/copy/revoke/delete）+ seL4 `src/kernel/mdb.c`（MDB 派生链）+ openEuler OLK-6.6 `security/commoncap.c`（POSIX cap 检查）+ `include/linux/cred.h`（credential 结构）
...
| 权限粒度 | 对象级 | 38 个粗粒度 cap | 对象 + 权限掩码 | seL4 + POSIX 混合 |
...
- **POSIX capability**：用于传统 Linux 兼容性（38 个标准 cap，进程级位图）
...
4. **IPC 传递**：capability 可通过 IPC 消息跨进程传递（`AGENTRT_IPC_FLAG_CAP_CARRY`）
...
```c
/**
 * struct agentrt_cnode - Capability Node（单个 capability 槽位）
 *
 * @cap_type:    capability 类型（AGENTRT_CAP_TYPE_*）
...
```c
/**
 * enum agentrt_cap_type - capability 类型
 *
 * 对齐 security_types.h [SC] 共享头文件中的类型定义。
 */
enum agentrt_cap_type {
    AGENTRT_CAP_TYPE_ENDPOINT    = 0,   /* IPC 端点 capability */
    AGENTRT_CAP_TYPE_TASK        = 1,   /* Agent 任务 capability */
    AGENTRT_CAP_TYPE_MEMORY      = 2,   /* 内存访问 capability */
    AGENTRT_CAP_TYPE_ROVOL       = 3,   /* 记忆卷载 capability */
    AGENTRT_CAP_TYPE_SCHED       = 4,   /* 调度 capability */
    AGENTRT_CAP_TYPE_FILE        = 5,   /* 文件系统 capability */
    AGENTRT_CAP_TYPE_NETWORK     = 6,   /* 网络访问 capability */
    AGENTRT_CAP_TYPE_WASM        = 7,   /* Wasm 模块 capability */
    AGENTRT_CAP_TYPE_CAP_MGMT    = 8,   /* capability 管理 capability（元 capability） */
    AGENTRT_CAP_TYPE_MAX,
};
```
...
```c
/**
 * struct agentrt_cspace - Agent 的 Capability Space
 *
...
```c
/**
 * agentrt_cap_mint - 缩减权限派生 capability
 * @src_cap:   源 capability ID
 * @mask:      权限掩码（缩减后 badge = src.badge & mask）
 * @new_cap_out: 新 capability ID 输出
 *
 * 返回: 0 成功，-AGENTRT_EINVAL 源 cap 无效，
 *       -AGENTRT_EPERM 调用者不拥有源 cap，-AGENTRT_ENOMEM 分配失败
 *
 * 借鉴 seL4 cnode.c 的 mint 操作。
 * 源 capability 保留不变，生成新的派生 capability。
 */
int agentrt_cap_mint(uint32_t src_cap, uint64_t mask, uint32_t *new_cap_out);
```
 ...
 */
int agentrt_cap_mintcopy(uint32_t src_cap, uint32_t dest_agent,
...
```c
/**
 * agentrt_cap_derive - 全权派生 capability
 * @src_cap:    源 capability ID
 * @dest_agent: 目标 Agent ID
 * @new_cap_out: 新 capability ID 输出
 *
 ...
 * 用于委托全权给受信任的子任务。
 */
int agentrt_cap_derive(uint32_t src_cap, uint32_t dest_agent,
 ...
 * @since 1.0.1
 */
int agentrt_cap_revoke(uint32_t src_cap);
```
...
对齐 `security_types.h`（[SC] 共享头文件）定义的 POSIX capability 38 ID 枚举。agentrt-linux 在 Linux 6.6 标准 40 个 POSIX capability 基础上，整合为 38 个有效枚举（排除 2 个已废弃）：

```c
/**
 * POSIX capability ID 枚举（38 个）
 * 对齐 Linux 6.6 include/uapi/linux/capability.h
 * 排除 CAP_DEPRECATED_1 / CAP_DEPRECATED_2
 */
#define AGENTRT_CAP_CHOWN              0   /* 修改文件所有者 */
#define AGENTRT_CAP_DAC_OVERRIDE       1   /* 绕过 DAC 读/写/执行检查 */
#define AGENTRT_CAP_DAC_READ_SEARCH    2   /* 绕过 DAC 读/搜索 */
#define AGENTRT_CAP_FOWNER             3   /* 绕过文件权限检查 */
#define AGENTRT_CAP_FSETID            4   /* 设置 setuid/setgid */
#define AGENTRT_CAP_KILL               5   /* 绕过发送信号权限检查 */
#define AGENTRT_CAP_SETGID            6   /* 设置 GID */
#define AGENTRT_CAP_SETUID            7   /* 设置 UID */
#define AGENTRT_CAP_SETPCAP           8   /* 设置进程 capability */
#define AGENTRT_CAP_LINUX_IMMUTABLE    9   /* 设置不可变文件 */
#define AGENTRT_CAP_NET_BIND_SERVICE  10  /* 绑定 <1024 端口 */
#define AGENTRT_CAP_NET_BROADCAST     11  /* 广播/监听 */
#define AGENTRT_CAP_NET_ADMIN         12  /* 网络管理 */
#define AGENTRT_CAP_NET_RAW           13  /* 原始 socket */
/* ... 完整 38 个枚举对齐 security_types.h ... */
#define AGENTRT_CAP_LAST_CAP          37  /* 最后一个 cap */
#define AGENTRT_CAP_AGENT_MAX         38  /* agentrt-linux 专属上限 */
```
 ...
 *   3. 两者都通过才允许
 */
int agentrt_cap_check(uint32_t agent_id, uint32_t cap_id,
...
```c
enum agentrt_cap_state {
    AGENTRT_CAP_STATE_CREATED     = 0,  /* 已创建，待激活 */
    AGENTRT_CAP_STATE_ACTIVE       = 1,  /* 活跃，可使用 */
    AGENTRT_CAP_STATE_DERIVED      = 2,  /* 已派生 */
    AGENTRT_CAP_STATE_USED         = 3,  /* 使用中 */
    AGENTRT_CAP_STATE_TRANSFERRED  = 4,  /* 传递中 */
    AGENTRT_CAP_STATE_REVOKED      = 5,  /* 已撤销 */
    AGENTRT_CAP_STATE_EXPIRED      = 6,  /* 已过期 */
    AGENTRT_CAP_STATE_DENIED       = 7,  /* 被拒绝 */
};
```
...
```c
/**
 * enum agentrt_cap_verdict - capability 策略裁决结果
 *
 * 4 值裁决模型对齐 SELinux 的 allow/deny/audit/deny_audit 语义。
 */
enum agentrt_cap_verdict {
    AGENTRT_CAP_ALLOW         = 0,  /* 允许，不记录 */
    AGENTRT_CAP_DENY          = 1,  /* 拒绝，不记录 */
    AGENTRT_CAP_AUDIT         = 2,  /* 允许，记录审计日志 */
    AGENTRT_CAP_DENY_AUDIT    = 3,  /* 拒绝，记录审计日志 */
};
```
...
```c
/**
 * agentrt_cap_adjudicate - 策略裁决
 * @agent_id:  Agent ID
 * @cap_type:  capability 类型
 * @resource:  资源路径
 * @operation: 请求的操作
 *
 * 返回: AGENTRT_CAP_* 裁决结果
 *
 * 裁决顺序:
 *   1. POSIX cap 检查（快速路径）
 ...
 *   3. Cupolas 策略引擎（慢速路径）
 */
enum agentrt_cap_verdict agentrt_cap_adjudicate(
    uint32_t agent_id,
    enum agentrt_cap_type cap_type,
    const char *resource,
    uint32_t operation);
```
...
```c
/**
 * struct agentrt_vault_backend - Vault 后端抽象
 *
 * @name:         后端名称
 * @store:        存储 capability
...
```c
/**
 * agentrt_vault_seal_tpm - 使用 TPM 封存 capability
 ...
 * @since 1.0.1
 */
int agentrt_vault_seal_tpm(uint32_t cap_id, uint32_t tpm_handle);
```
...
| 编号 | 系统调用 | 功能 | 对应操作 |
|------|---------|------|---------|
| 592 | `agentrt_sys_capability_request` | 申请 capability 令牌 | 创建 → ACTIVE |
| 593 | `agentrt_sys_capability_revoke` | 撤销 capability | 递归撤销 |
| 594 | `agentrt_sys_lsm_load_policy` | 加载策略 | 策略引擎 |
| 595 | `agentrt_sys_capability_derive` | 全权派生 | derive |
| 596 | `agentrt_sys_capability_mint` | 缩减派生 | mint |
| 597 | `agentrt_sys_capability_mintcopy` | 复制+缩减 | mintcopy |
| 598 | `agentrt_sys_capability_inspect` | 检查权限 | 查询 |
| 599 | `agentrt_sys_lsm_audit_query` | 查询审计日志 | 审计 |
| 600 | `agentrt_sys_capability_transfer` | IPC 传递 | transfer |

### 9.2 核心 C 接口

```c
/**
 * agentrt_sys_capability_request - 申请 capability 令牌
 * @capability: 权限名称（如 "file.read"、"ipc.send"）
 * @resource:   资源路径（如 "/etc/agentrt/config.yaml"）
 *
 * 返回: >0 capability ID（成功），<0 AGENTRT_E* 错误码
 *       -AGENTRT_EPERM 权限拒绝
 *
 * capability 令牌由内核生成、不可伪造（seL4 风格）。
 * 通过 IPC 消息可跨进程传递（AGENTRT_IPC_FLAG_CAP_CARRY 标志）。
 *
 * @since 1.0.1
...
/**
 * agentrt_sys_capability_revoke - 递归撤销 capability
 * @cap_slot: capability 句柄（ID）
 *
 * 返回: 0 成功，-AGENTRT_EINVAL 句柄无效
 *
 * 借鉴 seL4 cteRevoke 算法，递归撤销所有派生 capability。
 *
 * @since 1.0.1
 */
AGENTRT_API int agentrt_sys_capability_revoke(uint32_t cap_slot);

/**
 * agentrt_sys_capability_mintcopy - 复制 + 缩减权限
 * @src_cap:     源 capability ID
 * @dest_agent:  目标 Agent ID
 * @mask:        权限掩码
 * @new_cap_out: 新 capability ID 输出
 ...
 * @since 1.0.1
 */
AGENTRT_API int agentrt_sys_capability_mintcopy(
    uint32_t src_cap, uint32_t dest_agent,
    uint64_t mask, uint32_t *new_cap_out);

/**
 * agentrt_sys_capability_transfer - 通过 IPC 传递 capability
 * @cap_id:     capability ID
 * @target_ep:  目标 IPC 端点
 * @flags:      传递标志（AGENTRT_CAP_TRANSFER_*）
 *
 * 返回: 0 成功，<0 AGENTRT_E* 错误码
 *
 ...
 * @since 1.0.1
 */
AGENTRT_API int agentrt_sys_capability_transfer(
...
static struct security_hook_list capability_hooks[] __lsm_ro_after_init = {
    LSM_HOOK_INIT(capable,           agentrt_cap_capable),
    LSM_HOOK_INIT(inode_permission,  agentrt_cap_inode_perm),
    LSM_HOOK_INIT(file_open,         agentrt_cap_file_open),
    LSM_HOOK_INIT(task_create,       agentrt_cap_task_create),
    LSM_HOOK_INIT(cred_prepare,      agentrt_cap_cred_prepare),
    LSM_HOOK_INIT(cred_free,         agentrt_cap_cred_free),
};
```
 ...
 * 用于安全敏感系统调用入口处
 */
#define AGENTRT_CAP_GUARD(cap_name, resource) ({            \
    int __cap = agentrt_sys_capability_request(             \
        (cap_name), (resource));                            \
    if (__cap < 0) {                                        \
...
#define AGENTRT_CAP_GUARD_END(cap_slot)  \
    agentrt_sys_capability_revoke(cap_slot)
```
...
```c
/* 安全敏感系统调用：IPC 发送 */
int agentrt_sys_ipc_send(const struct agentrt_ipc_msg_hdr *hdr,
                         const void *payload)
{
    /* 1. capability 守卫 */
    int cap = AGENTRT_CAP_GUARD("ipc.send", NULL);
...
capability 可通过 IPC 消息的 `AGENTRT_IPC_FLAG_CAP_CARRY` 标志跨进程传递：

```c
/* 发送方：携带 capability */
struct agentrt_ipc_msg_hdr hdr = {
    .magic   = AGENTRT_IPC_MAGIC,
    .flags   = AGENTRT_IPC_FLAG_CAP_CARRY,
    .cap_id  = my_cap_id,  /* 携带的 capability */
    /* ... */
};
agentrt_sys_ipc_send(&hdr, payload);

/* 接收方：获取派生 capability */
struct agentrt_ipc_msg_hdr recv_hdr;
agentrt_sys_ipc_recv(&recv_hdr, buf, len);
if (recv_hdr.flags & AGENTRT_IPC_FLAG_CAP_CARRY) {
...
| 共享内容 | 影响 |
|---------|------|
| POSIX capability 38 ID 枚举 | 编号一致 |
| LSM 钩子 254 ID 枚举 | 钩子编号一致 |
...
struct agentrt_task_config cfg = {
    .capability_flags = AGENTRT_CAP_COGNITION | AGENTRT_CAP_IPC | AGENTRT_CAP_ROVOL,
};
uint32_t agent_id;
agentrt_sys_task_register(&cfg, &agent_id);
...
int cap = agentrt_sys_capability_request("file.read", "/data/shared.txt");

/* 2. Agent A 通过 mintcopy 传递受限 capability */
uint32_t new_cap;
agentrt_sys_capability_mintcopy(cap, agent_b_id, 0x01, &new_cap);
/* 0x01 = 仅读权限（badge 缩减） */
...
void agent_terminate_cleanup(uint32_t agent_id)
{
    ...
    /* 2. 递归撤销根 capability */
    agentrt_cap_revoke(cspace->root_cap);

    /* 3. 清理 CSpace */
    agentrt_cspace_destroy(cspace);
...
static void test_cap_revoke_recursive(struct kunit *test)
{
    ...
    KUNIT_EXPECT_EQ(test, 0,
        agentrt_cap_create(AGENT_TYPE_ENDPOINT, 0xFF, &root_cap));

    /* 2. 派生两个子 capability */
    KUNIT_EXPECT_EQ(test, 0,
        agentrt_cap_mintcopy(root_cap, agent_b, 0x0F, &child1));
    KUNIT_EXPECT_EQ(test, 0,
        agentrt_cap_mintcopy(root_cap, agent_c, 0x33, &child2));

    /* 3. child1 再派生孙 capability */
    KUNIT_EXPECT_EQ(test, 0,
        agentrt_cap_mintcopy(child1, agent_d, 0x03, &grandchild));

    /* 4. 撤销根 capability */
    KUNIT_EXPECT_EQ(test, 0, agentrt_cap_revoke(root_cap));

    /* 5. 验证所有派生 capability 已撤销 */
    KUNIT_EXPECT_EQ(test, get_cap_state(child1), AGENTRT_CAP_STATE_REVOKED);
    KUNIT_EXPECT_EQ(test, get_cap_state(child2), AGENTRT_CAP_STATE_REVOKED);
    KUNIT_EXPECT_EQ(test, get_cap_state(grandchild), AGENTRT_CAP_STATE_REVOKED);
}
```
...
| 版本 | 日期 | 变更 |
|------|------|------|
| 0.1.1 | 2026-07-09 | 初始版本。定义 seL4 风格 + POSIX 混合 capability 模型：CNode/CSpace 数据结构、MDB 派生链、四种派生操作（mint/mintcopy/derive/revoke）、POSIX 38 ID 枚举、令牌 7 状态生命周期、Cupolas blob 四类布局（cred/inode/file/task）、4 值策略裁决（ALLOW/DENY/AUDIT/DENY_AUDIT）、Vault backend 抽象（memory/TPM/CVM）、系统调用集成（592-600）、LSM_ORDER_FIRST 集成、capability 守卫流程、IPC 传递、KUnit 测试 |
