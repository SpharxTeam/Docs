...
agentrt-linux 的 `CONFIG_LSM` 默认配置为：`capability,landlock,yama,airy`（其中 capability 与 integrity 分别由 `LSM_ORDER_FIRST` / `LSM_ORDER_LAST` 强制首尾；airy 借助 `LSM_ORDER_FIRST` 与 capability 共同占据首位，实际顺序由 `CONFIG_LSM` 列表中的位置决定，capability 永远在 airy 之前执行）。详见 [07-airy-lsm-design.md](07-airy-lsm-design.md) §2.1。
...
Cupolas 通过 `DEFINE_LSM` 宏在编译期注册自身（模块名 `airy`，初始化顺序 `LSM_ORDER_FIRST`，与 capability 共同占据首位，实际顺序由 `CONFIG_LSM` 列表决定，capability 永远在 airy 之前执行）：
...
DEFINE_LSM(airy) = {
    .name  = "airy",
    .order = LSM_ORDER_FIRST,   /* 与 capability 共同占据首位，capability 永远在前 */
    .init  = airy_lsm_init,
...
```c
/* include/airymax/security_types.h —— IRON-9 v3 [SC] 共享契约层（节选） */
...
| 初始化阶段 | Agent 启动时 | `LSM_ORDER_FIRST`（airy 在 capability 之后第一个初始化，对齐 07-airy-lsm-design.md §2.1） |
...
```c
/* include/airymax/security_types.h —— IRON-9 v3 [SC] 共享契约层 */
 ...
 *
 * 初始化顺序: LSM_ORDER_FIRST（airy 与 capability 共同占据首位，capability 永远在前）
 ...
 *
 * @LSM_ORDER_FIRST:   强制第一位（capability 与 airy 使用，实际顺序由 CONFIG_LSM 列表决定，capability 永远在 airy 之前）
 * @LSM_ORDER_MUTABLE: 可配置顺序（Landlock/Yama 使用）
 ...
 * 对齐 Linux 6.6 include/linux/lsm_hooks.h
 */
#define DEFINE_LSM(name) \
    static struct lsm_info __lsm_info_##name \
        __used __section(".lsm_info") \
 ...
 * 宏只声明变量，初始化由调用方以 = { ... }; 形式提供。
 * 使用形式：DEFINE_EARLY_LSM(name) = { .name = "name", .init = name_init, ... };
 * 用于 lockdown、纯 C LSM 模块（对齐 openEuler 实践，使用 DEFINE_LSM + LSM_HOOK_INIT 宏注册，对齐 Landlock/Yama/Tomoyo 等 Linux 主线纯 C LSM）。
 *
 * 对齐 Linux 6.6 include/linux/lsm_hooks.h
 */
#define DEFINE_EARLY_LSM(name) \
    static struct lsm_info __early_lsm_info_##name \
        __used __section(".early_lsm_info") \
        __aligned(sizeof(unsigned long))
```
