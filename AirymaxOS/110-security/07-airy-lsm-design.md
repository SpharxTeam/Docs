...
### 1.3 设计目标（v1.0.1: fastpath/slowpath 职责分割）

纯 C LSM 模块的核心设计目标：

1. **Capability 校验**（v1.0.1 职责分割）：fastpath C-S9 内联 Badge 校验（~10ns）+ slowpath LSM 钩子接管异常（冷酷执法）
2. **io_uring 安全**：opcode 白名单校验（§4）
3. **LSM_ORDER_MUTABLE + CONFIG_LSM 优先位置**（v1.0.1 修正）：通过 `CONFIG_LSM` 默认值将 `airy` 置于 `capability` 之后、其他 LSM 之前，实现"capability 优先"语义。**不使用 `LSM_ORDER_FIRST`**——OLK 6.6 `include/linux/lsm_hooks.h:113` 注释明确 `LSM_ORDER_FIRST` "This is only for capabilities"
...
```c
/* security/airy/airy_lsm.c —— 纯 C LSM 模块注册（v1.0.1，OLK 6.6 对齐） */
 ...
 *   };
 *
 * v1.0.1 修正: airy 不使用 LSM_ORDER_FIRST（OLK 6.6 注释明确仅用于 capabilities），
...
DEFINE_LSM(airy) = {
    .name = "airy",
    /* v1.0.1: 不设置 .order 或设置为 LSM_ORDER_MUTABLE（默认值）
...
/* 模块加载时初始化（v1.0.1: agent_caps[] 静态数组替代 radix tree） */
static int __init airy_lsm_init(void)
{
    /* 1. 初始化 agent_caps[] 静态数组 + 全局 Epoch（v1.0.1 替代 radix tree）
     * agent_caps[1024] 是 128KB 内核静态数组，sec_d 唯一写者
     ...
     * v1.0.1: uring_cmd 钩子仅在 slowpath 被 C-S9 失败时调用
     */
...
### 2.2 LSM_ORDER_MUTABLE + CONFIG_LSM 优先位置（v1.0.1 修正）

**v1.0.1 修正**：airy 不使用 `LSM_ORDER_FIRST`（OLK 6.6 注释明确"仅用于 capabilities"），改用 `LSM_ORDER_MUTABLE`（默认值），通过 `CONFIG_LSM` 默认值将 `airy` 置于 `capability` 之后、其他 LSM 之前。
...
config LSM
    ...
    # v1.0.1: 将 airy 添加到默认值，位于 safesetid 之后、selinux 之前
...
| 1（CONFIG_LSM 列表首位） | **airy** | `LSM_ORDER_MUTABLE` | v1.0.1: `CONFIG_LSM` 默认值将 airy 置于列表首位 |
...
## §3 Capability 校验钩子：v1.0.1 fastpath C-S9 + slowpath LSM（职责分割）

### 3.1 v1.0.1 架构：fastpath/slowpath 职责分割

**v1.0.1 重构**：Capability 校验从"独立前置 `airy_cap_check()` + radix tree 查找"重构为 **fastpath C-S9 内联 Badge 校验** + **slowpath LSM 钩子接管**。这是 Capability Folding 单平面架构的核心设计——IPC 不是承载能力校验，IPC 就是能力校验。
...
`security_uring_cmd` 钩子在 v1.0.1 中**仅 slowpath 被 C-S9 失败时调用**，做策略裁决与冷酷执法。这是 Micro-Supervisor 的核心实现，详见 [09-kernel-agent-supervisor.md §2.3](../20-modules/09-kernel-agent-supervisor.md)。
 ...
 *      （v1.0.1: 对齐 LSM_ORDER_MUTABLE + CONFIG_LSM 首位设计，airy 在 SELinux/AppArmor 之前执行）
...
static int airy_uring_cmd_check(struct io_uring_cmd *ioucmd)
{
     ...
     * 此处做二次校验确保 LSM 链中 capability 优先（v1.0.1: LSM_ORDER_MUTABLE + CONFIG_LSM 首位设计）。
...
#### 3.3.3 钩子注册与 LSM_ORDER_MUTABLE（v1.0.1 修正）

```c
/* security/airy/airy_lsm.c —— 钩子注册（v1.0.1，OLK 6.6 对齐）
 *
 ...
 *
 * OLK 6.6 中不存在 uring_sqe / uring_register_buffers 钩子（v1.0 文档误注册，v1.0.1 已删除）。
...
static struct security_hook_list airy_hooks[] __lsm_ro_after_init = {
    /* Capability 校验钩子 —— v1.0.1: LSM_ORDER_MUTABLE + CONFIG_LSM 首位（替代 LSM_ORDER_FIRST） */
...
DEFINE_LSM(airy) = {
    .name    = "airy",
    .order   = LSM_ORDER_MUTABLE,   /* v1.0.1: 不使用 LSM_ORDER_FIRST（OLK 6.6 仅用于 capabilities） */
    .enabled = &airy_enabled,
    .init    = airy_lsm_init,
...
### 3.4 Capability 校验流程（v1.0.1 fastpath C-S9 + slowpath LSM）
...
| 钩子 | 触发场景 | 校验内容 | v1.0.1 变更 |
|------|---------|---------|----------|
| `security_uring_cmd` | io_uring_cmd 提交 | Badge 校验（核心，IPC fastpath）+ opcode 白名单分发 | v1.0.1: 仅 slowpath，fastpath C-S9 内联；opcode 白名单改在本钩子内部分发 |
| `security_task_kill` | kill 信号 | CAP_KILL 权限 | 无变更 |
| `security_file_open` | 文件打开 | CAP_FILE_OPEN 权限 | 无变更 |
| `security_file_ioctl` | 文件 ioctl | buffer 注册类操作校验（替代虚构的 uring_register_buffers） | v1.0.1: 替代 v1.0 误注册的 uring_register_buffers |

> **OLK 6.6 对齐说明**：v1.0 文档曾注册 `security_uring_sqe` 和 `security_uring_register_buffers` 两个虚构 LSM 钩子，但 OLK 6.6 `include/linux/lsm_hook_defs.h` 中**仅定义 3 个 io_uring 钩子**（`uring_override_creds`/`uring_sqpoll`/`uring_cmd`），不存在上述两个钩子。v1.0.1 已删除虚构钩子，opcode 白名单改在 `security_uring_cmd` 内部基于 `ioucmd->cmd_op` 分发，buffer 注册校验改由 `security_file_ioctl` 等既有钩子承担。
...
## §5 与 seL4 Capability 模型的关系：借鉴 seL4 capability-based security（v1.0.1: Badge 64-bit Native Word）
...
|-----------|---------------------|------|
| CNode（Capability Node） | `struct airy_cap_slot`（agent_caps[] 数组项） | v1.0.1: 物理存储替代 airy_cnode（逻辑概念保留，详见 03-capability-model.md §2.1） |
| CSpace（Capability Space） | `agent_caps[1024]` 静态数组（128KB） | v1.0.1: 替代 radix tree |
| mint/copy/move/revoke | `airy_cap_badge_compile/revoke`（sec_d 编译 + atomic_inc 撤销） | v1.0.1: sec_d 唯一写者，撤销 = 1 行 atomic_inc |
| badge（权限掩码） | Badge 64-bit Native Word（`Epoch<<48 \| RandomTag<<16 \| Perms`） | v1.0.1: 完整 64-bit Badge，含 Epoch + RandomTag + Perms |
...
## §6 性能：纯 C 无 BPF 间接调用开销（v1.0.1: fastpath C-S9 ~10ns）

### 6.1 性能对比（v1.0.1: fastpath C-S9 Badge 校验 ~10ns）
...
| 内存开销 | 128KB 静态数组（agent_caps[]） | BPF 程序 + map | ~3x |
| 编译开销 | 编译期 | 运行时 JIT | — |

### 6.2 fastpath C-S9 性能拆解（v1.0.1）

v1.0.1 fastpath C-S9 Badge 校验的性能拆解（无 RCU 锁、无 radix tree 查找）：
...
### 6.3 与 IPC fastpath 的关系（v1.0.1: FAST_SEND ~158ns）

v1.0.1 fastpath C-S9 是 IPC fastpath 的一部分，总延迟在 SLO 内：
...
### 7.2 借鉴的纯 C 模式（v1.0.1: agent_caps[] 替代 radix tree）
...
| `security/selinux/ss/policydb.c` | 策略数据库 | v1.0.1: `agent_caps[1024]` 静态数组（替代 radix tree，128KB 无锁多读者） |
| `security/landlock/setup.c` | `DEFINE_LSM` + 默认 `LSM_ORDER_MUTABLE` | v1.0.1: airy 同样使用 `LSM_ORDER_MUTABLE` + `CONFIG_LSM` 首位 |
...
Airymax 纯 C LSM 与其他 LSM 共存，v1.0.1 执行顺序为 `LSM_ORDER_MUTABLE` + `CONFIG_LSM` 首位（capability 强制首位由 OLK 6.6 硬编码）：

| 顺序 | LSM | order | 执行内容 | Airymax 关系 |
|------|-----|-------|---------|-------------|
| 0 | capability | `LSM_ORDER_FIRST` | POSIX capability 检查 | OLK 6.6 硬编码首位 |
| 1 | **Airymax** | `LSM_ORDER_MUTABLE` | Badge 校验（C-S9 fastpath + LSM slowpath） | v1.0.1: `CONFIG_LSM` 默认值置于 capability 之后、其他 LSM 之前 |
...
### 9.2 性能 SLO（v1.0.1: fastpath C-S9 ~10ns）

| 指标 | SLO | 实测 | 说明 |
|------|-----|------|------|
| fastpath C-S9 Badge 校验 | ≤20ns | ~10ns | v1.0.1: 3×READ_ONCE + 位运算 + 比较 |
| FAST_SEND 总延迟 | ≤200ns | ~158ns | v1.0.1: 含 C-S9 + Ring Buffer 写入 |
| SLOW_SEND（C-S9 失败） | ≤1μs | ~600ns-5.5μs | v1.0.1: LSM 钩子 + 冷酷执法 |
| opcode 白名单检查 | ≤10ns | ~5ns | 无变更 |
| Badge 撤销（atomic_inc） | ≤5ns | ~1ns | v1.0.1: 1 行 atomic_inc |
| LSM 注册开销 | 编译期 | 0（运行时） | 无变更 |
| 与 BPF LSM 对比 | 更快 | ~20x faster（fastpath C-S9） | v1.0.1: ~10ns vs ~200ns+ |

---

## §10 相关文档

- [01-lsm-framework.md](01-lsm-framework.md) —— LSM 框架（上级设计，v1.0.1: §8.4 fastpath C-S9 与 LSM 钩子职责分割）
- [10-unify-design.md](../10-architecture/10-unify-design.md) §7 —— A-ULS 模块（Micro-Supervisor 基于 LSM）
- [09-kernel-agent-supervisor.md](../20-modules/09-kernel-agent-supervisor.md) —— Micro-Supervisor（v1.0.1: §2.3 slowpath LSM 接管 + §6.4 Badge 撤销 atomic_inc）
- [03-capability-model.md](03-capability-model.md) —— Capability 模型（v1.0.1: Badge 64-bit Native Word，§2.5 Perms 位定义）
- [02-ipc-protocol.md](../30-interfaces/02-ipc-protocol.md) §3 —— opcode 表（v1.0.1: FREEZE 0x0005）
...
| 2 | **Cache 侧信道** | `agent_caps[1024]` 128KB 静态数组的 cache line 占用模式 | 同 CPU 核心或共享 LLC 的恶意 Agent，通过 Prime+Probe / Flush+Reload 推断 | 推断 fastpath C-S9 读取 `agent_caps[agent_id]` 的访问模式（哪个 Agent 槽位被读） |
...
> **文档结束** | 纯 C LSM 模块设计 | v1.0.1 | 2026-07-18
