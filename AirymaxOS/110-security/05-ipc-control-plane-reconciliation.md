...
1. 扫描数据面状态（io_uring_cmd RECONCILE_SCAN）
    │  ├── 扫描所有存活 Ring（内核 radix tree 遍历）
    │  ├── 扫描 Capability 缓存（radix tree 遍历）
    │  ├── 扫描离线队列（暂存消息）
...
static void reconcile_timeout_handler(int sig, siginfo_t *info, void *ctx)
{
    ...
    /* 3. 数据面继续自治运行（基于现有缓存） */
    /*    新 Capability 请求直接拒绝，返回 AIRY_ECAP_OFFLINE */
...
| 新 Ring 创建 | 控制面授权 | 拒绝（返回 `AIRY_ECAP_OFFLINE`） |
...
© 2025-2026 SPHARX Ltd. All Rights Reserved. | IPC 控制面 Reconciliation 设计 | v1.0 | 2026-07-17
