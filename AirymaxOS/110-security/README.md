...
> **文档定位**： agentrt-linux（AirymaxOS）安全工程体系主索引
> **版本**： 0.1.1
> **最后更新**： 2026-07-13
...
agentrt-linux 安全加固体系是系统可信运行的核心保障。它继承 Linux 内核 30+ 年沉淀的多层安全哲学（LSM 框架 + Landlock 用户态沙箱 + capability + 模块签名 + Lockdown + 4 层密钥环），并在其上扩展智能体操作系统专属的 Cupolas 安全穹顶、机密计算、国密算法等。
...
agentrt-linux 采用 seL4 风格的 capability 安全模型：
...
├── 07-cupolas-dome.md              # agentrt-linux 专属：Cupolas 安全穹顶
├── 08-confidential-computing.md    # agentrt-linux 专属：机密计算（VirtCCA）
└── 09-cryptography-compliance.md   # agentrt-linux 专属：国密算法合规
```

### 3.1 0.1.1 版本范围

README + 01 + 02 + 03 文档（4 文档奠基）。03-capability-model 补充完整 capability 数据模型、CNode/MDB 派生链、四种派生操作（mint/mintcopy/derive/revoke）、POSIX 41 ID 枚举、令牌 7 状态生命周期、Cupolas blob 四类布局、4 值策略裁决、Vault backend 抽象、系统调用 592-600 集成、LSM_ORDER_FIRST 共存机制。其余 6 文档（04-module-signing 至 09-cryptography-compliance）在 1.0.1 版本完成。

### 3.2 1.0.1 版本范围

完成剩余 6 文档（04-module-signing 至 09-cryptography-compliance），实施安全工程标准，进行代码级验证。
