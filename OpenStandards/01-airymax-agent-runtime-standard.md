...
| 转移 | 触发接口 | 约束 |
|------|---------|------|
| CREATED → REGISTERED | `agentrt_agent_register` | 必须提供合法元数据 |
| REGISTERED → SCHEDULED | `agentrt_agent_schedule` | 必须通过能力检查 |
| SCHEDULED → RUNNING | 调度器选中 | 自动 |
| RUNNING → SUSPENDED | `agentrt_agent_suspend` | 必须保存上下文 |
| SUSPENDED → SCHEDULED | `agentrt_agent_resume` | 必须恢复上下文 |
| RUNNING → TERMINATED | Agent 自然结束 或 `agentrt_agent_terminate` | 必须清理临时资源 |
| 任意 → DESTROYED | `agentrt_agent_destroy` | 必须回收全部资源 |

非法转移**必须**返回 `AGENTRT_EINVAL`（AOS-STD-RT-003）。
...
```c
/**
 * agentrt_agent_create - 创建 Agent 实例
 ...
 * 返回值:
 *   0:                  成功
 *   -AGENTRT_EINVAL:   元数据格式非法
 *   -AGENTRT_ENOMEM:   内存不足
 *   -AGENTRT_EPERM:    权限不足
 */
int agentrt_agent_create(const uint8_t *meta, size_t meta_len,
                         agentrt_agent_t *agent_out);

/**
 * agentrt_agent_register - 注册 Agent 到运行时
 * @agent:        Agent 句柄
 *
 ...
 * 返回值:
 *   0:                  成功
 *   -AGENTRT_EINVAL:   Agent 处于非法状态
 *   -AGENTRT_EEXIST:   Agent ID 已存在
 *   -AGENTRT_EPERM:    能力检查失败
 */
int agentrt_agent_register(agentrt_agent_t agent);

/**
 * agentrt_agent_schedule - 调度 Agent
 * @agent:        Agent 句柄
 ...
 *   0:                  成功
 *   -AGENTRT_EINVAL:   参数非法
 *   -AGENTRT_EBUSY:    调度资源已满（达到 MAC_MAX_AGENTS）
 */
int agentrt_agent_schedule(agentrt_agent_t agent, uint32_t priority,
                           agentrt_token_budget_t budget);

/**
 * agentrt_agent_suspend - 挂起 Agent
 * @agent:        Agent 句柄
 *
 * 返回值:
 *   0:                  成功
 *   -AGENTRT_EINVAL:   Agent 未运行
 */
int agentrt_agent_suspend(agentrt_agent_t agent);

/**
 * agentrt_agent_resume - 恢复 Agent
 * @agent:        Agent 句柄
 *
 * 返回值:
 *   0:                  成功
 *   -AGENTRT_EINVAL:   Agent 未挂起
 */
int agentrt_agent_resume(agentrt_agent_t agent);

/**
 * agentrt_agent_destroy - 销毁 Agent
 * @agent:        Agent 句柄
 *
 ...
 * 返回值:
 *   0:                  成功
 *   -AGENTRT_EINVAL:   Agent 句柄非法
 */
int agentrt_agent_destroy(agentrt_agent_t agent);
```
...
| 要素 | 含义 |
|------|------|
| `hard_limit` | 硬上限，超过**必须**终止 Agent 并返回 `AGENTRT_EBUDGET` |
...
1. 扣减量 = `实际消耗 token 数`。
2. 若 `remaining` 扣减后 < 0，**必须**终止 Agent 并返回 `AGENTRT_EBUDGET`（错误码 -1004 区段，详见 03 标准）。
3. 若 `remaining` < `soft_limit`，**应当**触发 `AGENT_EVENT_BUDGET_LOW` 事件。
4. 令牌桶补充由调度器在每秒节拍执行，补充量 = `replenish_rate`。

### 5.4 预算继承

子 Agent 通过 `agentrt_agent_create` 创建时**可以**继承父 Agent 的预算（AOS-STD-RT-024）：
...
| AOS-STD-RT-004 | agentrt_agent_create 接口 | L1 |
| AOS-STD-RT-005 | agentrt_agent_register 接口 | L1 |
| AOS-STD-RT-006 | agentrt_agent_schedule 接口 | L1 |
| AOS-STD-RT-007 | agentrt_agent_suspend 接口 | L1 |
| AOS-STD-RT-008 | agentrt_agent_resume 接口 | L1 |
| AOS-STD-RT-009 | agentrt_agent_destroy 接口 | L1 |
...
/* Agent 入口函数，签名由运行时约定 */
int weather_agent_main(agentrt_agent_t self, int argc, char **argv)
{
    /* 1. 获取自身能力令牌（运行时根据元数据自动授予） */
    are_cap_t net_cap, ipc_cap;
    if (agentrt_cap_lookup(self, AGENT_CAP_DOMAIN_NET,  &net_cap)  != 0) return -1;
    if (agentrt_cap_lookup(self, AGENT_CAP_DOMAIN_IPC, &ipc_cap) != 0) return -1;

    /* 2. 通过能力令牌发起 HTTP 请求（受约束：仅访问 api.weather.example） */
    char response[4096];
    int rc = agentrt_http_get(net_cap, "/v1/forecast?city=beijing",
    ...
    hdr.flags        = AGENTRT_IPC_FLAG_NOREPLY;
    hdr.src_pid      = agentrt_self_pid(self);
    hdr.dst_pid      = 0;                       /* 广播 */
    rc = agentrt_ipc_send(ipc_cap, &hdr, response, (size_t)rc);
    return rc;
}
```
...
int load_and_run_weather_agent(void)
{
    ...
    agentrt_agent_t agent;
    agentrt_token_budget_t budget = { .hard_limit = 4096, .soft_limit = 3072,
                                      .replenish_rate = 50 };

    int rc = agentrt_agent_create(meta, meta_len, &agent);          /* CREATED */
    if (rc) return rc;
    rc = agentrt_agent_register(agent);                            /* REGISTERED */
    if (rc) goto destroy;
    rc = agentrt_agent_schedule(agent, 100 /* 交互响应 */, budget);/* SCHEDULED */
    if (rc) goto destroy;
...
| `AGENTRT_EEXIST` | -17 | 已存在 |
| `AGENTRT_EBUSY` | -16 | 资源忙 |
