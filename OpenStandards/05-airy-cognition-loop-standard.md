...
```c
typedef enum {
    AIRY_COG_PHASE_PERCEPTION = 0,
    AIRY_COG_PHASE_THINKING   = 1,
    AIRY_COG_PHASE_ACTION      = 2,
} airy_cog_phase_t;
```
...
非法转移**必须**返回 `AIRY_EINVAL`（AOS-STD-COG-003）。

### 2.3 CoreLoopThree 上下文结构

```c
#define AIRY_COG_MAGIC 0x434f4731u  /* 'COG1' */
...
} airy_cog_context_t;             /* 总长 128 字节 */

_Static_assert(sizeof(airy_cog_context_t) == 128,
               "airy_cog_context_t must be 128 bytes");
```
...
```c
typedef enum {
    AIRY_THINK_SYSTEM1_FAST = 0,
    AIRY_THINK_SYSTEM2_SLOW = 1,
} airy_think_mode_t;
```
...
```c
typedef enum {
    AIRY_LLM_PHASE_PREFILL     = 0,
    AIRY_LLM_PHASE_DECODE      = 1,
    AIRY_LLM_PHASE_SPECULATIVE = 2,
} airy_llm_phase_t;
```
...
```c
/**
 * airy_cog_report_efficiency - 上报能效指标
 ...
 *   0: 成功
 */
int airy_cog_report_efficiency(const airy_cog_context_t *ctx,
                                   const airy_token_efficiency_t *efficiency);
```
 ...
 *   -AIRY_EINVAL:   参数无效
 */
int airy_cog_loop_init(airy_cog_context_t *ctx,
                          uint64_t agent_id,
                          uint64_t token_budget,
                          uint64_t deadline_ns);

/**
 * airy_cog_loop_step - 推进一个阶段
 * @ctx:     上下文
 * @input:   阶段输入
 * @output:  阶段输出
 *
...
/**
 * airy_cog_loop_finalize - 终止循环
 * @ctx: 上下文
 */
int airy_cog_loop_finalize(airy_cog_context_t *ctx);
```

### 6.2 阶段处理函数注册

```c
typedef int (*airy_cog_phase_handler_t)(airy_cog_context_t *ctx,
                                            const void *input,
                                            void *output);

/**
 * airy_cog_register_phase - 注册阶段处理函数
 * @phase:    阶段
 * @handler:  处理函数
 */
int airy_cog_register_phase(airy_cog_phase_t phase,
                                airy_cog_phase_handler_t handler);
```
...
```c
/* 注册阶段处理函数 */
airy_cog_register_phase(AIRY_COG_PHASE_PERCEPTION, handle_perception);
airy_cog_register_phase(AIRY_COG_PHASE_THINKING,   handle_thinking);
airy_cog_register_phase(AIRY_COG_PHASE_ACTION,     handle_action);

/* 运行认知循环 */
airy_cog_context_t ctx;
airy_cog_loop_init(&ctx,
                      /* agent_id */ 42,
                      /* budget */ 4096,
                      /* deadline */ 0);
...
while (ctx.current_phase != /* terminated */ 99) {
    int rc = airy_cog_loop_step(&ctx, input, output);
    if (rc == -AIRY_EBUDGET) {
...
airy_cog_loop_finalize(&ctx);
```

### 10.2 Thinkdual 模式选择

```c
int handle_thinking(airy_cog_context_t *ctx, const void *input, void *output)
{
    /* 根据意图复杂度选择模式 */
    intent_t *intent = (intent_t *)input;
    if (intent->complexity < 0.3 && ctx->token_budget - ctx->token_consumed > 500) {
        ctx->think_mode = AIRY_THINK_SYSTEM1_FAST;
    } else {
        ctx->think_mode = AIRY_THINK_SYSTEM2_SLOW;
    }
...
airy_cog_report_efficiency(&ctx, &eff);
```
...
- [Airymax 记忆卷载标准](./04-airy-memory-rovol-standard.md)
