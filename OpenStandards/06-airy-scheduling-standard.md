...
```c
#define AIRY_TASK_MAGIC       0x41475453u  /* 'A''G''T''S' */
#define AIRY_TASK_VERSION     0x0100u
#define AIRY_TASK_DESC_SIZE   128u
...
_Static_assert(sizeof(struct airy_task_desc) == AIRY_TASK_DESC_SIZE,
               "struct airy_task_desc must be 128 bytes");
```
...
```c
typedef uint64_t airy_q16_t;

#define AIRY_Q16_ONE        (1ULL << 16)   /* 1.0 的 Q16.16 表示 = 65536 */
#define AIRY_Q16_SHIFT      16

/* 转换：整数 -> Q16.16 */
static inline airy_q16_t airy_q16_from_int(uint64_t i) {
    return i << AIRY_Q16_SHIFT;
}

/* 转换：Q16.16 -> 整数（截断小数） */
static inline uint64_t airy_q16_to_int(airy_q16_t q) {
    return q >> AIRY_Q16_SHIFT;
}
```
...
| 超过时 `airy_agent_schedule` **必须**返回 `AIRY_EBUSY` | AOS-STD-SCHED-029 |
...
struct {
    ...
    __type(key, u32);
    __type(value, struct airy_task_desc);
    __uint(max_entries, MAC_MAX_AGENTS);  /* 1024 */
} task_map SEC(".maps");

/* 7.2 入队：选择下一个运行的任务 */
s32 BPF_STRUCT_OPS(airy_enqueue, struct task_struct *p, u64 enq_flags)
{
    u32 pid = p->pid;
    struct airy_task_desc *desc = bpf_map_lookup_elem(&task_map, &pid);
    if (!desc || desc->magic != AIRY_TASK_MAGIC) {
    ...
    u64 weight  = airy_q16_weight(desc->priority);  /* Q16.16 权重 */
    desc->vtime += (runtime * weight) >> AIRY_Q16_SHIFT;

    /* 入就绪队列（按 vtime 排序的 BPF rbtree） */
    return airy_bpf_enqueue_vtime(p, desc->vtime, enq_flags);
}

/* 7.3 出队：选择 vtime 最小的任务 */
struct task_struct *BPF_STRUCT_OPS(airy_pick_next, struct task_struct *prev)
{
    return airy_bpf_pick_min_vtime();
}

/* 7.4 调度类注册 */
struct sched_ext_ops airy_sched_ops = {
    .enqueue    = (void *)airy_enqueue,
    .pick_next  = (void *)airy_pick_next,
    .name       = "airy_sched",
};
SEC(".struct_ops.link")
struct sched_ext_ops *airy_sched_ops_link = &airy_sched_ops;
```

### 7.2 Q16.16 权重计算（BPF 内联）

```c
static __always_inline u64 airy_q16_weight(u32 priority)
{
...
```c
/**
 * airy_sched_submit - 提交任务到调度器
 * @desc: 任务描述符（已填充）
 *
 * 返回值:
 *   0:                  成功
 *   -AIRY_EINVAL:   描述符非法（magic/version 不匹配）
 *   -AIRY_EBUSY:    并发数达到 MAC_MAX_AGENTS
 *   -AIRY_EPERM:    优先级超权限
 */
int airy_sched_submit(struct airy_task_desc *desc);
```
...
```c
/**
 * airy_sched_cancel - 撤销任务
 * @task_id: 任务 ID
 */
int airy_sched_cancel(uint64_t task_id);
```
...
```c
/**
 * airy_sched_query - 查询任务调度状态
 * @task_id:  任务 ID
 * @state_out: 输出：当前状态（vtime、剩余预算等）
 */
int airy_sched_query(uint64_t task_id, struct airy_task_desc *state_out);
```
...
|--------|------|------|
| `AIRY_EOK` | 0 | 成功 |
| `AIRY_EINVAL` | -2 | 描述符非法（magic/version 不匹配） |
| `AIRY_EBUSY` | -1601 | 并发数达到上限 |
| `AIRY_EPERM` | -1302 | 优先级超权限 |
| `AIRY_ESCHED` | -1602 | 调度器未初始化 |
| `AIRY_ENOENT` | -2 | 任务不存在 |
...
```c
struct airy_task_desc desc = {0};
desc.magic         = AIRY_TASK_MAGIC;   /* 0x41475453 */
desc.version       = AIRY_TASK_VERSION;  /* 0x0100 */
desc.priority      = 100;                   /* Agent 认知类 */
desc.agent_id      = 42;
desc.task_id       = next_task_id();
desc.vtime         = airy_q16_from_int(0); /* 初始 vtime = 0 */
desc.deadline_ns   = 0;                     /* 无截止 */
desc.token_budget  = 4096;
desc.token_consumed = 0;
desc.started_ns    = now_ns();
desc.cog_phase     = AIRY_COG_PHASE_PERCEPTION;  /* 对齐 05 标准 */
desc.think_mode    = AIRY_THINK_SYSTEM2_SLOW;
desc.cap_id        = 1001;

int rc = airy_sched_submit(&desc);
if (rc == -AIRY_EBUSY) {
    /* 并发已满 */
}
```
...
airy_q16_t delta = (runtime * weight) >> AIRY_Q16_SHIFT;
...
sudo bpftool struct_ops register airy_sched.bpf.o
