...
- Cupolas capability 安全模型（41 ID 枚举 + 派生模型：mint/mintcopy/derive/revoke）
- LSM 钩子标准（252 ID 枚举）
...
本标准在 Linux 6.6 标准 41 个 capability 基础上扩展为 44 ID 枚举（前 41 个与 Linux 一致，42-44 为 Airymax 专属）：
...
枚举值**永久稳定**（AOS-STD-SEC-002，L1 稳定级）；新增 capability **必须**追加到 ID 44 之后。
...
本标准定义 252 个 LSM 钩子 ID，覆盖 9 个类别：
...
| 内存管理 | 9 | 243-251 | 内存分配、映射、防护 |
| **总计** | **252** | **0-251** | |

ID 范围**永久稳定**（AOS-STD-SEC-015，L1 稳定级）；新增钩子**必须**追加到 252 之后，**不得**修改既有 ID。

### 4.2 钩子类别命名

```c
typedef enum {
    AIRY_LSM_PROC    = 0,   /* 进程管理：0-41 */
    AIRY_LSM_FILE    = 1,   /* 文件系统：42-99 */
    AIRY_LSM_IPC     = 2,   /* IPC：100-134 */
    AIRY_LSM_NET     = 3,   /* 网络：135-175 */
    AIRY_LSM_CAP     = 4,   /* 能力管理：176-197 */
    AIRY_LSM_SANDBOX = 5,   /* 沙箱管理：198-215 */
    AIRY_LSM_AUDIT   = 6,   /* 审计管理：216-230 */
    AIRY_LSM_COG     = 7,   /* 认知管理：231-242 */
    AIRY_LSM_MEM     = 8,   /* 内存管理：243-251 */
} airy_lsm_category_t;
```
 ...
 */
int airy_lsm_register(const struct airy_security_ops *ops,
                         const char *name);
```
...
```c
struct airy_agent_security_ops {
    /* 能力管理钩子（ID 176-197） */
...
```c
typedef enum {
    AIRY_DECISION_ALLOW   = 0,  /* 允许操作 */
    AIRY_DECISION_DENY    = 1,  /* 拒绝操作 */
    AIRY_DECISION_AUDIT   = 2,  /* 允许但记录审计 */
    AIRY_DECISION_ENFORCE = 3,  /* 强制执行（如强制沙箱升级） */
} airy_decision_t;
```
...
| AOS-STD-SEC-002 | POSIX capability 41+3 ID 枚举 | L1 |
...
| AOS-STD-SEC-015 | LSM 钩子 252 ID | L1 |
...
| 沙箱 | -1400 ~ -1499 | 沙箱错误 | `AIRY_SANDBOX_FULL=-1401` |
| 能力 | -1500 ~ -1599 | 能力错误 | `AIRY_CAP_EXPIRED=-1501` |
| 调度 | -1600 ~ -1699 | 调度错误 | `AIRY_SCHED_EXHAUSTED=-1601` |
| 认知 | -1700 ~ -1799 | 认知错误 | `AIRY_COG_TIMEOUT=-1701` |
| 审计 | -1800 ~ -1899 | 审计错误 | `AIRY_AUDIT_DISK_FULL=-1801` |
...
| SEC-T-011 | LSM 钩子 ID | 252 个 ID 范围精确匹配 |
...
/* 派生子能力：只读，TTL 3600 秒 */
are_cap_t child_cap;
int rc = airy_cap_derive(parent_cap,
...
```c
int final_decision = AIRY_DECISION_ALLOW;
...
/* 最终：DENY（优先级最高） */
assert(final_decision == AIRY_DECISION_DENY);

int merge_decision(int current, int incoming) {
    /* 优先级：DENY > ENFORCE > AUDIT > ALLOW */
    if (incoming == AIRY_DECISION_DENY)    return AIRY_DECISION_DENY;
    if (incoming == AIRY_DECISION_ENFORCE && current != AIRY_DECISION_DENY)
        return AIRY_DECISION_ENFORCE;
    if (incoming == AIRY_DECISION_AUDIT && current == AIRY_DECISION_ALLOW)
        return AIRY_DECISION_AUDIT;
    return current;
}
```
...
| 版本 | 日期 | 变更 |
|------|------|------|
| 1.0 | 2026-07-09 | 初始草案：capability 41+3 ID 枚举、派生模型 mint/mintcopy/derive/revoke、LSM 252 ID、裁决 4 值、最小权限原则 |
