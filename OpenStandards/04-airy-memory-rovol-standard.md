...
```c
#define AIRY_MEM_MAGIC  0x4d454d31u  /* 'MEM1' */
...
} airy_mem_entry_hdr_t;  /* 头部 64 字节 */

_Static_assert(sizeof(airy_mem_entry_hdr_t) == 64,
               "airy_mem_entry_hdr_t must be 64 bytes");
```
...
typedef struct {
    ...
    uint32_t reserved;
    airy_mem_entry_hdr_t *entries; /* 条目指针数组 */
...
typedef struct {
    ...
    /* 哈希索引：key -> entry */
    struct airy_mem_l2_index *index;
    /* 热点 LRU 链表 */
    struct airy_mem_l2_lru   *lru;
    /* TTL 优先队列（按 expire_ns 排序） */
    struct airy_mem_l2_ttl  *ttl_queue;
} airy_mem_l2_t;
```
...
typedef struct {
    /* 向量索引（HNSW） */
    struct airy_hnsw_index *vectors;
    /* 文本索引（BM25） */
    struct airy_bm25_index *texts;
    /* 知识图谱 */
    struct airy_kg_graph    *knowledge_graph;
    /* VSA 绑定算子表 */
    struct airy_vsa_table  *vsa;
} airy_mem_l3_t;
```
...
typedef struct {
    ...
    char     raw_path[256];
    /* 模式层（聚类结果） */
    struct airy_hdbscan_result *clusters;
    struct airy_0d_persistence  *persistence;
    /* 后端类型 */
    enum { AIRY_MEM_BACKEND_FILE, AIRY_MEM_BACKEND_S3 } backend;
} airy_mem_l4_t;
```
...
| 位 | 名称 | 值 | 语义 | 调度优先级影响 |
|----|------|-----|------|---------------|
| 0 | `AIRY_GFP_L1` | 0x00000001 | 分配 L1 工作记忆 | 实时（不可阻塞） |
| 1 | `AIRY_GFP_L2` | 0x00000002 | 分配 L2 短期记忆 | 高（可短暂阻塞） |
| 2 | `AIRY_GFP_L3` | 0x00000004 | 分配 L3 长期记忆 | 普通 |
| 3 | `AIRY_GFP_L4` | 0x00000008 | 分配 L4 持久化记忆 | 后台 |
| 4 | `AIRY_GFP_NOWAIT` | 0x00000010 | 不等待，内存不足立即返回 | — |
| 5 | `AIRY_GFP_PERSIST` | 0x00000020 | 持久化分配（写入 PMEM） | — |
| 6 | `AIRY_GFP_COMPRESS` | 0x00000040 | 启用 ZSTD 压缩 | — |
| 7 | `AIRY_GFP_ENCRYPT` | 0x00000080 | 启用加密（SM4） | — |
| 8 | `AIRY_GFP_ATOMIC` | 0x00000100 | 原子分配（不可中断） | — |
| 9 | `AIRY_GFP_ZERO` | 0x00000200 | 分配后清零 | — |
| 10 | `AIRY_GFP_RECLAIMABLE` | 0x00000400 | 可回收（压力时自动释放） | — |
| 11-15 | 保留 | — | **必须**为 0 | — |
 ...
 */
void *airy_mem_alloc(uint32_t gfp, size_t size);

/**
 * airy_mem_free - 释放记忆
 * @ptr: airy_mem_alloc 返回的指针
 */
void airy_mem_free(void *ptr);
```
...
| 接口 | 语义 | 持久性保证 |
|------|------|-----------|
| `airy_pmem_write` | 写入持久化记忆 | 调用返回时数据已落盘 |
| `airy_pmem_read` | 读取持久化记忆 | — |
| `airy_pmem_flush` | 刷新缓存到持久介质 | 显式 barrier |
| `airy_pmem_sync` | 同步所有未落盘写入 | 全局 barrier |

### 5.2 PMEM 接口定义

```c
/**
 * airy_pmem_write - 写入持久化记忆
 * @backend:   PMEM 后端句柄
 * @key:       键（用于去重）
 * @key_len:   键长度
 * @value:     值
 ...
 *   -AIRY_EIO:      I/O 错误
 */
int airy_pmem_write(airy_pmem_t backend,
 ...
 * 读取最新版本。
 */
int airy_pmem_read(airy_pmem_t backend,
                      const void *key, size_t key_len,
                      void *value, size_t *value_len);

/**
 * airy_pmem_flush - 刷新到持久介质
 */
int airy_pmem_flush(airy_pmem_t backend);

/**
 * airy_pmem_sync - 全局同步
 */
int airy_pmem_sync(void);
```
...
```c
typedef enum {
    AIRY_FORGET_NONE          = 0,  /* 不遗忘（仅 L1） */
    AIRY_FORGET_EBBINGHAUS    = 1,  /* 艾宾浩斯曲线（默认） */
    AIRY_FORGET_LINEAR        = 2,  /* 线性衰减 */
    AIRY_FORGET_ACCESS_BASED  = 3,  /* 基于访问次数 */
} airy_forget_strategy_t;
```
 ...
 *   <  0: 错误码
 */
int airy_forget_run(airy_forget_strategy_t strategy,
 ...
 * 返回值:
 *   0:                  成功
 *   -AIRY_EINVAL:   参数非法（层级与 gfp 不匹配）
 *   -AIRY_ENOMEM:   内存不足
 */
int airy_mem_store(uint16_t layer, uint32_t gfp,
                      const void *key, size_t key_len,
                      const void *value, size_t value_len,
                      uint64_t ttl);

/**
 * airy_mem_load - 从指定层加载记忆
 * @layer:     源层级
 * @key:       键
 * @key_len:   键长度
 * @value:     输出缓冲区
 * @value_len: 输入：缓冲区大小；输出：实际读取字节数
 */
int airy_mem_load(uint16_t layer,
                     const void *key, size_t key_len,
                     void *value, size_t *value_len);

/**
 * airy_mem_search - 在指定层检索
 * @layer:   层级
 * @query:   查询（向量或文本）
 * @query_len: 查询长度
 * @k:       返回 top-k 结果
 ...
 */
int airy_mem_search(uint16_t layer,
                       const void *query, size_t query_len,
                       uint32_t k,
                       airy_mem_result_t *results,
 ...
 * 迁移：写入目标层 + 删除源层（原子操作）。
 */
int airy_mem_migrate(uint16_t from_layer, uint16_t to_layer,
                        const void *key, size_t key_len);
```
...
|--------|------|------|
| `AIRY_EOK` | 0 | 成功 |
| `AIRY_EINVAL` | -2 | 参数非法（层级与 gfp 不匹配） |
| `AIRY_ENOMEM` | -1100 | 内存不足 |
| `AIRY_ENOENT` | -2 | 记忆条目不存在 |
| `AIRY_EMSGSIZE` | -1109 | 记忆条目超过容量 |
| `AIRY_EIO` | -5 | I/O 错误（PMEM） |
| `AIRY_EPERSIST` | -1101 | 持久化失败 |
...
airy_mem_store(/* layer */ 1,
                  /* gfp   */ AIRY_GFP_L1 | AIRY_GFP_NOWAIT,
                  key, strlen(key),
                  value, strlen(value),
                  /* ttl */ 0);
...
airy_mem_load(1, key, strlen(key), buf, &len);
```
...
airy_mem_result_t results[10];
uint32_t result_count = 10;
airy_mem_search(/* layer */ 3,
...
```c
airy_pmem_t pmem = airy_pmem_open("/var/lib/airymax/pmem/l4.raw");
const char *key   = "doc-2026-07-09-001";
const char *value = "原始文档内容...";
airy_pmem_write(pmem,
                   key, strlen(key),
                   value, strlen(value),
                   AIRY_GFP_L4 | AIRY_GFP_PERSIST |
                   AIRY_GFP_COMPRESS | AIRY_GFP_ENCRYPT);
airy_pmem_flush(pmem);  /* 确保落盘 */
```
...
```c
/* 每日运行艾宾浩斯遗忘 */
int processed = airy_forget_run(AIRY_FORGET_EBBINGHAUS,
                                    /* tau */ 604800,  /* 7 天 */
                                    /* r_min */ 0.1);
printf("已处理 %d 条记忆\n", processed);
```
...
- [Airymax 认知循环标准](./05-airy-cognition-loop-standard.md)
