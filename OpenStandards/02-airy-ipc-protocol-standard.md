...
```c
#define AIRY_IPC_MSG_HDR_SIZE  128u
...
struct airy_ipc_msg_hdr {
    ...
    __u64   timestamp_ns;   /* 偏移 16: 纳秒时间戳（CLOCK_MONOTONIC） */
...
_Static_assert(sizeof(struct airy_ipc_msg_hdr) == AIRY_IPC_MSG_HDR_SIZE,
...
| timestamp_ns | 16 | 8 | __u64 LE | 纳秒时间戳（CLOCK_MONOTONIC） | 单调递增 |
...
| 标志位 | 值 | 语义 | 安全约束 |
|--------|-----|------|---------|
| `AIRY_IPC_FLAG_ZEROCOPY` | 0x00000001 | 零拷贝路径 | 发送方必须注册 buffer |
| `AIRY_IPC_FLAG_CAP_CARRY` | 0x00000002 | 携带 capability 令牌 | 内核校验令牌有效性 |
...
| `AIRY_IPC_FLAG_URGENT` | 0x00000010 | 紧急消息，优先投递 | 仅实时调度类可用 |
| `AIRY_IPC_FLAG_NOREPLY` | 0x00000020 | 无需响应（单向通知） | 仅 EVENT/NOTIFICATION 类型可用 |
| `AIRY_IPC_FLAG_BROADCAST` | 0x00000040 | 广播消息 | dst_task 必须为 0 |
...
| `timeout_ms` | 超时后发送方**必须**返回 `AIRY_ETIMEOUT` |
...
可选 HMAC-SM3 校验（通过 `AIRY_IPC_FLAG_ENCRYPT` 标志启用）。启用后 payload 末尾追加 32 字节 HMAC-SM3 标签。
